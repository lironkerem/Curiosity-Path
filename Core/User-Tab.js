...
`;
  ...
  renderAboutHTML() {
    return `<div class="accordion-inner"><p><strong>The Curiosity Path</strong> by Aanandoham, 2026.</p><p>A digital way, for a digital practitioner, to continue practicing Spirituality in the 21st Century.</p><p>This App was built to share tools, practices and ancient wisdom - digitally, from your device.</p><p>It is a convenient, accessible way, to stay connected to your 'Self', by small daily practices.</p><p>My hope is that you will utilize it to enhance your life, one small function at a time.</p></div>`;
  }

  renderContactHTML() {
    return `<div class="accordion-inner"><p>Contact me for questions, private sessions, classes, retreats, guidance or any technical issues.</p><a href="https://lironkerem.wixsite.com/project-curiosity" target="_blank" style="font-weight:bold;text-decoration:underline;color:var(--neuro-accent);">Official website</a><br><a href="mailto:lironkerem@gmail.com" style="font-weight:bold;text-decoration:underline;color:var(--neuro-accent);">Email me</a><br><a href="https://www.facebook.com/AanandohamsProjectCuriosity" target="_blank" style="font-weight:bold;text-decoration:underline;color:var(--neuro-accent);">Facebook Page</a></div>`;
  }
  ...
  renderBillingHTML() {
    return `<div class="accordion-inner"><p><strong>Free</strong> - basic tools, ads free forever.</p><p><strong>Practitioner</strong> - full Premium packs, monthly.</p><p><strong>Adept</strong> - Premium packs + Sessons discounts, monthly.</p><p><strong>Master</strong> - Premium packs + Discounts + 1-on-1 calls.</p><button class="btn-link">Choose plan</button></div>`;
  }
...
`;
  ...
  init() {
    ...
    this.dropdown.addEventListener('click', (e) => {
      ...
      } else if (actionBtn && actionBtn.dataset.action === 'logout') {
  ...
  handleSectionClick(section) {
    ...
    if (!isOpen) {
      ...
      // Render content if not already filled
  ...
  renderSection(section, panel) {
    const renderers = {
      profile: () => { panel.innerHTML = this.renderProfileHTML(); this.attachProfileHandlers(); },
      settings: () => { panel.innerHTML = this.renderSettingsHTML(); this.attachSettingsHandlers(); },
      notifications: () => { panel.innerHTML = this.renderNotificationsHTML(); this.attachNotificationsHandlers(); },
      automations: () => { panel.innerHTML = this.renderAutomationsHTML(); this.attachAutomationsHandlers(); },
      about: () => { panel.innerHTML = this.renderAboutHTML(); },
      rules: () => { panel.innerHTML = this.renderRulesHTML(); this.attachRulesHandlers(panel); },
      contact: () => { panel.innerHTML = this.renderContactHTML(); },
      export: () => { panel.innerHTML = this.renderExportHTML(); },
      billing: () => { panel.innerHTML = this.renderBillingHTML(); },
      admin: () => { panel.innerHTML = this.renderAdminHTML(); this.loadAdminPanel(); }
    };
  ...
  attachSettingsHandlers() {
    ...
    // Theme toggles with event delegation
  ...
  attachNotificationsHandlers() {
    ...
    const updateFreq = () => {
      const any = quotesToggle?.checked || affirmationsToggle?.checked;
      freqSelect.disabled = !any;
      freqSelect.parentElement.style.opacity = any ? '1' : '.4';
      freqSelect.parentElement.style.pointerEvents = any ? 'auto' : 'none';
    };
  ...
  attachAutomationsHandlers() {
    ['self-reset', 'full-body-scan', 'nervous-system', 'tension-sweep'].forEach(t => {
      const checkbox = document.getElementById(`auto-${t}`);
      const interval = document.getElementById(`interval-${t}`);
      const controls = checkbox?.closest('.automation-group')?.querySelector('.automation-controls');
      
      checkbox?.addEventListener('change', (e) => {
        if (interval) interval.disabled = !e.target.checked;
  ...
  saveAutomations() {
    ...
    if (notifSettings.enabled && notifSettings.wellness?.enabled) {
      this.scheduleNotifications(notifSettings);
    }
  ...
  loadActiveTheme() {
    try {
      ...
      if (theme && theme !== 'default') {
        setTimeout(() => this.switchTheme(theme), 100);
      }
    } catch (e) {
      console.warn(e);
  ...
  async enablePushNotifications() {
    if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
      this.app.showToast('❌ Push notifications not supported', 'error');
      return false;
    }
    ...
    try {
      ...
      if (permission !== 'granted') {
        this.app.showToast('❌ Notification permission denied', 'error');
        return false;
      }
      ...
      if (!sub) {
        ...
        const payload = this.app.state?.currentUser?.id
          ? { ...sub.toJSON(), user_id: this.app.state.currentUser.id }
          : sub;
      ...
      console.error(err);
      this.app.showToast('❌ Failed to enable notifications', 'error');
      return false;
    }
  }
  ...
  saveNotificationSettings() {
    ...
    supabase.from('notification_prefs')
      ...
      .then(({ error }) => {
        if (error) {
          console.error(error);
          this.app.showToast('⚠️ Saved locally, but cloud sync failed', 'error');
        } else {
          this.app.showToast('✅ Settings saved & scheduled', 'success');
        }
      });
  ...
  async sendTestNotification() {
    try {
      ...
      if (!subs.length) {
        this.app.showToast('❌ No subscriptions found', 'error');
        return;
      }
      ...
      if (response.ok) {
        this.app.showToast('📱 Test notification sent!', 'success');
      } else {
  ...
  scheduleNotifications(settings) {
    ...
    Object.entries(settings.reminders).forEach(([period, config]) => {
      if (config.enabled) this.scheduleDailyNotification(period, config.time);
    });

    // Schedule inspirational content
  ...
  async sendScheduledNotification(period) {
    const messages = {
  ...
  scheduleInspirationalNotifications(settings) {
    const intervals = {
      light: 4 * 60 * 60 * 1000,
    ...
    const send = async () => {
      let content;
      const both = settings.quotes.enabled && settings.affirmations.enabled;

      if (both) {
        if (Math.random() < 0.5 && window.QuotesData?.getRandomQuote) {
          const q = window.QuotesData.getRandomQuote();
          content = { title: '💭 Daily Quote', body: `"${q.text}" - ${q.author}` };
        } else if (window.affirmations?.length) {
          content = {
            title: '🌟 Daily Affirmation',
            body: window.affirmations[Math.floor(Math.random() * window.affirmations.length)]
          };
        }
  ...
  scheduleWellnessNotifications(settings) {
    ...
    Object.entries(automations).forEach(([key, config]) => {
      if (config.enabled && config.interval) {
        this.scheduleWellnessReminder(key, config.interval, messages[key]);
      }
    });
  }

  scheduleWellnessReminder(type, intervalMinutes, message) {
