// User-Tab.js – patched for static-css + lock-aware skins 2026-01-20
...
export default class UserTab {
  ...
  handleSectionClick(section) {
    ...
    if (!isOpen) {
      ...
      if (!panel.dataset.filled) { this.renderSection(section, panel); panel.dataset.filled = '1'; }
    }
  }
  ...
  attachProfileHandlers() {
    ...
    document.getElementById('avatar-upload')?.addEventListener('change', () => {
      ...
      if (!file) return;
      if (file.size > 5_242_880) { this.app.showToast('Image > 5 MB', 'error'); return; }
  ...
  async hydrateUserProfile() {
    ...
    if (!data) try { data = JSON.parse(localStorage.getItem(`profile_${uid}`)); } catch (e) {}
    if (data) {
  ...
  attachSkinsHandlers() {
    ...
    // premium-skin radios
    document.querySelectorAll('.theme-toggle').forEach(radio => {
      radio.addEventListener('change', e => {
        if (!e.target.checked) return;
        const name = e.target.dataset.theme;
        // ----- LOCK CHECK -----
        if (e.target.dataset.locked === 'true') {
          e.target.checked = false;                 // revert UI
          this.app.showToast('🔒 Purchase required', 'warning');
          return;
        }
        window.setSkin(name);
        if (name === 'matrix-code') {
  ...
  switchTheme(name) { window.setSkin(name); }

  loadActiveTheme() {
    try {
      const theme = localStorage.getItem('activeTheme') || 'default';
      if (theme !== 'default') window.setSkin(theme);
    } catch (e) {
  ...
  async attachNotificationsHandlers() {
    ...
    const checkWindowSize = () => {
      ...
      warning.style.display = (diff < 360 && frequency.value === 'full') ? 'block' : 'none';
    };
    const autoSave = () => { clearTimeout(saveTimeout); saveTimeout = setTimeout(() => this.saveNotificationSettings(), 1500); };
    ...
    master?.addEventListener('change', async (e) => {
      if (e.target.checked) {
        ...
        await this.disablePushNotifications(); options.style.opacity = '.4'; options.style.pointerEvents = 'none';
      }
      autoSave();
    });
    startTime?.addEventListener('change', () => { if (validateWindow()) { checkWindowSize(); autoSave(); } });
    endTime?.addEventListener('change', () => { if (validateWindow()) { checkWindowSize(); autoSave(); } });
    frequency?.addEventListener('change', () => { checkWindowSize(); autoSave(); });
    document.getElementById('save-notification-settings')?.addEventListener('click', () => {
      if (validateWindow()) { clearTimeout(saveTimeout); this.saveNotificationSettings(); }
    });
    checkWindowSize();
  }
    ...
    try {
      ...
      if (!sub) {
        ...
        const res = await fetch('/api/save-sub', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', ...(this.app.auth?.session?.access_token && { Authorization: `Bearer ${this.app.auth.session.access_token}` }) },
  ...
  restoreNotificationUI(settings) {
    ...
    if (timezoneDisplay) timezoneDisplay.textContent = settings.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone;
  }
  ...
  async loadAdminPanel() {
    ...
    try {
      ...
      mount.innerHTML = '';
      if (typeof content === 'string') mount.innerHTML = content;
      else if (content instanceof HTMLElement) mount.appendChild(content);
      else mount.innerHTML = '<div style="color:#ff4757;padding:10px;">Invalid content returned</div>';
    } catch (err) {
  ...
  showPricingModal() {
    const overlay = document.getElementById('pricing-modal-overlay');
    if (!overlay) return;
    const themeClass = ['champagne-gold', 'royal-indigo', 'earth-luxury', 'matrix-code'].find(cls => document.body.classList.contains(cls));
  ...
  async handleLogout() {
    try {
      ...
      if (this.app && typeof this.app.logout === 'function') await this.app.logout();
      else { console.error('app.logout() not available'); this.app?.showToast('❌ Logout failed', 'error'); }
    } catch (err) {
