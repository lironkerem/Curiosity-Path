...
export default class AuthManager {
  ...
  _generateLoginHTML(isLocked, lockoutMessage) {
    return `<div class="auth-screen-wrapper mobile-optimized"><div class="auth-card-enhanced">${this._generateLogoHeader('The Curiosity Path', 'by Aanandoham', 'Join 10,000+ seekers on their journey')}${isLocked ? `<div class="lockout-warning">${lockoutMessage}</div>` : ''}${this._generateGoogleButton('Continue with Google', 'handleGoogleLogin', isLocked)}<div class=divider><span>or</span></div>${this._generateLoginForm(isLocked)}<p class="text-center mt-4 text-sm" style="margin-top:16px"><a href="#" onclick="window.app.auth.showSignupForm(); return false;" class="auth-link">Create an account</a></p>${this._generateFooter()}</div></div>${this._generateStyles()}`;
  }

  showSignupForm() {
    const html = `<div class="auth-screen-wrapper mobile-optimized"><div class="auth-card-enhanced">${this._generateLogoHeader('Create Account', 'Join The Curiosity Path', 'Join 10,000+ seekers on their journey')}${this._generateGoogleButton('Sign up with Google', 'handleGoogleSignup', false)}<div class=divider><span>or</span></div>${this._generateSignupForm()}<p class="text-center mt-4 text-sm" style="margin-top:16px"><a href="#" onclick="window.app.auth.renderAuthScreen(); return false;" class="auth-link">Already have an account? Sign in</a></p>${this._generateFooter()}</div></div>${this._generateStyles()}`;
    document.getElementById('auth-screen').innerHTML = html;
  }

  showForgotPassword() {
    const html = `<div class="auth-screen-wrapper mobile-optimized"><div class="auth-card-enhanced">${this._generateLogoHeader('Reset Password', 'Enter your email to receive a reset link', '')}<form id=forgot-form onsubmit="window.app.auth.handleForgotPassword(event)" class="space-y-4" style="margin-top:1.5rem"><div class=form-group style="margin-bottom:16px"><label class=form-label style="margin-bottom:8px;display:block">Email</label><input type=email class="form-input-enhanced" placeholder="your@email.com" autocomplete="email" inputmode="email" required><span class="error-message" style="display:none"></span></div>${this._generateSubmitButton('Send Reset Link')}</form><p class="text-center mt-4 text-sm" style="margin-top:16px"><a href="#" onclick="window.app.auth.renderAuthScreen(); return false;" class="auth-link">Back to sign in</a></p>${this._generateSecurityBadge()}</div></div>`;
    document.getElementById('auth-screen').innerHTML = html;
  }

  _generateLogoHeader(title, subtitle, description) {
    return `<div class="text-center mb-6 fade-in"><img class="header-image" src="${ASSETS.LOGO_URL}" alt="Aanandoham Header" loading="eager" style="margin:0 auto 1rem"><hr style="border:none;border-top:1px solid var(--neuro-shadow-dark);margin:0.75rem 0"><h1 class="text-3xl font-bold mb-2">${title}</h1><p class="auth-subtitle">${subtitle}</p>${description ? `<p class="auth-desc">${description}</p>` : ''}</div>`;
  }

  _generateGoogleButton(text, handler, disabled) {
    return `<button onclick="window.app.auth.${handler}()" class="btn-google w-full mb-4 touch-target" style="margin-top:1.5rem" ${disabled ? 'disabled' : ''}><svg width="20" height="20" viewBox="0 0 48 48" style="margin-right:12px"><path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/><path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.03h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.66z"/><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/><path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/></svg><span style="font-weight:500;color:var(--neuro-text)">${text}</span></button>`;
  }

  _generateLoginForm(disabled) {
    return `<form id=login-form onsubmit="window.app.auth.handleLogin(event)" class="space-y-4" style="margin-top:1.5rem">${this._generateEmailField(disabled)}${this._generatePasswordField(disabled, false)}<div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px;margin-bottom:16px"><label style="display:flex;align-items:center;font-size:0.875rem;color:var(--neuro-text-light);cursor:pointer"><input type=checkbox id="remember-me" style="margin-right:6px" ${disabled ? 'disabled' : ''}>Remember me</label><a href="#" onclick="window.app.auth.showForgotPassword(); return false;" class="auth-link" style="font-size:0.875rem">Forgot password?</a></div>${this._generateSubmitButton('Sign In', disabled)}</form>`;
  }

  _generateSignupForm() {
    return `<form id=signup-form onsubmit="window.app.auth.handleSignup(event)" class="space-y-4" style="margin-top:1.5rem"><div class=form-group style="margin-bottom:16px"><label class=form-label style="margin-bottom:8px;display:block">Name</label><input type=text class="form-input-enhanced" placeholder="Your spiritual name" autocomplete="name" required><span class="error-message" style="display:none"></span></div>${this._generateEmailField(false)}${this._generatePasswordField(false, true)}${this._generateSubmitButton('Sign Up')}</form>`;
  }
  ...
  _generateStyles() {
    return `<style>
      ...
      .auth-card-enhanced { background:var(--neuro-bg); border-radius:var(--radius-2xl); box-shadow:var(--shadow-raised-lg); padding:32px 32px; max-width:440px; width:100%; }
      @media (max-width:640px) { .auth-card-enhanced { padding:24px 20px; border-radius:var(--radius-xl); } .tooltip { white-space:normal; max-width:250px; } }
