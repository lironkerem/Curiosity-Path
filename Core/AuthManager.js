...
const ASSETS = {
  LOGO_URL: 'https://raw.githubusercontent.com/lironkerem/Digital-Curiosiry/main/Public/Watermarks/Logo.svg'
};
...
export default class AuthManager {
  ...
  async checkAuth() {
    ...
    if (session?.user) {
      await this._setAuthenticated(session.user);
      return true;
    }
  ...
  /**
   * Attempts to restore user from localStorage
   * @private
   * @returns {boolean} True if user restored successfully
   */
  _restoreLocalUser() {
    try {
  ...
  renderAuthScreen() {
    ...
    const html = this._generateLoginHTML(isLocked, lockoutMessage);
    document.getElementById('auth-screen').innerHTML = html;
  }

  /**
   * Generates login screen HTML
   * @private
   */
...
<div class="min-h-screen flex items-center justify-center p-4 mobile-optimized" style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%)">
  ...
  _generateLogoHeader(title, subtitle, description) {
    ...
    <div class="text-center mb-8 fade-in">
      <div class="logo-icon" style="width:144px;height:144px;display:flex;align-items:center;justify-content:center;margin:0 auto">
        <img src="${ASSETS.LOGO_URL}" alt="Aanandoham" style="width:120px;height:120px;object-fit:contain" loading="eager">
      </div>
      <h1 class="text-3xl font-bold mb-2">${title}</h1>
      <p style="color:#6c757d;font-weight:bold;font-size:1.1rem">${subtitle}</p>
      ${description ? `<p style="color:#9ca3af;font-size:0.9rem;margin-top:8px">${description}</p>` : ''}
    </div>`;
  }

  /**
   * Generates Google button
   * @private
   */
  _generateGoogleButton(text, handler, disabled) {
    return `
    <button onclick="window.app.auth.${handler}()" class="btn-google w-full mb-4 touch-target" style="margin-top:2.5rem" ${disabled ? 'disabled' : ''}>
      <svg width="20" height="20" viewBox="0 0 48 48" style="margin-right:12px">
      ...
      <span style="font-weight:500;color:#3c4043">${text}</span>
    </button>`;
  }
    ...
    <form id=login-form onsubmit="window.app.auth.handleLogin(event)" class="space-y-4" style="margin-top:2.5rem">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:12px;margin-bottom:24px">
        <label style="display:flex;align-items:center;font-size:0.875rem;color:#6c757d;cursor:pointer">
          <input type=checkbox id="remember-me" style="margin-right:6px" ${disabled ? 'disabled' : ''}>
          Remember me
        </label>
  ...
  _generateEmailField(disabled) {
    ...
    <div class=form-group style="margin-bottom:20px">
      <label class=form-label style="margin-bottom:8px;display:block">Email</label>
      <input type=email class="form-input-enhanced" placeholder="your@email.com" autocomplete="email" inputmode="email" required ${disabled ? 'disabled' : ''}>
      <span class="error-message" style="display:none"></span>
    </div>`;
  }
  ...
  _generatePasswordField(disabled, showStrength) {
    ...
    <div class=form-group style="margin-bottom:20px">
      <label class=form-label style="margin-bottom:8px;display:block">
        Password
        ${this._generatePasswordHint()}
      </label>
      <div style="position:relative">
        <input type=password class="form-input-enhanced" placeholder="••••••••" ${minlength} autocomplete="${autocomplete}" required ${oninput} onkeyup="window.app.auth.checkCapsLock(event)" ${disabled ? 'disabled' : ''}>
      </div>
  ...
  /**
   * Generates password hint tooltip
   * @private
   */
  ...
  /**
   * Generates submit button with loading state
   * @private
   */
  _generateSubmitButton(text, disabled = false) {
    return `
    <button type=submit class="btn-primary-enhanced w-full touch-target" ${disabled ? 'disabled' : ''} style="margin-top:24px">
  ...
  /**
   * Generates footer with links
   * @private
   */
  _generateFooter() {
    return `
    <div class="text-center mt-6 text-sm" style="color:#6c757d;margin-top:32px">
      ${this._generateSecurityBadge().replace('<div class="text-center mt-6 text-sm" style="color:#6c757d;margin-top:32px">', '').replace('</div>', '')}
...
// ==========================================================================
  ...
  /**
   * Checks if Caps Lock is on and shows warning
  ...
  /**
   * Shows password hint tooltip
   * @param {SVGElement} svg - SVG icon element
   */
  ...
  /**
   * Shows error message on input field
  ...
  _isAccountLocked() {
    ...
    try {
      ...
      console.warn('Failed to parse lockout data:', error);
  ...
  /**
   * Gets lockout message with remaining time
   * @private
  ...
  _recordFailedAttempt() {
    ...
    if (this.failedAttempts >= this.maxFailedAttempts) {
      ...
      this.renderAuthScreen(); // Re-render to show lockout message
    }
  }
    ...
    try {
      ...
      if (data.user) {
        ...
        if (data.user.identities && data.user.identities.length === 0) {
          ...
          // Initialize user progress and authenticate
      ...
      if (error.message.includes('already registered')) {
        ...
        this.showError(passInput, error.message);
      } else {
        this.showError(emailInput, error.message);
      }
    } finally {
  ...
  /**
   * Initializes user progress in database
   * @private
  ...
  async _initUserProgress(uid) {
    try {
      ...
      console.error('Failed to init user_progress:', error);
    }
  }

  /**
   * Ensures user progress record exists
   * @private
   * @param {string} uid - User ID
   */
  async _ensureUserProgress(uid) {
    try {
      ...
      if (!data) {
        await this._initUserProgress(uid);
      }
    } catch (error) {
      console.error('Failed to ensure user_progress:', error);
    }
  }

  /**
   * Sets user as authenticated and initializes app
   * @private
   * @param {Object} u - Supabase user object
   */
  async _setAuthenticated(u) {
    ...
    // Ensure user progress exists for Google users
    if (isGoogle) {
      await this._ensureUserProgress(user.id);
    }

    // Mark theme as loaded (makes app shell visible)
...
// ==========================================================================
  // MODAL HELPERS - TERMS & PRIVACY
  // ==========================================================================

  /**
   * Shows Terms of Service modal
   */
  showTerms() {
    this._showModal('Terms of Service', this._getTermsContent());
  }
   ...
   */
  showPrivacy() {
    this._showModal('Privacy Policy', this._getPrivacyContent());
  }
...
// ============================================================================
// PROFILE HELPERS - Late-attach after app initialization
// ============================================================================

/**
 * Attaches profile management helpers to app instance
 * These functions need to wait for window.app to be initialized
 */
function attachProfileHelpers() {
  // Wait for app to be ready
  if (!window.app) {
    requestAnimationFrame(attachProfileHelpers);
    return;
  }

  /**
   * Saves user profile data (quick profile update)
   * Attempts cloud save first, falls back to localStorage
   */
  window.app.saveQuickProfile = async function () {
    const uid = this.state?.currentUser?.id;
    
    if (!uid) {
      return this.showToast('Not logged in', 'error');
    }
    ...
    try {
      ...
      console.warn('Profile server save failed:', error);
    }
    ...
    // Sync avatar if UserTab is available
  ...
  /**
   * Hydrates user profile from cloud or localStorage
   * Cloud-first strategy with localStorage fallback
   */
  window.app.hydrateUserProfile = async function () {
    ...
    // Fallback to localStorage if cloud fetch failed
    if (!data) {
      try {
    ...
    // Apply profile data to current user
    if (data) {
