...
export class GamificationEngine {
  constructor(app) {
    ...
    // PATCHED: debouncedBadgeCheck initialised in constructor (not as class field)
    this.debouncedBadgeCheck = this.debounce(this._flushBadgeQueue.bind(this), CONFIG.BADGE_CHECK_DEBOUNCE_MS);
  ...
  defaultState() {
    return {
      ...
      logs:[], totalWellnessRuns:0, totalTarotSpreads:0,
      ...
      dailyQuestStreak:0, activeBoosts:[], skipCaps:{},
  ...
  initializeQuests() {
    ...
    if (!this.state.quests.daily || this.state.quests.daily.length === 0) {
      ...
      ['daily','weekly','monthly'].forEach(type => {
        ...
        defs.forEach(def => { if (!merged.find(q => q.id === def.id)) merged.push({ ...def }); });
  ...
  getQuestDefinitions() {
    return {
      daily: [
        ...
        { id:'flip_script',       tab:'flip-script',  icon:'🔄', name:'Flip The Script',              inspirational:'Every negative thought holds the seed of its opposite.',       target:'Flip 1 negative thought',        goal:1,  progress:0, xpReward:40, completed:false, karmaReward:4 }
      ],
      weekly: [
        { id:'gratitude_streak_7',    tab:'gratitude',   icon:'💖', name:'Gratitude Streak',          inspirational:'Consistency breeds abundance.',                    target:'Log 70 gratitudes across the week to complete this quest.',                                   goal:70, progress:0, xpReward:100, completed:false, karmaReward:10 },
        { id:'journal_5',             tab:'journal',     icon:'📝', name:'Journal Writer',            inspirational:'Your story matters.',                              target:'Save 5 journal entries across the week to complete this quest.',                              goal:5,  progress:0, xpReward:150, completed:false, karmaReward:15 },
        { id:'energy_7',              tab:'energy',      icon:'⚡', name:'Weekly Energy Check-ins',   inspirational:'Track your rhythm, honor your cycles.',           target:'Tick 14 energy check-ins (day & night) across the week to complete this quest.',            goal:14, progress:0, xpReward:80,  completed:false, karmaReward:8  },
        { id:'happiness_boosters_20', tab:'happiness',   icon:'🎨', name:'Happy and Motivated Week', inspirational:'Feed your mind with positivity.',                  target:'Refresh happiness cards 35 times across the week to complete this quest.',                   goal:35, progress:0, xpReward:120, completed:false, karmaReward:12 },
        { id:'tarot_4_days',          tab:'tarot',       icon:'🔮', name:'Tarot Lover',               inspirational:'Seek wisdom in the cards.',                        target:'Complete five 6-card (or larger) spreads across the week to complete this quest.',          goal:5,  progress:0, xpReward:100, completed:false, karmaReward:10 },
        { id:'meditate_3',            tab:'meditations', icon:'🌟', name:'Meditating Adept',          inspirational:'Stillness is strength.',                          target:'Finish 5 meditation sessions across the week to complete this quest.',                       goal:5,  progress:0, xpReward:120, completed:false, karmaReward:12 },
        { id:'flip_script_5',         tab:'flip-script', icon:'🔄', name:'Script Flipper',            inspirational:'Rewire your mind one thought at a time.',         target:'Flip 5 negative thoughts across the week to complete this quest.',                          goal:5,  progress:0, xpReward:150, completed:false, karmaReward:15 }
      ],
      monthly: [
        { id:'monthly_flip_15',        tab:'flip-script',  icon:'🔄', name:'Master Script Flipper',  inspirational:'You are the author of your own story.',                                       target:'Flip 15 negative thoughts during the month to complete this quest.',                             goal:15,  progress:0, xpReward:400, completed:false, karmaReward:40 },
        { id:'monthly_energy_28',      tab:'energy',       icon:'⚡', name:'Monthly Energy Check-ins',inspirational:'Know thyself through daily awareness.',                                       target:'Tick 60 energy check-ins (day & night) during the month to complete this quest.',              goal:60,  progress:0, xpReward:300, completed:false, karmaReward:30 },
        { id:'monthly_tarot_15',       tab:'tarot',        icon:'🔮', name:'Tarot Enthusiast',        inspirational:'The universe speaks through symbols.',                                        target:'Complete twenty 6-card (or larger) spreads during the month to complete this quest.',          goal:20,  progress:0, xpReward:400, completed:false, karmaReward:40 },
        { id:'monthly_gratitude_28',   tab:'gratitude',    icon:'💖', name:'Gratitude Master',        inspirational:'Gratitude unlocks the fullness of life.',                                     target:'Log 300 gratitudes during the month to complete this quest.',                                   goal:300, progress:0, xpReward:350, completed:false, karmaReward:35 },
        { id:'monthly_journal_20',     tab:'journal',      icon:'📝', name:'A Journalist',            inspirational:'Write to understand, reflect to grow.',                                       target:'Save 20 journal entries during the month to complete this quest.',                             goal:20,  progress:0, xpReward:500, completed:false, karmaReward:50 },
        { id:'monthly_happiness_100',  tab:'happiness',    icon:'🎨', name:'Super Good Month',        inspirational:'Choose joy every single day.',                                                target:'Refresh happiness cards 150 times during the month to complete this quest.',                   goal:150, progress:0, xpReward:450, completed:false, karmaReward:45 },
        { id:'monthly_meditation_15',  tab:'meditations',  icon:'🌟', name:'Meditating Healer',       inspirational:'Through stillness, we find our true power.',                                  target:'Finish 20 meditation sessions during the month to complete this quest.',                       goal:20,  progress:0, xpReward:600, completed:false, karmaReward:60 }
      ]
    };
  }
  ...
  saveState() {
    ...
    this.saveTimeout = setTimeout(() => {
      try {
        try { localStorage.setItem('gamificationState', JSON.stringify(this.state)); } catch { /* quota exceeded */ }
        if (this.app?.state) {
          const { logs: _logs, ...cloudState } = this.state;
          this.app.state.data = { ...this.app.state.data, ...cloudState };
          this.app.state.saveAppData();
        }
      } catch (error) { console.error('Failed to save gamification state:', error); }
  ...
  loadState() {
    try {
      if (this.app?.state?.data && this.app.state.data.xp !== undefined) {
        ...
        const deprecated = ['streak.lastCheckIn','energyLevel','alignmentScore','chakraProgress','totalPracticeMinutes'];
        deprecated.forEach(field => {
          const parts = field.split('.');
          if (parts.length === 2) { if (cloud[parts[0]]) delete cloud[parts[0]][parts[1]]; }
          else delete cloud[field];
        });
        return { ...this.defaultState(), ...cloud };
      }
      let local = null;
      try { local = localStorage.getItem('gamificationState'); } catch { /* private browsing */ }
      if (local) return { ...this.defaultState(), ...JSON.parse(local) };
      return null;
    } catch (error) { console.error('Failed to load gamification state:', error); return null; }
  }
  ...
  async reloadFromDatabase() {
    ...
    try {
      ...
      this.checkAllBadges();
    } catch (error) { console.error('Failed to reload gamification state:', error); }
  }
  ...
  reset() {
    try {
      ...
      this.saveState();
    } catch (error) { console.error('Failed to reset gamification state:', error); }
  }
  ...
  emit(event, data) {
    ...
    this.listeners[event].forEach(cb => {
      try { cb(data); } catch (error) { console.error(`Error in event listener for ${event}:`, error); }
    });
  }

  destroy() { clearTimeout(this.saveTimeout); this.listeners = {}; }
  ...
  incrementWellnessRuns()   { this._incrementCounter('totalWellnessRuns',   'wellness', 'wellnessRunCompleted');  }
  incrementTarotSpreads()   { this._incrementCounter('totalTarotSpreads',   'tarot',    'tarotSpreadCompleted');  }
  incrementJournalEntries() { this._incrementCounter('totalJournalEntries', 'journal',  'journalEntrySaved');     }
  incrementHappinessViews() { this._incrementCounter('totalHappinessViews', 'happiness','happinessViewAdded');    }
  ...
  // PATCHED: plain method instead of class field arrow function
  _flushBadgeQueue() {
  ...
  addBoth(xp, karma, source = 'general') {
    if (!xp && !karma) return;
    let finalXP = xp, finalKarma = karma;
    if (xp > 0) {
  ...
  hasActiveXPBoost() {
    try {
      const now = Date.now();
      return (this.state.activeBoosts || []).some(b =>
  ...
  activateBoost(id, durationMs) {
    ...
    this.state.activeBoosts.push({ id, expiresAt: Date.now() + durationMs });
    this.saveState();
    this.emit('boostActivated', { id, expiresAt: Date.now() + durationMs });
  }
  ...
  checkLevelUp() {
    ...
    if (level > prev) {
      ...
      if (this.app?.showToast) this.app.showToast(`🎉 Level ${level} – ${title}  +${CONFIG.LEVEL_UP_BONUS_XP} XP`, 'success');
      showLevelUpSpectacle({ level, title, xp: CONFIG.LEVEL_UP_BONUS_XP, karma: 0 });
  ...
  updateStreak() {
    ...
    if (!last || diff > this.state.settings.streakResetDays) this.state.streak.current = 1;
    else this.state.streak.current += 1;
    if (this.state.streak.current > (this.state.streak.best || 0)) this.state.streak.best = this.state.streak.current;
  ...
  checkCrossFeatureBadges(badges) {
    ...
    if (total >= 1) this.checkAndGrantBadge('first_step', badges);
    ...
    const todayMeditation = (data.meditationEntries || []).some(e => new Date(e.timestamp).toDateString() === today);
    ...
    if ([counts.gratitude,counts.journal,counts.energy,counts.tarot,counts.meditation,counts.happiness,counts.wellness].every(c => c > 0))
      this.checkAndGrantBadge('complete_explorer', badges);

    if (Object.values(counts).filter(c => c >= 10).length >= 5)
  ...
  checkChakraBadges(chakras) {
    ...
    if (chakraKeys.every(k => (chakras[k] || 0) >= 8)) this.checkAndGrantBadge('chakra_balancer', badges);
    if (chakraKeys.every(k => (chakras[k] || 0) >= 9)) this.checkAndGrantBadge('chakra_master',   badges);
  }
  ...
  grantBadge(badge) {
    ...
    if (karma) { this.state.karma += karma; this.logAction('karma', { amount: karma, source: `Badge: ${badge.id}` }); }
  ...
  progressEnergyCheckin(timeOfDay) {
    ...
    if (quest.progress >= quest.goal) {
      ...
      if (this.state.quests.daily.every(q => q.completed)) {
        ...
        if (!this.state._bulkMode && this.app?.showToast) this.app.showToast(`Daily quests finished! +${xp} XP +${karma} Karma`, 'success');
  ...
  _resetQuests(type) {
    ...
    if (allComplete) {
      if (type === 'daily') {
        ...
        this.state.karma += karma;
        if (this.app?.showToast) this.app.showToast(`Weekly quests finished! +${xp} XP +${karma} Karma`, 'success');
        ...
        this.state.karma += karma;
        if (this.app?.showToast) this.app.showToast(`Monthly quests finished! +${xp} XP +${karma} Karma`, 'success');
  ...
  _completeBatch(type) {
    ...
    quests.forEach(q => {
      if (!q.completed) { this.completeQuest(type, q.id); done++; xp += q.xpReward || 50; karma += q.karmaReward || 0; }
    });
  ...
  // PATCHED: removed redundant saveState() call inside logAction
  // (callers always call saveState() themselves)
  ...
  getStatusSummary() {
    ...
    return {
      ...
      badges: this.state.badges, unlockedFeatures: this.state.unlockedFeatures,
...
function showLevelUpSpectacle({ level, title, karma = 0, xp = 0 }) {
  ...
  st.textContent = `
    ...
    .lvl-title{font-size:clamp(3rem,12vmin,8rem);font-weight:900;letter-spacing:0.05em;background:linear-gradient(135deg,#fbbf24 0%,#f59e0b 50%,#fde68a 100%);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;}
