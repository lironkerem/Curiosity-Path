...
export function openTriggerReleaseModal(entry = null) {
  ...
  coreSelect.addEventListener('change', () => {
    const sel = coreSelect.value;
    if (sel && coreTriggers[sel]) {
      emotionSelect.disabled   = false;
      emotionSelect.style.color= '';
      emotionSelect.innerHTML  = coreTriggers[sel].map(e => `<option value="${e}">${e}</option>`).join('');
    } else {
  ...
  modal.querySelector('#saveTriggerEntry').addEventListener('click', () => {
    ...
    const intensity = parseInt(slider.value, 10); // FIXED: radix 10
    ...
    if (isEditing) {
      ...
      state.triggers.push({ id: `trigger-${Date.now()}`, date: new Date().toISOString(), source, text, coreTrigger: core, emotion, intensity });
