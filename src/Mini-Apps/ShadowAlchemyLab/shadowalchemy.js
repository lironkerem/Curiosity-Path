class ShadowAlchemyEngine {
  ...
  async render() {
    ...
    const headerHTML = `
      <header class="main-header project-curiosity"
        style="--header-img:url('/Tabs/NavShadow.webp');--header-title:'';--header-tag:'Transform your Shadows into Golden wisdom'">
    ...
    if (isLocked) {
      tab.innerHTML = `
        <div style="padding:1.5rem;min-height:100vh"><div class="universal-content">
          ${headerHTML}
          <div class="card relative" style="padding:3rem;text-align:center;opacity:0.75">
            ...
            <p style="color:var(--neuro-text-light);font-size:1.2rem;margin-bottom:2rem">Unlock the Shadow Alchemy Lab in the Karma Shop to access this powerful transformation tool.</p>
    ...
    tab.innerHTML = `
      <div style="background:var(--neuro-bg);padding:1.5rem;min-height:100vh"><div class="universal-content">
        ${headerHTML}
  ...
  async initializeShadowAlchemy() {
    try {
      ...
      if (main) {
        // FIXED: use textContent for err.message to prevent XSS
        const card = document.createElement('div');
        card.className = 'card';
        card.style.cssText = 'text-align:center;padding:var(--spacing-xl)';
        card.innerHTML = '<h3 style="color:var(--neuro-text)">Failed to Load</h3><p style="color:var(--neuro-text-light)">Please refresh the page and try again.</p><p id="err-detail" style="color:var(--neuro-text-lighter);font-size:0.9rem;margin-top:1rem"></p>';
        card.querySelector('#err-detail').textContent = err.message;
