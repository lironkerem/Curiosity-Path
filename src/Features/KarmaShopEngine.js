...
export class KarmaShopEngine {
  ...
  static SKIP_CAP_LIMITS = { dailySkips: 2, weeklySkips: 2, monthlySkips: 3 };
  ...
  async loadFromCloud() {
    try {
      ...
      if (data?.karmaShop) {
        ...
        if (shop.purchaseHistory) localStorage.setItem('karma_purchase_history', JSON.stringify(shop.purchaseHistory));
  ...
  _useSkipCap(type)  { if (this._isAdmin()) return; this.skipCaps[type].used += 1; this._persistSkipCaps(); this.queueRender(); }
  _skipCapUsed(type) { return this.skipCaps[type]?.used || 0; }
  _skipCapMax(type)  { return KarmaShopEngine.SKIP_CAP_LIMITS[type] || 0; }
  _whenResets(itemId) {
    const m = { skip_all_daily: 'tonight at midnight', skip_all_weekly: 'next Sunday', skip_all_monthly: 'the 1st of next month' };
    return m[itemId] || 'soon';
  }
  ...
  buildCatalog() {
    ...
    this.items = [
      ...
      { id:'advance_tarot_spreads',     name:'Advance Tarot Spreads',                  description:'Unlock premium spreads and TarotVision AI',                                                                       cost:200,  icon:'🃏',    category:'Premium Features',  consumable:false, rarity:'epic'      },
      { id:'shadow_alchemy_lab',        name:'Shadow Alchemy Lab',                     description:'Transform shadows into personal growth tools',                                                                    cost:300,  icon:'🌑',    category:'Premium Features',  consumable:false, rarity:'legendary' },
      { id:'self_analysis_pro',         name:'Self-Analysis Pro',                      description:'Full Numerology, Astrology & Tree of Life personal analysis with PDF export',                                    cost:400,  icon:'🔢',    category:'Premium Features',  consumable:false, rarity:'legendary' },
  ...
  saveActiveBoostsLocal() { localStorage.setItem('karma_active_boosts', JSON.stringify(this.activeBoosts)); }
  ...
  checkExpiredBoosts() {
    ...
    this.activeBoosts = this.activeBoosts.filter(b =>
      b.id.startsWith('skip_all_') ? now < (b.resetAt || 0) : b.expiresAt > now
    );
  ...
  isBoostActive(boostId) {
    if (boostId.startsWith('skip_all_')) {
      const b = this.activeBoosts.find(b => b.id === boostId);
      return b ? Date.now() < (b.resetAt || 0) : false;
    }
  ...
  canPurchase(itemId) {
    ...
    if (!this._isAdmin() && KarmaShopEngine.CAP_MAP[itemId]) {
      ...
      if (this._skipCapUsed(type) >= this._skipCapMax(type)) return { can: false, reason: `Cap reached (${this._skipCapMax(type)}/${this._skipCapMax(type)})` };
    }
    return { can: true };
  }
  ...
  isItemOwned(itemId) {
    return this.getPurchaseHistory().some(p => p.itemId === itemId)
      || this.app.gamification?.state?.unlockedFeatures?.includes(itemId);
  }
  ...
  applyItemEffect(itemId, item) {
    try {
      ...
      else if (itemId === 'self_analysis_pro')     { this.safeUnlockFeature('self_analysis_pro');     this.app.showToast('Self-Analysis Pro unlocked!', 'success'); }
  ...
  // PATCHED: escape userName and item.name to prevent XSS
  showMasterPurchasePopup(item) {
    const userName = this._esc(this.app.state.currentUser?.name || 'Friend');
    const karma    = item.cost;
    const rawMsg   = `${item.name} bought using ${karma} Karma for ${this.app.state.currentUser?.name || 'Friend'}.`;
    const safeMsg  = `${this._esc(item.name)} bought using ${karma} Karma for ${userName}.`;
    ...
    overlay.innerHTML = `
      <div class="card karma-shop-master-card">
        ...
        <p class="karma-shop-master-message">${safeMsg}</p>
        ...
        <a href="https://wa.me/+972524588767?text=${encodeURIComponent(rawMsg)}" target="_blank" rel="noopener noreferrer" class="btn btn-primary karma-shop-master-btn-wa">Open WhatsApp</a>
  ...
  render() {
    ...
    if (!this._initialized) {
      tab.innerHTML = `<div class="karma-shop-container"><div class="karma-shop-content"><div class="card" style="text-align:center;padding:3rem;"><h3>Loading Karma Shop...</h3><p style="color:var(--neuro-text-light);margin-top:1rem;">Syncing your data...</p></div></div></div>`;
      ...
      <div class="karma-shop-container">
        <div class="karma-shop-content">
          <header class="main-header project-curiosity"
                  style="--header-img:url('/Tabs/NavShop.webp');--header-title:'';--header-tag:'Exchange your Karma tokens for goodies and upgrades'">
  ...
  renderActiveBoosts() {
    ...
    const content = this.activeBoosts.map(boost => {
      ...
      return `<div class="karma-shop-boost-item"><span class="karma-shop-boost-name">${KarmaShopEngine.NICE_NAMES[boost.id] || boost.id}</span><span class="karma-shop-boost-time">${this._fmtDuration(msLeft)}</span></div>`;
    }).join('');
    return `
  ...
  startBoostTicker() {
    ...
    const tick = () => {
      ...
      if (!box) { this._clearTicker(); return; }
    ...
    setTimeout(() => {
      if (document.getElementById('boosts-list-live')) this._boostTicker = setInterval(tick, 5000);
    }, 100);
  }
  ...
  renderShopItem(item) {
    ...
    return `
      <div class="card karma-shop-item" data-rarity="${item.rarity}" style="background:${this.getRarityColor(item.rarity)}">
        ...
        <div class="karma-shop-item-content">
          ...
          <!-- PATCHED: white-space:pre-line so \n renders as line break -->
          <p class="karma-shop-item-description" style="white-space:pre-line">${item.description}</p>
        </div>
      ...
      <div class="card karma-shop-history">
        ...
        <div class="karma-shop-history-list">
          ${purchaseHistory.slice(-10).reverse().map(purchase => {
            const it   = this.items.find(i => i.id === purchase.itemId);
            const date = new Date(purchase.timestamp).toLocaleDateString();
            return `<div class="karma-shop-history-item"><span class="karma-shop-history-item-name">${it?.icon || '📦'} ${it?.name || purchase.itemId}</span><span class="karma-shop-history-item-meta">${date} • ${purchase.cost} Karma</span></div>`;
          }).join('')}
        </div>
      </div>`;
  }
