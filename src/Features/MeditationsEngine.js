...
class MeditationsEngine {
  ...
  loadYouTubeAPI() {
    if (window.YT && window.YT.Player) { window.ytReady = true; return; }
  ...
  showMeditationSchedule(roomKey) {
    ...
    const now = Date.now(), cycleMs = cfg.cycleSec*1000, openMs = cfg.openSec*1000;
    ...
    const rows = Array.from({ length:6 }, (_, i) => {
      ...
      const badge      = isOpen   ? `<span style="background:#22c55e;color:white;font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;margin-left:8px;">OPEN NOW</span>`
                       : isInSess ? `<span style="background:rgba(239,68,68,0.15);color:#ef4444;font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;margin-left:8px;">IN SESSION</span>`
                       : '';
  ...
  buildMeditationCTA() {
    const calcCycle = (cycleSec, openSec) => {
      const now = Date.now(), cycleMs = cycleSec*1000, openMs = openSec*1000;
      const timeInCycle = now % cycleMs;
      if (timeInCycle < openMs) return null;
      const msUntilOpen = cycleMs - timeInCycle;
      const m = Math.floor(msUntilOpen / 60000), s = Math.floor((msUntilOpen % 60000) / 1000);
    ...
    const scheduleBtn = (roomKey) =>
      `<button type="button" onclick="event.stopPropagation();window.featuresManager.engines.meditations.showMeditationSchedule('${roomKey}')"
    ...
    return this.meditations.map(med => {
      ...
      return `
        <div class="meditation-card ${isLocked ? 'locked' : ''}"
          ...
          ${med.premium ? '<span class="premium-badge">PREMIUM</span>' : ''}
  ...
  // PATCHED: inject styles only once using ID guard
  renderStyles() {
    if (document.getElementById('meditations-engine-styles')) return '';
    return `
  ...
  attachEventListeners() {
    // No-op: player lifecycle managed by stopMeditation() / play methods
  }
  ...
  playAudio(id) {
    ...
    if (med.premium && !isPrivileged && !this.app.gamification?.state?.unlockedFeatures?.includes('advanced_meditations')) {
      this.app.showToast('Unlock Advanced Meditations in the Karma Shop!', 'info'); return;
    }
    this._play(med, false);
  }

  // PATCHED: removed dead isPrivileged2, reuses isPrivileged
  playVideo(id) {
    const med = this.meditations.find(m => m.id === id);
    if (!med) return;
    const isPrivileged = this.app.state?.currentUser?.isAdmin || this.app.state?.currentUser?.isVip;
      ...
      if (!this.ytPlayer || typeof this.ytPlayer.playVideo !== 'function') {
        ...
        this.ytPlayer = new YT.Player('yt-player-container', {
          ...
          events: {
            ...
            // PATCHED: use 'this' directly — no window.featuresManager reference
  ...
  // PATCHED: uses 'this' instead of window.featuresManager
  _handleYouTubeStateChange(event) {
    if (event.data === YT.PlayerState.ENDED && this.currentMeditation) {
  ...
  stopMeditation() {
    try {
      ...
      this._hideVideoPane();
    } catch (error) { console.error('Error stopping meditation:', error); }
  }
  ...
  updateRing(current, duration) {
    ...
    } catch (error) { console.error('Error updating ring:', error); }
  }
  ...
  onMeditationComplete() {
    try {
      ...
      const sessionData = {
        ...
        duration,
        chakra:           this.getChakraFromMeditation(this.currentMeditation.category),
      ...
      this.sessionStartTime = null;
    } catch (error) { console.error('Error completing meditation:', error); }
  }
  ...
  getChakraFromMeditation(category) {
    const map = { Grounding:'root', Energy:'sacral', Chakras:'heart', Spiritual:'crown', Healing:'heart', Manifestation:'solar', Premium:'crown' };
  ...
  openPDFGuide() {
    ...
    else this.app.showToast('PDF Guide is not yet available.', 'info');
  }
