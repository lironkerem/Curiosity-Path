...
class ReikiRoom extends PracticeRoom {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // CHAKRA DATA INITIALIZATION (OPTIMIZATION #1 - EXTERNAL JSON)
    // ═══════════════════════════════════════════════════════════════════════
    
    async initializeChakraData() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                this.loadInlineChakraData();
            }
        } catch (error) {
            console.warn('Failed to load external chakra data, using inline fallback');
            this.loadInlineChakraData();
        }
        
        // Build chakra options from schedule
        this.buildChakraOptions();
    }
    
    // Fallback: inline data if JSON file unavailable
    loadInlineChakraData() {
    ...
    buildChakraOptions() {
        ...
        // Link practices from schedule to options
    ...
    buildChakraDisplay(chakra, imageIndex, onPrev, onNext) {
        ...
        return `
            ...
            <div style="margin-bottom: 40px; padding: 32px; background: linear-gradient(135deg, rgba(139, 92, 246, 0.05) 0%, rgba(167, 139, 250, 0.02) 100%); border: 2px solid var(--border); border-radius: var(--radius-lg); text-align: center;">
                <h4 style="margin: 0 0 16px 0; font-size: 18px; font-weight: 600; color: var(--accent);">💭 ${chakra === this.CHAKRA_SCHEDULE[this.state.currentDay] ? "Today's" : "Guiding"} Inquiry</h4>
    ...
    startPersonalSession() {
        ...
        // Use shared chakra display component
