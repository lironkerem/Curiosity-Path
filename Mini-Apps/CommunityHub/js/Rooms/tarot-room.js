 ...
 *   - Daily card is now date-seeded: same card for all users, resets at midnight
 *   - switchTab() moved to shared TabRoomMixin pattern (inline here)
 *   - buildParticipantList() aligned with ReikiRoom (no duplication)
 *   - Chat input shows current user's real avatar
 *   - initializeTarotData() async race guard added (ready flag)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
class TarotRoom extends PracticeRoom {
    constructor() {
        ...
        // Initialize tarot data (async — sets _tarotDataReady when done)
    ...
    async _initializeTarotData() {
        ...
        // If onEnter already ran before data was ready, draw now
    ...
    /**
     * Draw the daily card using today's date seed.
     * All users get the same card on the same UTC day.
     * Result is cached in this.state.dailyCard.
     */
    _drawDailyCard() {
        const seed = this._todaySeed();
        const rng  = this._seededRng(seed);
        const deck = this._buildFullDeck();
        const idx  = Math.floor(rng() * deck.length);
        this.state.dailyCard = deck[idx];
    }
    ...
    _renderDailyCard() {
        ...
        container.innerHTML = this._buildCardDisplay(this.state.dailyCard);
    }
    ...
    onEnter() {
        ...
        // Draw daily card if data ready; otherwise set pending flag
    ...
    _buildCardDisplay(card) {
        return `
            ...
            <h4 style="font-family: var(--serif); font-size: 20px; margin: 8px 0; text-align: center;">
    ...
    _buildParticipantList() {
        // Delegates to the shared helper on PracticeRoom — no duplication with ReikiRoom.
