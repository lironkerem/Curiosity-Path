...
class PracticeRoom {
    ...
    async _blessRoom() {
        ...
        if (!blessData) {
            // DB call failed — still give visual feedback
            Core.showToast('Blessing sent ✨');
        ...
        // Always show animation for the clicker — whether inside or outside the room.
        // Users inside the room get it via _subscribeToBlessings realtime callback.
        // The clicker (who may be on the hub card, outside the room) gets it here.
        const senderPayload = blessData.profiles ? { name: blessData.profiles.name } : null;
        this._showBlessingAnimation(senderPayload);

        // Optimistically update the card badge immediately for the clicker
        ...
        if (myUid && !this._blessingRows.some(r => (r.user_id || r.userId) === myUid)) {
            this._blessingRows = [...this._blessingRows, { user_id: myUid, profiles: blessData.profiles }];
        }
