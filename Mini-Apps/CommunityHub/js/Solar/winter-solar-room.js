...
WinterSolarRoom.config = {
  ...
  startMonth: 11, startDay: 21, endMonth: 1, endDay: 19,
  floatingEmojis:  ['❄️', '⛄', '🌨️'],
...
WinterSolarRoom.practices = {
  intentionReflection: {
    id: 'intentionReflection', title: 'Intention and Reflection', hasSaveData: true,
    description: 'Consolidate insights and prepare for renewal', color: 'var(--ring-silent)',
  ...
  futureAlignment: {
    id: 'futureAlignment', title: 'Future Alignment Visualization',
    description: 'Integrate insight and prepare for future growth', color: 'var(--ring-available)',
  ...
  bodyGrounding: {
    id: 'bodyGrounding', title: 'Body Grounding and Breath',
    description: 'Signal safety, stability, and rejuvenation', color: 'var(--ring-deep)',
  ...
  energyAwareness: {
    id: 'energyAwareness', title: 'Energy and Awareness',
    description: 'Consolidate internal energy and protect vital reserves', color: 'var(--ring-resonant)',
  ...
  environmentalClearing: {
    id: 'environmentalClearing', title: 'Environmental Clearing',
    description: 'Create supportive external space for inner calm', color: 'var(--ring-silent)',
  ...
  roleShedding: {
    id: 'roleShedding', title: 'Role Shedding Practice',
    description: 'Consciously release or soften roles that drain energy', color: 'var(--ring-available)',
  ...
  paceRecalibration: {
    id: 'paceRecalibration', title: 'Pace Recalibration Practice',
    description: "Align personal tempo with Winter's stillness", color: 'var(--ring-deep)',
    icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>`,
  },
  relationshipWinterAudit: {
    id: 'relationshipWinterAudit', title: 'Relationship Winter Audit',
    description: 'Identify connections to rest, nurture, or let go', color: 'var(--ring-resonant)',
...
WinterSolarRoom.getIntentionReflectionContent = function() {
  return SolarConfig.generateIntentionPracticeContent(this.userData, this.prebuiltAffirmations, {
...
WinterSolarRoom.getBodyGroundingContent = function() {
  return SolarConfig.generateBodyPracticeContent({
    ...
    steps:          ['Stand or sit comfortably, feet hip-width apart, knees soft', 'Inhale deeply through the nose, exhale slowly through the mouth', 'Repeat 5 times'],
...
WinterSolarRoom.getEnergyAwarenessContent = function() {
  return SolarConfig.generateEnergyAwarenessContent({
    ...
    energySteps:     ['Place hands on heart and lower belly', 'Imagine energy gathering inward and settling into the center'],
...
WinterSolarRoom.getPaceRecalibrationContent = function() {
  return SolarConfig.generatePacePracticeContent({
    ...
    adjustmentExamples: ['Pause before starting tasks', 'Reduce transitions', 'Allow more rest periods'],
...
WinterSolarRoom.getRelationshipWinterAuditContent = function() {
  return SolarConfig.generateRelationshipAuditContent({
    ...
    actionExamples:    ['Express gratitude', 'Limit interaction', 'Step back', 'Simply observe'],
