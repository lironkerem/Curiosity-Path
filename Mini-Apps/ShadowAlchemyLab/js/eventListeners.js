/* js/eventListeners.js (PATCHED FOR BIG-APP INTEGRATION) */
...
export function attachDashboardListeners() {
  ...
  document.addEventListener('click', async e => {
    ...
    if (id === 'startNewJourney' || t.closest('#startNewJourney')) {
      e.preventDefault();
      const { openShadowGuidedProcessModal } = await import('/Mini-Apps/ShadowAlchemyLab/js/features/shadowGuidedProcess.js');
      return openShadowGuidedProcessModal();
    }
    if (id === 'openFreeJournal' || t.closest('#openFreeJournal')) {
      e.preventDefault();
      const { openShadowDialogueModal } = await import('/Mini-Apps/ShadowAlchemyLab/js/features/shadowDialogue.js');
      return openShadowDialogueModal();
    }
    if (id === 'openTriggerLog' || t.closest('#openTriggerLog')) {
      e.preventDefault();
      const { openTriggerReleaseModal } = await import('/Mini-Apps/ShadowAlchemyLab/js/features/triggerRelease.js');
      return openTriggerReleaseModal();
    }
    if (id === 'open-sub-shadows-lab' || t.closest('#open-sub-shadows-lab')) {
      e.preventDefault();
      const { openSubShadowsLabGrid } = await import('/Mini-Apps/ShadowAlchemyLab/js/features/subShadowsLabGrid.js');
    ...
    if (id === 'continue-last-session' || t.closest('#continue-last-session')) {
      ...
      if (engine.state.activeShadowId) {
        const { openSubShadowJourneyModal } = await import('/Mini-Apps/ShadowAlchemyLab/js/features/subShadowJourney.js');
    ...
    if (isEdit || isView || isDel) {
      ...
      if (isView) {
        if (entryType === 'journey') {
          ...
          if (entry) {
            const { openShadowGuidedProcessViewModal } = await import('/Mini-Apps/ShadowAlchemyLab/js/features/shadowGuidedProcessViewer.js');
