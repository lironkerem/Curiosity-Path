...
export class KarmaShopEngine {
  static NICE_NAMES = Object.freeze({
  ...
  static SKIP_CAP_LIMITS = Object.freeze({
    dailySkips:   2,
    weeklySkips:  2,
    monthlySkips: 3
  });

  static CAP_MAP = Object.freeze({
    skip_all_daily:   'dailySkips',
    skip_all_weekly:  'weeklySkips',
    skip_all_monthly: 'monthlySkips'
  });

  // Whitelist of valid item IDs — used to validate purchase calls
  static VALID_ITEM_IDS = new Set([
    'xp_multiplier', 'karma_multiplier', 'double_boost',
    'skip_all_daily', 'skip_all_weekly', 'skip_all_monthly',
    'advanced_meditations', 'shadow_alchemy_lab', 'advance_tarot_spreads',
    'luxury_champagne_gold_skin', 'royal_indigo_skin', 'earth_luxury_skin',
    'private_consultation', 'private_tarot_reading', 'reiki_healing'
  ]);
  ...
  async loadFromCloud() {
    try {
      ...
      if (data?.karmaShop) {
        ...
        if (shop.purchaseHistory) ls.set('karma_purchase_history', JSON.stringify(shop.purchaseHistory));
        ls.set('karma_active_boosts', JSON.stringify(this.activeBoosts));
        return;
      }
  ...
  /* ---------- Admin Helper ---------- */
  _isAdmin() { return Boolean(this.app.state.currentUser?.isAdmin); }
  ...
  _persistSkipCaps() {
    ...
    Object.entries(this.skipCaps).forEach(([type, obj]) => {
      ls.set(`karma_skip_caps_${type}`, JSON.stringify(obj));
    });
  }

  _weekKey(t)  { const d = new Date(t); const y = d.getFullYear(); const w = Math.ceil((d - new Date(y, 0, 1)) / 604800000); return `${y}-W${String(w).padStart(2, '0')}`; }
  _monthKey(t) { return new Date(t).toISOString().slice(0, 7); }
  _yearKey(t)  { return new Date(t).getFullYear().toString(); }
  ...
  _skipCapUsed(type) { return this.skipCaps[type]?.used || 0; }
  _skipCapMax(type)  { return KarmaShopEngine.SKIP_CAP_LIMITS[type] || 0; }
  ...
  saveActiveBoostsLocal(){ ls.set('karma_active_boosts', JSON.stringify(this.activeBoosts)); }
  saveActiveBoosts()     { this.saveActiveBoostsLocal(); this.saveToCloud(); }

  _getLocalStorage(key, fallback) {
    try { const d = ls.get(key); return d ? JSON.parse(d) : fallback; } catch { return fallback; }
  }

  /* ---------- Quest Reset Helpers ---------- */
  _nextQuestDailyReset()   { const t = new Date(); t.setHours(24, 0, 0, 0); if (t <= Date.now()) t.setDate(t.getDate() + 1); return t.getTime(); }
  _nextQuestWeeklyReset()  { const t = new Date(); const d = (7 - t.getDay()) % 7 || 7; t.setDate(t.getDate() + d); t.setHours(0, 0, 0, 0); return t.getTime(); }
  ...
  checkExpiredBoosts() {
    ...
    this.activeBoosts = this.activeBoosts.filter(b => {
      if (b.id.startsWith('skip_all_')) return now < (b.resetAt || 0);
  ...
  isBoostActive(boostId) {
    if (boostId.startsWith('skip_all_')) {
      ...
      return boost ? Date.now() < (boost.resetAt || 0) : false;
    }
    return this.activeBoosts.some(b => b.id === boostId);
  }

  activateBoost(boostId, duration) {
    if (this.app?.gamification?.activateBoost) this.app.gamification.activateBoost(boostId, duration);
  ...
  purchase(itemId) {
    // Whitelist check — itemId is used in DOM attribute and localStorage keys
    if (!KarmaShopEngine.VALID_ITEM_IDS.has(itemId)) {
      this.app.showToast('Invalid item', 'error');
      return false;
    }
    const item = this.items.find(i => i.id === itemId);
    ...
    if (KarmaShopEngine.CAP_MAP[itemId]) this._useSkipCap(KarmaShopEngine.CAP_MAP[itemId]);

    const history = this.getPurchaseHistory();
    if (history.length === 1) {
      this.app.gamification.grantBadge({ id: 'first_purchase', name: 'First Purchase', icon: '🛒', description: 'First purchase in the Karma Shop', xp: 50, rarity: 'epic' });
    }
  ...
  recordPurchase(itemId, cost) {
    ...
    ls.set('karma_purchase_history', JSON.stringify(history));
    this.saveToCloud();
  }
  ...
  applyItemEffect(itemId, item) {
    try {
      if      (itemId === 'xp_multiplier')    { this.activateBoost('xp_multiplier',    item.duration); this.app.showToast('2× XP active for 24 h!', 'success'); }
      else if (itemId === 'karma_multiplier')  { this.activateBoost('karma_multiplier', item.duration); this.app.showToast('2× Karma active for 24 h!', 'success'); }
      else if (itemId === 'double_boost')      { this.activateBoost('double_boost',     item.duration); this.app.showToast('2× XP + 2× Karma active for 48 h!', 'success'); }
      else if (itemId.startsWith('skip_all_')) this._applyQuestSkip(itemId);
      else if (itemId === 'advanced_meditations')    { this.safeUnlockFeature('advanced_meditations');    this.app.showToast('Advanced meditations unlocked!', 'success'); }
      else if (itemId === 'shadow_alchemy_lab')      { this.safeUnlockFeature('shadow_alchemy_lab');      this.app.showToast('Shadow Alchemy Lab unlocked!', 'success'); }
      else if (itemId === 'advance_tarot_spreads')   { this.safeUnlockFeature('advance_tarot_spreads'); this.safeUnlockFeature('tarot_vision_ai'); this.app.showToast('Advanced Tarot Spreads & Tarot Vision AI unlocked!', 'success'); }
      else if (itemId === 'luxury_champagne_gold_skin') { this.safeUnlockFeature('luxury_champagne_gold_skin'); this.app.showToast('Luxury Champagne-Gold Skin unlocked!', 'success'); }
      else if (itemId === 'royal_indigo_skin')          { this.safeUnlockFeature('royal_indigo_skin');   this.app.showToast('Royal Indigo Skin unlocked!', 'success'); }
      else if (itemId === 'earth_luxury_skin')          { this.safeUnlockFeature('earth_luxury_skin');   this.app.showToast('Earth Luxury Skin unlocked!', 'success'); }
  ...
  _applyQuestSkip(skipType) {
    ...
    let xp = 0, karma = 0;
    todo.forEach(q => { xp += q.xpReward ?? 0; karma += q.karmaReward ?? 0; q.completed = true; });
  ...
  showMasterPurchasePopup(item) {
    // Build the message safely — no user-controlled content in the WhatsApp URL path
    const userName = esc(this.app.state.currentUser?.name || 'Friend');
    const itemName = esc(item.name);
    const msg      = `${itemName} bought using ${item.cost} Karma for ${userName}.`;
    const waMsg    = encodeURIComponent(`${item.name} bought using ${item.cost} Karma for ${this.app.state.currentUser?.name || 'Friend'}.`);

    const overlay = document.createElement('div');
    overlay.className = 'karma-shop-master-overlay';

    const card = document.createElement('div');
    card.className = 'card karma-shop-master-card';

    const icon = document.createElement('div');
    icon.className = 'karma-shop-master-icon';
    icon.textContent = '🧘';

    const title = document.createElement('h3');
    title.className = 'karma-shop-master-title';
    title.textContent = 'Meet the Master';

    const msgEl = document.createElement('p');
    msgEl.className = 'karma-shop-master-message';
    msgEl.textContent = `${item.name} bought using ${item.cost} Karma for ${this.app.state.currentUser?.name || 'Friend'}.`;

    const instructions = document.createElement('p');
    instructions.className = 'karma-shop-master-instructions';
    instructions.textContent = 'Screenshot or save this message, then contact Aanandoham via WhatsApp to schedule your session:';

    const waLink = document.createElement('a');
    waLink.href    = `https://wa.me/+972524588767?text=${waMsg}`;
    waLink.target  = '_blank';
    waLink.rel     = 'noopener noreferrer';
    waLink.className = 'btn btn-primary karma-shop-master-btn-wa';
    waLink.textContent = 'Open WhatsApp';

    const closeBtn = document.createElement('button');
    closeBtn.type = 'button';
    closeBtn.className = 'btn btn-secondary karma-shop-master-btn-close';
    closeBtn.textContent = 'Close';
    closeBtn.addEventListener('click', () => overlay.remove());

    card.append(icon, title, msgEl, instructions, waLink, closeBtn);
  ...
  render() {
          ...
          ${categories.map(category => {
            ...
            return `
              <div class="karma-shop-category">
                <h3 class="karma-shop-category-title">${esc(category)}</h3>
    ...
    this._attachPurchaseHandlers(tab);
    this.startBoostTicker();
  }

  /** Attach purchase button listeners via delegation — avoids inline onclick with itemId injection */
  _attachPurchaseHandlers(tab) {
    tab.addEventListener('click', e => {
      const btn = e.target.closest('[data-purchase-id]');
      if (!btn || btn.disabled) return;
      const itemId = btn.dataset.purchaseId;
      if (KarmaShopEngine.VALID_ITEM_IDS.has(itemId)) this.purchase(itemId);
    });
  }

  renderActiveBoosts() {
    this.checkExpiredBoosts();
    if (!this.activeBoosts.length) return '';
    const content = this.activeBoosts.map(boost => {
      ...
      return `
        <div class="karma-shop-boost-item">
          <span class="karma-shop-boost-name">${esc(KarmaShopEngine.NICE_NAMES[boost.id] || boost.id)}</span>
    ...
    return `
      <div class="card karma-shop-boosts">
        <h3 class="karma-shop-boosts-title" style="display:flex;align-items:center;gap:0.5rem;">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" aria-hidden="true" focusable="false"><rect width="16" height="10" x="2" y="7" rx="2" ry="2"/><path d="M22 11v2"/></svg>
          Active Boosts
        </h3>
        <div class="karma-shop-boosts-list" id="boosts-list-live">${content}</div>
      </div>`;
  }
  ...
  startBoostTicker() {
    ...
    const tick = () => {
      ...
      if (!box) { console.warn('[KarmaShop] boosts-list-live not found'); this._clearTicker(); return; }
    ...
    setTimeout(() => {
      if (document.getElementById('boosts-list-live')) {
  ...
  renderShopItem(item) {
    ...
    // Determine button label — no inline onclick
    let btnLabel;
    if (isOwned)       btnLabel = '✓ Owned';
    else if (isActive) btnLabel = '✓ Active';
    else if (canBuy.can) btnLabel = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="display:inline;vertical-align:middle;margin-right:0.3rem;" aria-hidden="true" focusable="false"><circle cx="8" cy="21" r="1"/><circle cx="19" cy="21" r="1"/><path d="M2.05 2.05h2l2.66 12.42a2 2 0 0 0 2 1.58h9.78a2 2 0 0 0 1.95-1.57l1.65-7.43H5.12"/></svg>Purchase`;
    else               btnLabel = esc(canBuy.reason);

    return `
      <div class="card karma-shop-item" data-rarity="${esc(item.rarity)}" style="background:${rarityColor}">
        ${badgeText ? `<div class="karma-shop-item-owned-badge">${esc(badgeText)}</div>` : ''}
        <div class="karma-shop-item-content">
          <div class="karma-shop-item-icon">${esc(item.icon)}</div>
          <h4 class="karma-shop-item-name">${esc(item.name)}</h4>
          <p class="karma-shop-item-description">${esc(item.description)}</p>
        </div>
        <div class="karma-shop-item-footer">
          <div class="karma-shop-item-meta">
            <span class="karma-shop-item-rarity karma-shop-rarity-${esc(item.rarity)}">${esc(item.rarity)}</span>
            <span class="karma-shop-item-rarity karma-shop-item-cost karma-shop-rarity-${esc(item.rarity)}">
              ${item.cost}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="display:inline;vertical-align:middle;width:14px;height:14px;" aria-hidden="true" focusable="false"><path d="M6 3h12l4 6-10 13L2 9Z"/><path d="M11 3 8 9l4 13 4-13-3-6"/><path d="M2 9h20"/></svg>
            </span>
          </div>
          <button type="button"
                  data-purchase-id="${esc(item.id)}"
                  class="btn ${canBuy.can ? 'btn-primary' : 'btn-secondary'} karma-shop-item-btn"
                  ${(!canBuy.can || isOwned || isActive) ? 'disabled' : ''}
                  aria-label="${isOwned ? 'Already owned' : isActive ? 'Currently active' : canBuy.can ? `Purchase ${esc(item.name)}` : esc(canBuy.reason)}">
            ${btnLabel}
          </button>
        </div>
      </div>`;
  ...
  renderPurchaseHistory(purchaseHistory) {
    return `
      <div class="card karma-shop-history">
        <h3 class="karma-shop-history-title" style="display:flex;align-items:center;gap:0.5rem;">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" aria-hidden="true" focusable="false"><path d="M15 12h-5"/><path d="M15 8h-5"/><path d="M19 17V5a2 2 0 0 0-2-2H4"/><path d="M8 21h12a2 2 0 0 0 2-2v-1a1 1 0 0 0-1-1H11a1 1 0 0 0-1 1v1a2 2 0 1 1-4 0V5a2 2 0 1 0-4 0v2a1 1 0 0 0 1 1h3"/></svg>
          Purchase History
        </h3>
          ...
          ${purchaseHistory.slice(-10).reverse().map(purchase => {
            ...
            return `
              <div class="karma-shop-history-item">
                <span class="karma-shop-history-item-name">${esc(item?.icon || '📦')} ${esc(item?.name || purchase.itemId)}</span>
                <span class="karma-shop-history-item-meta">${esc(date)} • ${purchase.cost} Karma</span>
              </div>`;
          }).join('')}
        </div>
      </div>`;
  ...
  getRarityColor(rarity) {
    const gradients = {
      common:    'linear-gradient(135deg,rgba(245,245,247,0.85) 0%,rgba(210,214,220,0.85) 100%)',
      uncommon:  'linear-gradient(135deg,rgba(0,224,132,0.85) 0%,rgba(0,185,108,0.85) 100%)',
      rare:      'linear-gradient(135deg,rgba(0,168,255,0.85) 0%,rgba(0,123,204,0.85) 100%)',
      epic:      'linear-gradient(135deg,rgba(184,0,230,0.85) 0%,rgba(142,0,204,0.85) 100%)',
      legendary: 'linear-gradient(135deg,rgba(255,195,0,0.85) 0%,rgba(255,135,0,0.85) 100%)'
    };
