...
class HappinessEngine {
  ...
  async loadBoosters() {
    try {
      ...
      console.error('HappinessEngine: Failed to load boosters:', e);
      this.boostersLoaded = true; // use defaults
    }
  }
  ...
  trackView() {
    ...
    const data  = this._getStorageData(today);
    data.count += 1;
    ls.set('daily_booster_views', JSON.stringify(data));
  ...
  _getStorageData(today) {
    try {
      const stored = ls.get('daily_booster_views');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed.date === today) return parsed;
      }
    } catch (e) {
      console.error('HappinessEngine: Failed to parse storage data:', e);
    }
  ...
  _toggleQuestCompleteBanner(viewCount) {
    ...
    if (viewCount >= HappinessEngine.QUEST_TARGET && !banner) {
      ...
      // Use textContent / DOM construction — no unsafe innerHTML
      const p = document.createElement('p');
      p.className = 'text-center';
      p.style.cssText = 'color:#22c55e;display:flex;align-items:center;justify-content:center;gap:0.5rem;';
      p.textContent = `Daily quest complete! Keep exploring if you'd like!`;
  ...
  refreshQuote()       { this._refreshContent('Quote',       this.getRandomQuote,       'quote-card',   'quote',       'Quote',       this._formatQuote); }
  refreshAffirmation() { this._refreshContent('Affirmation', this.getRandomAffirmation, 'affirm-card',  'affirmation', 'Affirmation', this._formatAffirmation); }
  refreshInquiry()     { this._refreshContent('Inquiry',     this.getRandomInquiry,     'inquiry-card', 'inquiry',     'Inquiry',     this._formatInquiry); }

  // ── HTML Formatters ────────────────────────────────────────────────────────

  _refreshBtn(method) {
    return `<button type="button" onclick="window.featuresManager.engines.happiness.${method}()" class="btn btn-secondary" style="display:inline-flex;align-items:center;gap:0.5rem;" aria-label="Refresh ${method.replace('refresh','')}"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" aria-hidden="true" focusable="false"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg> Refresh</button>`;
  }

  _formatBooster(booster) {
    return `
      <div class="flex items-center" style="margin-bottom:1rem;">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="margin-right:0.5rem;flex-shrink:0;" aria-hidden="true" focusable="false"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>
        <h2 class="text-2xl font-bold" style="color:var(--neuro-text);">A Quick Happiness Booster</h2>
      </div>
      <div class="text-center">
        <h3 class="text-2xl font-bold" style="color:var(--neuro-accent);">${esc(booster.title)}</h3>
        <p class="mt-2 text-lg">${esc(booster.description)}</p>
        <div class="mt-4 text-sm" style="color:var(--neuro-text-light);">
          <span>${esc(booster.duration)}</span> • <span>${esc(booster.category)}</span> • <span>${esc(booster.emoji)}</span>
        </div>
      </div>
      <div style="margin-top:2rem;display:flex;justify-content:flex-end;">${this._refreshBtn('refreshBooster')}</div>`;
  }

  _formatQuote(quote) {
    return `
      <div class="flex items-center" style="margin-bottom:1rem;">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="margin-right:0.5rem;flex-shrink:0;" aria-hidden="true" focusable="false"><path d="M15 12h-5"/><path d="M15 8h-5"/><path d="M19 17V5a2 2 0 0 0-2-2H4"/><path d="M8 21h12a2 2 0 0 0 2-2v-1a1 1 0 0 0-1-1H11a1 1 0 0 0-1 1v1a2 2 0 1 1-4 0V5a2 2 0 1 0-4 0v2a1 1 0 0 0 1 1h3"/></svg>
        <h2 class="text-2xl font-bold" style="color:var(--neuro-text);">Inspirational Quote</h2>
      </div>
      <p class="text-2xl font-semibold text-center" style="color:var(--neuro-accent);">"${esc(quote.text)}"</p>
      <p class="mt-3 text-center text-lg" style="color:var(--neuro-text);">- ${esc(quote.author)}</p>
      <div style="margin-top:2rem;display:flex;justify-content:flex-end;">${this._refreshBtn('refreshQuote')}</div>`;
  }

  _formatAffirmation(affirmation) {
    return `
      <div class="flex items-center" style="margin-bottom:1rem;">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="margin-right:0.5rem;flex-shrink:0;" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="10"/><path d="M8 13s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>
        <h2 class="text-2xl font-bold" style="color:var(--neuro-text);">Positive Affirmation</h2>
      </div>
      <p class="text-2xl font-semibold text-center" style="color:var(--neuro-accent);">"${esc(affirmation)}"</p>
      <div style="margin-top:2rem;display:flex;justify-content:flex-end;">${this._refreshBtn('refreshAffirmation')}</div>`;
  }
  ...
  _formatInquiry(inquiry) {
    ...
    return `
      <div class="flex items-center" style="margin-bottom:1rem;">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="margin-right:0.5rem;flex-shrink:0;" aria-hidden="true" focusable="false"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/></svg>
      ...
      <div class="text-center">
        <div style="margin-bottom:1rem;padding:0.5rem;background:var(--neuro-bg-secondary);border-radius:8px;display:inline-block;">
          <span style="font-size:0.75rem;text-transform:uppercase;font-weight:700;color:var(--neuro-accent);">${esc(inquiry.domain)}</span>
        </div>
        <p class="text-2xl font-semibold" style="color:var(--neuro-accent);line-height:1.4;margin-bottom:1rem;">${esc(inquiry.question)}</p>
        <p class="mt-2 text-lg" style="font-style:italic;color:var(--neuro-text-secondary);">${esc(inquiry.holding)}</p>
        <div class="mt-4 text-sm" style="color:var(--neuro-text-light);">
          <span>Level ${esc(String(inquiry.intensity))}</span> • <span>${emoji} Self-Inquiry</span>
        </div>
      </div>
      <div style="margin-top:2rem;display:flex;justify-content:flex-end;">${this._refreshBtn('refreshInquiry')}</div>`;
  }
      ...
      <div style="padding:1.5rem;min-height:100vh;">
        <div class="universal-content">
          <header class="main-header project-curiosity"
                  style="--header-img:url('/public/Tabs/NavHappiness.webp');
                         --header-title:'';
          ...
          <div class="flex items-center justify-between" style="margin-bottom:2rem;padding:1rem;background:rgba(102,126,234,0.05);border-radius:8px;">
            ...
            <span class="badge ${viewCount >= HappinessEngine.QUEST_TARGET ? 'badge-success' : 'badge-primary'}" aria-live="polite">
              ${viewCount} / ${HappinessEngine.QUEST_TARGET} (Quest)
            </span>
          </div>

          ${viewCount >= HappinessEngine.QUEST_TARGET ? `
            <div id="happiness-quest-complete" role="status" style="margin-bottom:2rem;padding:1rem;border-radius:0.5rem;background:rgba(34,197,94,.08);border:1px solid rgba(34,197,94,.25);">
              <p class="text-center" style="color:#22c55e;display:flex;align-items:center;justify-content:center;gap:0.5rem;">
                ...
                Daily quest complete! Keep exploring if you'd like!
              </p>
            </div>
          ` : ''}
          ...
          <main class="space-y-6">
            ${this._renderCard('affirm-card', 'affirmation', 'Positive Affirmation',
              `<p class="text-2xl font-semibold text-center" style="color:var(--neuro-accent);">"${esc(this.currentAffirmation)}"</p>`,
              'refreshAffirmation')}

            ${this._renderCard('quote-card', 'quote', 'Inspirational Quote',
              `<p class="text-2xl font-semibold text-center" style="color:var(--neuro-accent);">"${esc(this.currentQuote.text)}"</p>
               <p class="mt-3 text-center text-lg" style="color:var(--neuro-text);">- ${esc(this.currentQuote.author)}</p>`,
              'refreshQuote')}

            ${this._renderCard('booster-card', 'booster', 'A Quick Happiness Booster',
              `<h3 class="text-2xl font-bold" style="color:var(--neuro-accent);">${esc(this.currentBooster.title)}</h3>
               <p class="mt-2 text-lg">${esc(this.currentBooster.description)}</p>
               <div class="mt-4 text-sm" style="color:var(--neuro-text-light);">
                 <span>${esc(this.currentBooster.duration)}</span> • <span>${esc(this.currentBooster.category)}</span>
               </div>`,
              'refreshBooster')}

            ${this._renderCard('inquiry-card', 'inquiry', 'Self Inquiry',
              `<div style="margin-bottom:1rem;padding:0.5rem;background:var(--neuro-bg-secondary);border-radius:8px;display:inline-block;">
                 <span style="font-size:0.75rem;text-transform:uppercase;font-weight:700;color:var(--neuro-accent);">${esc(this.currentInquiry.domain)}</span>
               </div>
               <p class="text-2xl font-semibold" style="color:var(--neuro-accent);line-height:1.4;margin-bottom:1rem;">${esc(this.currentInquiry.question)}</p>
               <p class="mt-2 text-lg" style="font-style:italic;color:var(--neuro-text-secondary);">${esc(this.currentInquiry.holding)}</p>
               <div class="mt-4 text-sm" style="color:var(--neuro-text-light);">
                 <span>Level ${esc(String(this.currentInquiry.intensity))}</span> • <span>Self-Inquiry</span>
               </div>`,
              'refreshInquiry')}
  ...
  _renderCard(id, iconKey, title, content, refreshMethod) {
    const ICONS = {
      ...
      booster:     `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="margin-right:0.5rem;flex-shrink:0;" aria-hidden="true" focusable="false"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>`,
      inquiry:     `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="margin-right:0.5rem;flex-shrink:0;" aria-hidden="true" focusable="false"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/></svg>`
    };
    const icon = ICONS[iconKey] || '';
    return `
      <div class="neuro-card flip-card" id="${esc(id)}">
        <div class="flip-card-inner">
          <div class="flip-card-front">
            <div class="flex items-center" style="margin-bottom:1rem;">
              ${icon}
              <h2 class="text-2xl font-bold" style="color:var(--neuro-text);">${esc(title)}</h2>
            </div>
            <div class="text-center">${content}</div>
            <div style="margin-top:2rem;display:flex;justify-content:flex-end;">
              <button type="button"
                      onclick="window.featuresManager.engines.happiness.${refreshMethod}()"
                      class="btn btn-secondary"
                      style="display:inline-flex;align-items:center;gap:0.5rem;"
                      aria-label="Refresh ${esc(title)}">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" aria-hidden="true" focusable="false"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>
                Refresh
              </button>
            </div>
          </div>
