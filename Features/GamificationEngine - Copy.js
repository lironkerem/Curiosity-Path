...
async reloadFromDatabase() {
  ...
  hasActiveXPBoost() {
    try {
      ...
      console.error('Error checking XP boost:', error);
      return false;
    }
  }
  ...
  updateStreak() {
    ...
    const last = this.state.streak.lastCheckIn ? 
                 new Date(this.state.streak.lastCheckIn) : null;
...
// =========================================================
//  GamificationEngine.js — Optimized (Part 2)
//  Badge Definitions & Category-Based Checking
// =========================================================

// Add these methods to the GamificationEngine class from Part 1
...
// =========================================================
//  GamificationEngine.js — Optimized (Part 3)
//  Cross-Feature Badges, Badge Granting, Chakra, Quests
// =========================================================

// Add these methods to the GamificationEngine class
  ...
  checkCrossFeatureBadges(badges) {
    ...
    // Daily checks
    const todayGratitude = (data.gratitudeEntries || [])
      .some(e => new Date(e.timestamp).toDateString() === today);
    const todayJournal = (data.journalEntries || [])
      .some(e => new Date(e.timestamp).toDateString() === today);
    const todayEnergy = (data.energyEntries || [])
      .some(e => new Date(e.timestamp).toDateString() === today);
    const todayTarot = (data.tarotReadings || [])
      .some(e => new Date(e.timestamp).toDateString() === today);
    const todayMeditation = (data.meditationHistory || [])
      .some(e => new Date(e.timestamp).toDateString() === today);

    const todayFeatures = [
      todayGratitude, todayJournal, todayEnergy, todayTarot, todayMeditation
    ].filter(Boolean).length;

    if (todayFeatures >= 3) {
      this.checkAndGrantBadge('triple_threat', badges);
    }
  ...
  /* ---------------------------------------------------------
     ALL BADGES CHECK (for initialization)
  --------------------------------------------------------- */
  checkAllBadges() {
  ...
  /* ---------------------------------------------------------
     BADGE UNLOCK — XP + KARMA BY RARITY
  --------------------------------------------------------- */
  grantBadge(badge) {
    ...
    // 3. XP reward
    if (badge.xp) {
      this.addXP(badge.xp, `Badge: ${badge.id}`);
    }

    // 4. Inspirational toast
    if (badge.inspirational) {
      this.emit('inspirationalMessage', badge.inspirational);
    }
  ...
  progressEnergyCheckin(timeOfDay) {
    ...
    const key = timeOfDay === 'day' ? 'day' : 
                timeOfDay === 'night' ? 'night' : null;
  ...
  progressQuest(questType, questId, increment = 1) {
    ...
    if (quest.progress >= quest.target) {
      ...
      if (quest.karmaReward) {
        this.state.karma += quest.karmaReward;
      }
      
      if (quest.badge) {
        this.grantBadge(quest.badge);
      }
    ...
    if (allComplete) {
      if (type === 'daily') {
        this.state.dailyQuestCompletions = 
          (this.state.dailyQuestCompletions || 0) + 1;
        this.state.dailyQuestStreak = 
          (this.state.dailyQuestStreak || 0) + 1;
      } else if (type === 'weekly') {
        ...
        this.state.weeklyQuestCompletions = 
          (this.state.weeklyQuestCompletions || 0) + 1;
        ...
        this.state.monthlyQuestCompletions = 
          (this.state.monthlyQuestCompletions || 0) + 1;
      }
...
// =========================================================
//  GamificationEngine.js — Optimized (Part 4 - Final)
//  Feature Unlocks, Logging, Status, Level-Up Spectacle
// =========================================================

// Add these final methods to the GamificationEngine class
