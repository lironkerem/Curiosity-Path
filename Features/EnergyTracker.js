 ...
 * Overall energy, chakra-specific ratings, mood tagging, day/night check-ins,
 * weekly trends, historical journal
 */
class EnergyEngineEnhanced {
  static CHAKRAS = Object.freeze([
  ...
  // Whitelist of valid mood tags — used for data-mood attribute and classList toggle
  static MOODS = Object.freeze([
    'grounded', 'anxious', 'calm', 'happy', 'creative',
    'tired', 'focused', 'grateful', 'curious', 'confident'
  ]);

  static MOOD_EMOJIS = Object.freeze({
  ...
  getTodayCheckin() {
    try {
      ...
      const found   = entries.find(e => this.getISODate(e.timestamp) === today);
      if (found?.chakras) {
        return {
          overallEnergy:  found.energy || 6,
          moodTags:       found.moodTags || [],
          chakras:        found.chakras,
          notes:          found.notes || '',
          practicesDone:  found.practicesDone || [],
          timestamp:      found.timestamp,
          dayCheckin:     found.dayCheckin  || false,
          nightCheckin:   found.nightCheckin || false
        };
      }
      return this.createDefaultCheckin();
    } catch (err) {
      console.error('EnergyTracker: Error loading check-in:', err);
      return this.createDefaultCheckin();
    }
  }
  ...
  saveCheckin(commit = false) {
    try {
      ...
      // Sanitise notes
      const safeNotes = String(this.currentCheckin.notes || '').slice(0, EnergyEngineEnhanced.MAX_NOTES_LEN);
      ...
      console.error('EnergyTracker: Error saving check-in:', err);
  ...
  getChakraAverages() {
    ...
    const averages = {};
    EnergyEngineEnhanced.CHAKRAS.forEach(c => { averages[c.key] = Math.round((totals[c.key] / count) * 10) / 10; });
    return averages;
  }
  ...
  render() {
    ...
    try {
      ...
      console.error('EnergyTracker: Render error:', err);
      ...
      <div class="community-link-card" style="padding-top:0;">
        ...
        <div style="display:flex;gap:0.75rem;flex-wrap:wrap;">
          ...
          <button type="button"
                  ...
                  class="btn btn-primary"
                  style="display:inline-flex;align-items:center;justify-content:center;gap:0.5rem;flex:1 1 40%;white-space:nowrap;"
                  aria-label="Enter the Reiki Room">
  ...
  buildCheckinHeader(stats, greeting, period, checkinStatus) {
    return `
      <div class="flex items-center justify-between" style="margin-bottom:2rem;">
        <div>
          <h3 class="text-2xl font-bold" style="color:var(--neuro-text);">Good ${esc(greeting)}</h3>
        ...
        <div class="text-right">
          ...
          <p class="text-sm" style="color:var(--neuro-text-light);">Streak: ${esc(String(stats.currentStreak))} day(s)</p>
          <div class="flex gap-2 mt-2 justify-end text-xs">
            <span class="${this.currentCheckin.dayCheckin ? 'badge badge-success' : 'badge'}"
                  ...
                  aria-label="Day check-in ${this.currentCheckin.dayCheckin ? 'complete' : 'pending'}">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="width:12px;height:12px;" aria-hidden="true" focusable="false"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M6.34 17.66l-1.41 1.41M19.07 4.93l-1.41 1.41"/></svg>
              ${this.currentCheckin.dayCheckin ? '✓' : ''}
            </span>
            <span class="${this.currentCheckin.nightCheckin ? 'badge badge-success' : 'badge'}"
                  style="padding:4px 8px;display:inline-flex;align-items:center;gap:4px;"
                  aria-label="Night check-in ${this.currentCheckin.nightCheckin ? 'complete' : 'pending'}">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="width:12px;height:12px;" aria-hidden="true" focusable="false"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"/></svg>
  ...
  buildOverallEnergySlider() {
    return `
      <div style="margin-bottom:2rem;">
        <label class="form-label" for="energy-overall-slider">Overall Energy Level</label>
        <div class="flex items-center gap-4">
          <input type="range" id="energy-overall-slider"
                 min="0" max="10" step="0.5"
                 value="${this.currentCheckin.overallEnergy}"
                 class="flex-1"
                 aria-label="Overall energy level (0–10)"
                 aria-valuenow="${this.currentCheckin.overallEnergy}"
  ...
  buildMoodSelector() {
    return `
      <div style="margin-bottom:2rem;">
        ...
        <div id="mood-chips" class="flex flex-wrap gap-2" role="group" aria-label="Mood selection">
          ${EnergyEngineEnhanced.MOODS.map(mood => `
            <button type="button" class="chip ${this.currentCheckin.moodTags.includes(mood) ? 'active' : ''}"
                    data-mood="${esc(mood)}"
                    aria-pressed="${this.currentCheckin.moodTags.includes(mood) ? 'true' : 'false'}">
    ...
    return `
      <div style="margin-bottom:2rem;">
        ...
        <textarea id="energy-notes" class="form-input"
                  ...
                  maxlength="${EnergyEngineEnhanced.MAX_NOTES_LEN}"
                  aria-label="Energy notes"
        >${esc(this.currentCheckin.notes || '')}</textarea>
      </div>
    `;
  }
  ...
  buildActionButtons(period) {
    return `
      <div class="flex gap-3 flex-wrap">
        <button type="button" id="btn-save-checkin" class="btn btn-primary"
                style="display:inline-flex;align-items:center;gap:0.5rem;"
                aria-label="Save ${period === 'day' ? 'day' : 'night'} check-in">
        ...
        <button type="button" id="btn-reset-today" class="btn btn-secondary"
                style="display:inline-flex;align-items:center;gap:0.5rem;"
  ...
  buildChakraRow() {
    return EnergyEngineEnhanced.CHAKRAS.map(chakra => {
      ...
      return `
        <div class="chakra-mini-card" data-chakra="${esc(chakra.key)}">
          <div class="chakra-pulse"
               style="box-shadow:0 0 ${pulseSize}px ${chakra.color}${pulseHex};opacity:${intensity > 0 ? 1 : 0};transform:scale(${1 + intensity * 0.01})">
          </div>
          <div class="chakra-icon" style="background:${chakra.color}" aria-hidden="true">
            ${chakra.name.charAt(0)}
          </div>
          <div style="font-size:13px;font-weight:700;text-align:center">${esc(chakra.name)}</div>
          <input type="range" class="chakra-slider" data-chakra="${esc(chakra.key)}"
                 min="0" max="10" step="0.5" value="${value}" style="width:100%"
                 aria-label="${esc(chakra.name)} energy level"
                 aria-valuenow="${value}" aria-valuemin="0" aria-valuemax="10">
          <div class="chakra-value" style="font-size:13px;font-weight:700" aria-live="polite">${value}</div>
        </div>
      `;
    }).join('');
  }
          ...
          <div class="grid grid-cols-4 gap-2 mt-3 text-xs text-center">
            ${EnergyEngineEnhanced.CHAKRAS.map(c => `
              <div>
                ...
                <div class="text-gray-500">${esc(c.name)}</div>
              </div>
            `).join('')}
          </div>
        </div>
    ...
    return `
      <div class="card calc-expandable-card" id="journal-collapsible-card">
        <div class="calc-expandable-header" id="journal-collapsible-header"
             role="button" tabindex="0" aria-expanded="false" aria-controls="journal-collapsible-content"
             onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();this.click();}">
          <span class="chevron" aria-hidden="true">›</span>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor"
               stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
               style="color:var(--neuro-accent);flex-shrink:0;" aria-hidden="true" focusable="false">
        ...
        <div class="calc-expandable-content" id="journal-collapsible-content">
          <div style="margin-bottom:2rem;">
            <label for="journal-search" class="form-label">Search</label>
            <input type="search" id="journal-search" class="form-input"
      ...
      return `
        <div class="card text-center" style="padding:4rem;">
          <div style="display:flex;justify-content:center;margin-bottom:1rem;">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="width:56px;height:56px;color:var(--neuro-text-light);" aria-hidden="true" focusable="false"><rect width="8" height="4" x="8" y="2" rx="1" ry="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/></svg>
          </div>
    ...
    return `
      <div class="card" style="border-left:4px solid var(--neuro-accent)">
        <div class="flex justify-between items-start" style="margin-bottom:1rem;">
          <div>
            <div class="font-bold text-lg" style="color:var(--neuro-text)">${esc(dateStr)}</div>
            <div class="text-sm" style="color:var(--neuro-text-light)">${esc(timeStr)}</div>
      ...
      <div class="flex flex-wrap gap-2" style="margin-bottom:1rem;">
        ${entry.moodTags.map(mood => `
          <span class="badge" style="font-size:0.85rem">
            ${this.getMoodEmoji(mood)} ${esc(this.capitalize(mood))}
          </span>
        `).join('')}
      </div>
    `;
  ...
  buildEntryNotes(entry) {
    ...
    return `<div style="color:var(--neuro-text);line-height:1.6;white-space:pre-wrap">${esc(entry.notes)}</div>`;
  }
        ...
        <div class="grid grid-cols-4 md:grid-cols-7 gap-2">
          ${EnergyEngineEnhanced.CHAKRAS.map(c => `
            <div class="text-center">
              <div class="text-xs" style="color:var(--neuro-text-light);margin-bottom:0.25rem;">${esc(c.name)}</div>
  ...
  renderWeeklyChart(points) {
    ...
    const GAP          = DAY_WIDTH * GAP_PCT;
    const BAR_WIDTH    = DAY_WIDTH * NARROW - GAP;
    ...
    let svg = `<svg viewBox="0 ${-TOP_SPACE} ${WIDTH} ${VIEW_HEIGHT}" style="width:100%;height:100%;display:block;" role="img" aria-label="Weekly energy chart">`;
    ...
    const average = nonZero.length
      ? (nonZero.reduce((s, v) => s + v, 0) / nonZero.length).toFixed(1)
      : '-';
  ...
  renderRadarChart(values, size) {
    ...
    const points = keys.map((key, i) => {
      ...
      return `${centerX + Math.cos(angle) * maxRadius * (value / 10)},${centerY + Math.sin(angle) * maxRadius * (value / 10)}`;
    }).join(' ');

    return `
      <svg viewBox="0 0 ${size} ${size}" style="max-width:100%;height:auto" role="img" aria-label="Chakra balance radar chart">
        ${[0.25, 0.5, 0.75, 1].map(ratio => `
          <circle cx="${centerX}" cy="${centerY}" r="${maxRadius * ratio}" stroke="#e6eef4" fill="none" stroke-width="1"/>
        `).join('')}
        ${keys.map((key, i) => {
          const angle = (Math.PI * 2 * i) / keys.length - Math.PI / 2;
          return `<line x1="${centerX}" y1="${centerY}" x2="${centerX + Math.cos(angle) * maxRadius}" y2="${centerY + Math.sin(angle) * maxRadius}" stroke="#eef6fa" stroke-width="1"/>`;
        }).join('')}
        ...
        ${EnergyEngineEnhanced.CHAKRAS.map((c, i) => {
          ...
          return `<text x="${x}" y="${y}" text-anchor="middle" dominant-baseline="middle" font-size="10" fill="var(--neuro-text-light)">${esc(c.name)}</text>`;
        }).join('')}
      </svg>
    `;
  }
  ...
  attachNotesListener() {
    ...
    if (notes) {
      notes.addEventListener('input', e => {
        this.currentCheckin.notes = e.target.value.slice(0, EnergyEngineEnhanced.MAX_NOTES_LEN);
      });
    }
  }
  ...
  attachMoodListeners() {
    if (!this._boundMoodHandler) {
      this._boundMoodHandler = e => {
        ...
        // Validate against whitelist before modifying state
        if (!EnergyEngineEnhanced.MOODS.includes(mood)) return;
  ...
  attachChakraListeners() {
    // Whitelist of valid chakra keys
    const validChakras = new Set(EnergyEngineEnhanced.CHAKRAS.map(c => c.key));
    document.querySelectorAll('.chakra-slider').forEach(slider => {
      slider.addEventListener('input', e => {
        const key = e.target.dataset.chakra;
        if (!validChakras.has(key)) return; // whitelist guard
        const value = parseFloat(e.target.value);
  ...
  attachButtonListeners() {
    this.getElement('btn-save-checkin')?.addEventListener('click', () => this.saveCheckin(false));
    this.getElement('btn-reset-today')?.addEventListener('click', () => {
  ...
  buildStyles() {
    return `
      <style>
        ...
        @media (max-width:768px) { .chakra-row { flex-wrap:wrap; } .chakra-mini-card { min-width:calc(50% - 6px); } }
      </style>
    `;
  }
