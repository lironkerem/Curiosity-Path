      ...
      <div style="padding:1.5rem;min-height:100vh;">
        <div class="universal-content">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6" style="margin-bottom: 3rem;">
            ${Object.entries(this.spreads).map(([a,l])=>{var g,h,w,c,r,o,u;const d=["options","pyramid","cross"].includes(a),m=((h=(g=this.app.state)==null?void 0:g.currentUser)==null?void 0:h.isAdmin)||((c=(w=this.app.state)==null?void 0:w.currentUser)==null?void 0:c.isVip),y=d&&!m&&!((u=(o=(r=this.app.gamification)==null?void 0:r.state)==null?void 0:o.unlockedFeatures)!=null&&u.includes("advance_tarot_spreads"));return`
              <div onclick="window.featuresManager.engines.tarot.selectSpread('${a}')"
                ...
                ${d?'<span class="premium-badge-tr">PREMIUM</span>':""}
                ...
                <h4 class="text-xl font-bold" style="color: var(--neuro-text);margin-bottom: 0.5rem;">${l.name}</h4>
                <p style="color: var(--neuro-text-light);" class="text-sm">${l.desc}</p>
              </div>`}).join("")}
          </div>

          <div style="margin-bottom: 3rem; padding: 0 1.5rem;">
            ${(()=>{var m,y,g,h,w,c,r,o,u,f;const d=!(((g=(y=(m=this.app)==null?void 0:m.state)==null?void 0:y.currentUser)==null?void 0:g.isAdmin)||((c=(w=(h=this.app)==null?void 0:h.state)==null?void 0:w.currentUser)==null?void 0:c.isVip)||((f=(u=(o=(r=this.app)==null?void 0:r.gamification)==null?void 0:o.state)==null?void 0:u.unlockedFeatures)==null?void 0:f.includes("tarot_vision_ai")));return`
                <button id="tarot-vision-ai-btn"
                        class="btn ${d?"opacity-50 cursor-not-allowed":"hover:scale-[1.02]"}"
                  ...
                  <span style="display:flex;flex-direction:column;gap:0.2rem;flex:1;min-width:0;padding-right:${d?"3rem":"4rem"};">
                  ...
                  ${d?'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:1.75rem;height:1.75rem;opacity:0.45;flex-shrink:0;margin-left:auto;"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>':""}
                </button>`})()}
          </div>
...
<style>
  ...
  `,a=document.createElement("style");a.textContent=n,document.head.appendChild(a);const l={ctx:null};function d(){try{l.ctx||(l.ctx=new(window.AudioContext||window.webkitAudioContext));const c=l.ctx,r=c.currentTime,o=c.createOscillator(),u=c.createOscillator(),f=c.createGain(),b=c.createBiquadFilter();o.type="sine",u.type="sine",o.frequency.value=660,u.frequency.value=990,u.detune.value=6,b.type="lowpass",b.frequency.value=2600,f.gain.setValueAtTime(1e-4,r),f.gain.exponentialRampToValueAtTime(.28,r+.02),f.gain.exponentialRampToValueAtTime(.001,r+1.5),o.connect(b),u.connect(b),b.connect(f),f.connect(c.destination),o.start(r),u.start(r),o.stop(r+1.5),u.stop(r+1.5)}catch(c){console.warn("WellnessKit: Audio playback failed",c)}}class m{constructor(r){if(!r||!r.id||!r.duration||!r.type)throw new Error("WellnessKit: Invalid tool configuration");this.config=r,this.state={remaining:r.duration,mainInterval:null,phaseTimeout:null,pulseInterval:null,currentIndex:0,isRunning:!1,countdownInterval:null},this.createUI(),this.initPulsePool(),this.attachEvents(),this.resetState()}createUI(){const r=document.createElement("div");r.innerHTML=`
      ...
      `,document.body.appendChild(r),this.elements={overlay:document.getElementById(`wk-${this.config.id}-overlay`),box:document.getElementById(`wk-${this.config.id}-box`),timer:document.getElementById(`wk-${this.config.id}-timer`),progress:document.getElementById(`wk-${this.config.id}-progress`),pulses:document.getElementById(`wk-${this.config.id}-pulses`),anim:document.getElementById(`wk-${this.config.id}-anim`),text:document.getElementById(`wk-${this.config.id}-text`),count:document.getElementById(`wk-${this.config.id}-count`),btnStart:document.getElementById(`wk-${this.config.id}-start`),btnFinish:document.getElementById(`wk-${this.config.id}-finish`),btnClose:document.getElementById(`wk-${this.config.id}-close`)},this.elements.box.style.background=this.config.gradient,this.R=40,this.CIRC=2*Math.PI*this.R,this.elements.progress.style.strokeDasharray=this.CIRC}initPulsePool(){this.pulsePool=[],this.pulseIndex=0;for(let r=0;r<t.PULSE_POOL_SIZE;r++){const o=document.createElement("div");o.className="wk-pulse",this.elements.pulses.appendChild(o),this.pulsePool.push(o)}}spawnPulse(){const r=this.pulsePool[this.pulseIndex];this.pulseIndex=(this.pulseIndex+1)%t.PULSE_POOL_SIZE,r.classList.remove("active"),r.offsetWidth,r.classList.add("active")}spawnPulseWithInterval(r){this.spawnPulse(),this.state.pulseInterval&&clearInterval(this.state.pulseInterval),this.state.pulseInterval=setInterval(()=>this.spawnPulse(),r)}setProgress(r,o){const u=Math.max(0,Math.min(1,r/o)),f=this.CIRC*(1-u);this.elements.progress.style.strokeDashoffset=f}clearTimers(){[this.state.mainInterval,this.state.phaseTimeout,this.state.pulseInterval,this.state.countdownInterval].forEach(o=>{o&&(clearInterval(o),clearTimeout(o))}),this.state.mainInterval=null,this.state.phaseTimeout=null,this.state.pulseInterval=null,this.state.countdownInterval=null}resetAnimationTransition(){this.elements.anim.style.transition=""}resetState(){this.clearTimers(),this.state.remaining=this.config.duration,this.state.currentIndex=0,this.state.isRunning=!1,this.elements.timer.textContent=String(this.config.duration),this.setProgress(this.config.duration,this.config.duration),this.elements.btnFinish.classList.add("hidden"),this.elements.btnStart.classList.remove("hidden"),this.elements.btnStart.textContent="Start",this.elements.anim.style.transform="scale(1.0)",this.elements.anim.style.opacity="0.9",this.resetAnimationTransition(),this.config.type==="breathing"?(this.elements.text.textContent=i.inhale,this.elements.count&&(this.elements.count.textContent=String(this.config.breathIn))):this.config.type==="zones"?this.elements.text.textContent=this.config.zones[0]:this.config.type==="steps"&&(this.elements.text.textContent=this.config.steps[0])}startMainTimer(){this.state.mainInterval||(this.state.isRunning=!0,this.elements.btnStart.textContent="Stop",this.config.type==="breathing"?this.startBreathingCycle():this.startPhaseLoop(),this.state.mainInterval=setInterval(()=>{this.state.remaining-=1,this.state.remaining<0&&(this.state.remaining=0),this.elements.timer.textContent=String(this.state.remaining),this.setProgress(this.state.remaining,this.config.duration),this.state.remaining<=0&&(this.clearTimers(),this.finalizeSession())},1e3))}stopMainTimer(){this.clearTimers(),this.state.isRunning=!1,this.elements.btnStart.textContent="Start",this.resetAnimationTransition()}getCompletedCycles(){const r=this.config.duration-this.state.remaining,o=this.config.breathIn+this.config.breathHold+this.config.breathOut;return Math.floor(r/o)}startBreathingCycle(){const r=()=>{d(),this.setBreathPhase("inhale",this.config.breathIn),this.elements.anim.style.transition=`transform ${this.config.breathIn}s linear`,this.elements.anim.style.transform="scale(1.14)",this.spawnPulseWithInterval(e.BREATH_PULSE_INTERVAL),this.state.phaseTimeout=setTimeout(()=>{this.resetAnimationTransition(),o()},this.config.breathIn*1e3)},o=()=>{this.state.remaining<=0||(this.setBreathPhase("hold",this.config.breathHold),this.state.pulseInterval&&(clearInterval(this.state.pulseInterval),this.state.pulseInterval=null),this.elements.anim.style.transition=`transform ${this.config.breathHold}s linear`,this.elements.anim.style.transform="scale(1.18)",this.state.phaseTimeout=setTimeout(()=>{this.resetAnimationTransition(),u()},this.config.breathHold*1e3))},u=()=>{this.state.remaining<=0||(this.setBreathPhase("exhale",this.config.breathOut),this.elements.anim.style.transition=`transform ${this.config.breathOut}s linear`,this.elements.anim.style.transform="scale(0.94)",this.spawnPulseWithInterval(e.EXHALE_PULSE_INTERVAL),this.state.phaseTimeout=setTimeout(()=>{this.resetAnimationTransition(),f()},this.config.breathOut*1e3))},f=()=>{if(this.state.remaining<=0)return;this.state.pulseInterval&&(clearInterval(this.state.pulseInterval),this.state.pulseInterval=null),this.getCompletedCycles()>=this.config.completeRounds?b():r()},b=()=>{this.setBreathPhase("relax",null),this.state.pulseInterval&&(clearInterval(this.state.pulseInterval),this.state.pulseInterval=null),this.elements.anim.style.transition=`transform ${e.RELAX_TRANSITION}ms ease-in-out`,this.elements.anim.style.transform="scale(1.0)",this.spawnPulseWithInterval(e.RELAX_PULSE_INTERVAL)};r()}setBreathPhase(r,o){if(this.elements.text.textContent=i[r]||"",r==="relax"){this.elements.count&&(this.elements.count.textContent="");return}if(this.elements.count&&o){this.state.countdownInterval&&clearInterval(this.state.countdownInterval);let u=o;this.elements.count.textContent=String(u),this.state.countdownInterval=setInterval(()=>{u-=1,u<=0?(this.elements.count.textContent="0",clearInterval(this.state.countdownInterval),this.state.countdownInterval=null):this.elements.count.textContent=String(u)},1e3)}}startPhaseLoop(){const r=this.config.zones||this.config.steps,o=()=>{if(!(this.state.remaining<=0)&&(this.elements.text.classList.add("changing"),setTimeout(()=>{this.elements.text.textContent=r[this.state.currentIndex],this.elements.text.classList.remove("changing"),this.spawnPulse(),this.elements.anim.style.transition="transform 0.24s ease-out",this.elements.anim.style.transform="scale(1.08)",setTimeout(()=>{this.elements.anim.style.transform="scale(1.0)",setTimeout(()=>this.resetAnimationTransition(),e.SCALE_BOUNCE)},e.SCALE_BOUNCE)},e.TEXT_FADE),this.state.currentIndex<r.length-1)){const f=this.config.stepDurations?this.config.stepDurations[this.state.currentIndex]*1e3:this.config.duration/r.length*1e3;this.state.phaseTimeout=setTimeout(()=>{this.state.currentIndex++,o()},f)}};this.elements.text.textContent=r[0],this.spawnPulse();const u=this.config.stepDurations?this.config.stepDurations[0]*1e3:this.config.duration/r.length*1e3;this.state.phaseTimeout=setTimeout(()=>{this.state.currentIndex++,o()},u)}completeSession(){var r;d();try{(r=window.app)!=null&&r.gamification&&(window.app.gamification.incrementWellnessRuns(),window.app.gamification.addBoth(10,1,"Wellness Practice"))}catch(o){console.warn("WellnessKit: Gamification integration failed",o)}}finalizeSession(){this.completeSession(),this.elements.btnStart.classList.add("hidden"),this.elements.btnFinish.classList.remove("hidden"),setTimeout(()=>this.close(),1200)}open(){this.resetState(),this.elements.overlay.style.display="flex",setTimeout(()=>this.elements.btnStart.focus(),100)}close(){this.stopMainTimer(),this.elements.overlay.style.display="none",this.resetState()}attachEvents(){this.elements.btnStart.addEventListener("click",()=>{this.state.mainInterval?this.stopMainTimer():(this.state.remaining<=0&&this.resetState(),this.startMainTimer())}),this.elements.btnFinish.addEventListener("click",()=>{this.completeSession(),this.close()}),this.elements.btnClose.addEventListener("click",()=>this.close()),this.elements.overlay.addEventListener("click",r=>{r.target===this.elements.overlay&&this.close()})}getStats(){return{autoTriggerEnabled:t.AUTO_TRIGGER}}destroy(){this.clearTimers(),this.elements.overlay&&this.elements.overlay.parentNode&&this.elements.overlay.parentNode.removeChild(this.elements.overlay)}}const y=new Set;let g=null;document.addEventListener("keydown",c=>{if(c.key==="Escape"){if(g)return;g=setTimeout(()=>g=null,e.ESCAPE_DEBOUNCE),y.forEach(r=>{r.elements.overlay.style.display==="flex"&&r.close()})}});const h={selfReset:new m(s.selfReset),fullBodyScan:new m(s.fullBodyScan),nervousSystem:new m(s.nervousSystem),tensionSweep:new m(s.tensionSweep)};Object.values(h).forEach(c=>y.add(c)),window.addEventListener("beforeunload",()=>{if(Object.values(h).forEach(c=>{c.clearTimers(),c.destroy()}),l.ctx)try{l.ctx.close(),l.ctx=null}catch(c){console.warn("WellnessKit: Audio context cleanup failed",c)}}),window.WellnessKit={openSelfReset:()=>h.selfReset.open(),openFullBodyScan:()=>h.fullBodyScan.open(),openNervousReset:()=>h.nervousSystem.open(),openTensionSweep:()=>h.tensionSweep.open(),closeSelfReset:()=>h.selfReset.close(),closeFullBodyScan:()=>h.fullBodyScan.close(),closeNervousReset:()=>h.nervousSystem.close(),closeTensionSweep:()=>h.tensionSweep.close(),getSelfResetStats:()=>h.selfReset.getStats(),getFullBodyScanStats:()=>h.fullBodyScan.getStats(),getNervousResetStats:()=>h.nervousSystem.getStats(),getTensionSweepStats:()=>h.tensionSweep.getStats(),getAllStats:()=>({selfReset:h.selfReset.getStats(),fullBodyScan:h.fullBodyScan.getStats(),nervousSystem:h.nervousSystem.getStats(),tensionSweep:h.tensionSweep.getStats()}),playChime:d},["SelfReset","FullBodyScan","NervousReset","TensionSweep"].forEach(c=>{window[`open${c}`]=window.WellnessKit[`open${c}`],window[`close${c}`]=window.WellnessKit[`close${c}`],window[`get${c}Stats`]=window.WellnessKit[`get${c}Stats`]})})();window.innerWidth<=767&&p(()=>Promise.resolve({}),__vite__mapDeps([11]));let U=!1;window.lazyLoadCommunityHub=async function(){U||(U=!0,await Promise.all([p(()=>Promise.resolve({}),__vite__mapDeps([12])),p(()=>import("./community-hub-Bv-lWZW8.js").then(t=>t.e),__vite__mapDeps([5,6]))]))};"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("/service-worker.js",{scope:"/"}).catch(t=>console.error("[SW] Registration failed:",t))});async function z(){try{const[t,{QUOTES:e,getRandomQuote:i,getQuoteOfTheDay:s},n,{default:a}]=await Promise.all([p(()=>import("./AffirmationsList-BRihhJ1z.js"),[]),p(()=>import("./QuotesList-BeF9xuKi.js"),[]),p(()=>import("./Index-BEVcoeoi.js"),__vite__mapDeps([13,14,5,6,7,2,4,15])),p(()=>import("./User-Tab-Z5k3fMrT.js"),__vite__mapDeps([14,5,6,7]))]);window.affirmations=t.default,window.QuotesData={QUOTES:e,getRandomQuote:i,getQuoteOfTheDay:s},window.app=new n.ProjectCuriosityApp({AppState:n.AppState,AuthManager:n.AuthManager,NavigationManager:n.NavigationManager,DashboardManager:n.DashboardManager,UserTab:a}),window.app.init();const l=()=>{p(()=>import("./features-lazy-bN0WDG2L.js").then(d=>d.T),[]),p(()=>import("./features-lazy-bN0WDG2L.js").then(d=>d.i),[]),p(()=>import("./features-lazy-bN0WDG2L.js").then(d=>d.s),[])};typeof requestIdleCallback=="function"?requestIdleCallback(l,{timeout:3e3}):setTimeout(l,1500)}catch(t){console.error("[FATAL] Bootstrap failed:",t),document.body.innerHTML='<div style="padding:2rem;text-align:center;font-family:system-ui"><h1>Loading Error</h1><p>Unable to load the application. Please refresh the page.</p><button onclick="location.reload()" style="margin-top:1.5rem;padding:0.75rem 2rem;background:#6b9b37;color:#fff;border:none;border-radius:8px;font-size:1rem;cursor:pointer">Retry</button></div>'}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",z):z();export{V as T,q as a,Z as c,we as g,ve as i,be as s};
