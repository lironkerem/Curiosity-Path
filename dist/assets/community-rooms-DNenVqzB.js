import{C as p,a as D,S as $e}from"./community-hub-BdcPwhoU.js";const W={ENDED:0,PLAYING:1},pe={initPlayerState(){this.state.player=null,this.state.playerReady=!1,this.state.playerInitialized=!1,this.state.sessionStarted=!1,this._progressInterval=null},preloadYouTubeAPI(){if(document.querySelector('script[src*="youtube.com/iframe_api"]'))return;const n=document.createElement("script");n.src="https://www.youtube.com/iframe_api",document.head.appendChild(n)},loadYouTubeAPI(){var e;if((e=window.YT)!=null&&e.Player){this.initPlayer();return}const n=window.onYouTubeIframeAPIReady;window.onYouTubeIframeAPIReady=()=>{n==null||n(),this.initPlayer()}},initPlayer(){if(this.state.playerInitialized)return;const n=this.getCurrentSession();if(!(n!=null&&n.videoId)){console.warn(`[${this.roomId}] No video session available`);return}this.state.player=new YT.Player(`${this.roomId}-youtube-player`,{videoId:n.videoId,playerVars:{autoplay:0,controls:1,modestbranding:1,rel:0,mute:1},events:{onReady:e=>this.onPlayerReady(e),onStateChange:e=>this.onPlayerStateChange(e)}}),this.state.playerInitialized=!0},onPlayerReady(n){var e,t,i,o;this.state.playerReady=!0,n.target.cueVideoById((e=this.getCurrentSession())==null?void 0:e.videoId),this.state.isInSession&&((i=(t=window.Core.state)==null?void 0:t.currentUser)!=null&&i.is_admin)&&this._startAtCycleOffset(n.target),(o=this.onPlayerReadyCustom)==null||o.call(this,n)},_startAtCycleOffset(n){const e=this.config.cycleDuration*1e3,t=this.config.openDuration*1e3,o=(Date.now()-this.state.cycleStartTime)%e,a=Math.max(0,(o-t)/1e3);n.seekTo(a,!0),n.unMute(),n.setVolume(100),n.playVideo(),this.state.sessionStarted=!0,this._showPlayer();const s=this.getCurrentSession();s&&window.Core.showToast(`${s.emoji} Joining session in progress…`)},onPlayerStateChange(n){n.data===W.PLAYING?this.onVideoPlaying():n.data===W.ENDED&&this.onVideoEnded()},onVideoPlaying(){this._showPlayer(),this.startProgressTracking()},onVideoEnded(){this.stopProgressTracking(),window.Core.showToast("Session complete")},startSession(){var e,t,i;if(!this.state.playerReady||this.state.sessionStarted)return;const n=this.getCurrentSession();n&&(this._showPlayer(),(e=this.state.player)==null||e.unMute(),(t=this.state.player)==null||t.setVolume(100),(i=this.state.player)==null||i.playVideo(),this.state.sessionStarted=!0,window.Core.showToast(`${n.emoji} Session starting…`))},togglePlayPause(){if(!this.state.player)return;const n=this.state.player.getPlayerState()===W.PLAYING;n?this.state.player.pauseVideo():this.state.player.playVideo();const e=document.getElementById(`${this.roomId}PlayBtn`);e&&(e.innerHTML=`<span style="font-size:24px;">${n?"▶":"⏸"}</span>`)},_seek(n){this.state.player&&this.state.player.seekTo(Math.max(0,this.state.player.getCurrentTime()+n))},skipBackward(){this._seek(-10)},skipForward(){this._seek(10)},startProgressTracking(){this._progressInterval||(this._progressInterval=setInterval(()=>this.updateTimeDisplay(),1e3))},stopProgressTracking(){this._progressInterval&&(clearInterval(this._progressInterval),this._progressInterval=null)},updateTimeDisplay(){var e;if(!((e=this.state.player)!=null&&e.getCurrentTime))return;const n=document.getElementById(`${this.roomId}TimeDisplay`);if(n){const t=Math.floor(this.state.player.getCurrentTime()),i=Math.floor(this.state.player.getDuration());n.textContent=`${this.formatTime(t)} / ${this.formatTime(i)}`}},_showPlayer(){const n=document.getElementById(`${this.roomId}PlayerOverlay`);n&&(n.style.display="none");const e=document.getElementById(`${this.roomId}Controls`);e&&(e.style.display="flex")},buildPlayerContainer(){const n=this.getCurrentSession();return`
        <div class="guided-player-container" role="region" aria-label="Video player">
            ...
            <div class="player-overlay" id="${this.roomId}PlayerOverlay">
                <div class="session-info" aria-live="polite">
                    <div class="session-emoji"    id="${this.roomId}SessionEmoji"   >${(n==null?void 0:n.emoji)||"🎧"}</div>
                    <div class="session-title"    id="${this.roomId}SessionTitle"   >${(n==null?void 0:n.title)||"Loading..."}</div>
                    <div class="session-duration" id="${this.roomId}SessionDuration">${(n==null?void 0:n.duration)||"00:00"}</div>
        ...
        </div>`},cleanupPlayer(){var n;this.stopProgressTracking(),(n=this.state.player)==null||n.destroy(),this.state.player=null,this.state.playerReady=!1,this.state.playerInitialized=!1,this.state.sessionStarted=!1}},Le=Object.freeze(Object.defineProperty({__proto__:null,YouTubePlayerMixin:pe},Symbol.toStringTag,{value:"Module"})),me={initCycleState(){this.state.isOpen=!1,this.state.isInSession=!1,this.state.cycleStartTime=null,this.state.nextOpenTime=null,this.state.nextSessionStart=null,this._cycleInterval=null},initializeCycle(){var t;const n=Date.now(),e=this.config.cycleDuration*1e3;this.state.cycleStartTime=n-n%e,(t=this.setSessions)==null||t.call(this,n),this.calculateCycleState(),this._cycleInterval&&clearInterval(this._cycleInterval),this._cycleInterval=setInterval(()=>{this.calculateCycleState(),this.updateRoomCard()},1e3)},calculateCycleState(){var s,r;const n=Date.now(),e=this.config.cycleDuration*1e3,t=this.config.openDuration*1e3,i=(n-this.state.cycleStartTime)%e,o=n-i;o!==this.state.cycleStartTime&&(this.state.cycleStartTime=o,(s=this.setSessions)==null||s.call(this,n));const a=i<t;if(this.state.isOpen=a,this.state.isInSession=!a,a){const l=t-i;this.state.nextSessionStart=n+l,this.state.nextOpenTime=n+l+this.config.sessionDuration*1e3}else this.state.nextOpenTime=n+(e-i),this.state.nextSessionStart=null;this.state.isInSession&&!this.state.sessionStarted&&this.isUserInRoom()&&((r=this.startSession)==null||r.call(this))},isUserInRoom(){var n,e;return((e=(n=window.Core)==null?void 0:n.state)==null?void 0:e.currentRoom)===this.roomId},canEnterRoom(){var n,e;return((e=(n=window.Core.state)==null?void 0:n.currentUser)==null?void 0:e.is_admin)||this.state.isOpen},_checkCycleWindow(){return this.state.isOpen},_formatCountdown(n,e){if(!n)return"";const t=Math.max(0,n-Date.now()),i=Math.floor(t/6e4),o=Math.floor(t%6e4/1e3);return`${e} ${i}:${String(o).padStart(2,"0")}`},getTimeUntilSessionStarts(){return this._formatCountdown(this.state.nextSessionStart,"Session begins in")},getCountdownToNextOpen(){return this._formatCountdown(this.state.nextOpenTime,"Opens in")},getTimerText(){var t,i;const n=((i=(t=this.getNextSession)==null?void 0:t.call(this))==null?void 0:i.title)??"Next Session",e=this.state.isOpen?this.getTimeUntilSessionStarts():this.getCountdownToNextOpen();return`<strong>Next:</strong> ${n}<br>${e}`},buildScheduleLink(){return`
        ...
        </button>`},cleanupCycle(){this._cycleInterval&&(clearInterval(this._cycleInterval),this._cycleInterval=null)}},je=Object.freeze(Object.defineProperty({__proto__:null,CycleStateMixin:me},Symbol.toStringTag,{value:"Module"})),H=n=>n.charAt(0).toUpperCase()+n.slice(1);function q(n,e,t,i){var l,c;const o=t||n.charAt(0).toUpperCase(),a=((c=(l=window.Core)==null?void 0:l.getAvatarGradient)==null?void 0:c.call(l,i||n))??"linear-gradient(135deg, #667eea 0%, #764ba2 100%)",s=e?`<img src="${e}" width="36" height="36" style="width:36px;height:36px;object-fit:cover;border-radius:50%;display:block;flex-shrink:0;" alt="${n}" loading="lazy" decoding="async">`:`<span style="color:white;font-size:13px;font-weight:600;line-height:1;">${o}</span>`,r=e?"background:transparent;":`background:${a};`;return{inner:s,bg:r,gradient:a,initial:o}}const ee=n=>new Date(n).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});function Te(n){return!n||typeof n!="string"?"":n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}const z={initChatState(n=["main"]){this.chatChannels=n,this.state.messages=Object.fromEntries(n.map(e=>[e,[]]))},async initializeChat(){await Promise.all(this.chatChannels.map(n=>this.loadRoomChatFromDB(n))),setTimeout(()=>this._injectSenderAvatar(),200)},_getDbRoomId(n="main"){return n==="main"?this.roomId:`${this.roomId}-${n}`},_msgContainerId(n){return`${this.roomId}${H(n)}Messages`},_inputId(n){return`${this.roomId}${H(n)}Input`},_rowToMsgData(n,e=!1){var d,u;const t=n.profiles||{},i=((u=(d=window.Core)==null?void 0:d.state)==null?void 0:u.currentUser)||{},o=e?i.name||"You":t.name||"Member",a=e?i.avatar_url||"":t.avatar_url||"",s=e?i.emoji||"":t.emoji||"",r=t.id||null,{gradient:l,initial:c}=q(o,a,s,r);return{name:o,initial:c,avatarUrl:a,avatarBg:l,userId:r,time:ee(n.created_at),text:n.message,country:null}},async sendMessage(n="main",e=null){var d,u,m,h;const t=document.getElementById(e||this._inputId(n));if(!(t!=null&&t.value.trim()))return;const i=t.value.trim(),o=document.getElementById(this._msgContainerId(n)),a=((u=(d=window.Core)==null?void 0:d.state)==null?void 0:u.currentUser)||{},s=a.name||"You",{gradient:r,initial:l}=q(s,a.avatar_url||"",a.emoji||"",a.id),c={name:s,initial:l,avatarUrl:a.avatar_url||"",avatarBg:r,userId:a.id||null,time:ee(new Date),text:i,country:null,timestamp:Date.now(),...(m=this.getCustomMessageData)==null?void 0:m.call(this,n)};if(this.state.messages[n].push(c),o&&(o.insertAdjacentHTML("beforeend",this.buildMessageHTML(c)),o.scrollTop=o.scrollHeight),t.value="",window.Core.showToast("Message sent"),(h=p)!=null&&h.ready)try{await p.sendRoomMessage(this._getDbRoomId(n),i)}catch(y){console.error("[ChatMixin] sendRoomMessage error:",y)}},async loadRoomChatFromDB(n="main"){var o,a,s,r;if(!((o=p)!=null&&o.ready)){this.loadMessagesFromStorage(n),this.renderSavedMessages(n);return}const e=this._getDbRoomId(n),t=document.getElementById(this._msgContainerId(n)),i=(r=(s=(a=window.Core)==null?void 0:a.state)==null?void 0:s.currentUser)==null?void 0:r.id;try{const[l,c]=await Promise.all([p.getRoomMessages(e,50),p.getBlockedUsers()]);if(t&&l.length){const d=l.filter(u=>{var m;return!c.has((m=u.profiles)==null?void 0:m.id)}).map(u=>{var h;const m=((h=u.profiles)==null?void 0:h.id)===i;return this.buildMessageHTML(this._rowToMsgData(u,m))}).join("");d&&(t.insertAdjacentHTML("beforeend",d),t.scrollTop=t.scrollHeight)}p.subscribeToRoomChat(e,async d=>{var m,h;((m=d.profiles)==null?void 0:m.id)===i||(await p.getBlockedUsers()).has((h=d.profiles)==null?void 0:h.id)||!t||(t.insertAdjacentHTML("beforeend",this.buildMessageHTML(this._rowToMsgData(d))),t.scrollTop=t.scrollHeight)})}catch(l){console.error(`[ChatMixin] loadRoomChatFromDB error (${n}):`,l)}},buildMessageHTML(n){const e=n.avatarUrl?`background-image:url('${n.avatarUrl}');background-size:cover;background-position:center;`:`background:${n.avatarBg};`,t=n.avatarUrl?"":`<span style="color:white;font-size:13px;font-weight:600;line-height:1;">${n.initial}</span>`,i=n.userId?`<span class="campfire-msg-name" role="button" tabindex="0" style="cursor:pointer;" onclick="openMemberProfileAboveRoom('${n.userId}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openMemberProfileAboveRoom('${n.userId}');}" aria-label="View profile of ${n.name}">${n.name}</span>`:`<span class="campfire-msg-name">${n.name}</span>`;return`
        ...
        </div>`},buildSoundSettings(){const n=this.roomId,e=Se.map(o=>this._soundOption("Bell",o.value,o.label,"previewSound","selectBell")).join(""),t=Me.map(o=>this._soundOption("Completion",o.value,o.label,"previewSound","selectCompletion")).join(""),i=Ee.map(o=>this._soundOption("Ambient",o.value,o.label,"previewAmbient","selectAmbient")).join("");return`
        ...
        </button>`}},He=Object.freeze(Object.defineProperty({__proto__:null,SoundSettingsMixin:Y},Symbol.toStringTag,{value:"Module"})),K=180,ie=+(2*Math.PI*K).toFixed(2),Be=60,Re=7200,oe={idle:"Begin",running:"Pause",paused:"Continue",done:"Complete"},Q={initTimerState(n=1200){this.state.timerRunning=!1,this.state.timeLeft=n,this.state.initialTime=n,this._timerInterval=null,this._timerHiddenAt=null,this._timerVisibilityHandler=null},toggleTimer(){this.state.timerRunning?this.pauseTimer():this.startTimer()},startTimer(){this.state.timerRunning||(this.state.timerRunning=!0,this._timerTickStart=Date.now(),this._setTimerBtn("running"),this._setTimerGlow("running"),this._timerInterval=setInterval(()=>{var n;this.state.timeLeft--,this._updateTimer(),this.state.timeLeft>0&&this.state.timeLeft%300===0&&this.state.fiveMinBellEnabled&&((n=this.play5MinBell)==null||n.call(this)),this.state.timeLeft<=0&&this.completeTimer()},1e3),this._attachVisibilityHandler(),window.Core.showToast("Timer started"))},pauseTimer(){this._clearInterval(),this._detachVisibilityHandler(),this.state.timerRunning=!1,this._setTimerBtn("paused"),this._setTimerGlow("paused")},completeTimer(){var n,e;this._clearInterval(),this._detachVisibilityHandler(),this.state.timerRunning=!1,this.state.timeLeft=0,this._setTimerBtn("done"),this._updateTimer(),this._setTimerGlow("idle"),(n=this.playCompletionSound)==null||n.call(this),window.Core.showToast("Session complete!"),(e=this.onTimerComplete)==null||e.call(this)},resetTimer(){this._clearInterval(),this._detachVisibilityHandler(),this.state.timerRunning=!1,this.state.timeLeft=this.state.initialTime,this._setTimerBtn("idle"),this._updateTimer(),this._setTimerGlow("idle")},adjustTime(n){this.state.timerRunning||(this.state.timeLeft=Math.min(Re,Math.max(Be,this.state.timeLeft+n*60)),this.state.initialTime=this.state.timeLeft,this._updateTimer())},cleanupTimer(){this._clearInterval(),this._detachVisibilityHandler(),this.state.timerRunning=!1},_attachVisibilityHandler(){this._timerVisibilityHandler||(this._timerVisibilityHandler=()=>{if(document.hidden)this._timerHiddenAt=Date.now(),this._clearInterval();else{if(this._timerHiddenAt!==null){const n=Math.round((Date.now()-this._timerHiddenAt)/1e3);if(this._timerHiddenAt=null,this.state.timeLeft=Math.max(0,this.state.timeLeft-n),this._updateTimer(),this.state.timeLeft<=0){this.completeTimer();return}}this.state.timerRunning&&!this._timerInterval&&(this._timerInterval=setInterval(()=>{var n;this.state.timeLeft--,this._updateTimer(),this.state.timeLeft>0&&this.state.timeLeft%300===0&&this.state.fiveMinBellEnabled&&((n=this.play5MinBell)==null||n.call(this)),this.state.timeLeft<=0&&this.completeTimer()},1e3))}},document.addEventListener("visibilitychange",this._timerVisibilityHandler))},_detachVisibilityHandler(){this._timerVisibilityHandler&&(document.removeEventListener("visibilitychange",this._timerVisibilityHandler),this._timerVisibilityHandler=null),this._timerHiddenAt=null},_clearInterval(){this._timerInterval&&(clearInterval(this._timerInterval),this._timerInterval=null)},_setTimerBtn(n){const e=document.getElementById(`${this.roomId}TimerBtn`);e&&(e.textContent=oe[n])},_updateTimer(){const n=document.getElementById(`${this.roomId}TimerDisplay`);n&&(n.textContent=this.formatTime(this.state.timeLeft));const e=document.getElementById(`${this.roomId}TimerRing`);if(e){const t=this.state.timeLeft/this.state.initialTime;e.style.strokeDashoffset=ie*(1-t)}},updateTimerDisplay(){this._updateTimer()},updateTimerRing(){this._updateTimer()},buildTimerContainer({subtitle:n="in practice",gradientId:e=`timerGrad_${this.roomId}`,color1:t="#a78bfa",color2:i="#c084fc",glowColor:o="rgba(167,139,250,0.35)",subtitleHtml:a=null}={}){this._glowColor=o;const s=ie,r=163,l=+(2*Math.PI*r).toFixed(2),c=+(l*35/360).toFixed(2),d=+(l*120/360).toFixed(2),u=+(l-c).toFixed(2),m=+(l-d).toFixed(2),h=a??`<div style="font-size:13px;text-transform:uppercase;letter-spacing:0.22em;opacity:0.5;font-weight:500;">${n}</div>`;return`
        <style>
        ...
        </div>`},_setTimerGlow(n){const e=document.getElementById(`${this.roomId}TimerRingWrap`),t=document.getElementById(`${this.roomId}OuterSvg`);!e||!t||(clearTimeout(this._glowTransitionTimer),e.classList.remove("running","paused"),n==="running"&&e.classList.add("running"),n==="paused"&&e.classList.add("paused"),n==="running"?(t.style.animation="",t.style.transition="filter 1.2s ease",t.style.filter=`drop-shadow(0 0 12px ${this._glowColor}) drop-shadow(0 0 35px ${this._glowColor}) drop-shadow(0 0 70px ${this._glowColor})`,this._glowTransitionTimer=setTimeout(()=>{e.classList.add("running"),requestAnimationFrame(()=>{t.style.transition="",t.style.filter=""})},1200)):(t.style.animation="none",t.style.transition="filter 1.2s ease",t.style.filter="none"))}},ze=Object.freeze(Object.defineProperty({__proto__:null,TimerMixin:Q},Symbol.toStringTag,{value:"Module"})),N='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;opacity:0.8;"><path d="M12 3L13.5 8.5L19 10L13.5 11.5L12 17L10.5 11.5L5 10L10.5 8.5Z"/><path d="M5 3L5.75 5.25L8 6L5.75 6.75L5 9L4.25 6.75L2 6L4.25 5.25Z"/><path d="M19 14L19.75 16.25L22 17L19.75 17.75L19 20L18.25 17.75L16 17L18.25 16.25Z"/></svg>',ne='<span style="background:rgba(239,68,68,0.85);color:white;font-size:11px;font-weight:700;letter-spacing:0.08em;padding:4px 10px;border-radius:4px;text-transform:uppercase;">In Session</span>',F={MAX:5,fallbackGradient:"linear-gradient(135deg, #667eea 0%, #764ba2 100%)"};function _(n){return n?String(n).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;"):""}const S={bless:'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;opacity:0.8;"><path d="M12 3L13.5 8.5L19 10L13.5 11.5L12 17L10.5 11.5L5 10L10.5 8.5Z"/><path d="M5 3L5.75 5.25L8 6L5.75 6.75L5 9L4.25 6.75L2 6L4.25 5.25Z"/><path d="M19 14L19.75 16.25L22 17L19.75 17.75L19 20L18.25 17.75L16 17L18.25 16.25Z"/></svg>',leave:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M13 4h3a2 2 0 0 1 2 2v14"/><path d="M2 20h3"/><path d="M13 20h9"/><path d="M10 12v.01"/><path d="M13 4l-6 2v14l6 2"/></svg>',shield:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>',book:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>',alert:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>',block:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><circle cx="12" cy="12" r="10"/><path d="m4.9 4.9 14.2 14.2"/></svg>',mute:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>',help:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="4"/><line x1="4.93" y1="4.93" x2="9.17" y2="9.17"/><line x1="14.83" y1="14.83" x2="19.07" y2="19.07"/><line x1="14.83" y1="9.17" x2="19.07" y2="4.93"/><line x1="4.93" y1="19.07" x2="9.17" y2="14.83"/></svg>'},J={data:null,fetchedAt:0,TTL_MS:12e4,async get(){const n=Date.now();if(this.data&&n-this.fetchedAt<this.TTL_MS)return this.data;try{this.data=await p.getBlockedUsers(),this.fetchedAt=n}catch{this.data=this.data??new Set}return this.data},invalidate(){this.data=null,this.fetchedAt=0}};class g{constructor(e){this.roomId=e.roomId,this.roomType=e.roomType,this.config={name:"Practice Room",icon:"🧘",description:"A space for practice",energy:"Peaceful",statusRingColor:"var(--neuro-accent)",imageUrl:"",...e},this.state={participants:e.participants||0,isActive:!0,...e.state},this.eventListeners=[],this._actionsCache=null}_dbReady(){var e;return!!((e=p)!=null&&e.ready)}init(){var e;this.updateRoomCard(),window[`${this.roomId}_blessRoom`]=()=>this._blessRoom(),window[`${this.roomId}_showScheduleModal`]=()=>{var t;return(t=this.showScheduleModal)==null?void 0:t.call(this)},window[`${this.roomId}_closeScheduleModal`]=()=>{var t;return(t=this.closeScheduleModal)==null?void 0:t.call(this)},this.mountHubModals(),this._loadBlessingCount(),(e=this.onInit)==null||e.call(this)}mountHubModals(){var o,a;const e=((o=this.buildHubModals)==null?void 0:o.call(this))||"";if(!e)return;let t=document.getElementById("roomHubModals");t||(t=Object.assign(document.createElement("div"),{id:"roomHubModals"}),t.style.cssText="position:relative;z-index:200000;",document.body.appendChild(t)),(a=document.getElementById(`${this.roomId}HubModalsSlot`))==null||a.remove();const i=Object.assign(document.createElement("div"),{id:`${this.roomId}HubModalsSlot`,innerHTML:e});t.appendChild(i)}buildHubModals(){return""}enterRoom(){var e;if(!this.canEnterRoom()){window.Core.showToast("Session in progress. Please wait for the next opening.");return}g.stopHubPresence(),this._createPracticeViewDeferred(),window.Core.navigateTo("practiceRoomView"),window.Core.showToast(`${this.config.icon} Entered ${this.config.name}`),this._actionsCache=null,this.setupEventListeners(),(e=this.onEnter)==null||e.call(this),this._setRoomPresence(this.roomId),D&&!D.state.isContributing&&D._startWave(),requestAnimationFrame(()=>{this._refreshParticipantSidebar(`${this.roomId}ParticipantListEl`,`${this.roomId}ParticipantCount`),requestAnimationFrame(()=>{this._refreshBlessingCounter(),this._subscribeToBlessings()})}),p.logRoomEntry(this.roomId).then(t=>{this._roomEntryId=t}).catch(()=>{})}_createPracticeViewDeferred(){var a;$e.injectModals();const e=document.getElementById("dynamicRoomContent"),t=this.buildHeader(),i=[this.buildInstructionsModal(),((a=this.buildAdditionalModals)==null?void 0:a.call(this))??""].join(""),o=`<div class="ps-body" id="${this.roomId}BodyPlaceholder" style="flex:1;display:flex;align-items:center;justify-content:center;min-height:200px;"><div style="opacity:0.4;font-size:14px;">Loading…</div></div>`;if(e)e.innerHTML=t+o+i;else{if(document.getElementById(`${this.roomId}View`))return;const s=document.createElement("div");s.className="view practice-space",s.id=`${this.roomId}View`,s.innerHTML=t+o+i,document.body.appendChild(s)}requestAnimationFrame(()=>{const s=document.getElementById(`${this.roomId}BodyPlaceholder`),r=this.buildBody();if(s){const l=document.createElement("div");l.innerHTML=r,s.replaceWith(...Array.from(l.childNodes))}else e&&(e.innerHTML=t+r+i);this._onEnterDeferred()})}_onEnterDeferred(){var e;(e=this.onEnter)==null||e.call(this),requestAnimationFrame(()=>{this._refreshParticipantSidebar(`${this.roomId}ParticipantListEl`,`${this.roomId}ParticipantCount`),requestAnimationFrame(()=>{this._refreshBlessingCounter(),this._subscribeToBlessings()})})}createPracticeView(){this._createPracticeViewDeferred()}leaveRoom(){var e;this._exitCleanup(),window.Core.navigateTo("hubView"),window.Core.showToast(`Left ${this.config.name}`),(e=this.onLeave)==null||e.call(this),g.startHubPresence()}gentlyLeave(){var e;this._exitCleanup(),(e=this.onLeave)==null||e.call(this),g.startHubPresence(),window.Rituals?Rituals.showClosing():window.Core.navigateTo("hubView")}_exitCleanup(){var e;this._teardownDelegatedListeners(),(e=D)!=null&&e.state.isContributing&&D._endWave(),this._clearRoomPresence(),J.invalidate(),this._lastParticipantKey=null,this._participantFetchInFlight=!1,this._blessingRows=null,this._actionsCache=null,clearTimeout(this._blessingRefreshTimer),this._roomEntryId&&(p.logRoomExit(this._roomEntryId).catch(()=>{}),this._roomEntryId=null);for(const t of["_presenceSub","_sidebarPresenceSub"])if(this[t]){try{this[t].unsubscribe()}catch{}this[t]=null}p&&p.unsubscribeFromBlessings(this.roomId),this.cleanup()}canEnterRoom(){return!0}_isWithinOpenWindow(){return this.roomType!=="timed"?!0:typeof this._checkCycleWindow=="function"?this._checkCycleWindow():!0}cleanup(){var e;this.eventListeners.forEach(({element:t,event:i,handler:o})=>t.removeEventListener(i,o)),this.eventListeners=[],(e=this.onCleanup)==null||e.call(this)}_setRoomPresence(e){var t;if(this._dbReady())try{const i=g.ROOM_ACTIVITIES[e]??`${this.config.icon} ${this.config.name}`;p.setPresence("online",i,e),(t=window.Core)!=null&&t.state&&(window.Core.state.currentRoom=e,window.Core.state.currentUser&&(window.Core.state.currentUser.activity=i))}catch(i){console.error("[PracticeRoom] _setRoomPresence error:",i)}}_clearRoomPresence(){var e;if(this._dbReady())try{p.setPresence("online","✨ Available",null),(e=window.Core)!=null&&e.state&&(window.Core.state.currentRoom=null,window.Core.state.currentUser&&(window.Core.state.currentUser.activity="✨ Available"))}catch(t){console.error("[PracticeRoom] _clearRoomPresence error:",t)}}async fetchRoomParticipants(){if(this._dbReady())try{const[e,t]=await Promise.all([p.getRoomParticipants(this.roomId),J.get()]),i=e.filter(o=>!t.has(o.user_id));this.state.participants=i.length,this._updateParticipantUI(i),this._updateRoomCardCount(i.length)}catch(e){console.error(`[${this.roomId}] fetchRoomParticipants error:`,e)}}_updateParticipantUI(e){const t=document.getElementById(`${this.roomId}ParticipantCount`);t&&(t.textContent=this.getParticipantText());const i=document.getElementById(`${this.roomId}ParticipantStack`);i&&(i.innerHTML=this._buildAvatarStack(e))}async _refreshParticipantSidebar(e,t=null){var o;if(!this._dbReady())return;const i=async()=>{if(this._participantFetchInFlight)return;this._participantFetchInFlight=!0;const[a,s]=await Promise.all([p.getRoomParticipants(this.roomId),J.get()]),r=a.filter(d=>!s.has(d.user_id));this._participantFetchInFlight=!1;const l=r.map(d=>d.user_id).sort().join(",");if(l===this._lastParticipantKey)return;this._lastParticipantKey=l,this.state.participants=r.length,this._updateParticipantUI(r);const c=document.getElementById(e);if(c&&this._renderParticipantList(c,r),t){const d=document.getElementById(t);d&&(d.textContent=`${r.length} present`)}this._updateRoomCardCount(r.length)};try{await i()}catch(a){this._participantFetchInFlight=!1,console.error(`[${this.roomId}] _refreshParticipantSidebar error:`,a);return}(o=this._sidebarPresenceSub)==null||o.unsubscribe(),this._sidebarPresenceSub=p.subscribeToPresence(async()=>{var a;document.getElementById(e)?await i():((a=this._sidebarPresenceSub)==null||a.unsubscribe(),this._sidebarPresenceSub=null)})}_updateRoomCardCount(e){this.state.participants=e;const t=document.querySelector(`[data-room-id="${this.roomId}"] .room-participants`);t&&(t.textContent=this.getParticipantText())}_buildAvatarStack(e,t=22,i=9){const{MAX:o,fallbackGradient:a}=F,s=e.slice(0,o),r=e.length-o,l=s.map(d=>{var v,k;const u=d.profiles||{},m=_(u.name||"Member"),h=_(u.avatar_url||""),f=_(u.emoji||"")||m.charAt(0).toUpperCase(),b=((k=(v=window.Core)==null?void 0:v.getAvatarGradient)==null?void 0:k.call(v,d.user_id||m))??a,C=h?`<img src="${h}" width="40" height="40" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" alt="${m}" loading="lazy" decoding="async">`:`<span style="color:white;font-size:${i}px;font-weight:700;">${f}</span>`;return`<div title="${m}" style="width:${t}px;height:${t}px;border-radius:50%;background:${h?"transparent":b};border:2px solid var(--surface);display:flex;align-items:center;justify-content:center;overflow:hidden;margin-left:-6px;flex-shrink:0;">${C}</div>`}).join(""),c=r>0?`<span style="font-size:10px;color:var(--text-muted);margin-left:4px;">+${r}</span>`:"";return l+c}_buildRealAvatars(e){const{MAX:t,fallbackGradient:i}=F,o=e.slice(0,t),a=e.length-t;return o.map(r=>{var x,T;const l=r.profiles||{},c=r.user_id||l.id||"",d=_(l.name||"Member"),u=_(l.avatar_url||""),m=_(l.emoji||""),h=((T=(x=window.Core)==null?void 0:x.getAvatarGradient)==null?void 0:T.call(x,c||d))??i,y=u?`<img src="${u}" width="40" height="40" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" alt="${d}" loading="lazy" decoding="async">`:`<span aria-hidden="true">${m||d.charAt(0).toUpperCase()}</span>`,f=u?"background:transparent;":`background:${h};`,b=c?`onclick="openMemberProfileAboveRoom('${c}')"`:"",C=c?"button":"img",M=c?'tabindex="0"':"",v=c?`onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openMemberProfileAboveRoom('${c}');}"`:"",k=`aria-label="${d}"`;return`<div class="p-avatar" style="${f}" title="${d}" role="${C}" ${M} ${k} ${b} ${v}>${y}</div>`}).join("")+(a>0?`<div class="p-avatar" style="background:var(--surface);color:var(--text-muted);font-size:11px;">+${a}</div>`:"")}_renderParticipantList(e,t){if(!t.length){e.innerHTML='<div style="color:var(--text-muted);font-size:13px;padding:8px;">Just you here 🕯️</div>';return}const{fallbackGradient:i}=F;e.innerHTML=t.map(o=>{var b,C;const a=o.profiles||{},s=o.user_id||a.id||"",r=_(a.name||"Member"),l=_(a.avatar_url||""),c=_(a.emoji||""),d=((C=(b=window.Core)==null?void 0:b.getAvatarGradient)==null?void 0:C.call(b,s||r))??i,u=l?`<img src="${l}" referrerpolicy="no-referrer" width="40" height="40" style="width:40px;height:40px;min-width:40px;min-height:40px;object-fit:cover;border-radius:50%;display:block;" alt="${r}" loading="lazy" decoding="async">`:`<span style="color:white;font-weight:600;font-size:13px;">${c||r.charAt(0).toUpperCase()}</span>`,m=l?"background:transparent;":`background:${d};`,h=s?`onclick="openMemberProfileAboveRoom('${s}')"`:"",y=s?`onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openMemberProfileAboveRoom('${s}');}"`:"";return`
            <div class="campfire-participant" ${h} ${y} ${s?'role="button" tabindex="0"':""} style="${s?"cursor:pointer;":""}">
                ...
                <div class="campfire-participant-info">
                    ...
                    <div class="campfire-participant-country">${_(o.activity||"✨ Available")}</div>
                </div>
            </div>`}).join("")}async _blessRoom(){var r;const e=document.getElementById(`${this.roomId}BlessBtn`);if(!this._dbReady()){window.Core.showToast("Blessing sent ✨"),this._showBlessingAnimation(null);return}const t=Date.now(),i=this._lastBlessedAt??0,o=6e4;if(t-i<o){const l=Math.ceil((o-(t-i))/1e3);window.Core.showToast(`✦ Room is holding your blessing — try again in ${l}s`);return}if(e){if(e.dataset.blessed)return;e.dataset.blessed="1",e.style.opacity="0.7",e.innerHTML=`${N} Blessed ✦`,setTimeout(()=>{delete e.dataset.blessed,e.style.opacity="1";const l='<div class="bless-dots"><div class="bless-dot"></div><div class="bless-dot"></div><div class="bless-dot"></div></div>';e.innerHTML=`${l} ${N} Bless this room ${l}`},3e3)}this._lastBlessedAt=t,this._showBlessingAnimation(null),window.Core.showToast("Blessing sent ✨");const a=await p.blessRoom(this.roomId);if(a.status==="cooldown"){window.Core.showToast("✦ This room is still holding your last blessing"),this._lastBlessedAt=t;return}if(a.status==="error")return;this._blessingRows=this._blessingRows??[];const s=p.userId;if(s&&!this._blessingRows.some(l=>(l.user_id||l.userId)===s)){const l=((r=a.data)==null?void 0:r.profiles)??null;this._blessingRows=[...this._blessingRows,{user_id:s,profiles:l}]}this._updateCardBlessingBadge(this._blessingRows.length),this._refreshBlessingCounter()}async _loadBlessingCount(){if(this._dbReady())try{const e=await p.getRoomBlessings(this.roomId);this._blessingRows=e,this._updateCardBlessingBadge(e.length)}catch{}}_updateCardBlessingBadge(e){const t=document.getElementById(`${this.roomId}BlessBtn`);if(!t)return;const i='<div class="bless-dots"><div class="bless-dot"></div><div class="bless-dot"></div><div class="bless-dot"></div></div>';t.style.color="#fff",t.innerHTML=`${i} ${N} Bless this room ${i}`}_subscribeToBlessings(){this._dbReady()&&p.subscribeToBlessings(this.roomId,e=>{this._showBlessingAnimation(e),this._optimisticBlessingBump(e),this._debouncedRefreshBlessingCounter()})}_optimisticBlessingBump(e){this._blessingRows=this._blessingRows??[];const t=(e==null?void 0:e.userId)||(e==null?void 0:e.user_id);t&&!this._blessingRows.some(i=>(i.user_id||i.userId)===t)&&(this._blessingRows=[...this._blessingRows,e]),this._renderBlessingCounter(this._blessingRows),this._updateCardBlessingBadge(this._blessingRows.length)}_debouncedRefreshBlessingCounter(){clearTimeout(this._blessingRefreshTimer),this._blessingRefreshTimer=setTimeout(()=>this._refreshBlessingCounter(),3e3)}async _refreshBlessingCounter(){if(this._dbReady())try{const e=await p.getRoomBlessings(this.roomId);this._blessingRows=e,this._renderBlessingCounter(e),this._updateCardBlessingBadge(e.length)}catch{}}_normaliseBlessingRow(e){return e.profiles?e:{user_id:e.userId||e.user_id,profiles:{name:e.name||"A member",avatar_url:e.avatarUrl||"",emoji:e.emoji||""}}}_renderBlessingCounter(e){const t=document.getElementById(`${this.roomId}BlessedCounter`);if(!t)return;if(!e.length){t.innerHTML='<span style="font-size:12px;color:var(--text-muted);opacity:0.6;">No blessings yet</span>';return}const i=e.map(r=>this._normaliseBlessingRow(r)),o=this._buildAvatarStack(i),a=Math.max(0,e.length-F.MAX),s=a>0?`<span style="font-size:10px;color:var(--text-muted);margin-left:4px;">+${a}</span>`:"";t.innerHTML=`
            <span style="font-size:11px;color:var(--text-muted);white-space:nowrap;margin-right:4px;display:inline-flex;align-items:center;gap:0.25rem;">${S.bless} Blessed by</span>
            <div style="display:flex;align-items:center;margin-left:6px;">${o}</div>
            ${s}`}_showBlessingAnimation(e){if(document.getElementById("blessingAnimationOverlay"))return;const t=3e3,i=window.innerWidth<768,o=window.innerWidth,a=window.innerHeight,s=o/2,r=a/2,l=document.createElement("div");l.id="blessingAnimationOverlay",l.style.cssText=["position:fixed;inset:0;pointer-events:none;z-index:999999;","backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);","background:rgba(0,0,0,0);","transition:background 0.2s ease;"].join(""),document.body.appendChild(l),requestAnimationFrame(()=>{l.style.background="rgba(0,0,0,0.25)"});const c=document.createElement("canvas");c.width=o,c.height=a,c.style.cssText="position:absolute;inset:0;width:100%;height:100%;background:transparent;",l.appendChild(c);const d=c.getContext("2d"),u=document.createElement("div");u.style.cssText=["position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) scale(0.6);","color:rgba(255,230,120,0.95);font-size:clamp(36px,8vw,72px);font-weight:300;","letter-spacing:0.25em;text-transform:uppercase;white-space:nowrap;","font-family:var(--serif,Georgia,serif);","text-shadow:0 0 40px rgba(255,200,60,0.8),0 0 80px rgba(255,180,40,0.4);","opacity:0;transition:opacity 0.35s ease,transform 0.5s cubic-bezier(0.34,1.56,0.64,1);","pointer-events:none;"].join(""),u.textContent="✦  Blessed  ✦",l.appendChild(u),setTimeout(()=>{u.style.opacity="1",u.style.transform="translate(-50%,-50%) scale(1)"},t*.42),setTimeout(()=>{u.style.opacity="0",u.style.transform="translate(-50%,-50%) scale(1.15)",u.style.transition="opacity 0.4s ease,transform 0.4s ease"},t-600);const m=(e==null?void 0:e.name)??null;if(m){const h=document.createElement("div");h.style.cssText=["position:absolute;bottom:18%;left:50%;transform:translateX(-50%);","color:rgba(255,230,160,0.9);font-size:15px;font-weight:400;","letter-spacing:0.18em;text-transform:uppercase;white-space:nowrap;","font-family:var(--serif,Georgia,serif);","opacity:0;transition:opacity 0.6s ease 0.6s;"].join(""),h.textContent=`✦  ${m} blesses this room  ✦`,l.appendChild(h),requestAnimationFrame(()=>{h.style.opacity="1"}),setTimeout(()=>{h.style.opacity="0",h.style.transition="opacity 0.4s ease"},t-500)}requestAnimationFrame(()=>{const h=i?120:260,y=[];for(let M=0;M<h;M++){const v=Math.random()*Math.PI*2,k=20+Math.random()*Math.max(o,a)*.55,x=Math.random()<.75,T=x?38+Math.random()*18:265+Math.random()*25,w=x?85+Math.random()*15:70+Math.random()*20,I=60+Math.random()*25;y.push({angle:v,radius:k,x:s+Math.cos(v)*k,y:r+Math.sin(v)*k,size:1+Math.random()*(i?2:3),hue:T,sat:w,lit:I,delay:Math.random()*.3,speed:.8+Math.random()})}const f=performance.now();let b;const C=M=>{const v=(M-f)/t,k=.42,x=.48,T=1;if(d.globalCompositeOperation="destination-out",d.globalAlpha=.18,d.fillStyle="rgba(0,0,0,1)",d.fillRect(0,0,o,a),d.globalCompositeOperation="source-over",d.globalAlpha=1,v>k*.6&&v<T*.85){const w=v<x?(v-k*.6)/(x-k*.6):1-(v-x)/(T-x),I=Math.max(0,Math.min(1,w)),E=80+w*120,$=d.createRadialGradient(s,r,0,s,r,E);$.addColorStop(0,`rgba(255,220,100,${.55*I})`),$.addColorStop(.5,`rgba(200,150, 60,${.25*I})`),$.addColorStop(1,"rgba(200,150, 60,0)"),d.globalAlpha=1,d.fillStyle=$,d.beginPath(),d.arc(s,r,E,0,Math.PI*2),d.fill()}y.forEach(w=>{const I=Math.max(0,v-w.delay*.3);let E,$,R;if(I<k){const A=I/k,P=1-(1-A)*(1-A),L=w.radius*(1-P*.97),j=P*Math.PI*2.5*(w.radius>0?1:-1);E=s+Math.cos(w.angle+j)*L,$=r+Math.sin(w.angle+j)*L,R=Math.min(1,P*2.5)}else if(I<x)E=s+(Math.random()-.5)*8,$=r+(Math.random()-.5)*8,R=1;else{const A=(I-x)/(T-x),P=A*A,L=w.angle+Math.PI*(.7+Math.random()*.6),j=P*Math.max(o,a)*.7*w.speed;E=s+Math.cos(L)*j,$=r+Math.sin(L)*j,R=Math.max(0,1-A*1.15)}R<=0||(d.globalAlpha=R,d.fillStyle=`hsl(${w.hue},${w.sat}%,${w.lit}%)`,d.shadowBlur=i?4:8,d.shadowColor=`hsl(${w.hue},100%,70%)`,d.beginPath(),d.arc(E,$,w.size,0,Math.PI*2),d.fill())}),d.globalAlpha=1,d.shadowBlur=0,v<1?b=requestAnimationFrame(C):(l.style.transition="opacity 0.3s ease",l.style.opacity="0",setTimeout(()=>l.remove(),350),cancelAnimationFrame(b))};b=requestAnimationFrame(C)})}buildHeader(){var e;return`
        <header class="ps-header" style="padding:12px 16px;display:flex;flex-direction:column;gap:12px;${this.getHeaderGradient()}">
            ...
            <div class="ps-header-btn-grid">
                ...
                <button type="button" class="ps-leave ps-header-btn" data-action="gentlyLeave">
                    ${S.leave} Gently Leave
                </button>
            </div>
        </header>`}buildSafetyDropdown(){const e=[["CommunityModule.showReportModal()",S.alert,"Report Issue"],["CommunityModule.showBlockModal()",S.block,"Block User"],["CommunityModule.muteChat()",S.mute,"Mute Chat"],["CommunityModule.showHelpModal()",S.help,"Get Help"]];return`
        <div style="position:relative;" id="${this.roomId}SafetyDropdownContainer">
            <button type="button" class="ps-leave" data-action="toggleSafety"
                ...
                ${S.shield} Safety <span style="font-size:12px;">▼</span>
            </button>
            <div id="${this.roomId}SafetyDropdown"
                ...
                <button type="button" data-action="showInstructions"
                    ...
                    <span aria-hidden="true">${S.book}</span> Instructions
                </button>
        ...
        </div>`}updateRoomCard(){var i;const e=document.querySelector(`[data-room-id="${this.roomId}"]`);if(!e)return;const t=e.querySelector(".room-energy span");if(t&&(t.textContent=this.getParticipantText()),this.roomType==="timed"){const o=this.canEnterRoom(),a=this._isWithinOpenWindow();e.style.cursor=o?"pointer":"not-allowed",e.style.opacity=a?"1":"0.55",e.style.border=`3px solid ${a?"#22c55e":"#ef4444"}`,e.onclick=o?()=>this.enterRoom():null,e.classList.toggle("active",a),e.classList.toggle("in-session",!a);let s=e.querySelector(".in-session-label");!a&&!s?(s=document.createElement("div"),s.className="in-session-label",s.style.cssText="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:2;pointer-events:none;",s.innerHTML=ne,e.appendChild(s)):a&&s&&s.remove();const r=e.querySelector(".room-timer");r&&this.getTimerText&&(r.innerHTML=this.getTimerText())}else e.style.border="3px solid #22c55e",e.classList.add("active");(i=this.onUpdateCard)==null||i.call(this,e)}getRoomCardHTML(){const e=this.canEnterRoom(),t=this.roomType==="timed",i=!t||this._isWithinOpenWindow(),o=`${this.roomId}_enterRoom`;return window[o]=()=>this.enterRoom(),`
        ...
        </div>`}setupEventListeners(){this._teardownDelegatedListeners(),this._actionsCache=this.getActions();const e=i=>{const o=i.target.closest("[data-action]");if(o){i.stopPropagation();const a=this._actionsCache[o.dataset.action];if(a){a(i);return}console.warn(`[${this.roomId}] Unknown data-action: "${o.dataset.action}"`);return}this._handleOutsideClick(i)},t=i=>{if(i.target.tagName!=="SELECT")return;const o=i.target.closest("[data-action]");if(!o)return;const a=this._actionsCache[o.dataset.action];a&&a(i)};document.addEventListener("click",e),document.addEventListener("change",t),this._delegatedClickHandler=e,this._delegatedChangeHandler=t,this.eventListeners.push({element:document,event:"click",handler:e},{element:document,event:"change",handler:t})}_teardownDelegatedListeners(){this._delegatedClickHandler&&(document.removeEventListener("click",this._delegatedClickHandler),document.removeEventListener("change",this._delegatedChangeHandler),this.eventListeners=this.eventListeners.filter(({handler:e})=>e!==this._delegatedClickHandler&&e!==this._delegatedChangeHandler),this._delegatedClickHandler=null,this._delegatedChangeHandler=null)}getActions(){return{gentlyLeave:()=>this.gentlyLeave(),toggleSafety:e=>this.toggleSafetyDropdown(e),showInstructions:()=>this.showInstructions(),closeInstructions:()=>this.closeInstructions(),showSchedule:()=>{var e;return(e=this.showScheduleModal)==null?void 0:e.call(this)},closeScheduleModal:()=>{var e;return(e=this.closeScheduleModal)==null?void 0:e.call(this)},toggleDimMode:()=>{var e;return(e=this.toggleDimMode)==null?void 0:e.call(this)},toggleTimer:()=>{var e;return(e=this.toggleTimer)==null?void 0:e.call(this)},adjustTime:e=>{var t;return(t=this.adjustTime)==null?void 0:t.call(this,+this._actionEl(e).dataset.minutes)},toggleSoundSettings:()=>{var e;return(e=this.toggleSoundSettings)==null?void 0:e.call(this)},toggle5minBell:()=>{var e;return(e=this.toggle5minBell)==null?void 0:e.call(this)},toggleAmbientSound:()=>{var e;return(e=this.toggleAmbientSound)==null?void 0:e.call(this)},selectBell:e=>{var t;return(t=this.selectBell)==null?void 0:t.call(this,this._actionEl(e).dataset.value)},selectCompletion:e=>{var t;return(t=this.selectCompletion)==null?void 0:t.call(this,this._actionEl(e).dataset.value)},selectAmbient:e=>{var t;return(t=this.selectAmbient)==null?void 0:t.call(this,this._actionEl(e).dataset.value)},previewSound:e=>{var t;return(t=this.previewSound)==null?void 0:t.call(this,this._actionEl(e).dataset.value,e)},previewAmbient:e=>{var t;return(t=this.previewAmbient)==null?void 0:t.call(this,this._actionEl(e).dataset.value,e)},sendMessage:e=>{var t;return(t=this.sendMessage)==null?void 0:t.call(this,this._actionEl(e).dataset.channel)},switchTab:e=>{var t;return(t=this.switchTab)==null?void 0:t.call(this,this._actionEl(e).dataset.tab)},skipBackward:()=>{var e;return(e=this.skipBackward)==null?void 0:e.call(this)},togglePlayPause:()=>{var e;return(e=this.togglePlayPause)==null?void 0:e.call(this)},skipForward:()=>{var e;return(e=this.skipForward)==null?void 0:e.call(this)}}}_actionEl(e){return e.target.closest("[data-action]")}_handleOutsideClick(e){var i;const t=document.getElementById(`${this.roomId}SafetyDropdownContainer`);if(t&&!t.contains(e.target)){const o=document.getElementById(`${this.roomId}SafetyDropdown`);o&&(o.style.display="none")}(i=this.onOutsideClick)==null||i.call(this,e)}toggleSafetyDropdown(e){e==null||e.stopPropagation();const t=document.getElementById(`${this.roomId}SafetyDropdown`),i=document.getElementById(`${this.roomId}SafetyDropdownContainer`);if(t){if(t.style.display==="block"){t.style.display="none";return}if(i){const{bottom:o,right:a}=i.getBoundingClientRect();Object.assign(t.style,{position:"fixed",top:`${o+6}px`,right:`${window.innerWidth-a}px`,left:"auto",zIndex:"2147483640",marginTop:"0"})}t.style.display="block"}}showInstructions(){var e,t;(e=document.getElementById(`${this.roomId}SafetyDropdown`))==null||e.style.setProperty("display","none"),(t=document.getElementById(`${this.roomId}InstructionsModal`))==null||t.classList.add("active")}closeInstructions(){var e;(e=document.getElementById(`${this.roomId}InstructionsModal`))==null||e.classList.remove("active")}getParticipantText(){return`${this.state.participants} present`}getHeaderGradient(){return"background:linear-gradient(135deg,rgba(139,92,246,0.1) 0%,rgba(168,85,247,0.05) 100%);"}getDevModeBadge(){return""}formatTime(e){return`${Math.floor(e/60)}:${String(e%60).padStart(2,"0")}`}registerEventListener(e,t,i){e.addEventListener(t,i),this.eventListeners.push({element:e,event:t,handler:i})}}g._hubPresenceSub=null;g._hubRooms=[];g.ROOM_ACTIVITIES={"silent-room":"🧘 Silent practice","guided-room":"🎧 Guided session","breathwork-room":"💨 Breathwork","campfire-room":"🔥 Around the fire","osho-room":"🌀 Osho space","deepwork-room":"🎯 Deep work","tarot-room":"🔮 Tarot reading","reiki-room":"✨ Reiki session"};g.startHubPresence=async function(n){var i;n&&(g._hubRooms=n);const e=g._hubRooms;if(!e.length)return;if(!((i=p)!=null&&i.ready)){const o=setInterval(()=>{var a;(a=p)!=null&&a.ready&&(clearInterval(o),g.startHubPresence())},500);return}g.stopHubPresence();const t=async()=>{const o=await p.getActiveMembers(),a=o.filter(s=>s.is_phantom).length;e.forEach(s=>{const r=o.filter(l=>l.room_id===s.roomId).length;s.state.participants=r+a,s._updateRoomCardCount(r+a)})};await t(),g._hubPresenceSub=p.subscribeToPresence(t)};g.stopHubPresence=function(){if(g._hubPresenceSub){try{g._hubPresenceSub.unsubscribe()}catch{}g._hubPresenceSub=null}};window.PracticeRoom=g;window.dispatchRoomReady=function(n){document.dispatchEvent(new CustomEvent("practiceRoomReady",{detail:{roomKey:n}}))};window.openMemberProfileAboveRoom=function(n){!window.MemberProfileModal||!n||(MemberProfileModal.open(n),requestAnimationFrame(()=>{const e=openMemberProfileAboveRoom._cachedEls;(!e||e.some(t=>!t.isConnected))&&(openMemberProfileAboveRoom._cachedEls=[document.getElementById("memberProfileModal"),document.getElementById("memberProfileOverlay"),document.querySelector(".member-profile-overlay"),document.querySelector(".member-profile-modal"),document.querySelector('[class*="member"][class*="modal"]'),document.querySelector('[id*="memberProfile"]')].filter(Boolean)),openMemberProfileAboveRoom._cachedEls.forEach(t=>{t.style.zIndex="200000"})}))};const Oe=Object.freeze(Object.defineProperty({__proto__:null,PracticeRoom:g},Symbol.toStringTag,{value:"Module"}));class B extends g{constructor(e){super(e),this.initPlayerState(),this.initCycleState(),this.sessions=[],this.scheduleModalTitle="📅 Upcoming Sessions",this.state.currentSession=null,this.state.nextSession=null}onInit(){this.initializeCycle(),this.preloadYouTubeAPI()}onEnter(){this.loadYouTubeAPI()}onCleanup(){this.cleanupPlayer(),this.cleanupCycle()}setSessions(e){const t=Math.floor(e/(this.config.cycleDuration*1e3))%this.sessions.length;this.state.currentSession=this.sessions[t],this.state.nextSession=this.sessions[(t+1)%this.sessions.length]}getCurrentSession(){return this.state.currentSession}getNextSession(){return this.state.nextSession}buildBody(){const e=this.getCurrentSession();return`
        <div class="ps-body">
            <main class="ps-main">
        ...
        <div class="schedule-item${a?" current":""}"
            ...
            <div style="display:flex;align-items:center;gap:12px;flex:1;">
                ...
                <div>
                    <div style="font-weight:600;font-size:14px;">${l.title}${h}</div>
        ...
        <div class="ps-body">
            ...
            </ul>`}}const fe=new ye;window.GuidedRoom=fe;var se;(se=window.dispatchRoomReady)==null||se.call(window,"guided");const Ge=Object.freeze(Object.defineProperty({__proto__:null,GuidedRoom:ye,guidedRoom:fe},Symbol.toStringTag,{value:"Module"}));class ve extends B{constructor(){super({roomId:"osho",roomType:"timed",name:"OSHO Active",icon:"💃",description:"OSHO Active Meditations practices (with instructions). View Schedule for details.",energy:"Dynamic",imageUrl:"/Community/OSHO.webp",participants:0,cycleDuration:5400,openDuration:600,sessionDuration:4800}),this.scheduleModalTitle="📅 Upcoming OSHO Sessions",this.sessions=[{title:"OSHO Dynamic Meditation",duration:"77:00",category:"Energy",introVideoId:"Q_PFlkHH7IA",videoId:"tLUxtq3peR8",emoji:"🔥"},{title:"OSHO Kundalini Meditation",duration:"77:00",category:"Movement",introVideoId:"O3-wH2VBdN8",videoId:"vEyageQp6w8",emoji:"💃"},{title:"OSHO Nadabrahma Meditation",duration:"77:00",category:"Humming",introVideoId:"tnVsMXf88Pw",videoId:"yVGhzBVT64A",emoji:"🕉️"},{title:"OSHO Nataraj Meditation",duration:"77:00",category:"Dance",introVideoId:"pxg3FmOeQhk",videoId:"grSjP12Q4Oc",emoji:"🎭"},{title:"OSHO Whirling Meditation",duration:"77:00",category:"Spinning",introVideoId:"Jk2AaABIKTY",videoId:"EKvLFs9niXY",emoji:"🌀"}]}onEnter(){this._playingIntro=!1,super.onEnter()}initPlayer(){if(this.state.playerInitialized)return;const e=this.getCurrentSession();if(!(e!=null&&e.introVideoId)){super.initPlayer();return}this._playingIntro=!0,this.state.player=new YT.Player(`${this.roomId}-youtube-player`,{videoId:e.introVideoId,playerVars:{autoplay:0,controls:1,modestbranding:1,rel:0,mute:1},events:{onReady:t=>this.onPlayerReady(t),onStateChange:t=>this.onPlayerStateChange(t)}}),this.state.playerInitialized=!0}startSession(){var t,i,o;if(!this.state.playerReady||this.state.sessionStarted)return;const e=this.getCurrentSession();e&&(this._playingIntro=!0,this._showPlayer(),(t=this.state.player)==null||t.unMute(),(i=this.state.player)==null||i.setVolume(100),(o=this.state.player)==null||o.playVideo(),this.state.sessionStarted=!0,window.Core.showToast(`${e.emoji} Intro starting…`))}onVideoEnded(){if(this._playingIntro){this._playingIntro=!1;const e=this.getCurrentSession();e!=null&&e.videoId&&this.state.player&&(this.state.player.loadVideoById(e.videoId),this.state.player.playVideo(),window.Core.showToast(`${e.emoji} Practice starting…`))}else this.stopProgressTracking(),window.Core.showToast("Session complete")}getInstructions(){return`
        ...
        <div class="modal-overlay" id="${this.roomId}SetupModal" style="z-index:200001;">
            ...
            </ul>`}}Object.assign(O.prototype,Q);Object.assign(O.prototype,Y);Object.assign(O.prototype,z);const ke=new O;window.DeepWorkRoom=ke;var de;(de=window.dispatchRoomReady)==null||de.call(window,"deepwork");const qe=Object.freeze(Object.defineProperty({__proto__:null,DeepWorkRoom:O,deepWorkRoom:ke},Symbol.toStringTag,{value:"Module"}));class X extends g{constructor(){super({roomId:"campfire",roomType:"always-open",name:"Community Campfire",icon:"🔥",description:"A warm space to share, reflect, and connect with the community. Pull up a chair. Real conversations, real people, real connection.",energy:"Social",imageUrl:"/Community/Campfire.webp",participants:12}),this.initChatState(["main"])}onEnter(){this.loadRoomChatFromDB("main"),this._injectSenderAvatar("main")}getHeaderGradient(){return"background:linear-gradient(135deg,rgba(234,88,12,0.1) 0%,rgba(251,146,60,0.05) 100%);"}getParticipantText(){return`${this.state.participants} around the fire`}buildBody(){return`
        <div class="ps-body">
        ...
        ${i?`<p style="font-family:var(--serif);font-size:16px;font-weight:700;color:var(--text);text-align:center;margin:0;letter-spacing:.01em;">${i}</p>`:""}`}_getEnrichedHTML(e,t=!0){const i=`${e.suit}-${e.number}-${t}`;return this._enrichedCache[i]||(this._enrichedCache[i]=this._buildEnrichedSections(e,t)),this._enrichedCache[i]}_buildEnrichedSections(e,t=!0){const i=this.getCardData(e.number,e.suit);if(!i)return"";const o={pentacles:"Earth",swords:"Air",cups:"Water",wands:"Fire"},a={pentacles:"Physical",swords:"Mental",cups:"Emotional",wands:"Spiritual"},s={Air:"Air",Fire:"Fire",Water:"Water",Earth:"Earth"};let r=null,l=null;e.suit==="major"?r=s[i.correspondence]??null:(r=o[e.suit]??null,l=a[e.suit]??null);const c=(i.keywords||[]).map(y=>`<span style="display:inline-block;padding:4px 12px;border:1px solid var(--border);border-radius:20px;font-size:12px;color:var(--text-muted);margin:3px 4px;white-space:nowrap;">${y}</span>`).join(""),d=[];i.astrology&&i.astrology!=="-"&&d.push({label:"Astrology",value:i.astrology}),r&&d.push({label:"Element",value:r}),l&&d.push({label:"Aspect",value:l}),i.treeOfLife&&d.push({label:"Tree of Life",value:i.treeOfLife});const u=d.map(y=>`
            ...
            </div>`).join(""),m=(i.symbols||[]).map(y=>`<li style="margin:6px 0;font-size:14px;color:var(--text-muted);line-height:1.5;">${y}</li>`).join(""),h=this.roomId;return`
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:24px;margin-bottom:12px;">
            ...
            <div id="${h}MeaningText" style="text-align:center;font-size:15px;line-height:1.7;max-width:560px;margin:0 auto;">${i.upright||""}</div>
            <div id="${h}MeaningTextReversed" style="display:none;text-align:center;font-size:15px;line-height:1.7;max-width:560px;margin:0 auto;color:var(--text-muted);">${i.reversed||""}</div>
            ${m?`
        ...
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:24px;margin-bottom:12px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                ...
                <button onclick="window.TarotRoom._toggleJournalLog()" id="${h}JournalLogBtn"
            ...
            <div id="${h}JournalForm">
                <textarea id="${h}JournalInput"
            ...
            <div id="${h}JournalLog" style="display:none;">
                <div id="${h}JournalLogList" style="display:flex;flex-direction:column;gap:10px;max-height:340px;overflow-y:auto;"></div>
            </div>
        </div>
        ${t?`
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:24px;margin-bottom:12px;">
            ...
            <div style="display:flex;gap:8px;margin-bottom:16px;">
                <input id="${h}InterpInput" type="text" maxlength="140"
            ...
            <div id="${h}InterpList" style="display:flex;flex-direction:column;gap:8px;max-height:220px;overflow-y:auto;">
                <div style="font-size:13px;color:var(--text-muted);text-align:center;padding:12px;">Loading interpretations…</div>
            </div>
        </div>`:""}`}_setMeaningTab(e){const t=document.getElementById(`${this.roomId}UprightBtn`),i=document.getElementById(`${this.roomId}ReversedBtn`),o=document.getElementById(`${this.roomId}MeaningText`),a=document.getElementById(`${this.roomId}MeaningTextReversed`);t&&(e==="upright"?(t.style.background="var(--accent)",t.style.color="#fff",t.style.borderColor="var(--accent)",i.style.background="transparent",i.style.color="var(--text-muted)",i.style.borderColor="var(--border)",o.style.display="",a.style.display="none"):(i.style.background="var(--accent)",i.style.color="#fff",i.style.borderColor="var(--accent)",t.style.background="transparent",t.style.color="var(--text-muted)",t.style.borderColor="var(--border)",o.style.display="none",a.style.display=""))}_toggleJournalLog(){const e=document.getElementById(`${this.roomId}JournalLog`),t=document.getElementById(`${this.roomId}JournalForm`),i=document.getElementById(`${this.roomId}JournalLogBtn`);if(!e)return;const o=e.style.display!=="none";e.style.display=o?"none":"",t.style.display=o?"":"none",i.textContent=o?"View Log":"Write Entry",o||this._loadJournalLog()}async _saveJournalEntry(){var s;const e=document.getElementById(`${this.roomId}JournalInput`),t=(s=e==null?void 0:e.value)==null?void 0:s.trim();if(!t){window.Core.showToast("Please write something first");return}const i=window.Core.state.currentUser,o=this.state.dailyCard,a=new Date().toISOString().slice(0,10);try{await p._sb.from("tarot_reflections").insert({user_id:i==null?void 0:i.id,card_key:`${o.suit}-${o.number}`,card_name:this.getCardName(o.number,o.suit),date:a,reflection:t}),e.value="",window.Core.showToast("Saved to your journal ✨")}catch(r){console.warn("[TarotRoom] journal save failed",r),window.Core.showToast("Could not save — please try again")}}async _loadJournalLog(){const e=window.Core.state.currentUser,t=document.getElementById(`${this.roomId}JournalLogList`);if(!(!t||!(e!=null&&e.id))){t.innerHTML='<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:12px;">Loading…</div>';try{const{data:i,error:o}=await p._sb.from("tarot_reflections").select("id, card_name, date, reflection").eq("user_id",e.id).order("created_at",{ascending:!1}).limit(30);if(o)throw o;if(!i||i.length===0){t.innerHTML='<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:12px;">No journal entries yet.</div>';return}t.innerHTML=i.map(a=>`
                ...
                </div>`).join("")}catch(i){console.warn("[TarotRoom] load journal failed",i),t.innerHTML='<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:12px;">Could not load journal.</div>'}}}_editJournalEntry(e){var t,i;(t=document.getElementById(`jr-text-${e}`))!=null&&t.style&&(document.getElementById(`jr-text-${e}`).style.display="none"),(i=document.getElementById(`jr-edit-${e}`))!=null&&i.style&&(document.getElementById(`jr-edit-${e}`).style.display="")}_cancelEditEntry(e){var t,i;(t=document.getElementById(`jr-text-${e}`))!=null&&t.style&&(document.getElementById(`jr-text-${e}`).style.display=""),(i=document.getElementById(`jr-edit-${e}`))!=null&&i.style&&(document.getElementById(`jr-edit-${e}`).style.display="none")}async _saveEditedEntry(e){var a;const t=document.getElementById(`jr-edit-${e}`),i=t==null?void 0:t.querySelector("textarea"),o=(a=i==null?void 0:i.value)==null?void 0:a.trim();if(o)try{await p._sb.from("tarot_reflections").update({reflection:o}).eq("id",e);const s=document.getElementById(`jr-text-${e}`);s&&(s.textContent=o),this._cancelEditEntry(e),window.Core.showToast("Entry updated")}catch(s){console.warn("[TarotRoom] edit failed",s),window.Core.showToast("Could not update entry")}}async _deleteJournalEntry(e){var t;try{await p._sb.from("tarot_reflections").delete().eq("id",e),(t=document.getElementById(`jr-${e}`))==null||t.remove(),window.Core.showToast("Entry deleted")}catch(i){console.warn("[TarotRoom] delete failed",i),window.Core.showToast("Could not delete entry")}}async _submitInterpretation(){var s;const e=document.getElementById(`${this.roomId}InterpInput`),t=(s=e==null?void 0:e.value)==null?void 0:s.trim();if(!t)return;const i=window.Core.state.currentUser,o=this.state.dailyCard,a=new Date().toISOString().slice(0,10);try{await p._sb.from("tarot_interpretations").insert({user_id:i==null?void 0:i.id,display_name:(i==null?void 0:i.display_name)||(i==null?void 0:i.username)||"A seeker",card_key:`${o.suit}-${o.number}`,date:a,interpretation:t}),e.value="",await this._loadInterpretations(),window.Core.showToast("Shared 🌀")}catch(r){console.warn("[TarotRoom] interpretation save failed",r),window.Core.showToast("Could not share — please try again")}}async _loadInterpretations(){const e=this.state.dailyCard,t=new Date().toISOString().slice(0,10),i=document.getElementById(`${this.roomId}InterpList`);if(!(!i||!e)){this._interpPurgedToday||(this._interpPurgedToday=!0,p._sb.from("tarot_interpretations").delete().lt("date",t).then(()=>{}));try{const{data:o,error:a}=await p._sb.from("tarot_interpretations").select("display_name, interpretation, created_at").eq("card_key",`${e.suit}-${e.number}`).eq("date",t).order("created_at",{ascending:!1}).limit(50);if(a)throw a;if(!o||o.length===0){i.innerHTML='<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:12px;">Be the first to share an interpretation.</div>';return}i.innerHTML=o.map(s=>`
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:var(--surface);border-radius:var(--radius-md);border:1px solid var(--border);">
                    <div style="display:flex;align-items:center;gap:8px;">
                        <span style="font-size:16px;">${s[r.card_type]||"🃏"}</span>
                        <span style="font-size:14px;font-weight:500;">${this._escapeHtml(r.card_name)}</span>
                    </div>
                    <span style="font-size:11px;color:var(--text-muted);white-space:nowrap;">${l} · ${c}</span>
                </div>`}).join(""),this._personalDataLoaded=!0}catch(a){console.warn("[TarotRoom] loadDrawHistory failed",a)}}async _loadMastery(){var a,s;const e=document.getElementById(`${this.roomId}MasteryCount`),t=document.getElementById(`${this.roomId}MasteryProgress`),i=document.getElementById(`${this.roomId}MasteryCards`);if(!e||!p._sb)return;const o=(s=(a=window.Core)==null?void 0:a.state)==null?void 0:s.currentUser;if(o!=null&&o.id)try{const{data:r}=await p._sb.from("tarot_draws").select("card_key").eq("user_id",o.id).eq("card_type","major"),l=new Set((r||[]).map(m=>m.card_key)),c=22,d=l.size,u=Math.round(d/c*100);if(e&&(e.textContent=`${d} / ${c}`),t&&(t.style.width=`${u}%`),i){const m=this.MAJOR_ARCANA_NAMES||{};i.innerHTML=Object.entries(m).map(([h,y])=>{const f=`major-${h}`,b=l.has(f);return`<span style="padding:3px 8px;border-radius:12px;font-size:12px;border:1px solid var(--border);
                        background:${b?"var(--neuro-accent)":"transparent"};
                        color:${b?"white":"var(--text-muted)"};
                        opacity:${b?"1":"0.6"};"
                        title="${y}">${b?"✓ ":""}${y}</span>`}).join("")}}catch(r){console.warn("[TarotRoom] loadMastery failed",r)}}async _clearDrawHistory(){var t,i;if(!p._sb)return;const e=(i=(t=window.Core)==null?void 0:t.state)==null?void 0:i.currentUser;if(e!=null&&e.id&&confirm("Clear your entire draw history? This cannot be undone."))try{await p._sb.from("tarot_draws").delete().eq("user_id",e.id),this._personalDataLoaded=!1,this._loadDrawHistory(),this._loadMastery(),window.Core.showToast("Draw history cleared")}catch(o){console.warn("[TarotRoom] clearDrawHistory failed",o)}}onEnter(){this.state.personalDeck.length||(this.state.personalDeck=this._buildFullDeck()),this._personalTabHydrated=!1,this._tarotDataReady?this._drawAndRenderDaily():this._pendingDailyRender=!0,this.initializeChat(),requestAnimationFrame(()=>{var e;return(e=document.querySelector(`#${this.roomId}View .tarot-main`))==null?void 0:e.scrollTo(0,0)})}getParticipantText(){return`${this.state.participants} seeking guidance`}buildCardFooter(){const e=this.state.dailyCard,t=e?this.getCardName(e.number,e.suit):null;return`
        ...
        </div>`}_onPersonalSelectChange(e){const t=this.roomId;["Major","Minor","Court"].forEach(s=>{if(s.toLowerCase()!==e){const r=document.getElementById(`${t}${s}Select`);r&&(r.value="")}});const i=document.getElementById(`${t}${e.charAt(0).toUpperCase()+e.slice(1)}Select`);if(!(i!=null&&i.value))return;const o=i.value.split(":");let a;o[0]==="major"?a={type:"major",number:parseInt(o[1]),suit:"major"}:o[0]==="minor"?a={type:"minor",number:parseInt(o[1]),suit:o[2]}:a={type:"court",number:parseInt(o[1]),suit:o[2]},this._renderPersonalCard(a)}getActions(){return{...super.getActions(),drawPersonalCard:()=>this.drawPersonalCard(),clearDrawHistory:()=>this._clearDrawHistory(),personalSelectChange:e=>this._onPersonalSelectChange(this._actionEl(e).dataset.source)}}getInstructions(){return`
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:4px;">
            ...
            </div>`).join(""),c=(e.commonIssues||[]).map(f=>`<li style="margin:6px 0;font-size:14px;color:var(--text-muted);line-height:1.5;">${f}</li>`).join(""),d=(e.guidedReflections||[]).map(f=>`<li style="margin:8px 0;font-size:14px;line-height:1.6;color:var(--text);">${f}</li>`).join(""),u=this.roomId,m=i?"daily":"personal",h=i?"Daily":"Personal";return`
        <div style="text-align:center;margin-bottom:24px;display:flex;align-items:center;justify-content:center;gap:8px;">
            ...
            <img width="500" height="400" id="${u}${h}CarouselImg"
        ...
        ${(y=e.fundamentalTruths)!=null&&y.length||e.visualization?`
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:24px;margin-bottom:12px;">
        ...
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:24px;margin-bottom:12px;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
                ...
                <button type="button" data-action="toggleChakraJournal" data-prefix="${h}"
                    id="${u}${h}JournalLogBtn"
                    style="font-size:12px;color:var(--accent);background:none;border:none;cursor:pointer;padding:0;font-weight:600;">View Log</button>
            </div>
            ...
            <div id="${u}${h}JournalForm">
                <textarea id="${u}${h}JournalInput"
                ...
                <button type="button" data-action="saveChakraJournal"
                    data-prefix="${h}" data-key="${e.key}"
                    style="margin-top:10px;padding:9px 22px;border:1px solid var(--accent);background:var(--accent);color:#fff;border-radius:var(--radius-md);font-size:14px;font-weight:600;cursor:pointer;">
                    Save to Journal
                </button>
            </div>
            <div id="${u}${h}JournalLog" style="display:none;">
                <div id="${u}${h}JournalLogList" style="display:flex;flex-direction:column;gap:10px;max-height:340px;overflow-y:auto;"></div>
            </div>
        </div>
        ${o?`
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:24px;margin-bottom:12px;">
        ...
        <div style="border:2px solid var(--border);border-radius:var(--radius-lg);padding:20px;background:var(--surface);margin-top:16px;">
            ...
            <div id="${this.roomId}MasteryChakras" style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center;">
        ...
        </div>`}_toggleChakraJournalLog(e){const t=this.roomId,i=document.getElementById(`${t}${e}JournalLog`),o=document.getElementById(`${t}${e}JournalForm`),a=document.getElementById(`${t}${e}JournalLogBtn`);if(!i)return;const s=i.style.display!=="none";i.style.display=s?"none":"",o.style.display=s?"":"none",a.textContent=s?"View Log":"Write Entry",s||this._loadChakraJournalLog(e)}async _saveChakraJournalEntry(e,t){var d,u;const i=this.roomId,o=document.getElementById(`${i}${e}JournalInput`),a=(d=o==null?void 0:o.value)==null?void 0:d.trim();if(!a){window.Core.showToast("Please write something first");return}const s=window.Core.state.currentUser,r=new Date().toISOString().slice(0,10),c=((u=Object.values(this.CHAKRA_SCHEDULE||{}).find(m=>m.key===t))==null?void 0:u.name)||t;try{await p._sb.from("reiki_sessions").insert({user_id:s==null?void 0:s.id,chakra_key:t,chakra_name:c,date:r,entry:a}),o.value="",window.Core.showToast("Saved to your chakra journal ✨"),this._masteryLoaded=!1,e==="Personal"&&this._loadChakraMastery()}catch(m){console.warn("[ReikiRoom] journal save failed",m),window.Core.showToast("Could not save — please try again")}}async _loadChakraJournalLog(e){const t=this.roomId,i=window.Core.state.currentUser,o=document.getElementById(`${t}${e}JournalLogList`);if(!(!o||!(i!=null&&i.id))){o.innerHTML='<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:12px;">Loading…</div>';try{const{data:a,error:s}=await p._sb.from("reiki_sessions").select("id, chakra_name, date, entry").eq("user_id",i.id).order("created_at",{ascending:!1}).limit(30);if(s)throw s;if(!a||a.length===0){o.innerHTML='<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:12px;">No journal entries yet.</div>';return}o.innerHTML=a.map(r=>`
                ...
                </div>`).join("")}catch(a){console.warn("[ReikiRoom] load journal failed",a),o.innerHTML='<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:12px;">Could not load journal.</div>'}}}_editSessionEntry(e){var t,i;(t=document.getElementById(`rj-text-${e}`))!=null&&t.style&&(document.getElementById(`rj-text-${e}`).style.display="none"),(i=document.getElementById(`rj-edit-${e}`))!=null&&i.style&&(document.getElementById(`rj-edit-${e}`).style.display="")}_cancelEditEntry(e){var t,i;(t=document.getElementById(`rj-text-${e}`))!=null&&t.style&&(document.getElementById(`rj-text-${e}`).style.display=""),(i=document.getElementById(`rj-edit-${e}`))!=null&&i.style&&(document.getElementById(`rj-edit-${e}`).style.display="none")}async _saveEditedEntry(e){var a;const t=document.getElementById(`rj-edit-${e}`),i=t==null?void 0:t.querySelector("textarea"),o=(a=i==null?void 0:i.value)==null?void 0:a.trim();if(o)try{await p._sb.from("reiki_sessions").update({entry:o}).eq("id",e);const s=document.getElementById(`rj-text-${e}`);s&&(s.textContent=o),this._cancelEditEntry(e),window.Core.showToast("Entry updated")}catch(s){console.warn("[ReikiRoom] edit failed",s),window.Core.showToast("Could not update entry")}}async _deleteSessionEntry(e){var t;try{await p._sb.from("reiki_sessions").delete().eq("id",e),(t=document.getElementById(`rj-${e}`))==null||t.remove(),window.Core.showToast("Entry deleted")}catch(i){console.warn("[ReikiRoom] delete failed",i),window.Core.showToast("Could not delete entry")}}async _submitCommunityEnergy(){var s;const e=document.getElementById(`${this.roomId}EnergyInput`),t=(s=e==null?void 0:e.value)==null?void 0:s.trim();if(!t)return;const i=window.Core.state.currentUser,o=this.CHAKRA_SCHEDULE[this.state.currentDay],a=new Date().toISOString().slice(0,10);try{await p._sb.from("reiki_shares").insert({user_id:i==null?void 0:i.id,display_name:(i==null?void 0:i.display_name)||(i==null?void 0:i.username)||(i==null?void 0:i.name)||"A seeker",chakra_key:o==null?void 0:o.key,date:a,share:t}),e.value="",await this._loadCommunityEnergy(),window.Core.showToast("Shared 🌀")}catch(r){console.warn("[ReikiRoom] community energy save failed",r),window.Core.showToast("Could not share — please try again")}}async _loadCommunityEnergy(){const e=this.CHAKRA_SCHEDULE[this.state.currentDay],t=new Date().toISOString().slice(0,10),i=document.getElementById(`${this.roomId}EnergyList`);if(!(!i||!e)){this._energyPurgedToday||(this._energyPurgedToday=!0,p._sb.from("reiki_shares").delete().lt("date",t).then(()=>{}));try{const{data:o,error:a}=await p._sb.from("reiki_shares").select("display_name, share, created_at").eq("chakra_key",e.key).eq("date",t).order("created_at",{ascending:!1}).limit(50);if(a)throw a;if(!o||o.length===0){i.innerHTML=`<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:12px;">Be the first to share today's energy.</div>`;return}i.innerHTML=o.map(s=>`
                <div style="display:flex;gap:10px;align-items:baseline;padding:8px 10px;background:var(--background);border-radius:var(--radius-md);border:1px solid var(--border);">
                    <span style="font-size:12px;font-weight:600;color:var(--accent);white-space:nowrap;flex-shrink:0;">${this._escapeHtml(s.display_name)}</span>
                    <span style="font-size:14px;line-height:1.5;color:var(--text);">${this._escapeHtml(s.share)}</span>
                </div>`).join("")}catch(o){console.warn("[ReikiRoom] load community energy failed",o),i.innerHTML='<div style="font-size:13px;color:var(--text-muted);text-align:center;padding:12px;">Could not load.</div>'}}}async _loadChakraMastery(){const e=window.Core.state.currentUser,t=document.getElementById(`${this.roomId}MasteryCount`),i=document.getElementById(`${this.roomId}MasteryProgress`),o=document.getElementById(`${this.roomId}MasteryChakras`);if(!o)return;const a=[{key:"root",label:"Root",color:"#ef4444"},{key:"sacral",label:"Sacral",color:"#f97316"},{key:"solar",label:"Solar Plexus",color:"#eab308"},{key:"heart",label:"Heart",color:"#22c55e"},{key:"throat",label:"Throat",color:"#3b82f6"},{key:"third-eye",label:"Third Eye",color:"#6366f1"},{key:"crown",label:"Crown",color:"#a855f7"}];if(!(e!=null&&e.id)){o.innerHTML=a.map(s=>`<span style="padding:5px 14px;border-radius:20px;font-size:13px;font-weight:600;background:var(--border);color:var(--text-muted);">${s.label}</span>`).join(""),t&&(t.textContent="0 / 7");return}try{const{data:s,error:r}=await p._sb.from("reiki_sessions").select("chakra_key").eq("user_id",e.id);if(r)throw r;const l=new Set((s||[]).map(u=>u.chakra_key)),c=a.filter(u=>l.has(u.key)).length,d=Math.round(c/7*100);t&&(t.textContent=`${c} / 7`),i&&(i.style.width=`${d}%`),o.innerHTML=a.map(u=>{const m=l.has(u.key);return`<span style="padding:5px 14px;border-radius:20px;font-size:13px;font-weight:600;
                    background:${m?u.color:"var(--border)"};
                    color:${m?"#fff":"var(--text-muted)"};
                    opacity:${m?"1":"0.5"};
                    transition:all 0.3s ease;">${u.label}</span>`}).join(""),this._masteryLoaded=!0}catch(s){console.warn("[ReikiRoom] load mastery failed",s),t&&(t.textContent="— / 7")}}onEnter(){this._personalTabHydrated=!1,this._masteryLoaded=!1,this.initializeChat(),requestAnimationFrame(()=>{var i;const e=document.getElementById(`${this.roomId}DailyTabContent`),t=document.getElementById(`${this.roomId}DailyChatSection`);if(e&&this._chakraDataReady&&this.state.currentDay){const o=this.CHAKRA_SCHEDULE[this.state.currentDay];e.innerHTML=`
                ...
                ${this._getChakraDisplayHTML(o,this.state.dailyImageIndex,!0,!0)}`,t&&(t.style.display=""),this._loadCommunityEnergy()}(i=document.querySelector(`#${this.roomId}View .tarot-main`))==null||i.scrollTo(0,0)})}getActions(){return{...super.getActions(),startPersonalSession:()=>this.startPersonalSession(),prevChakraImage:e=>{this._actionEl(e).dataset.scope==="daily"?this.previousDailyImage():this.previousPersonalImage()},nextChakraImage:e=>{this._actionEl(e).dataset.scope==="daily"?this.nextDailyImage():this.nextPersonalImage()},toggleChakraJournal:e=>this._toggleChakraJournalLog(this._actionEl(e).dataset.prefix),saveChakraJournal:e=>{const t=this._actionEl(e);this._saveChakraJournalEntry(t.dataset.prefix,t.dataset.key)},submitCommunityEnergy:()=>this._submitCommunityEnergy()}}getInstructions(){return`
            ...
            </ul>`}}Object.assign(V.prototype,z);Object.assign(V.prototype,Z);const Ie=new V;window.ReikiRoom=Ie;var he;(he=window.dispatchRoomReady)==null||he.call(window,"reiki");const Ye=Object.freeze(Object.defineProperty({__proto__:null,ReikiRoom:V,reikiRoom:Ie},Symbol.toStringTag,{value:"Module"}));export{je as C,Oe as P,He as S,ze as T,Le as Y,De as a,Ne as b,Fe as c,We as d,qe as e,Je as f,Ge as g,Ve as o,Ye as r,Ue as s,Ke as t};
