      ...
      <div style="padding:1.5rem;min-height:100vh;">
        <div class="universal-content">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6" style="margin-bottom: 3rem;">
            ${Object.entries(this.spreads).map(([a,o])=>{const n=["options","pyramid","cross"].includes(a),d=this.app.state?.currentUser?.isAdmin||this.app.state?.currentUser?.isVip,s=n&&!d&&!this.app.gamification?.state?.unlockedFeatures?.includes("advance_tarot_spreads");return`
              <div onclick="window.featuresManager.engines.tarot.selectSpread('${a}')"
                   class="card cursor-pointer relative ${this.selectedSpread===a?"border-4":""} ${s?"opacity-75":""}"
                   style="${this.selectedSpread===a?"border-color: var(--neuro-accent);":""} padding: 1.5rem;"
                   title="${s?"Purchase Advanced Tarot Spreads in Karma Shop to unlock":""}">
                ${n?'<span class="premium-badge-tr">PREMIUM</span>':""}
                ${s?'<div style="position: absolute; top: 50%; right: 1rem; transform: translateY(-50%); font-size: 3rem; opacity: 0.3;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:3rem;height:3rem;"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div>':""}
          ...
          <div style="margin-bottom: 3rem; padding: 0 1.5rem;">
            ${(()=>{const n=!(this.app?.state?.currentUser?.isAdmin||this.app?.state?.currentUser?.isVip||this.app?.gamification?.state?.unlockedFeatures?.includes("tarot_vision_ai"));return`
  ...
  .premium-badge {
    ...
    `,this.init()}cardMarkup(t,e,i=!1){const r=i?"width:100%;":"width: clamp(140px, 24vw, 250px);",a=i?"clamp(80px, 16vw, 120px)":"clamp(60px, 12vw, 100px)";return`
      <div class="flex flex-col items-center mx-auto" style="${r}">
        ...
        </div>`}init(){setTimeout(()=>{const t=document.getElementById("tarot-vision-ai-btn");if(t){const e=()=>{!(this.app?.state?.currentUser?.isAdmin||this.app?.state?.currentUser?.isVip)&&!this.app?.gamification?.state?.unlockedFeatures?.includes("tarot_vision_ai")?this.app?.showToast?.("Unlock Tarot Vision AI in the Karma Shop!","info"):this.app?.showToast?.("Tarot Vision AI opening...","info")};t.onclick=e,this.cleanup=()=>{t.onclick=null}}},0)}selectSpread(t){const e=["options","pyramid","cross"];if(!(this.app?.state?.currentUser?.isAdmin||this.app?.state?.currentUser?.isVip)&&e.includes(t)&&!this.app?.gamification?.state?.unlockedFeatures?.includes("advance_tarot_spreads")){this.app?.showToast?.("Unlock Advanced Tarot Spreads in the Karma Shop!","info");return}this.selectedSpread=t,this.prepareReading(),this.render(),setTimeout(()=>{const r=document.querySelector("#tarot-cards-result");r&&r.scrollIntoView({behavior:"smooth",block:"start"})},h.SCROLL_DELAY)}reset(){this.selectSpread(this.selectedSpread)}}typeof window<"u"&&(window.TarotEngine=y);export{y as default};
