var At=Object.defineProperty;var Rt=(t,e,i)=>e in t?At(t,e,{enumerable:!0,configurable:!0,writable:!0,value:i}):t[e]=i;var it=(t,e,i)=>Rt(t,typeof e!="symbol"?e+"":e,i);import{_ as Ce}from"./features-lazy-COJZCo6-.js";const bt=window.AppSupabase||null;window.CommunitySupabase=bt;const u={_sb:null,_uid:null,_subs:{},_heartbeatTimer:null,async init(){if(this._sb=window.AppSupabase||bt,!this._sb)return console.error("[CommunityDB] CommunitySupabase not ready — window.AppSupabase is null"),!1;const{data:{user:t},error:e}=await this._sb.auth.getUser();return e||!t?(console.error("[CommunityDB] No authenticated user:",e==null?void 0:e.message),!1):(this._uid=t.id,!0)},get userId(){return this._uid},get ready(){return!!(this._sb&&this._uid)},_err(t,e){console.error(`[CommunityDB] ${t}:`,(e==null?void 0:e.message)??e)},_profileSelect:"id, name, emoji, avatar_url",_ago(t){return new Date(Date.now()-t).toISOString()},_todayUTC(){const t=new Date;return t.setUTCHours(0,0,0,0),t.toISOString()},async getMyProfile(){if(!this.ready)return null;const{data:t,error:e}=await this._sb.from("profiles").select("*").eq("id",this._uid).single();return e?(this._err("getMyProfile",e),null):t},async getProfile(t){if(!this.ready)return null;const{data:e,error:i}=await this._sb.from("profiles").select("id, name, emoji, avatar_url, inspiration, community_status, community_role, total_sessions, total_minutes, gifts_given, birthday, country").eq("id",t).single();return i?(this._err("getProfile",i),null):e},_parseProgress(t){var i,o,n,r,a,s;const e=typeof t=="string"?JSON.parse(t):t;return{xp:e.xp??0,karma:e.karma??0,level:e.level??1,badges:e.badges??[],unlockedFeatures:e.unlockedFeatures??[],streak:((i=e.streaks)==null?void 0:i.current)??((o=e.streak)==null?void 0:o.current)??0,longestStreak:((n=e.streaks)==null?void 0:n.longest)??0,totalSessions:((r=e.stats)==null?void 0:r.totalSessions)??0,totalMeditations:((a=e.stats)==null?void 0:a.totalMeditations)??0,totalReadings:((s=e.stats)==null?void 0:s.totalReadings)??0,totalTarotSpreads:e.totalTarotSpreads??0,totalJournalEntries:e.totalJournalEntries??0,totalWellnessRuns:e.totalWellnessRuns??0,totalHappinessViews:e.totalHappinessViews??0}},async getUserProgress(t){if(!this.ready)return null;try{const{data:e,error:i}=await this._sb.from("user_progress").select("payload").eq("user_id",t).single();return i||!e?null:this._parseProgress(e.payload)}catch(e){return this._err("getUserProgress",e),null}},getOwnGamificationState(){var e;const t=(e=window.app)==null?void 0:e.gamification;return t?this._parseProgress(t.state??t):null},async uploadAvatar(t){if(!this.ready)return null;try{const e=t.name.split(".").pop().toLowerCase()||"jpg",i=`avatars/${this._uid}.${e}`,{error:o}=await this._sb.storage.from("community-avatars").upload(i,t,{upsert:!0,contentType:t.type});if(o)return this._err("uploadAvatar upload",o),null;const{data:n}=this._sb.storage.from("community-avatars").getPublicUrl(i),r=n==null?void 0:n.publicUrl;if(!r)return null;const a=`${r}?t=${Date.now()}`;return await this.updateProfile({avatar_url:a})?a:null}catch(e){return this._err("uploadAvatar",e),null}},async updateProfile(t){if(!this.ready)return!1;const{error:e}=await this._sb.from("profiles").update({...t,updated_at:new Date().toISOString()}).eq("id",this._uid);return e?(this._err("updateProfile",e),!1):!0},async setPresence(t="online",e="✨ Available",i=null){if(!this.ready)return!1;const o=new Date().toISOString(),{error:n}=await this._sb.from("community_presence").upsert({user_id:this._uid,status:t,activity:e,room_id:i,last_seen:o,updated_at:o},{onConflict:"user_id"});return n&&this._err("setPresence",n),!n},async setOffline(){return this.setPresence("offline","💤 Offline",null)},async getActiveMembers(){if(!this.ready)return[];const{data:t,error:e}=await this._sb.from("community_presence").select(`user_id, status, activity, room_id, is_phantom, last_seen, profiles ( ${this._profileSelect} )`).neq("status","offline").gte("last_seen",this._ago(5*6e4)).order("is_phantom",{ascending:!1}).order("last_seen",{ascending:!1});return e?(this._err("getActiveMembers",e),[]):t||[]},async getRoomParticipants(t){if(!this.ready)return[];const{data:e,error:i}=await this._sb.from("community_presence").select(`user_id, status, activity, room_id, is_phantom, last_seen, profiles ( ${this._profileSelect} )`).or(`room_id.eq.${t},is_phantom.eq.true`).neq("status","offline").gte("last_seen",this._ago(5*6e4)).order("is_phantom",{ascending:!1}).order("last_seen",{ascending:!0});return i?(this._err("getRoomParticipants",i),[]):e||[]},subscribeToPresence(t){return this._subs.presence&&this._subs.presence.unsubscribe(),this._subs.presence=this._sb.channel("community-presence").on("postgres_changes",{event:"*",schema:"public",table:"community_presence"},async()=>t(await this.getActiveMembers())).subscribe(),this._subs.presence},startHeartbeat(t=6e4){this.stopHeartbeat(),this._heartbeatTimer=setInterval(async()=>{var i,o,n;if(!this.ready)return;const e=(i=typeof Core<"u"?Core:window.Core)==null?void 0:i.state;await this.setPresence(((o=e==null?void 0:e.currentUser)==null?void 0:o.status)||"online",((n=e==null?void 0:e.currentUser)==null?void 0:n.activity)||"✨ Available",(e==null?void 0:e.currentRoom)||null)},t),window.addEventListener("beforeunload",()=>this._cleanup())},stopHeartbeat(){this._heartbeatTimer&&(clearInterval(this._heartbeatTimer),this._heartbeatTimer=null)},_reflectionSelect:"id, content, appreciation_count, created_at, profiles ( id, name, emoji, avatar_url )",async getReflections(t=30){if(!this.ready)return[];const{data:e,error:i}=await this._sb.from("reflections").select(this._reflectionSelect).order("created_at",{ascending:!1}).limit(t);return i?(this._err("getReflections",i),[]):e||[]},async postReflection(t){if(!this.ready)return null;const{data:e,error:i}=await this._sb.from("reflections").insert({user_id:this._uid,content:t}).select(this._reflectionSelect).single();return i?(this._err("postReflection",i),null):e},async deleteReflection(t){if(!this.ready)return!1;const{error:e}=await this._sb.from("reflections").delete().eq("id",t);return e?(this._err("deleteReflection",e),!1):!0},async updateReflection(t,e){if(!this.ready)return!1;const{error:i}=await this._sb.from("reflections").update({content:e}).eq("id",t).eq("user_id",this._uid);return i?(this._err("updateReflection",i),!1):!0},subscribeToReflections(t){return this._subs.reflections&&this._subs.reflections.unsubscribe(),this._subs.reflections=this._sb.channel("community-reflections").on("postgres_changes",{event:"INSERT",schema:"public",table:"reflections"},async({new:e})=>{const{data:i}=await this._sb.from("reflections").select(this._reflectionSelect).eq("id",e.id).single();i&&t(i)}).subscribe(),this._subs.reflections},async getMyAppreciations(){if(!this.ready)return new Set;const{data:t,error:e}=await this._sb.from("appreciations").select("reflection_id").eq("user_id",this._uid);return e?(this._err("getMyAppreciations",e),new Set):new Set(t.map(i=>i.reflection_id))},async toggleAppreciation(t,e){if(!this.ready)return null;if(e){const{error:i}=await this._sb.from("appreciations").delete().eq("user_id",this._uid).eq("reflection_id",t);return i?(this._err("removeAppreciation",i),null):{appreciated:!1}}else{const{error:i}=await this._sb.from("appreciations").insert({user_id:this._uid,reflection_id:t});return i?(this._err("addAppreciation",i),null):{appreciated:!0}}},async getReflectionCount(t){if(!this.ready)return null;const{data:e,error:i}=await this._sb.from("reflections").select("appreciation_count").eq("id",t).single();return i?(this._err("getReflectionCount",i),null):(e==null?void 0:e.appreciation_count)??null},async getMyUserAppreciations(){if(!this.ready)return new Set;const{data:t,error:e}=await this._sb.from("user_appreciations").select("appreciated_user_id").eq("user_id",this._uid);return e?(this._err("getMyUserAppreciations",e),new Set):new Set(t.map(i=>i.appreciated_user_id))},async toggleUserAppreciation(t,e){if(!this.ready)return null;if(e){const{error:i}=await this._sb.from("user_appreciations").delete().eq("user_id",this._uid).eq("appreciated_user_id",t);return i?(this._err("removeUserAppreciation",i),null):{appreciated:!1}}else{const{error:i}=await this._sb.from("user_appreciations").insert({user_id:this._uid,appreciated_user_id:t});return i?(this._err("addUserAppreciation",i),null):{appreciated:!0}}},async getUserAppreciationCount(t){if(!this.ready)return 0;const{count:e,error:i}=await this._sb.from("user_appreciations").select("*",{count:"exact",head:!0}).eq("appreciated_user_id",t);return i?(this._err("getUserAppreciationCount",i),0):e||0},_roomMsgSelect:"id, message, created_at, profiles ( id, name, emoji, avatar_url )",async getRoomMessages(t,e=50){if(!this.ready)return[];const{data:i,error:o}=await this._sb.from("room_messages").select(this._roomMsgSelect).eq("room_id",t).order("created_at",{ascending:!0}).limit(e);return o?(this._err("getRoomMessages",o),[]):i||[]},async sendRoomMessage(t,e){if(!this.ready)return null;const{data:i,error:o}=await this._sb.from("room_messages").insert({user_id:this._uid,room_id:t,message:e}).select(this._roomMsgSelect).single();return o?(this._err("sendRoomMessage",o),null):i},subscribeToRoomChat(t,e){const i=`room-${t}`;return this._subs[i]&&this._subs[i].unsubscribe(),this._subs[i]=this._sb.channel(`room-chat-${t}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"room_messages",filter:`room_id=eq.${t}`},async({new:o})=>{const{data:n}=await this._sb.from("room_messages").select(this._roomMsgSelect).eq("id",o.id).single();n&&e(n)}).subscribe(),this._subs[i]},unsubscribeFromRoomChat(t){this._unsub(`room-${t}`)},async sendWhisper(t,e){if(!this.ready)return null;const{data:i,error:o}=await this._sb.from("whispers").insert({sender_id:this._uid,recipient_id:t,message:e}).select().single();return o?(this._err("sendWhisper",o),null):i},async getWhispers(t){if(!this.ready)return[];const{data:e,error:i}=await this._sb.from("whispers").select(`
            ...
            <div id="${e}" style="display:none;margin-top:6px;padding:8px;border-radius:10px;
                ...
                ${Bt.map(i=>`<button type="button" onclick="MemberProfileModal._pickEmoji('${t}','${e}','${i}')"
                    ...
                    style="width:100%;height:100%;object-fit:cover;border-radius:50%;" alt="avatar">`):r&&(s.style.background="",s.innerHTML=`<span>${Ze(r)}</span>`))});const e=(window.BADGE_REGISTRY||[]).map(o=>`<option value="${o.id}" data-icon="${o.icon}" data-rarity="${o.rarity}" data-xp="${o.xp}" data-desc="${o.desc}">${o.icon} ${o.name}</option>`).join(""),i=document.createElement("div");i.innerHTML=`
            ...
            <section class="section" aria-labelledby="activeMembersTitle-${this._uid}">
                ...
                ${o}
            </section>`}get _uid(){return this.__uid||(this.__uid=Math.random().toString(36).slice(2,7)),this.__uid}}function ot(t){return t!=null&&t.length?t.map(Nt).join(""):'<div style="color:var(--text-muted);font-size:13px;padding:12px">No members online</div>'}function Nt(t){var h;if(!t)return"";const e=t.profiles||{},i=e.name||"Member",o=e.emoji||"",n=e.avatar_url||"",r=t.status||"online",a=t.activity||"✨ Available",s=t.user_id,l=(h=window.Core)==null?void 0:h.getAvatarGradient(s||i),d=Fe(i),c=wt[r]||"offline",p=n?`<img src="${n}"
               style="width:100%;height:100%;object-fit:cover;border-radius:inherit;"
               alt="${d}" loading="lazy">`:o?`<span class="member-avatar-icon">${Ze(o)}</span>`:`<span>${Fe(i.charAt(0).toUpperCase())}</span>`;return`
        ...
        `)},formatTime(t){if(typeof t!="number"||t<0)return"0:00";const e=Math.floor(t/60),i=Math.floor(t%60);return`${e}:${i.toString().padStart(2,"0")}`},getAvatarGradient(t){(!t||typeof t!="string")&&(t="default");const e=t.split("").reduce((i,o)=>i+o.charCodeAt(0),0);return this.config.AVATAR_GRADIENTS[Math.abs(e)%this.config.AVATAR_GRADIENTS.length]}};window.Core=x;const Wt={state:{isOpen:!1,view:"inbox",threadPartnerId:null,threadPartnerName:null,realtimeSub:null,bgSub:null},init(){if(document.getElementById("whisperModal"))return;const t=document.createElement("div");t.innerHTML=`
            ...
            </div>`},async _showThread(t,e,i,o){var d;this.state.view="thread",this.state.threadPartnerId=t,this.state.threadPartnerName=e,this._setView("thread"),document.getElementById("whisperModalTitle").textContent=e;const n=document.getElementById("whisperModalSubtitle");n.textContent="Private whisper thread",n.style.display="block";const r=document.getElementById("whisperThreadLoading"),a=document.getElementById("whisperThreadMessages"),s=document.getElementById("whisperThreadView");r.style.display="block",a.innerHTML="";const[l]=await Promise.all([u.getWhispers(t),u.markConversationRead(t)]);r.style.display="none",this._renderThreadMessages(l),setTimeout(()=>{s.scrollTop=s.scrollHeight},50),(d=document.getElementById("whisperReplyText"))==null||d.focus(),await this.refreshUnreadBadge()},_renderThreadMessages(t){const e=document.getElementById("whisperThreadMessages");if(!e)return;if(!t.length){e.innerHTML=`
                <div style="text-align:center;padding:3rem;
                            color:var(--text-muted);font-size:0.85rem;">
                    No messages yet — say something ✨
                </div>`;return}const i=u._uid;e.innerHTML=t.map(o=>{var r;const n=o.sender_id===i||((r=o.sender)==null?void 0:r.id)===i;return this._messageBubbleHTML(n,this._escape(o.message),this._relativeTime(o.created_at))}).join("")},_appendMessage(t){var n,r;const e=document.getElementById("whisperThreadMessages");if(!e)return;(n=e.querySelector("[data-empty]"))==null||n.remove();const i=t.sender_id===u._uid,o=document.createElement("div");o.innerHTML=this._messageBubbleHTML(i,this._escape(t.message),this._relativeTime(t.created_at)),e.appendChild(o.firstElementChild),(r=document.getElementById("whisperThreadView"))==null||r.scrollTo({top:999999})},_messageBubbleHTML(t,e,i){return`
        ...
        <style>
            ...
            background:rgba(0,0,0,0.06);color:var(--neuro-text);`,f=document.createElement("input");f.type=o,f.value=i,f.maxLength=r,f.placeholder=n,f.style.cssText=h;const v=document.createElement("button");v.textContent="✓",v.style.cssText=m;const S=document.createElement("button");S.textContent="✕",S.style.cssText=g;const B=()=>{f.remove(),v.remove(),S.remove(),d&&(d.style.display=""),p&&(p.style.display="")},_=async()=>{var U;const k=f.value.trim().substring(0,r);if(s){const T=s(k);if(T){window.Core.showToast(T);return}}if(v.disabled=!0,v.textContent="...",!await((U=W())==null?void 0:U.updateProfile({[e]:k||null}))){window.Core.showToast("Could not save - please try again"),v.disabled=!1,v.textContent="✓";return}B(),l(k||null),window.Core.showToast(a)};v.onclick=_,S.onclick=B,f.addEventListener("keydown",k=>{k.key==="Enter"&&_(),k.key==="Escape"&&B()}),c.appendChild(f),c.appendChild(v),c.appendChild(S),f.focus()},editBirthday(){var n;const t=document.getElementById("profileBirthdayDisplay");if(!t)return;const e=((n=this._user())==null?void 0:n.birthday)||"",i=document.createElement("input");i.type="date",i.value=e,i.style.cssText=`padding:3px 8px;border-radius:8px;border:1px solid rgba(0,0,0,0.15);
    ...
    <div class="modal-overlay" id="guidelinesModal" onclick="SafetyBar.handleOverlayClick(event,'guidelines')">
        ...
        <div style="margin-bottom:12px;">
            ...
            ${e.map(o=>{var n,r,a,s,l,d,c;return`
                <div class="admin-notif-row ${o.read?"":"unread"}" id="adminNotif_${o.id}">
                    ...
                    <div style="flex:1;min-width:0;">
                        ...
                        <div style="font-size:0.82rem;color:var(--text-muted,#888);">
                            From: <strong>${G(((n=o.payload)==null?void 0:n.sender_name)||"Unknown")}</strong>
                            ${(r=o.payload)!=null&&r.room?`· ${G(o.payload.room)}`:""}
                        </div>
                        ${(a=o.payload)!=null&&a.message?`<div style="font-size:0.82rem;margin-top:4px;font-style:italic;">"${G(o.payload.message)}"</div>`:""}
                        ${(s=o.payload)!=null&&s.reason?`<div style="font-size:0.82rem;margin-top:4px;">Reason: ${G(o.payload.reason)}</div>`:""}
                        ${(l=o.payload)!=null&&l.details?`<div style="font-size:0.82rem;color:var(--text-muted,#888);">${G(o.payload.details)}</div>`:""}
                        ${(d=o.payload)!=null&&d.issueType?`<div style="font-size:0.82rem;margin-top:4px;">Type: ${G(o.payload.issueType)}</div>`:""}
                        ${(c=o.payload)!=null&&c.description?`<div style="font-size:0.82rem;color:var(--text-muted,#888);">${G(o.payload.description)}</div>`:""}
                    </div>
                    ${o.read?"":`
                ...
                <div style="display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:10px;
                    ...
                    <div style="flex:1;min-width:0;">
                        <div style="font-size:0.85rem;font-weight:600;">${G(l.name||"Member")}</div>
                        <div style="font-size:0.75rem;color:var(--text-muted,#888);">${G(p)}</div>
                    </div>
            ...
            <div class="admin-stat-grid">
                ...
                <div style="display:flex;align-items:center;gap:8px;padding:7px 10px;border-radius:8px;
                    ...
                    <span style="flex:1;font-size:0.85rem;font-weight:600;">${G(((l=r.profiles)==null?void 0:l.name)||"Member")}</span>
                ...
                <div>
                    ...
                    ${(o=e.streakMembers)!=null&&o.length?e.streakMembers.map(n=>`
                            <div style="display:flex;align-items:center;gap:8px;padding:6px 10px;border-radius:8px;
                                ...
                                <span>${n.emoji||"🧘"}</span>
                                <span style="font-size:0.83rem;font-weight:600;">${G(n.name||"Member")}</span>
                            </div>`).join(""):'<div style="color:var(--text-muted,#888);font-size:0.83rem;">No data yet.</div>'}
                </div>
            </div>`},async _renderBulk(t){const{data:e=[]}=await CommunityDB._sb.from("profiles").select("id, name, emoji, avatar_url, community_role").order("name");this._bulkMembers=e,this._bulkSelected=new Set,this._bulkRendered.clear();const i=e.map(o=>`
            <label id="bulkRow_${o.id}"
                ...
                <span style="font-size:0.85rem;font-weight:600;color:var(--neuro-text);">${G(o.name||"Member")}</span>
                <span style="font-size:0.75rem;color:var(--text-muted,#888);margin-left:auto;">${G(o.community_role||"Member")}</span>
            ...
            <div style="margin-bottom:16px;">
                ...
                <select id="bulkBadgeSelect" style="${e}">
                    ${Je.map(l=>`<option value="${l.id}" data-icon="${l.icon}" data-rarity="${l.rarity}" data-xp="${l.xp}" data-desc="${l.desc}">${l.icon} ${l.name}</option>`).join("")}
                </select>
        ...
        <header class="ps-header" style="padding:12px 16px;display:flex;flex-direction:column;gap:12px;${this.getHeaderGradient()}">
            ...
            <div class="ps-header-btn-grid">
                ...
                <button type="button" class="ps-leave ps-header-btn" data-action="gentlyLeave">
                    ${_e.leave} Gently Leave
                </button>
            </div>
        </header>`}buildSafetyDropdown(){const e=[["CommunityModule.showReportModal()",_e.alert,"Report Issue"],["CommunityModule.showBlockModal()",_e.block,"Block User"],["CommunityModule.muteChat()",_e.mute,"Mute Chat"],["CommunityModule.showHelpModal()",_e.help,"Get Help"]];return`
        <div style="position:relative;" id="${this.roomId}SafetyDropdownContainer">
            <button type="button" class="ps-leave" data-action="toggleSafety"
                ...
                ${_e.shield} Safety <span style="font-size:12px;">▼</span>
            </button>
            <div id="${this.roomId}SafetyDropdown"
                ...
                <button type="button" data-action="showInstructions"
                    ...
                    <span aria-hidden="true">${_e.book}</span> Instructions
                </button>
        ...
        <div class="practice-room ${o?"active":"in-session"}"
            ...
            ${!o&&i?`<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:2;pointer-events:none;">${rt}</div>`:""}
        ...
        </button>`},cleanupCycle(){this._cycleInterval&&(clearInterval(this._cycleInterval),this._cycleInterval=null)}},Ae=t=>t.charAt(0).toUpperCase()+t.slice(1);function Ye(t,e,i,o){var l,d;const n=i||t.charAt(0).toUpperCase(),r=((d=(l=window.Core)==null?void 0:l.getAvatarGradient)==null?void 0:d.call(l,o||t))??"linear-gradient(135deg, #667eea 0%, #764ba2 100%)",a=e?`<img src="${e}" width="36" height="36" style="width:36px;height:36px;object-fit:cover;border-radius:50%;display:block;flex-shrink:0;" alt="${t}" loading="lazy" decoding="async">`:`<span style="color:white;font-size:13px;font-weight:600;line-height:1;">${n}</span>`,s=e?"background:transparent;":`background:${r};`;return{inner:a,bg:s,gradient:r,initial:n}}const at=t=>new Date(t).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"});function Zt(t){return!t||typeof t!="string"?"":t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}const He={initChatState(t=["main"]){this.chatChannels=t,this.state.messages=Object.fromEntries(t.map(e=>[e,[]]))},async initializeChat(){await Promise.all(this.chatChannels.map(t=>this.loadRoomChatFromDB(t))),setTimeout(()=>this._injectSenderAvatar(),200)},_getDbRoomId(t="main"){return t==="main"?this.roomId:`${this.roomId}-${t}`},_msgContainerId(t){return`${this.roomId}${Ae(t)}Messages`},_inputId(t){return`${this.roomId}${Ae(t)}Input`},_rowToMsgData(t,e=!1){var c,p;const i=t.profiles||{},o=((p=(c=window.Core)==null?void 0:c.state)==null?void 0:p.currentUser)||{},n=e?o.name||"You":i.name||"Member",r=e?o.avatar_url||"":i.avatar_url||"",a=e?o.emoji||"":i.emoji||"",s=i.id||null,{gradient:l,initial:d}=Ye(n,r,a,s);return{name:n,initial:d,avatarUrl:r,avatarBg:l,userId:s,time:at(t.created_at),text:t.message,country:null}},async sendMessage(t="main",e=null){var c,p,h;const i=document.getElementById(e||this._inputId(t));if(!(i!=null&&i.value.trim()))return;const o=i.value.trim(),n=document.getElementById(this._msgContainerId(t)),r=((p=(c=window.Core)==null?void 0:c.state)==null?void 0:p.currentUser)||{},a=r.name||"You",{gradient:s,initial:l}=Ye(a,r.avatar_url||"",r.emoji||"",r.id),d={name:a,initial:l,avatarUrl:r.avatar_url||"",avatarBg:s,userId:r.id||null,time:at(new Date),text:o,country:null,timestamp:Date.now(),...(h=this.getCustomMessageData)==null?void 0:h.call(this,t)};if(this.state.messages[t].push(d),n&&(n.insertAdjacentHTML("beforeend",this.buildMessageHTML(d)),n.scrollTop=n.scrollHeight),i.value="",window.Core.showToast("Message sent"),u!=null&&u.ready)try{await u.sendRoomMessage(this._getDbRoomId(t),o)}catch(m){console.error("[ChatMixin] sendRoomMessage error:",m)}},async loadRoomChatFromDB(t="main"){var n,r,a;if(!(u!=null&&u.ready)){this.loadMessagesFromStorage(t),this.renderSavedMessages(t);return}const e=this._getDbRoomId(t),i=document.getElementById(this._msgContainerId(t)),o=(a=(r=(n=window.Core)==null?void 0:n.state)==null?void 0:r.currentUser)==null?void 0:a.id;try{const[s,l]=await Promise.all([u.getRoomMessages(e,50),u.getBlockedUsers()]);if(i&&s.length){const d=s.filter(c=>{var p;return!l.has((p=c.profiles)==null?void 0:p.id)}).map(c=>{var h;const p=((h=c.profiles)==null?void 0:h.id)===o;return this.buildMessageHTML(this._rowToMsgData(c,p))}).join("");d&&(i.insertAdjacentHTML("beforeend",d),i.scrollTop=i.scrollHeight)}u.subscribeToRoomChat(e,async d=>{var p,h;((p=d.profiles)==null?void 0:p.id)===o||(await u.getBlockedUsers()).has((h=d.profiles)==null?void 0:h.id)||!i||(i.insertAdjacentHTML("beforeend",this.buildMessageHTML(this._rowToMsgData(d))),i.scrollTop=i.scrollHeight)})}catch(s){console.error(`[ChatMixin] loadRoomChatFromDB error (${t}):`,s)}},buildMessageHTML(t){const e=t.avatarUrl?`background-image:url('${t.avatarUrl}');background-size:cover;background-position:center;`:`background:${t.avatarBg};`,i=t.avatarUrl?"":`<span style="color:white;font-size:13px;font-weight:600;line-height:1;">${t.initial}</span>`,o=t.userId?`<span class="campfire-msg-name" role="button" tabindex="0" style="cursor:pointer;" onclick="openMemberProfileAboveRoom('${t.userId}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openMemberProfileAboveRoom('${t.userId}');}" aria-label="View profile of ${t.name}">${t.name}</span>`:`<span class="campfire-msg-name">${t.name}</span>`;return`
        <div class="campfire-msg">
            <div class="campfire-msg-avatar" aria-hidden="true" style="${e}width:36px;height:36px;min-width:36px;overflow:hidden;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${i}</div>
        ...
        </div>`},buildChatContainer(t="main",e="Type your message..."){const i=Ae(t),o=`${this.roomId}${i}SenderAvatar`,n=`${this.roomId}${i}SendBtn`;return`
        ...
        </div>`},buildSoundSettings(){const t=this.roomId,e=Qt.map(n=>this._soundOption("Bell",n.value,n.label,"previewSound","selectBell")).join(""),i=ei.map(n=>this._soundOption("Completion",n.value,n.label,"previewSound","selectCompletion")).join(""),o=ti.map(n=>this._soundOption("Ambient",n.value,n.label,"previewAmbient","selectAmbient")).join("");return`
    ...
    `},generateRelationshipAuditContent(t){return`
      ...
      ${this._closingBlock(t.closingLine)}
    `}},O={generateIntentionPracticeContent(t,e,i){const{intention:o="",affirmation:n="",releaseList:r=""}=t,a=e.map(s=>`
            ...
            <div class="lunar-popup-section">
                ...
                <textarea id="intentionText" class="lunar-textarea-large"
                    ...
                    placeholder="${i.intentionPrompt}"
                >${this._escapeHtml(o)}</textarea>
                <p class="lunar-helper-text">${i.intentionHelper}</p>
            </div>

            <div class="lunar-popup-section">
                <h3>One ${i.affirmationType} Affirmation</h3>
                <p>${i.affirmationPrompt}</p>
                ...
                ${i.affirmationHelper?`<p class="lunar-helper-text">${i.affirmationHelper}</p>`:""}
            </div>

            <div class="lunar-popup-section">
                <h3>${i.listTitle}</h3>
                <p>${i.listPrompt}</p>
                ${i.listHelper?`<p class="lunar-helper-text">${i.listHelper}</p>`:""}
                <textarea id="releaseListText" class="lunar-textarea-large"
                    style="min-height:120px;" maxlength="1000"
                    aria-label="${i.listTitle}"
                    placeholder="1. &#10;2. &#10;3. "
                >${this._escapeHtml(r)}</textarea>
                ${i.listFooter?`<div class="lunar-popup-highlight"><p>${i.listFooter}</p></div>`:""}
            </div>
            ...
            </div>`},generateFutureAlignmentContent({purpose:t,steps:e,guideline:i}){return`
            <div class="lunar-popup-section">
                <h3>Purpose</h3>
                <p>${t}</p>
            </div>
            ...
            <div class="lunar-popup-highlight"><p>${i}</p></div>`},generateBodyPracticeContent({purpose:t,steps:e,guideline:i}){return`
            <div class="lunar-popup-section">
                <h3>Purpose</h3>
                <p>${t}</p>
            </div>
...
@media(prefers-reduced-motion:reduce){*{animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important;}}
        `}},Ke=()=>window.CommunityDB,ke={location:{latitude:31,longitude:0,name:"Default"},currentMoonData:null,currentLunarRoom:null,lunarRooms:[{phaseRanges:[[0,.068],[.932,1]],phaseName:"New Moon",icon:"🌑",roomName:"New Moon Intentions",description:"Set intentions and plant seeds for the lunar cycle",roomId:"new-moon",energy:"Beginning, Stillness, Potential",practices:["Silent meditation","Intention setting","Vision journaling","Seed planting ritual"]},{phaseRanges:[[.068,.432]],phaseName:"Waxing Moon",icon:"🌓",roomName:"Waxing Growth Practice",description:"Build momentum and cultivate expanding energy",roomId:"waxing-moon",energy:"Growth, Action, Building",practices:["Dynamic movement","Energy cultivation","Goal visualization","Action planning meditation"]},{phaseRanges:[[.432,.568]],phaseName:"Full Moon",icon:"🌕",roomName:"Full Moon Illumination",description:"Celebrate fullness and release what no longer serves",roomId:"full-moon",energy:"Culmination, Release, Clarity",practices:["Celebration ritual","Gratitude meditation","Release ceremony","Moon bathing"]},{phaseRanges:[[.568,.932]],phaseName:"Waning Moon",icon:"🌗",roomName:"Waning Release Practice",description:"Let go and reflect on the lunar journey",roomId:"waning-moon",energy:"Release, Reflection, Rest",practices:["Reflection meditation","Letting go ritual","Rest and restore","Completion ceremony"]}],init(){if(this._initialized)return;this._initialized=!0;const t=()=>this.updateAll();navigator.geolocation?navigator.geolocation.getCurrentPosition(({coords:e})=>{this.location={latitude:e.latitude,longitude:e.longitude,name:"Your Location"},t()},()=>t(),{timeout:5e3,maximumAge:36e5}):t(),setInterval(()=>this.updateAll(),6e5)},updateAll(){this.updateMoonData(),this.updateMoonVisualization(),this.updateLunarRoom(),this.renderLunarCard()},setLocation(t,e,i){this.location={latitude:t,longitude:e,name:i},this.updateAll()},updateMoonData(){const t=new Date,e=Be.getMoonIllumination(t),i=Be.getMoonTimes(t,this.location.latitude,this.location.longitude);this.currentMoonData={phase:e.phase,fraction:e.fraction,angle:e.angle,rise:i.rise,set:i.set,phaseName:this.getMoonPhaseName(e.phase),age:e.phase*29.53,nextFullMoon:this.getNextFullMoon(t)},this._renderMoonInfo()},_renderMoonInfo(){const t=this.currentMoonData;if(!t)return;const e=(o,n)=>{const r=document.getElementById(o);r&&(r.textContent=n)};e("moonPhaseName",t.phaseName),e("moonIllumination",`${Math.round(t.fraction*100)}% illuminated`),e("moonAge",`${t.age.toFixed(1)} days old`),e("moonrise",t.rise?this.formatTime(t.rise):"No rise today"),e("moonset",t.set?this.formatTime(t.set):"No set today");const i=Math.ceil((t.nextFullMoon-new Date)/864e5);e("nextPhase",`Next Full Moon: ${this.formatDate(t.nextFullMoon)} (${i} days)`)},updateMoonVisualization(){this.currentMoonData&&this.drawMoon(this.currentMoonData.phase,this.currentMoonData.angle)},drawMoon(t,e){const i=document.getElementById("moonSvg");if(!i)return;const o=50,n=60,r=60;i.innerHTML="";const a=document.createElementNS("http://www.w3.org/2000/svg","g");a.setAttribute("transform",`rotate(${(e*180/Math.PI*-1).toFixed(2)}, ${n}, ${r})`),i.appendChild(a);const s=document.createElementNS("http://www.w3.org/2000/svg","circle");s.setAttribute("cx",n),s.setAttribute("cy",r),s.setAttribute("r",o),s.setAttribute("fill","#2d3748"),a.appendChild(s);const l=Math.cos(t*2*Math.PI)*o,d=document.createElementNS("http://www.w3.org/2000/svg","path");d.setAttribute("fill","#f7fafc"),d.setAttribute("d",t<.5?`M ${n},${r-o} A ${o},${o} 0 0 1 ${n},${r+o} Q ${n+l},${r} ${n},${r-o}`:`M ${n},${r-o} Q ${n+l},${r} ${n},${r+o} A ${o},${o} 0 0 1 ${n},${r-o}`),a.appendChild(d),[{x:.3,y:-.2,r:.15},{x:-.2,y:.3,r:.1},{x:.1,y:.4,r:.08}].forEach(({x:c,y:p,r:h})=>{if(!(t<.5&&c>0||t>=.5&&c<0||t>.45&&t<.55))return;const g=document.createElementNS("http://www.w3.org/2000/svg","circle");g.setAttribute("cx",n+c*o),g.setAttribute("cy",r+p*o),g.setAttribute("r",h*o),g.setAttribute("fill","rgba(0,0,0,0.15)"),a.appendChild(g)})},getMoonPhaseName(t){return t<.03||t>.97?"New Moon":t<.22?"Waxing Crescent":t<.28?"First Quarter":t<.47?"Waxing Gibbous":t<.53?"Full Moon":t<.72?"Waning Gibbous":t<.78?"Last Quarter":"Waning Crescent"},getNextFullMoon(t){const e=Be.getMoonIllumination(t).phase,i=e<.5?(.5-e)*29.53:(1.5-e)*29.53;return new Date(t.getTime()+i*864e5)},getLunarRoomByPhase(t){return this.lunarRooms.find(e=>e.phaseRanges.some(([i,o])=>t>=i&&t<=o))??this.lunarRooms[0]},updateLunarRoom(){var e;if(!this.currentMoonData)return;const t=this.getLunarRoomByPhase(this.currentMoonData.phase);(e=this.currentLunarRoom)==null||e.roomId,t.roomId,this.currentLunarRoom=t},getCurrentRoom(){return this.currentLunarRoom},_ROOM_MODULES:{"new-moon":()=>Ce(()=>Promise.resolve().then(()=>bi),void 0),"waxing-moon":()=>Ce(()=>Promise.resolve().then(()=>xi),void 0),"full-moon":()=>Ce(()=>Promise.resolve().then(()=>ki),void 0),"waning-moon":()=>Ce(()=>Promise.resolve().then(()=>Si),void 0)},_roomExportName:{"new-moon":"NewMoonRoom","waxing-moon":"WaxingMoonRoom","full-moon":"FullMoonRoom","waning-moon":"WaningMoonRoom"},async _loadAndEnterRoom(t){var i,o,n;const e=this._ROOM_MODULES[t];if(!e){(i=window.Core)==null||i.showToast(`Unknown room: ${t}`);return}try{const a=(await e())[this._roomExportName[t]];a?a.enterRoom():(o=window.Core)==null||o.showToast(`${t} failed to initialise`)}catch(r){console.error(`[LunarEngine] Failed to load ${t}:`,r),(n=window.Core)==null||n.showToast(`Failed to load ${t}`)}},joinLunarRoom(){var t;if(!this.currentLunarRoom){(t=window.Core)==null||t.showToast("Lunar room not ready");return}this._loadAndEnterRoom(this.currentLunarRoom.roomId)},adminJoinRoom(t){this._loadAndEnterRoom(t)},renderLunarCard(){var n,r,a;const t=document.getElementById("lunarContainer");if(!t){console.warn("LunarEngine: #lunarContainer not found");return}if(Y.injectStyles(),!this.currentMoonData||!this.currentLunarRoom){t.innerHTML='<p style="color:var(--text-muted);padding:20px;">Loading lunar data...</p>';return}const{currentMoonData:e,currentLunarRoom:i}=this,o=(a=(r=(n=window.Core)==null?void 0:n.state)==null?void 0:r.currentUser)==null?void 0:a.is_admin;t.innerHTML=`
            ...
            </div>`},_attachAdminListeners(){const t=document.querySelector(".lunar-admin-header");if(t&&!t._lunarBound){const e=()=>{const i=document.getElementById(t.dataset.panel),o=document.getElementById(t.dataset.toggle),n=i.style.display!=="none";i.style.display=n?"none":"block",o.textContent=n?"▶":"▼",t.setAttribute("aria-expanded",String(!n))};t.addEventListener("click",e),t.addEventListener("keydown",i=>{(i.key==="Enter"||i.key===" ")&&(i.preventDefault(),e())}),t._lunarBound=!0}document.querySelectorAll(".lunar-admin-room-btn").forEach(e=>{e._lunarBound||(e.addEventListener("click",()=>this.adminJoinRoom(e.dataset.roomId)),e._lunarBound=!0)})},_refreshOuterCard(){var i,o,n;if(!((i=Ke())!=null&&i.ready)){this._outerCardRetry||(this._outerCardRetry=setTimeout(()=>{this._outerCardRetry=null,this._refreshOuterCard()},500));return}const t=(o=this.currentLunarRoom)==null?void 0:o.roomId;if(!t)return;const e=async()=>{var r;try{const a=await((r=Ke())==null?void 0:r.getRoomParticipants(t)),s=document.getElementById("lunarRoomPresence");s&&(s.textContent=`${a.length} present`)}catch(a){console.warn("[LunarEngine] _refreshOuterCard error:",a)}};if(e(),this._attachCardListeners(),this._attachAdminListeners(),this._outerCardSub)try{this._outerCardSub.unsubscribe()}catch{}this._outerCardSub=(n=Ke())==null?void 0:n.subscribeToPresence(e)},formatTime(t){return!t||isNaN(t.getTime())?"N/A":t.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})},formatDate(t){return t.toLocaleDateString("en-US",{month:"short",day:"numeric"})},getMoonData(){return this.currentMoonData},renderLunarRoom(){this.renderLunarCard()},injectAdminUI(){var i,o,n;if(!((n=(o=(i=window.Core)==null?void 0:i.state)==null?void 0:o.currentUser)==null?void 0:n.is_admin))return;const e=document.querySelector("#lunarContainer .celestial-card-full");if(e&&!document.getElementById("lunarAdminPanel")){const r=document.createElement("div");r.innerHTML=this._renderAdminSection(),e.appendChild(r.firstElementChild),this._attachAdminListeners()}}};window.LunarEngine=ke;const te=()=>window.CommunityDB,F=class F{constructor(e){if(!e)throw new Error("LunarRoom: phaseConfig is required");this.config=e,this.isActive=!1,this.currentPractice=null,this.collectiveTimer=null,this.collectiveWords=null,this.saveDebounceTimer=null,this._retryCheckTimeout=null,this._hasLoggedWaiting=!1,this.eventListeners=[],this.userWeekData=this._getDefaultUserData(),this.weekStartDate=null,this.weekEndDate=null,this.domCache={dynamicContent:null,popup:null},this._bindMethods()}_getDefaultUserData(){return{seedAffirmation:null,practiceCount:0,journalEntries:[],emotionalEchoes:[],intentionShared:!1,privateIntention:"",collectiveWord:"",intention:"",affirmation:"",releaseList:""}}_bindMethods(){["init","enterRoom","renderRoomDashboard","switchMode","showPracticePopup","showCollectiveIntentionPopup","startCollectiveStep2","startCollectiveStep3","startCollectiveStep4","startCollectiveStep5","startMeditationTimer","startWitnessingTimer","submitWordToCollective","completeCollectivePractice","closeCollectivePopup","closePracticePopup","saveIntentionPractice","submitClosure","showLockedMessage","enterGroupCircle","leaveRoom","_handlePopupClick","_clearTimer"].forEach(e=>{typeof this[e]=="function"&&(this[e]=this[e].bind(this))})}init(){try{this.checkIfWeekActive(),Y.injectStyles(this.config.cssPrefix)}catch(e){console.error(`${this.config.name} init error:`,e)}}checkIfWeekActive(){var e,i,o;try{if(!(ke!=null&&ke.currentMoonData)){this._hasLoggedWaiting||(this._hasLoggedWaiting=!0),this._retryCheckTimeout||(this._retryCheckTimeout=setTimeout(()=>{this._retryCheckTimeout=null,this.checkIfWeekActive()},500));return}if((o=(i=(e=window.Core)==null?void 0:e.state)==null?void 0:i.currentUser)==null?void 0:o.is_admin){this.isActive=!0,this.calculateWeekDates(),this.loadUserWeekData();return}const r=ke.currentMoonData.phase;this.isActive=this.config.phaseRanges.some(([a,s])=>r>=a&&r<=s),this.isActive&&(this.calculateWeekDates(),this.loadUserWeekData())}catch(n){console.error("checkIfWeekActive error:",n),this.isActive=!1}}calculateWeekDates(){var e,i,o;try{const n=ke==null?void 0:ke.currentMoonData;if(!n){console.error("No moon data");return}const{phase:r,age:a}=n,s=F.CONSTANTS,l=new Date,d=new Date(l-a*s.MS_PER_DAY);let c=this.config.phaseRanges.find(([p,h])=>r>=p&&r<=h);if(!c&&(c=((o=(i=(e=window.Core)==null?void 0:e.state)==null?void 0:i.currentUser)==null?void 0:o.is_admin)?this.config.phaseRanges[0]:null,!c)){this._calculateNextOccurrence(r,a);return}this.weekStartDate=new Date(d.getTime()+c[0]*s.LUNAR_CYCLE_DAYS*s.MS_PER_DAY),this.weekEndDate=new Date(d.getTime()+c[1]*s.LUNAR_CYCLE_DAYS*s.MS_PER_DAY)}catch(n){console.error("calculateWeekDates error:",n)}}_calculateNextOccurrence(e,i){try{const o=F.CONSTANTS,n=new Date,r=new Date(n-i*o.MS_PER_DAY),[a,s]=this.config.phaseRanges[0],l=e<a?r:new Date(r.getTime()+o.LUNAR_CYCLE_DAYS*o.MS_PER_DAY);this.weekStartDate=new Date(l.getTime()+a*o.LUNAR_CYCLE_DAYS*o.MS_PER_DAY),this.weekEndDate=new Date(l.getTime()+s*o.LUNAR_CYCLE_DAYS*o.MS_PER_DAY)}catch(o){console.error("_calculateNextOccurrence error:",o)}}getDaysRemaining(){return this.weekEndDate?Math.max(0,Math.ceil((this.weekEndDate-new Date)/F.CONSTANTS.MS_PER_DAY)):0}shouldShowClosure(){const e=this.getDaysRemaining();return e>0&&e<=2}loadUserWeekData(){try{const e=localStorage.getItem(this.config.storageKey);e&&(this.userWeekData={...this.userWeekData,...JSON.parse(e)})}catch(e){console.error("loadUserWeekData error:",e)}}saveUserWeekData(e=!1){if(e){this._performSave();return}clearTimeout(this.saveDebounceTimer),this.saveDebounceTimer=setTimeout(()=>this._performSave(),F.CONSTANTS.SAVE_DEBOUNCE_MS)}_performSave(){var e;try{localStorage.setItem(this.config.storageKey,JSON.stringify(this.userWeekData))}catch(i){console.error("_performSave error:",i),(e=window.Core)==null||e.showToast("Failed to save data")}}_sanitizeInput(e){return typeof e=="string"?e.replace(/<[^>]*>/g,"").trim():""}_validateIntention(e){const i=F.CONSTANTS;return e!=null&&e.trim()?e.length>i.MAX_INTENTION_LENGTH?{valid:!1,error:`Intention must be under ${i.MAX_INTENTION_LENGTH} characters`}:{valid:!0}:{valid:!1,error:"Intention cannot be empty"}}_validateAffirmation(e){const i=F.CONSTANTS;return e!=null&&e.trim()?e.length>i.MAX_AFFIRMATION_LENGTH?{valid:!1,error:`Affirmation must be under ${i.MAX_AFFIRMATION_LENGTH} characters`}:{valid:!0}:{valid:!1,error:"Affirmation cannot be empty"}}_validateCollectiveWord(e){return e!=null&&e.trim()?e.length>F.CONSTANTS.MAX_WORD_LENGTH?{valid:!1,error:`Word must be under ${F.CONSTANTS.MAX_WORD_LENGTH} characters`}:e.trim().includes(" ")?{valid:!1,error:"Please enter only one word"}:{valid:!0}:{valid:!1,error:"Word cannot be empty"}}enterRoom(){var e,i,o,n,r,a,s,l;try{const d=(o=(i=(e=window.Core)==null?void 0:e.state)==null?void 0:i.currentUser)==null?void 0:o.is_admin;if(!this.isActive&&!d){(n=window.Core)==null||n.showToast(`${this.config.emoji} ${this.config.name} room opens during the ${this.config.name} phase`);return}(r=window.Core)==null||r.navigateTo("practiceRoomView"),(s=(a=window.PracticeRoom)==null?void 0:a.stopHubPresence)==null||s.call(a),window.currentLunarRoom=this,this.renderRoomDashboard(),this._setPresence(),this._loadCollectiveWords(),setTimeout(()=>this._refreshLivePresence(),300)}catch(d){console.error("enterRoom error:",d),(l=window.Core)==null||l.showToast("Failed to enter room")}}leaveRoom(){var e,i,o;try{this._clearPresence(),["_presenceSub","_collectiveWordsSub"].forEach(n=>{if(this[n]){try{this[n].unsubscribe()}catch{}this[n]=null}}),this._clearTimer(),this._removeEventListeners(),window.currentLunarRoom=null,(i=(e=window.PracticeRoom)==null?void 0:e.startHubPresence)==null||i.call(e),(o=window.Core)==null||o.navigateTo("hubView")}catch(n){console.error("leaveRoom error:",n)}}renderRoomDashboard(){var e;try{const i=(e=this.domCache).dynamicContent??(e.dynamicContent=document.getElementById("dynamicRoomContent"));if(!i){console.error("dynamicRoomContent not found");return}const o=new Date,n=this.getDaysRemaining(),r=Math.ceil((this.weekStartDate-o)/F.CONSTANTS.MS_PER_DAY),a=this.getLivingPresenceCount(),s=this.isActive?`${n} ${n===1?"day":"days"} remaining`:`${r} ${r===1?"day":"days"} until ${this.config.name}`;i.innerHTML=`
                <div class="lunar-room-bg">
                    <div class="${this.config.cssPrefix}-starfield lunar-starfield">
                        ${Y.generateStarfield()}
                    </div>
                    ${Y.renderTopBar({emoji:this.config.emoji,name:this.config.name,daysText:s,livingPresenceCount:a,cssPrefix:this.config.cssPrefix})}
                    <div class="${this.config.cssPrefix}-content-wrapper lunar-content-wrapper">
                        ${Y.renderMoonVisual({cssPrefix:this.config.cssPrefix,sphereClass:`${this.config.cssPrefix}-moon-sphere`,glowClass:`${this.config.cssPrefix}-moon-glow`})}
                        ${Y.renderIntroCard({imageUrl:this.config.imageUrl,description:this.config.description})}
                        ${Y.renderModeToggle({cssPrefix:this.config.cssPrefix,globalName:this.config.globalName})}
                        ...
                        <div class="lunar-wisdom-section">${Y.renderWisdomText(this.config.wisdomQuote)}</div>
                    </div>
                </div>`,this._attachEventListeners(i)}catch(i){console.error("renderRoomDashboard error:",i)}}_renderSoloPracticeMode(){const e=!!(this.userWeekData.intention||this.userWeekData.affirmation);return`
            ...
            ${e?this._renderSavedInputs():""}`}_renderPracticeCards(){var n;const e=Object.entries(this.config.practices),i=(n=e[0])==null?void 0:n[0],o=!!(this.userWeekData.intention||this.userWeekData.affirmation);return e.map(([r,a])=>{const s=r===i&&o;return`
                ...
                </div>`}).join("")}_renderSavedInputs(){const{intention:e,affirmation:i,releaseList:o}=this.userWeekData,n=this.config.savedInputLabels,r=a=>O._escapeHtml(a);return`
            <div class="lunar-saved-inputs">
                ...
                ${i?`<div class="lunar-saved-item"><div class="lunar-saved-label">${n.affirmation}</div><div class="lunar-saved-text">${r(i)}</div></div>`:""}
                ${o?`<div class="lunar-saved-item"><div class="lunar-saved-label">${n.releaseList}</div><div class="lunar-saved-text">${r(o).replace(/\n/g,"<br>")}</div></div>`:""}
            ...
            <div class="lunar-group-intro">
                ...
                <div class="lunar-group-avatars">${Y.renderMockAvatars(e)}</div>
            ...
            </div>`}_attachEventListeners(e){this._removeEventListeners();const i=o=>{const n=o.target.closest(".lunar-practice-card");if(n&&(o.type==="click"||o.key==="Enter"||o.key===" ")){o.preventDefault(),n.dataset.locked==="true"?this.showLockedMessage():this.showPracticePopup(n.dataset.practice);return}if(o.type!=="click")return;const r=o.target.closest("[data-action]");if(r)switch(r.dataset.action){case"switch-mode":this.switchMode(r.dataset.mode);break;case"enter-group-circle":this.enterGroupCircle();break;case"submit-closure":this.submitClosure();break;case"back-to-hub":window.Rituals?Rituals.showClosing():this.leaveRoom();break}};e.addEventListener("click",i),e.addEventListener("keydown",i),this.eventListeners.push({element:e,type:"click",handler:i},{element:e,type:"keydown",handler:i})}_removeEventListeners(){this.eventListeners.forEach(({element:e,type:i,handler:o})=>e==null?void 0:e.removeEventListener(i,o)),this.eventListeners=[]}switchMode(e){document.querySelectorAll(".lunar-practice-mode").forEach(i=>i.classList.toggle("active",i.dataset.mode===e)),document.querySelectorAll(".lunar-mode-btn").forEach(i=>{const o=i.dataset.mode===e;i.classList.toggle("active",o),i.setAttribute("aria-pressed",String(o))})}showPracticePopup(e){try{const i=this.config.practices[e];if(!i){console.error(`Practice not found: ${e}`);return}this.currentPractice=e;const o=e===Object.keys(this.config.practices)[0],n=Y.createPopup({icon:i.icon,title:i.title,subtitle:i.description,content:this.getPracticeContent(e),cssPrefix:this.config.cssPrefix,hasFooter:!o,onClose:()=>this.closePracticePopup()});(document.getElementById("communityHubFullscreenContainer")??document.body).appendChild(n),this.domCache.popup=n,this._attachPopupListeners(n,e)}catch(i){console.error("showPracticePopup error:",i)}}_attachPopupListeners(e,i){var o;e.querySelectorAll('[data-action="close-popup"]').forEach(n=>n.addEventListener("click",this.closePracticePopup)),e.addEventListener("click",this._handlePopupClick),i===Object.keys(this.config.practices)[0]&&((o=e.querySelector('[data-action="save-practice"]'))==null||o.addEventListener("click",this.saveIntentionPractice),e.querySelectorAll("[data-affirmation]").forEach(n=>n.addEventListener("click",()=>{const r=e.querySelector("#affirmationText");r&&(r.value=n.dataset.affirmation)})))}_handlePopupClick(e){e.target===e.currentTarget&&this.closePracticePopup()}getPracticeContent(e){try{const i=this.config.practiceContent[e];return i?i(this.userWeekData,this.config.prebuiltAffirmations):`<div class="lunar-popup-section"><h3>Practice</h3><p>${this.config.practices[e].description}</p></div>`}catch(i){return console.error("getPracticeContent error:",i),"<p>Error loading practice content</p>"}}closePracticePopup(){const e=this.domCache.popup??document.querySelector(".lunar-practice-popup");e&&(e.remove(),this.domCache.popup=null)}saveIntentionPractice(){var e,i,o,n;try{const r=document.getElementById("intentionText"),a=document.getElementById("affirmationText"),s=document.getElementById("releaseListText");if(!r||!a||!s){console.error("Form elements not found");return}const l=this._sanitizeInput(r.value),d=this._sanitizeInput(a.value),c=this._sanitizeInput(s.value),p=this._validateIntention(l);if(!p.valid){(e=window.Core)==null||e.showToast(p.error);return}const h=this._validateAffirmation(d);if(!h.valid){(i=window.Core)==null||i.showToast(h.error);return}Object.assign(this.userWeekData,{intention:l,affirmation:d,releaseList:c}),this.userWeekData.practiceCount++,this.saveUserWeekData(!0),(o=window.Core)==null||o.showToast("Practice saved"),this.closePracticePopup(),this.renderRoomDashboard()}catch(r){console.error("saveIntentionPractice error:",r),(n=window.Core)==null||n.showToast("Failed to save practice")}}showLockedMessage(){var e;(e=window.Core)==null||e.showToast("✓ Practice completed for this cycle")}showCollectiveIntentionPopup(){var e;try{const i=Y.createPopup({icon:"🌙",title:"Collective Intention Circle",subtitle:`Join others in ${this.config.collectiveFocus}`,content:this._getCollectiveStep1Content(),cssPrefix:this.config.cssPrefix,hasFooter:!1,onClose:()=>this.closeCollectivePopup()});(document.getElementById("communityHubFullscreenContainer")??document.body).appendChild(i),this.domCache.popup=i,i.querySelectorAll('[data-action="close-popup"]').forEach(o=>o.addEventListener("click",this.closeCollectivePopup)),(e=i.querySelector('[data-action="begin-collective"]'))==null||e.addEventListener("click",this.startCollectiveStep2),i.addEventListener("click",this._handlePopupClick)}catch(i){console.error("showCollectiveIntentionPopup error:",i)}}_getCollectiveStep1Content(){return`
            ...
            </div>`}startCollectiveStep2(){var i,o;const e=document.getElementById("collectiveIntentionContent");e&&(e.innerHTML=`
            <div class="lunar-popup-section" style="text-align:center;">
            ...
            </div>`,(i=e.querySelector('[data-action="start-meditation"]'))==null||i.addEventListener("click",this.startMeditationTimer),(o=e.querySelector('[data-action="skip-meditation"]'))==null||o.addEventListener("click",this.startCollectiveStep3))}startMeditationTimer(){var e;(e=document.getElementById("startMeditationBtn"))==null||e.style.setProperty("display","none"),this._runTimer("meditationTimer",F.CONSTANTS.MEDITATION_DURATION,()=>{const i=document.getElementById("skipToIntentionBtn");i&&(i.textContent="Continue to Intention",i.style.display="block")}),setTimeout(()=>{const i=document.getElementById("skipToIntentionBtn");i&&(i.style.display="block")},1e4)}startCollectiveStep3(){var i;this._clearTimer();const e=document.getElementById("collectiveIntentionContent");e&&(e.innerHTML=`
            <div class="lunar-popup-section">
                ...
                >${O._escapeHtml(this.userWeekData.privateIntention||"")}</textarea>
                <button class="lunar-popup-btn" data-action="save-private-intention">Continue</button>
            </div>`,(i=e.querySelector('[data-action="save-private-intention"]'))==null||i.addEventListener("click",this.startCollectiveStep4))}startCollectiveStep4(){var o;const e=document.getElementById("privateIntentionText");e&&(this.userWeekData.privateIntention=this._sanitizeInput(e.value));const i=document.getElementById("collectiveIntentionContent");i&&(i.innerHTML=`
            <div class="lunar-popup-section">
                ...
                <div class="lunar-intention-preview">
                    ...
                    <p>${O._escapeHtml(this.userWeekData.privateIntention||"No intention set")}</p>
                </div>
            ...
            </div>`,(o=i.querySelector('[data-action="submit-collective-word"]'))==null||o.addEventListener("click",()=>{const n=document.getElementById("collectiveWordInput");n&&this.submitWordToCollective(n.value)}))}submitWordToCollective(e){var n,r,a;const i=this._sanitizeInput(e),o=this._validateCollectiveWord(i);if(!o.valid){(n=window.Core)==null||n.showToast(o.error);return}this.userWeekData.collectiveWord=i,this.userWeekData.intentionShared=!0,this.collectiveWords||(this.collectiveWords=this.getMockCollectiveWords()),this.collectiveWords.push({word:i,timestamp:Date.now()}),(r=te())!=null&&r.ready&&((a=te())==null||a.sendRoomMessage(`${this._getLunarRoomId()}-collective`,i).catch(s=>console.error("[LunarRoom] submitWordToCollective DB error:",s))),this.startCollectiveStep5()}startCollectiveStep5(){var i,o;const e=document.getElementById("collectiveIntentionContent");e&&(e.innerHTML=`
            <div class="lunar-popup-section" style="text-align:center;">
                ...
                <div id="wordCloud" class="lunar-word-cloud">
                    ${Y.renderWordCloud(this.collectiveWords??this.getMockCollectiveWords())}
                </div>
            ...
            </div>`,(i=e.querySelector('[data-action="start-witnessing"]'))==null||i.addEventListener("click",this.startWitnessingTimer),(o=e.querySelector('[data-action="complete-collective"]'))==null||o.addEventListener("click",this.completeCollectivePractice))}startWitnessingTimer(){var e;(e=document.getElementById("startWitnessingBtn"))==null||e.style.setProperty("display","none"),this._runTimer("witnessingTimer",F.CONSTANTS.WITNESSING_DURATION,()=>{const i=document.getElementById("completeBtn");i&&(i.style.display="block")})}_runTimer(e,i,o){let n=i;const r=document.getElementById(e);r&&(this.collectiveTimer=setInterval(()=>{n--;const a=Math.floor(n/60),s=n%60;r.textContent=`${a}:${String(s).padStart(2,"0")}`,n<=0&&(this._clearTimer(),r.textContent="Complete",o==null||o())},F.CONSTANTS.TIMER_INTERVAL_MS))}completeCollectivePractice(){var e;this.userWeekData.practiceCount++,this.saveUserWeekData(!0),(e=window.Core)==null||e.showToast("Intention planted with the collective"),this.closeCollectivePopup(),this.renderRoomDashboard()}closeCollectivePopup(){this._clearTimer(),this.closePracticePopup()}enterGroupCircle(){this.showCollectiveIntentionPopup()}submitClosure(){var e,i,o;try{const n=document.getElementById("closureReflection");n&&(this.userWeekData.closureReflection=this._sanitizeInput(n.value),this.saveUserWeekData(!0)),(e=window.Core)==null||e.showToast(this.config.completionMessage),this.userWeekData=this._getDefaultUserData(),this.saveUserWeekData(!0),(i=window.Core)==null||i.navigateTo("hubView")}catch(n){console.error("submitClosure error:",n),(o=window.Core)==null||o.showToast("Failed to submit closure")}}_clearTimer(){this.collectiveTimer&&(clearInterval(this.collectiveTimer),this.collectiveTimer=null)}getLivingPresenceCount(){return typeof this._cachedPresenceCount=="number"?this._cachedPresenceCount:0}getMockCollectiveWords(){const e=Date.now();return[{word:"Growth",timestamp:e-864e5},{word:"Peace",timestamp:e-72e6},{word:"Courage",timestamp:e-432e5},{word:"Clarity",timestamp:e-36e6},{word:"Healing",timestamp:e-216e5},{word:"Trust",timestamp:e-18e6},{word:"Love",timestamp:e-144e5},{word:"Balance",timestamp:e-72e5},{word:"Freedom",timestamp:e-36e5},{word:"Joy",timestamp:e-18e5}]}getCollectiveWordsCount(){return(this.collectiveWords??this.getMockCollectiveWords()).length}_getLunarRoomId(){return{newmoon:"new-moon",waxingmoon:"waxing-moon",fullmoon:"full-moon",waningmoon:"waning-moon"}[this.config.cssPrefix]??this.config.cssPrefix}_setPresence(){var e,i,o;if((e=te())!=null&&e.ready)try{const n=this._getLunarRoomId(),r=`${this.config.emoji} ${this.config.name}`;(i=te())==null||i.setPresence("online",r,n),(o=window.Core)!=null&&o.state&&(window.Core.state.currentRoom=n,window.Core.state.currentUser&&(window.Core.state.currentUser.activity=r))}catch(n){console.error("[LunarRoom] _setPresence error:",n)}}_clearPresence(){var e,i,o;if((e=te())!=null&&e.ready)try{(i=te())==null||i.setPresence("online","✨ Available",null),(o=window.Core)!=null&&o.state&&(window.Core.state.currentRoom=null,window.Core.state.currentUser&&(window.Core.state.currentUser.activity="✨ Available"))}catch(n){console.error("[LunarRoom] _clearPresence error:",n)}}async _refreshLivePresence(){var o,n;if(!((o=te())!=null&&o.ready))return;const e=this._getLunarRoomId(),i=async()=>{var r,a;try{const s=await((r=te())==null?void 0:r.getRoomParticipants(e)),l=await((a=te())==null?void 0:a.getBlockedUsers()),d=s.filter(m=>!l.has(m.user_id));this._cachedPresenceCount=d.length,this._cachedParticipants=d;const c=document.getElementById("lunarLiveCountTop");c&&(c.textContent=`${d.length} members practicing with you now`);const p=document.querySelector(".lunar-live-presence span");p&&(p.textContent=`${d.length} members in circle`);const h=document.querySelector(".lunar-group-avatars");h&&(h.innerHTML=this._buildRealAvatars(d))}catch(s){console.warn("[LunarRoom] _refreshLivePresence error:",s)}};if(await i(),this._presenceSub)try{this._presenceSub.unsubscribe()}catch{}this._presenceSub=(n=te())==null?void 0:n.subscribeToPresence(i)}_buildRealAvatars(e){const o=e.slice(0,5),n=e.length-5,r=o.map(s=>{var m,g;const l=s.profiles??{},d=l.name??l.display_name??"?",c=d.charAt(0).toUpperCase(),p=((g=(m=window.Core)==null?void 0:m.getAvatarGradient)==null?void 0:g.call(m,s.user_id))??"background:#8B7AFF";let h=l.avatar_url?`<img src="${l.avatar_url}" alt="${c}" width="40" height="40" loading="lazy" decoding="async" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`:l.emoji?`<span style="font-size:18px;">${l.emoji}</span>`:`<span style="font-size:14px;font-weight:600;">${c}</span>`;return`<div class="lunar-avatar" style="${p};display:flex;align-items:center;justify-content:center;" aria-label="${d}">${h}</div>`}).join(""),a=n>0?`<div class="lunar-avatar" style="background:#333;display:flex;align-items:center;justify-content:center;font-size:12px;">+${n}</div>`:`<div class="lunar-avatar lunar-join-avatar" role="button" tabindex="0" aria-label="Join circle" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();this.click();}"><span aria-hidden="true">+</span></div>`;return r+a}async _loadCollectiveWords(){var o,n;if(!((o=te())!=null&&o.ready))return;const e=`${this._getLunarRoomId()}-collective`,i=async()=>{var r;try{const a=await((r=te())==null?void 0:r.getRoomMessages(e,100));if(a!=null&&a.length){this.collectiveWords=a.map(d=>({word:d.message,timestamp:new Date(d.created_at).getTime()}));const s=document.getElementById("wordCloud");s&&(s.innerHTML=Y.renderWordCloud(this.collectiveWords));const l=document.querySelector(".lunar-word-count strong");l&&(l.textContent=this.collectiveWords.length)}}catch(a){console.warn("[LunarRoom] _loadCollectiveWords error:",a)}};if(await i(),this._collectiveWordsSub)try{this._collectiveWordsSub.unsubscribe()}catch{}this._collectiveWordsSub=(n=te())==null?void 0:n.subscribeToRoomChat(e,i)}cleanup(){this._clearTimer(),this._removeEventListeners(),clearTimeout(this.saveDebounceTimer),clearTimeout(this._retryCheckTimeout),["_presenceSub","_collectiveWordsSub"].forEach(e=>{if(this[e]){try{this[e].unsubscribe()}catch{}this[e]=null}}),this.domCache={dynamicContent:null,popup:null}}};it(F,"CONSTANTS",{MEDITATION_DURATION:180,WITNESSING_DURATION:120,SAVE_DEBOUNCE_MS:500,MAX_INTENTION_LENGTH:500,MAX_AFFIRMATION_LENGTH:300,MAX_RELEASE_LIST_LENGTH:1e3,MAX_WORD_LENGTH:50,TIMER_INTERVAL_MS:1e3,LUNAR_CYCLE_DAYS:29.53,MS_PER_DAY:864e5});let $e=F;class Pe extends z{constructor(e){super(e),this.initPlayerState(),this.initCycleState(),this.sessions=[],this.scheduleModalTitle="📅 Upcoming Sessions",this.state.currentSession=null,this.state.nextSession=null}onInit(){this.initializeCycle(),this.preloadYouTubeAPI()}onEnter(){this.loadYouTubeAPI()}onCleanup(){this.cleanupPlayer(),this.cleanupCycle()}setSessions(e){const i=Math.floor(e/(this.config.cycleDuration*1e3))%this.sessions.length;this.state.currentSession=this.sessions[i],this.state.nextSession=this.sessions[(i+1)%this.sessions.length]}getCurrentSession(){return this.state.currentSession}getNextSession(){return this.state.nextSession}buildBody(){const e=this.getCurrentSession();return`
        <div class="ps-body">
            <main class="ps-main">
        ...
        </div>`}},$={storage:{_key:t=>`${Se.STORAGE_PREFIX}${t}${Se.STORAGE_KEY_SUFFIX}`,get(t){const e=localStorage.getItem(this._key(t));if(!e)return null;try{return JSON.parse(e)}catch(i){return console.error(`[SolarUI] Error loading ${t} data:`,i),null}},set(t,e){try{return localStorage.setItem(this._key(t),JSON.stringify(e)),!0}catch(i){return console.error(`[SolarUI] Error saving ${t} data:`,i),!1}},clear(t){localStorage.removeItem(this._key(t))}},utils:{calculateDaysRemaining:t=>Math.ceil((t-new Date)/864e5),generateFloatingElements(t,e=Se.FLOATING_ELEMENT_COUNT){const{FLOATING_ELEMENT_DELAY_MAX:i,FLOATING_ELEMENT_DURATION_MIN:o,FLOATING_ELEMENT_DURATION_RANGE:n}=Se;let r="";for(let a=0;a<e;a++){const s=t[Math.floor(Math.random()*t.length)],l=(Math.random()*100).toFixed(2),d=(Math.random()*i).toFixed(2),c=(o+Math.random()*n).toFixed(2);r+=`<div class="solar-floating-element"
        ...
        </div>`},savedInputs(t,e){const{intention:i,affirmation:o,releaseList:n}=t;if(!i&&!o&&!n)return"";const r=s=>C.escapeHtml(s),a=(s,l,d)=>`<div class="solar-input-display" style="margin-bottom:1rem;">
           <h4>${s}</h4>
...
@media (max-width:380px) {
  ...
  .solar-inactive-card{padding:1.25rem 0.875rem !important;}
    ...
    `}};window.SolarUIManager=$;const Le={location:{latitude:31,longitude:0,name:"Default"},currentSolarData:null,currentSolarRoom:null,solarRooms:[{season:"spring",icon:"🌱",roomName:"Spring Awakening",roomId:"spring-equinox",description:"Renewal energy and fresh beginnings",energy:"Renewal, Growth, Awakening",practices:["Dawn meditation","Renewal rituals","Energy activation","New beginning visualization"]},{season:"summer",icon:"☀️",roomName:"Summer Radiance",roomId:"summer-solstice",description:"Peak vitality and expansive energy",energy:"Vitality, Expansion, Power",practices:["Solar noon meditation","Fire ceremony","Peak energy practice","Celebration ritual"]},{season:"autumn",icon:"🍂",roomName:"Autumn Harvest",roomId:"autumn-equinox",description:"Gratitude and preparation for rest",energy:"Gratitude, Harvest, Balance",practices:["Gratitude meditation","Harvest ritual","Balance practice","Reflection ceremony"]},{season:"winter",icon:"❄️",roomName:"Winter Stillness",roomId:"winter-solstice",description:"Deep rest and inner contemplation",energy:"Rest, Stillness, Contemplation",practices:["Deep rest meditation","Inner journey","Stillness practice","Contemplative silence"]}],init(){if(this._initialized)return;this._initialized=!0;const t=()=>this.updateAll();navigator.geolocation?navigator.geolocation.getCurrentPosition(({coords:e})=>{this.location={latitude:e.latitude,longitude:e.longitude,name:"Your Location"},t()},t,{timeout:5e3,maximumAge:36e5}):t(),setInterval(t,Se.UPDATE_INTERVAL_MS)},updateAll(){this.updateSolarData(),this.updateSolarVisualization(),this.updateSolarRoom(),this.renderSolarCard()},setLocation(t,e,i){this.location={latitude:t,longitude:e,name:i},this.updateAll()},updateSolarData(){const t=new Date,{latitude:e,longitude:i}=this.location,o=Be.getTimes(t,e,i),n=this.calculateSunDeclination(t),{name:r,days:a}=this._getNextTransition(t);this.currentSolarData={sunrise:o.sunrise,sunset:o.sunset,solarNoon:o.solarNoon,declination:n,seasonInfo:this.getSeasonInfo(t),currentSeason:this.getCurrentSeason(),nextSeasonName:r,daysToNextSeason:a,position:Be.getPosition(t,e,i)},this.renderSolarInfo()},renderSolarInfo(){const t=this.currentSolarData;if(!t)return;const e=(o,n)=>{const r=document.getElementById(o);r&&(r.textContent=n)};e("sunrise",this.formatTime(t.sunrise)),e("sunset",this.formatTime(t.sunset)),e("solarNoon",this.formatTime(t.solarNoon)),e("seasonInfo",t.seasonInfo);const i=document.getElementById("solarDeclination");if(i){const o=i.querySelector(".decl-value");o&&(o.textContent=`${Math.abs(t.declination).toFixed(1)}° ${t.declination>=0?"N":"S"}`)}},calculateSunDeclination(t){const e=this.getDayOfYear(t);return 23.44*Math.sin(2*Math.PI*(e-81)/365)},getDayOfYear(t){return Math.floor((t-new Date(t.getFullYear(),0,0))/864e5)},getSeasonDates(t){if(this._seasonDatesCache||(this._seasonDatesCache={}),this._seasonDatesCache[t])return this._seasonDatesCache[t];const e=(t-2e3)/1e3,i=m=>{const g={spring:[245162380984e-5,365242.37404,.05169,-.00411,-57e-5],summer:[245171656767e-5,365241.62603,.00325,.00888,-3e-4],autumn:[245181021715e-5,365242.01767,-.11575,.00337,78e-5],winter:[245190005952e-5,365242.74049,-.06223,-.00823,32e-5]}[m];return g[0]+g[1]*e+g[2]*e**2+g[3]*e**3+g[4]*e**4},o=m=>{const g=(m-2451545)/36525,f=35999.373*g-2.47,v=1+.0334*Math.cos(f*Math.PI/180)+7e-4*Math.cos(2*f*Math.PI/180),S=[[485,324.96,1934.136],[203,337.23,32964.467],[199,342.08,20.186],[182,27.85,445267.112],[156,73.14,45036.886],[136,171.52,22518.443],[77,222.54,65928.934],[74,296.72,3034.906],[70,243.58,9037.513],[58,119.81,33718.148],[52,297.17,150.678],[50,21.02,2281.226]].reduce((B,[_,k,R])=>B+_*Math.cos((k+R*g)*Math.PI/180),0);return m+1e-5*S/v},n=m=>new Date((m-24405875e-1)*864e5),r=m=>new Date(m.getTime()-864e5),a=m=>n(o(i(m))),s=a("spring"),l=a("summer"),d=a("autumn"),c=a("winter"),p=n(o((()=>{const m=(t+1-2e3)/1e3,g=[245162380984e-5,365242.37404,.05169,-.00411,-57e-5];return g[0]+g[1]*m+g[2]*m**2+g[3]*m**3+g[4]*m**4})())),h={spring:{start:s,end:r(l)},summer:{start:l,end:r(d)},autumn:{start:d,end:r(c)},winter:{start:c,end:r(p)}};return this._seasonDatesCache[t]=h,h},getCurrentSeason(){const t=new Date,e=t.getFullYear(),i=this.location.latitude>=0,o={spring:"autumn",summer:"winter",autumn:"spring",winter:"summer"},n=this.getSeasonDates(e-1);if(t>=n.winter.start&&t<=n.winter.end)return i?"winter":"summer";const r=this.getSeasonDates(e);for(const a of["spring","summer","autumn","winter"]){const{start:s,end:l}=r[a];if(t>=s&&t<=l)return i?a:o[a]}return"winter"},_getUpcomingTransitions(t){const e=t.getFullYear(),i=this.getSeasonDates(e),o=this.getSeasonDates(e+1);return[{name:"Spring Equinox",date:i.spring.start},{name:"Summer Solstice",date:i.summer.start},{name:"Autumn Equinox",date:i.autumn.start},{name:"Winter Solstice",date:i.winter.start},{name:"Spring Equinox",date:o.spring.start}].filter(n=>n.date>t).sort((n,r)=>n.date-r.date)},getSeasonInfo(t){const e=this._getUpcomingTransitions(t)[0];return`${Math.ceil((e.date-t)/864e5)} days to ${e.name}`},_getNextTransition(t){const e=this._getUpcomingTransitions(t)[0];return{name:e.name.split(" ")[0],days:Math.ceil((e.date-t)/864e5)}},getNextSeasonInfo(t){return this._getNextTransition(t)},updateSolarVisualization(){this.currentSolarData&&this.drawWheelOfYear(this.currentSolarData)},drawWheelOfYear(t){var ee;const e=document.getElementById("solarSvg");if(!e)return;const i=280,o=140,n=140,r=110,a=52;e.setAttribute("viewBox",`0 0 ${i} ${i}`),e.setAttribute("width","100%"),e.style.height="auto",e.innerHTML="";const s="http://www.w3.org/2000/svg",l=(E,M)=>{const J=document.createElementNS(s,E);return Object.entries(M).forEach(([X,ce])=>J.setAttribute(X,ce)),J},d=l("defs",{}),c=l("clipPath",{id:"woyClip"});c.appendChild(l("circle",{cx:o,cy:n,r:r-1})),d.appendChild(c),e.appendChild(d);const p=[{d:`M${o},${n} L${o},${n-r} L${o+r},${n-r} L${o+r},${n} Z`,fill:"#C0DD97"},{d:`M${o},${n} L${o+r},${n} L${o+r},${n+r} L${o},${n+r} Z`,fill:"#FAC775"},{d:`M${o},${n} L${o},${n+r} L${o-r},${n+r} L${o-r},${n} Z`,fill:"#F0957B"},{d:`M${o},${n} L${o-r},${n} L${o-r},${n-r} L${o},${n-r} Z`,fill:"#B5D4F4"}],h=l("g",{"clip-path":"url(#woyClip)"});p.forEach(({d:E,fill:M})=>h.appendChild(l("path",{d:E,fill:M,opacity:"0.45",stroke:"none"}))),e.appendChild(h),e.appendChild(l("circle",{cx:o,cy:n,r,fill:"none",stroke:"var(--neuro-shadow-light)","stroke-width":"1.5"})),e.appendChild(l("circle",{cx:o,cy:n,r:a,fill:"none",stroke:"var(--neuro-shadow-light)","stroke-width":"1","stroke-dasharray":"3 3",opacity:"0.6"})),e.appendChild(l("line",{x1:o-r,y1:n,x2:o+r,y2:n,stroke:"var(--neuro-shadow-light)","stroke-width":"0.8"})),e.appendChild(l("line",{x1:o,y1:n-r,x2:o,y2:n+r,stroke:"var(--neuro-shadow-light)","stroke-width":"0.8"}));const m=(E,M,J,X,ce,ue)=>{const pe=l("text",{x:E,y:M,"text-anchor":"middle",fill:ce,"font-size":"9","font-weight":"600"});pe.textContent=J,e.appendChild(pe);const y=l("text",{x:E,y:M+11,"text-anchor":"middle",fill:ue,"font-size":"7.5"});y.textContent=X,e.appendChild(y)};m(o+55,n-40,"Spring","Mar · Apr · May","#3B6D11","#639922"),m(o+55,n+52,"Summer","Jun · Jul · Aug","#854F0B","#BA7517"),m(o-55,n+52,"Autumn","Sep · Oct · Nov","#993C1D","#D85A30"),m(o-55,n-40,"Winter","Dec · Jan · Feb","#185FA5","#378ADD");const g=(E,M,J)=>{const X=l("text",{x:E,y:M,"text-anchor":"middle","font-size":"11"});X.textContent=J,e.appendChild(X)};g(o,n-r+8,"🌱"),g(o+r-6,n+4,"☀️"),g(o,n+r-2,"🍂"),g(o-r+6,n+4,"❄️"),[[o,n-r,o,n-r+8],[o+r,n,o+r-8,n],[o,n+r,o,n+r-8],[o-r,n,o-r+8,n]].forEach(([E,M,J,X])=>e.appendChild(l("line",{x1:E,y1:M,x2:J,y2:X,stroke:"var(--neuro-text)","stroke-width":"1.5","stroke-linecap":"round"})));const f=79,v=new Date,S=this.getDayOfYear(v),B=-Math.PI/2+(S-f)/365*2*Math.PI,_=o+r*Math.cos(B),k=n+r*Math.sin(B);for(let E=0;E<8;E++){const M=E*Math.PI/4;e.appendChild(l("line",{x1:_+Math.cos(M)*10,y1:k+Math.sin(M)*10,x2:_+Math.cos(M)*15,y2:k+Math.sin(M)*15,stroke:"#EF9F27","stroke-width":"1.5","stroke-linecap":"round"}))}e.appendChild(l("circle",{cx:_,cy:k,r:"8",fill:"#EF9F27",stroke:"#BA7517","stroke-width":"1.2"})),e.appendChild(l("circle",{cx:_,cy:k,r:"3.5",fill:"#FCDE5A",opacity:"0.9"}));const{nextSeasonName:R,daysToNextSeason:U}=t,T=v.toLocaleDateString("en-US",{month:"short",day:"numeric"}),j=this.getCurrentSeason(),K=j.charAt(0).toUpperCase()+j.slice(1),L=((ee=this.getSeasonDates(v.getFullYear())[j])==null?void 0:ee.start)||v,Z=Math.max(1,Math.floor((v-L)/864e5)+1),V=(E,M,J,X,ce)=>{const ue=l("text",{x:o,y:M,"text-anchor":"middle",fill:ce,"font-size":J,"font-weight":X});ue.textContent=E,e.appendChild(ue)};V(T,n-10,"9","700","var(--neuro-text)"),V(`${Z} days in ${K}`,n+3,"7.5","400","var(--neuro-text-light)"),V(`${U} days to ${R}`,n+15,"7.5","400","#BA7517")},updateSolarRoom(){const t=this.getCurrentSeason(),e=this.solarRooms.find(i=>i.season===t);e&&e!==this.currentSolarRoom&&(this.currentSolarRoom=e,this.renderSolarRoom(e))},renderSolarRoom(t){const e=(o,n)=>{const r=document.getElementById(o);r&&(r.textContent=n)};e("solarRoomIcon",t.icon),e("solarRoomName",t.roomName),e("solarRoomDesc",t.description);const i=document.getElementById("solarPracticeRoom");i&&(i.setAttribute("data-room-id",t.roomId),i.setAttribute("data-room-energy",t.energy))},getCurrentRoom(){return this.currentSolarRoom},renderSolarCard(){var r,a,s;const t=document.getElementById("solarContainer");if(!t){console.warn("SolarEngine: solarContainer not found");return}if(!this.currentSolarData||!this.currentSolarRoom){t.innerHTML='<p style="color:var(--text-muted);padding:20px;">Loading solar data...</p>';return}const{currentSolarData:e,currentSolarRoom:i}=this,o=(s=(a=(r=window.Core)==null?void 0:r.state)==null?void 0:a.currentUser)==null?void 0:s.is_admin,n=e.currentSeason.charAt(0).toUpperCase()+e.currentSeason.slice(1);t.innerHTML=`
      ...
      </div>`},adminJoinRoom(t){this._loadAndEnterRoom(t)},injectAdminUI(){var i,o,n;if(!((n=(o=(i=window.Core)==null?void 0:i.state)==null?void 0:o.currentUser)!=null&&n.is_admin))return;const t=document.getElementById("solarContainer");if(!t||t.querySelector("#solarAdminPanel"))return;const e=t.querySelector(".celestial-card-full");e&&e.insertAdjacentHTML("beforeend",this.renderAdminSection())},_ROOM_MODULES:{"spring-equinox":()=>Ce(()=>Promise.resolve().then(()=>Mi),void 0),"summer-solstice":()=>Ce(()=>Promise.resolve().then(()=>Ii),void 0),"autumn-equinox":()=>Ce(()=>Promise.resolve().then(()=>Ti),void 0),"winter-solstice":()=>Ce(()=>Promise.resolve().then(()=>Ei),void 0)},_roomExportName:{"spring-equinox":"SpringSolarRoom","summer-solstice":"SummerSolarRoom","autumn-equinox":"AutumnSolarRoom","winter-solstice":"WinterSolarRoom"},async _loadAndEnterRoom(t){var i,o,n,r,a,s;const e=this._ROOM_MODULES[t];if(!e){(o=(i=window.Core)==null?void 0:i.showToast)==null||o.call(i,`Unknown room: ${t}`);return}try{const d=(await e())[this._roomExportName[t]];d?d.enterRoom():(r=(n=window.Core)==null?void 0:n.showToast)==null||r.call(n,`${t} failed to initialise`)}catch(l){console.error(`[SolarEngine] _loadAndEnterRoom error for ${t}:`,l),(s=(a=window.Core)==null?void 0:a.showToast)==null||s.call(a,`Failed to load ${t}`)}},joinSolarRoom(){var t,e;if(!this.currentSolarRoom){(e=(t=window.Core)==null?void 0:t.showToast)==null||e.call(t,"Solar room not ready");return}this._loadAndEnterRoom(this.currentSolarRoom.roomId)},formatTime:t=>!t||isNaN(t.getTime())?"N/A":t.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0}),formatDate:t=>t.toLocaleDateString("en-US",{month:"short",day:"numeric"}),getSolarData(){return this.currentSolarData}};window.SolarEngine=Le;const Ne={config:{},practices:{},prebuiltAffirmations:[],userData:{intention:"",affirmation:"",releaseList:"",practiceCount:0,closureReflection:"",completedDate:null,privateIntention:"",collectiveWord:"",intentionShared:!1},isActive:!1,startDate:null,endDate:null,collectiveTimer:null,saveDebounceTimer:null,eventListeners:[],CONSTANTS:{MAX_INTENTION_LENGTH:500,MAX_AFFIRMATION_LENGTH:300,MAX_RELEASE_LIST_LENGTH:1e3,SAVE_DEBOUNCE_MS:500},_sanitizeInput(t){return typeof t!="string"?"":t.trim().slice(0,1e3).replace(/<script[^>]*>.*?<\/script>/gi,"").replace(/<iframe[^>]*>.*?<\/iframe>/gi,"").replace(/<object[^>]*>.*?<\/object>/gi,"")},_escapeHtml:t=>C.escapeHtml(t),_removeEventListeners(){this.eventListeners.forEach(({element:t,event:e,handler:i})=>t==null?void 0:t.removeEventListener(e,i)),this.eventListeners=[]},_clearTimer(){this.collectiveTimer&&(clearInterval(this.collectiveTimer),this.collectiveTimer=null)},cleanup(){this._clearTimer(),this._removeEventListeners(),clearTimeout(this.saveDebounceTimer),this.saveDebounceTimer=null,this._unsubPresence(),this._unsubCollectiveWords()},_unsubPresence(){var t;try{(t=this._presenceSub)==null||t.unsubscribe()}catch{}this._presenceSub=null},_unsubCollectiveWords(){var t;try{(t=this._collectiveWordsSub)==null||t.unsubscribe()}catch{}this._collectiveWordsSub=null},_startCountdownTimer(t,e,i,o){var n;try{(n=document.getElementById(e))!=null&&n.style&&(document.getElementById(e).style.display="none");const r=document.getElementById(t);if(!r)return;r.setAttribute("role","timer"),r.setAttribute("aria-live","off");let a=i;this._clearTimer(),this.collectiveTimer=setInterval(()=>{a--;const s=Math.floor(a/60),l=a%60;r.textContent=`${s}:${String(l).padStart(2,"0")}`,a<=0&&(this._clearTimer(),r.textContent="Complete",o==null||o())},1e3)}catch(r){console.error("[BaseSolarRoom] _startCountdownTimer error:",r),this._clearTimer()}},init(){try{this.checkIfActive(),this.isActive&&(this.calculateDates(),this.loadData())}catch(t){console.error(`${this.config.displayName} init error:`,t)}},checkIfActive(){var r,a,s;if((s=(a=(r=window.Core)==null?void 0:r.state)==null?void 0:a.currentUser)!=null&&s.is_admin){this.isActive=!0;return}const t=new Date,e=t.getFullYear(),i=this.config.name,o=Le.getSeasonDates(e)[i];if(t>=o.start&&t<=o.end){this.isActive=!0;return}const n=Le.getSeasonDates(e-1)[i];this.isActive=t>=n.start&&t<=n.end},calculateDates(){const t=new Date,e=this.config.name,i=Le.getSeasonDates(t.getFullYear()-1)[e];if(t>=i.start&&t<=i.end){this.startDate=i.start,this.endDate=i.end;return}const o=Le.getSeasonDates(t.getFullYear())[e];this.startDate=o.start,this.endDate=o.end},loadData(){var e;const t=$.storage.get(this.config.name);t&&new Date(t.seasonStartDate).getTime()===((e=this.startDate)==null?void 0:e.getTime())&&(this.userData=t.data)},saveData(){$.storage.set(this.config.name,{seasonStartDate:this.startDate.toISOString(),data:this.userData})||$.showToast("Could not save data. Check storage limits.")},enterRoom(){var e,i,o,n,r,a,s;const t=(o=(i=(e=window.Core)==null?void 0:e.state)==null?void 0:i.currentUser)==null?void 0:o.is_admin;if(!this.isActive&&!t){$.showToast(`${this.config.emoji} ${this.config.displayName} room opens during ${this.config.displayName.toLowerCase()} season`);return}this.startDate||this.calculateDates(),(r=(n=window.Core)==null?void 0:n.navigateTo)==null||r.call(n,"practiceRoomView"),(s=(a=window.PracticeRoom)==null?void 0:a.stopHubPresence)==null||s.call(a),document.body.classList.add("solar-room-active"),this.renderDashboard(),this._setPresence(),this._loadCollectiveWords()},leaveRoom(){var t,e,i,o;try{this._clearPresence(),this._unsubPresence(),this._unsubCollectiveWords(),this._clearTimer(),this._removeEventListeners(),document.body.classList.remove("solar-room-active"),window.currentSolarRoom=null,(e=(t=window.PracticeRoom)==null?void 0:t.startHubPresence)==null||e.call(t),(o=(i=window.Core)==null?void 0:i.navigateTo)==null||o.call(i,"hubView")}catch(n){console.error("Error leaving solar room:",n)}},renderDashboard(){$.injectStyles();const t=document.getElementById("dynamicRoomContent");if(!t){console.error("dynamicRoomContent not found");return}document.body.setAttribute("data-season",this.config.name),window.currentSolarRoom=this;const e=$.utils.calculateDaysRemaining(this.endDate),i=`${e} ${e===1?"day":"days"} remaining`,o=`${Se.IMAGE_BASE_URL}${this.config.displayName}.webp`,n=this.getLivingPresenceCount();t.innerHTML=`
      <div class="solar-room-bg">
        <div class="solar-floating-bg">
          ${$.utils.generateFloatingElements(this.config.floatingEmojis)}
        </div>

        ${$.renderers.topBar({seasonName:this.config.displayName,emoji:this.config.emoji,daysText:i,livingPresenceCount:n})}
        ...
        <div class="solar-content-wrapper">
          ...
          <div class="solar-intro-card">
            <picture>
              ...
              <img src="${o.replace(".webp",".png")}" alt="${this.config.displayName} Season"
      ...
      </div>`,this._attachEventListeners(t),setTimeout(()=>this._refreshLivePresence(),300)},_attachEventListeners(t){try{this._removeEventListeners();const e=(r,a,s)=>{r&&(r.addEventListener(a,s),this.eventListeners.push({element:r,event:a,handler:s}))},i=t.querySelector("#practicesGrid"),o=r=>{const a=r.target.closest(".solar-practice-card");a&&(r.type==="keydown"&&r.key!=="Enter"&&r.key!==" "||(r.type==="keydown"&&r.preventDefault(),a.dataset.locked==="true"?$.showToast("Complete the first practice to unlock others"):this.showPracticePopup(a.dataset.practice)))};e(i,"click",o),e(i,"keydown",o),e(t.querySelector(".solar-mode-toggle"),"click",r=>{const a=r.target.closest(".solar-mode-btn");a!=null&&a.dataset.mode&&$.switchMode(a.dataset.mode)});const n=t.querySelector('[data-action="submit-closure"]');e(n,"click",()=>this.submitClosure())}catch(e){console.error("Error attaching event listeners:",e)}},isPracticeLocked:()=>!1,getPracticeContent(t){const e=`get${t.charAt(0).toUpperCase()}${t.slice(1)}Content`;return typeof this[e]=="function"?this[e]():(console.error(`[BaseSolarRoom] Content method ${e} not found for ${t}`),'<div class="solar-popup-section"><p>Practice content not available.</p></div>')},showPracticePopup(t){try{const e=this.practices[t];if(!e){console.error(`Practice ${t} not found`);return}const i=document.createElement("div");i.id="practicePopup",i.innerHTML=$.renderers.popup({title:e.title,content:this.getPracticeContent(t),hasSaveButton:!!e.hasSaveData}),(document.getElementById("communityHubFullscreenContainer")||document.body).appendChild(i),this._attachPopupListeners(i,t)}catch(e){console.error("Error showing practice popup:",e)}},_attachPopupListeners(t,e){var i;try{t.querySelectorAll(".solar-affirmation-btn").forEach(o=>{o.addEventListener("click",()=>{const n=document.getElementById("affirmationText");n&&(n.value=o.dataset.affirmation)})}),(i=t.querySelector('[data-action="save-practice"]'))==null||i.addEventListener("click",()=>this.savePractice(e)),t.querySelectorAll('[data-action="close-popup"]').forEach(o=>o.addEventListener("click",()=>$.closePracticePopup()))}catch(o){console.error("Error attaching popup listeners:",o)}},savePractice(t){try{const e=this.practices[t];if(!(e!=null&&e.hasSaveData))return;const i=a=>{const s=document.getElementById(a);return s?this._sanitizeInput(s.value):null},o=i("intentionText"),n=i("affirmationText"),r=i("releaseListText");o!==null&&(this.userData.intention=o),n!==null&&(this.userData.affirmation=n),r!==null&&(this.userData.releaseList=r),this.userData.practiceCount++,this.saveData(),$.showToast("Practice saved"),$.closePracticePopup(),this.renderDashboard()}catch(e){console.error("Error saving practice:",e),$.showToast("Failed to save practice")}},submitClosure(){const t=document.getElementById("closureReflection");if(!t){$.showToast("Reflection field not found");return}const e=t.value.trim();if(!e){$.showToast("Please write your closing reflection");return}if(e.length<Se.MIN_REFLECTION_LENGTH){$.showToast(`Please write at least ${Se.MIN_REFLECTION_LENGTH} characters`);return}this.userData.closureReflection=e,this.userData.completedDate=new Date().toISOString(),this.saveData(),$.showToast(`${this.config.emoji} ${this.config.displayName} season complete. Rest well until the next cycle.`),setTimeout(()=>this.init(),1e3)},showCollectiveIntentionPopup(){try{const t=document.createElement("div");t.id="collectivePopup",t.className="solar-practice-popup",t.innerHTML=`
      ...
      <div class="solar-popup-section" style="text-align:center;">
        ...
        </div>`}catch(t){console.error("Error starting step 4:",t)}},submitWordToCollective(){var t;try{const e=((t=document.getElementById("collectiveWordInput"))==null?void 0:t.value.trim())||"";if(!e){$.showToast("Please enter a word");return}if(e.length>50){$.showToast("Word is too long");return}this.userData.collectiveWord=e,this.userData.intentionShared=!0,u!=null&&u.ready&&u.sendRoomMessage(`${this._getSolarRoomId()}-collective`,e).catch(i=>console.error("[BaseSolarRoom] submitWordToCollective DB error:",i)),this.startCollectiveStep5()}catch(e){console.error("Error submitting word:",e)}},startCollectiveStep5(){var t;try{const e=document.getElementById("collectiveIntentionContent");if(!e)return;const i=(t=this._dbCollectiveWords)!=null&&t.length?this._dbCollectiveWords:this._getMockCollectiveWords(),o=this._renderWordCloud(i);e.innerHTML=`
        <div class="solar-popup-section" style="text-align:center;">
        ...
        <p style="text-align:center;font-size:0.9rem;color:rgba(224,224,255,0.7);margin-bottom:2rem;">
          <span class="solar-word-count"><strong>${i.length}</strong></span> ${this.config.collectiveNoun||"intentions"} in this ${this.config.displayName} cycle
        </p>
        <div style="margin:2rem 0;">
        ...
        <div class="modal-overlay" id="${this.roomId}SetupModal" style="z-index:200001;">
            ...
            </ul>`}}Object.assign(Oe.prototype,kt);Object.assign(Oe.prototype,_t);Object.assign(Oe.prototype,He);const pi=new Oe;window.DeepWorkRoom=pi;var gt;(gt=window.dispatchRoomReady)==null||gt.call(window,"deepwork");class St extends z{constructor(){super({roomId:"campfire",roomType:"always-open",name:"Community Campfire",icon:"🔥",description:"A warm space to share, reflect, and connect with the community. Pull up a chair. Real conversations, real people, real connection.",energy:"Social",imageUrl:"/Community/Campfire.webp",participants:12}),this.initChatState(["main"])}onEnter(){this.loadRoomChatFromDB("main"),this._injectSenderAvatar("main")}getHeaderGradient(){return"background:linear-gradient(135deg,rgba(234,88,12,0.1) 0%,rgba(251,146,60,0.05) 100%);"}getParticipantText(){return`${this.state.participants} around the fire`}buildBody(){return`
        <div class="ps-body">
        ...
        <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:24px;margin-bottom:12px;">
            ...
            <div style="text-align:center;margin-bottom:20px;">
                ...
                <p style="font-family:var(--serif);font-style:italic;font-size:15px;line-height:1.8;color:var(--text-muted);max-width:560px;margin:0 auto;">${o.narrative}</p>
            </div>`:""}
            ${d?`
            ...
            <div id="${m}MeaningText" style="text-align:center;font-size:15px;line-height:1.7;max-width:560px;margin:0 auto;">${o.upright||""}</div>
            <div id="${m}MeaningTextReversed" style="display:none;text-align:center;font-size:15px;line-height:1.7;max-width:560px;margin:0 auto;color:var(--text-muted);">${o.reversed||""}</div>
            ${h?`
        ...
        <div style="border:2px solid var(--border);border-radius:var(--radius-lg);padding:20px;background:var(--surface);margin-top:16px;">
            ...
            <div id="${this.roomId}MasteryChakras" style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center;">
                ...
                <div style="display:flex;gap:10px;align-items:baseline;padding:8px 10px;background:var(--background);border-radius:var(--radius-md);border:1px solid var(--border);">
                    ...
                    opacity:${h?"1":"0.5"};
                    transition:all 0.3s ease;">${p.label}</span>`}).join(""),this._masteryLoaded=!0}catch(a){console.warn("[ReikiRoom] load mastery failed",a),i&&(i.textContent="— / 7")}}onEnter(){this._personalTabHydrated=!1,this._masteryLoaded=!1,this.initializeChat(),requestAnimationFrame(()=>{var o;const e=document.getElementById(`${this.roomId}DailyTabContent`),i=document.getElementById(`${this.roomId}DailyChatSection`);if(e&&this._chakraDataReady&&this.state.currentDay){const n=this.CHAKRA_SCHEDULE[this.state.currentDay];e.innerHTML=`
                ...
                ${this._getChakraDisplayHTML(n,this.state.dailyImageIndex,!0,!0)}`,i&&(i.style.display=""),this._loadCommunityEnergy()}(o=document.querySelector(`#${this.roomId}View .tarot-main`))==null||o.scrollTo(0,0)})}getActions(){return{...super.getActions(),startPersonalSession:()=>this.startPersonalSession(),prevChakraImage:e=>{this._actionEl(e).dataset.scope==="daily"?this.previousDailyImage():this.previousPersonalImage()},nextChakraImage:e=>{this._actionEl(e).dataset.scope==="daily"?this.nextDailyImage():this.nextPersonalImage()},toggleChakraJournal:e=>this._toggleChakraJournalLog(this._actionEl(e).dataset.prefix),saveChakraJournal:e=>{const i=this._actionEl(e);this._saveChakraJournalEntry(i.dataset.prefix,i.dataset.key)},submitCommunityEnergy:()=>this._submitCommunityEnergy()}}getInstructions(){return`
            ...
            </ul>`}}Object.assign(tt.prototype,He);Object.assign(tt.prototype,Ct);const gi=new tt;window.ReikiRoom=gi;var vt;(vt=window.dispatchRoomReady)==null||vt.call(window,"reiki");const yi={state:{isEnabled:!1,isRendered:!1},config:{FEATURE_ENABLED:!1},getHTML(){return this.config.FEATURE_ENABLED?`
