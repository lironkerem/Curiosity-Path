var p=Object.defineProperty;var h=(s,e,t)=>e in s?p(s,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):s[e]=t;var l=(s,e,t)=>h(s,typeof e!="symbol"?e+"":e,t);import m from"./AffirmationsList-BRihhJ1z.js";import{I as g}from"./Index-B1Fu9TSL.js";import"./User-Tab-BvNLwH6J.js";import"./community-hub-B89giXmc.js";import"./features-lazy-CnKaPKmh.js";import"./vendor-suncalc-BBoG6fSx.js";import"./supabase-DmJIYYE5.js";import"./gamification-C4jQodZo.js";const o=class o{constructor(e){this.app=e,this.boosters=[{id:1,title:"5-Minute Dance Party",emoji:"💃",description:"Put on your favorite song and move your body!",duration:"5 min",category:"Movement"},{id:2,title:"Gratitude Snapshot",emoji:"📸",description:"Quickly name 3 things you're grateful for right now.",duration:"3 min",category:"Mindfulness"},{id:3,title:"Power Pose",emoji:"🦸",description:"Stand like a superhero for 2 minutes to boost confidence.",duration:"2 min",category:"Confidence"},{id:4,title:"Mindful Sip",emoji:"🍵",description:"Drink a glass of water or tea, focusing only on the sensation.",duration:"4 min",category:"Calm"},{id:5,title:"Quick Stretch",emoji:"🤸",description:"Gently stretch your arms, neck, and back for 3 minutes.",duration:"3 min",category:"Body"},{id:6,title:"Listen to One Song",emoji:"🎶",description:"Listen to one favorite song without any distractions.",duration:"4 min",category:"Joy"}],this.boostersLoaded=!1,this.currentBooster=this.getRandomBooster(),this.currentQuote=null,this.currentAffirmation=null,this.currentInquiry=null,this.affirmations=m,this.inquiryEngine=new g("beginner"),this._lastTracked=0,this._cachedElements={},this.loadBoosters()}_esc(e){return e?String(e).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}async loadBoosters(){try{const e=await fetch("/Data/HappinessBoostersList.json");if(!e.ok)throw new Error(`HTTP ${e.status}`);const t=await e.json();this.boosters=t.boosters,this.boostersLoaded=!0,this._getElement("happiness-tab")?.classList.contains("active")&&this.render()}catch(e){console.error("Failed to load happiness boosters:",e),this.boostersLoaded=!0}}_getElement(e){return(!this._cachedElements[e]||!document.getElementById(e))&&(this._cachedElements[e]=document.getElementById(e)),this._cachedElements[e]}_getDayOfYear(){const e=new Date;return Math.floor((e-new Date(e.getFullYear(),0,0))/864e5)}getDailyBooster(){return this.boosters[this._getDayOfYear()%this.boosters.length]}getRandomBooster(){return this.app.randomFrom(this.boosters)}getDailyAffirmation(){const e=this.affirmations?.general_positive_affirmations;if(!e?.length)return"You are doing great.";const t=e[this._getDayOfYear()%e.length];return typeof t=="string"?t:t.text}getRandomAffirmation(){const e=Object.values(this.affirmations||{}).flat().filter(Boolean);if(!e.length)return"You are capable of amazing things.";const t=this.app.randomFrom(e);return t?.text||t||"You are capable of amazing things."}getRandomInquiry(){const e=this.inquiryEngine._getUniqueDomains();return this.inquiryEngine.getRandomQuestion(e[Math.floor(Math.random()*e.length)])}getRandomQuote(){return window.QuotesData?.getRandomQuote()||{text:"Stay positive!",author:"Unknown"}}trackView(){const e=Date.now();if(this._lastTracked&&e-this._lastTracked<o.DEBOUNCE_MS)return this.getTodayViewCount();this._lastTracked=e;const t=new Date().toDateString();let r=this._getStorageData(t);if(r.count+=1,localStorage.setItem("daily_booster_views",JSON.stringify(r)),this.app.gamification){const n=this.app.gamification;n.progressQuest("daily","daily_booster",1),n.progressQuest("weekly","happiness_boosters_20",1),n.progressQuest("monthly","monthly_happiness_100",1)}return r.count}getTodayViewCount(){return this._getStorageData(new Date().toDateString()).count}_getStorageData(e){try{const t=localStorage.getItem("daily_booster_views");if(t){const r=JSON.parse(t);if(r.date===e)return r}}catch(t){console.error("Failed to parse storage data:",t)}return{date:e,count:0}}updateQuestDisplay(){const e=this.getTodayViewCount(),t=document.getElementById("happiness-quest-badge");if(t){const r=e>=o.QUEST_TARGET;t.style.cssText=`display:inline-flex;align-items:center;justify-content:center;gap:0.25rem;white-space:nowrap;padding:0.35rem 0.85rem;border-radius:9999px;font-size:0.85rem;font-weight:600;line-height:1.4;${r?"background:rgba(34,197,94,0.15);color:#22c55e;border:1px solid rgba(34,197,94,0.3);":"background:rgba(102,126,234,0.15);color:var(--neuro-accent);border:1px solid rgba(102,126,234,0.3);"}`;const n=document.getElementById("happiness-quest-count");n&&(n.textContent=e)}this._toggleQuestCompleteBanner(e)}_toggleQuestCompleteBanner(e){const t="happiness-quest-complete";let r=document.getElementById(t);if(e>=o.QUEST_TARGET&&!r){r=document.createElement("div"),r.id=t,r.style.cssText="margin-bottom:2rem;padding:1rem;border-radius:0.5rem;background:rgba(34,197,94,.08);border:1px solid rgba(34,197,94,.25);",r.innerHTML=`<p class="text-center" style="color:#22c55e;display:flex;align-items:center;justify-content:center;gap:0.5rem;"><svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' class='lucide-icon'><path d='M18 6 7 17l-5-5'/><path d='m22 10-7.5 7.5L13 16'/></svg> Daily quest complete! Keep exploring if you'd like!</p>`;const n=document.querySelector("#happiness-tab .universal-content");n?.insertBefore(r,n.querySelector("main"))}else e<o.QUEST_TARGET&&r&&r.remove()}_flipCard(e,t){const r=this._getElement(e);if(!r)return;const n=r.querySelector(".flip-card-inner"),a=r.querySelector(".flip-card-back");a.innerHTML=t;const i=n.style.transform.match(/-?\d+\.?\d*/),d=i?parseFloat(i[0]):0;n.style.transform=`rotateY(${d+o.ROTATION_DEG}deg)`;const c=()=>{n.removeEventListener("transitionend",c),r.querySelector(".flip-card-front").innerHTML=t};n.addEventListener("transitionend",c)}_refreshContent(e,t,r,n,a){this[`current${e}`]=t.call(this);const i=this.trackView(),d=a.call(this,this[`current${e}`],n);if(this._flipCard(r,d),this.updateQuestDisplay(),this.app.gamification&&this.app.gamification.incrementHappinessViews(),this.app.showToast){const c=i>=o.QUEST_TARGET;this.app.showToast(c?`Quest complete! You've viewed ${o.QUEST_TARGET} items today!`:`New ${n.toLowerCase()} revealed! (${i}/${o.QUEST_TARGET})`,"success")}}refreshBooster(){this._refreshContent("Booster",this.getRandomBooster,"booster-card","Booster",this._formatBooster)}refreshQuote(){this._refreshContent("Quote",this.getRandomQuote,"quote-card","Quote",this._formatQuote)}refreshAffirmation(){this._refreshContent("Affirmation",this.getRandomAffirmation,"affirm-card","Affirmation",this._formatAffirmation)}refreshInquiry(){this._refreshContent("Inquiry",this.getRandomInquiry,"inquiry-card","Inquiry",this._formatInquiry)}_formatBooster(e){return`
      ...
      <div class="text-center">
        <h3 class="text-2xl font-bold" style="color:var(--neuro-accent);">${this._esc(e.title)}</h3>
        <p class="mt-2 text-lg">${this._esc(e.description)}</p>
        <div class="mt-4 text-sm" style="color:var(--neuro-text-light);">
          <span>${this._esc(e.duration)}</span> • <span>${this._esc(e.category)}</span> • <span>${this._esc(e.emoji)}</span>
        </div>
      </div>
      ...
      <p class="text-2xl font-semibold text-center" style="color:var(--neuro-accent);">"${this._esc(e.text)}"</p>
      <p class="mt-3 text-center text-lg" style="color:var(--neuro-text);">- ${this._esc(e.author)}</p>
      <div style="margin-top:2rem;display:flex;justify-content:flex-end;">
      ...
      <p class="text-2xl font-semibold text-center" style="color:var(--neuro-accent);">"${this._esc(e)}"</p>
      <div style="margin-top:2rem;display:flex;justify-content:flex-end;">
      ...
      <div class="text-center">
        <div style="margin-bottom:1rem;padding:0.5rem;background:var(--neuro-bg-secondary);border-radius:8px;display:inline-block;">
          <span style="font-size:0.75rem;text-transform:uppercase;font-weight:700;color:var(--neuro-accent);">${this._esc(e.domain)}</span>
        </div>
        <p class="text-2xl font-semibold" style="color:var(--neuro-accent);line-height:1.4;margin-bottom:1rem;">${this._esc(e.question)}</p>
        <p class="mt-2 text-lg" style="font-style:italic;color:var(--neuro-text-secondary);">${this._esc(e.holding)}</p>
        <div class="mt-4 text-sm" style="color:var(--neuro-text-light);">
  ...
  <div style="padding:1.5rem;min-height:100vh;">
    <div class="universal-content">
      ...
      <main class="space-y-6">
        ${this._renderCard("affirm-card","affirmation","Positive Affirmation",`<p class="text-2xl font-semibold text-center" style="color:var(--neuro-accent);">"${this._esc(this.currentAffirmation)}"</p>`,"refreshAffirmation")}

        ${this._renderCard("quote-card","quote","Inspirational Quote",`<p class="text-2xl font-semibold text-center" style="color:var(--neuro-accent);">"${this._esc(this.currentQuote.text)}"</p>
           <p class="mt-3 text-center text-lg" style="color:var(--neuro-text);">- ${this._esc(this.currentQuote.author)}</p>`,"refreshQuote")}

        ${this._renderCard("booster-card","booster","A Quick Happiness Booster",`<h3 class="text-2xl font-bold" style="color:var(--neuro-accent);">${this._esc(this.currentBooster.title)}</h3>
           <p class="mt-2 text-lg">${this._esc(this.currentBooster.description)}</p>
           <div class="mt-4 text-sm" style="color:var(--neuro-text-light);"><span>${this._esc(this.currentBooster.duration)}</span> • <span>${this._esc(this.currentBooster.category)}</span></div>`,"refreshBooster")}
        ...
        ${this._renderCard("inquiry-card","inquiry","Self Inquiry",`<div style="margin-bottom:1rem;padding:0.5rem;background:var(--neuro-bg-secondary);border-radius:8px;display:inline-block;">
           ...
           <div class="mt-4 text-sm" style="color:var(--neuro-text-light);"><span>Level ${this.currentInquiry.intensity}</span> • <span>Self-Inquiry</span></div>`,"refreshInquiry")}
      ...
      </div>`}};l(o,"QUEST_TARGET",5),l(o,"DEBOUNCE_MS",1e3),l(o,"ROTATION_DEG",360),l(o,"INTENSITY_EMOJIS",{1:"🌱",2:"🌿",3:"🌳",4:"🔥"});let u=o;export{u as default};
