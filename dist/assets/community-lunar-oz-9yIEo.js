var S=Object.defineProperty;var A=(n,e,t)=>e in n?S(n,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):n[e]=t;var _=(n,e,t)=>A(n,typeof e!="symbol"?e+"":e,t);import{_ as b}from"./features-lazy-BwdvZW0A.js";import{S as x}from"./vendor-suncalc-BBoG6fSx.js";const c={generateIntentionPracticeContent(n,e,t){const{intention:o="",affirmation:r="",releaseList:i=""}=n,a=e.map(s=>`
...
@media(prefers-reduced-motion:reduce){*{animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important;}}
        `}},G=Object.freeze(Object.defineProperty({__proto__:null,LunarUI:u},Symbol.toStringTag,{value:"Module"})),C=()=>window.CommunityDB,v={location:{latitude:31,longitude:0,name:"Default"},currentMoonData:null,currentLunarRoom:null,lunarRooms:[{phaseRanges:[[0,.068],[.932,1]],phaseName:"New Moon",icon:"🌑",roomName:"New Moon Intentions",description:"Set intentions and plant seeds for the lunar cycle",roomId:"new-moon",energy:"Beginning, Stillness, Potential",practices:["Silent meditation","Intention setting","Vision journaling","Seed planting ritual"]},{phaseRanges:[[.068,.432]],phaseName:"Waxing Moon",icon:"🌓",roomName:"Waxing Growth Practice",description:"Build momentum and cultivate expanding energy",roomId:"waxing-moon",energy:"Growth, Action, Building",practices:["Dynamic movement","Energy cultivation","Goal visualization","Action planning meditation"]},{phaseRanges:[[.432,.568]],phaseName:"Full Moon",icon:"🌕",roomName:"Full Moon Illumination",description:"Celebrate fullness and release what no longer serves",roomId:"full-moon",energy:"Culmination, Release, Clarity",practices:["Celebration ritual","Gratitude meditation","Release ceremony","Moon bathing"]},{phaseRanges:[[.568,.932]],phaseName:"Waning Moon",icon:"🌗",roomName:"Waning Release Practice",description:"Let go and reflect on the lunar journey",roomId:"waning-moon",energy:"Release, Reflection, Rest",practices:["Reflection meditation","Letting go ritual","Rest and restore","Completion ceremony"]}],init(){if(this._initialized)return;this._initialized=!0;const n=()=>this.updateAll();navigator.geolocation?navigator.geolocation.getCurrentPosition(({coords:e})=>{this.location={latitude:e.latitude,longitude:e.longitude,name:"Your Location"},n()},()=>n(),{timeout:5e3,maximumAge:36e5}):n(),setInterval(()=>this.updateAll(),6e5)},updateAll(){this.updateMoonData(),this.updateMoonVisualization(),this.updateLunarRoom(),this.renderLunarCard()},setLocation(n,e,t){this.location={latitude:n,longitude:e,name:t},this.updateAll()},updateMoonData(){const n=new Date,e=x.getMoonIllumination(n),t=x.getMoonTimes(n,this.location.latitude,this.location.longitude);this.currentMoonData={phase:e.phase,fraction:e.fraction,angle:e.angle,rise:t.rise,set:t.set,phaseName:this.getMoonPhaseName(e.phase),age:e.phase*29.53,nextFullMoon:this.getNextFullMoon(n)},this._renderMoonInfo()},_renderMoonInfo(){const n=this.currentMoonData;if(!n)return;const e=(o,r)=>{const i=document.getElementById(o);i&&(i.textContent=r)};e("moonPhaseName",n.phaseName),e("moonIllumination",`${Math.round(n.fraction*100)}% illuminated`),e("moonAge",`${n.age.toFixed(1)} days old`),e("moonrise",n.rise?this.formatTime(n.rise):"No rise today"),e("moonset",n.set?this.formatTime(n.set):"No set today");const t=Math.ceil((n.nextFullMoon-new Date)/864e5);e("nextPhase",`Next Full Moon: ${this.formatDate(n.nextFullMoon)} (${t} days)`)},updateMoonVisualization(){this.currentMoonData&&this.drawMoon(this.currentMoonData.phase,this.currentMoonData.angle)},drawMoon(n,e){const t=document.getElementById("moonSvg");if(!t)return;const o=50,r=60,i=60;t.innerHTML="";const a=document.createElementNS("http://www.w3.org/2000/svg","g");a.setAttribute("transform",`rotate(${(e*180/Math.PI*-1).toFixed(2)}, ${r}, ${i})`),t.appendChild(a);const s=document.createElementNS("http://www.w3.org/2000/svg","circle");s.setAttribute("cx",r),s.setAttribute("cy",i),s.setAttribute("r",o),s.setAttribute("fill","#2d3748"),a.appendChild(s);const l=Math.cos(n*2*Math.PI)*o,m=document.createElementNS("http://www.w3.org/2000/svg","path");m.setAttribute("fill","#f7fafc"),m.setAttribute("d",n<.5?`M ${r},${i-o} A ${o},${o} 0 0 1 ${r},${i+o} Q ${r+l},${i} ${r},${i-o}`:`M ${r},${i-o} Q ${r+l},${i} ${r},${i+o} A ${o},${o} 0 0 1 ${r},${i-o}`),a.appendChild(m),[{x:.3,y:-.2,r:.15},{x:-.2,y:.3,r:.1},{x:.1,y:.4,r:.08}].forEach(({x:h,y,r:w})=>{if(!(n<.5&&h>0||n>=.5&&h<0||n>.45&&n<.55))return;const f=document.createElementNS("http://www.w3.org/2000/svg","circle");f.setAttribute("cx",r+h*o),f.setAttribute("cy",i+y*o),f.setAttribute("r",w*o),f.setAttribute("fill","rgba(0,0,0,0.15)"),a.appendChild(f)})},getMoonPhaseName(n){return n<.03||n>.97?"New Moon":n<.22?"Waxing Crescent":n<.28?"First Quarter":n<.47?"Waxing Gibbous":n<.53?"Full Moon":n<.72?"Waning Gibbous":n<.78?"Last Quarter":"Waning Crescent"},getNextFullMoon(n){const e=x.getMoonIllumination(n).phase,t=e<.5?(.5-e)*29.53:(1.5-e)*29.53;return new Date(n.getTime()+t*864e5)},getLunarRoomByPhase(n){return this.lunarRooms.find(e=>e.phaseRanges.some(([t,o])=>n>=t&&n<=o))??this.lunarRooms[0]},updateLunarRoom(){if(!this.currentMoonData)return;const n=this.getLunarRoomByPhase(this.currentMoonData.phase);this.currentLunarRoom?.roomId,n.roomId,this.currentLunarRoom=n},getCurrentRoom(){return this.currentLunarRoom},_ROOM_MODULES:{"new-moon":()=>b(()=>Promise.resolve().then(()=>$),void 0),"waxing-moon":()=>b(()=>Promise.resolve().then(()=>R),void 0),"full-moon":()=>b(()=>Promise.resolve().then(()=>D),void 0),"waning-moon":()=>b(()=>Promise.resolve().then(()=>W),void 0)},_roomExportName:{"new-moon":"NewMoonRoom","waxing-moon":"WaxingMoonRoom","full-moon":"FullMoonRoom","waning-moon":"WaningMoonRoom"},async _loadAndEnterRoom(n){const e=this._ROOM_MODULES[n];if(!e){window.Core?.showToast(`Unknown room: ${n}`);return}try{const o=(await e())[this._roomExportName[n]];o?o.enterRoom():window.Core?.showToast(`${n} failed to initialise`)}catch(t){console.error(`[LunarEngine] Failed to load ${n}:`,t),window.Core?.showToast(`Failed to load ${n}`)}},joinLunarRoom(){if(!this.currentLunarRoom){window.Core?.showToast("Lunar room not ready");return}this._loadAndEnterRoom(this.currentLunarRoom.roomId)},adminJoinRoom(n){this._loadAndEnterRoom(n)},renderLunarCard(){const n=document.getElementById("lunarContainer");if(!n){console.warn("LunarEngine: #lunarContainer not found");return}if(u.injectStyles(),!this.currentMoonData||!this.currentLunarRoom){n.innerHTML='<p style="color:var(--text-muted);padding:20px;">Loading lunar data...</p>';return}const{currentMoonData:e,currentLunarRoom:t}=this,o=window.Core?.state?.currentUser?.is_admin;n.innerHTML=`
            ...
            </div>`},_attachAdminListeners(){const n=document.querySelector(".lunar-admin-header");if(n&&!n._lunarBound){const e=()=>{const t=document.getElementById(n.dataset.panel),o=document.getElementById(n.dataset.toggle),r=t.style.display!=="none";t.style.display=r?"none":"block",o.textContent=r?"▶":"▼",n.setAttribute("aria-expanded",String(!r))};n.addEventListener("click",e),n.addEventListener("keydown",t=>{(t.key==="Enter"||t.key===" ")&&(t.preventDefault(),e())}),n._lunarBound=!0}document.querySelectorAll(".lunar-admin-room-btn").forEach(e=>{e._lunarBound||(e.addEventListener("click",()=>this.adminJoinRoom(e.dataset.roomId)),e._lunarBound=!0)})},_refreshOuterCard(){if(!C()?.ready){this._outerCardRetry||(this._outerCardRetry=setTimeout(()=>{this._outerCardRetry=null,this._refreshOuterCard()},500));return}const n=this.currentLunarRoom?.roomId;if(!n)return;const e=async()=>{try{const t=await C()?.getRoomParticipants(n),o=document.getElementById("lunarRoomPresence");o&&(o.textContent=`${t.length} present`)}catch(t){console.warn("[LunarEngine] _refreshOuterCard error:",t)}};if(e(),this._attachCardListeners(),this._attachAdminListeners(),this._outerCardSub)try{this._outerCardSub.unsubscribe()}catch{}this._outerCardSub=C()?.subscribeToPresence(e)},formatTime(n){return!n||isNaN(n.getTime())?"N/A":n.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})},formatDate(n){return n.toLocaleDateString("en-US",{month:"short",day:"numeric"})},getMoonData(){return this.currentMoonData},renderLunarRoom(){this.renderLunarCard()},injectAdminUI(){if(!window.Core?.state?.currentUser?.is_admin)return;const e=document.querySelector("#lunarContainer .celestial-card-full");if(e&&!document.getElementById("lunarAdminPanel")){const t=document.createElement("div");t.innerHTML=this._renderAdminSection(),e.appendChild(t.firstElementChild),this._attachAdminListeners()}}};window.LunarEngine=v;const H=Object.freeze(Object.defineProperty({__proto__:null,LunarEngine:v},Symbol.toStringTag,{value:"Module"})),p=()=>window.CommunityDB,d=class d{constructor(e){if(!e)throw new Error("LunarRoom: phaseConfig is required");this.config=e,this.isActive=!1,this.currentPractice=null,this.collectiveTimer=null,this.collectiveWords=null,this.saveDebounceTimer=null,this._retryCheckTimeout=null,this._hasLoggedWaiting=!1,this.eventListeners=[],this.userWeekData=this._getDefaultUserData(),this.weekStartDate=null,this.weekEndDate=null,this.domCache={dynamicContent:null,popup:null},this._bindMethods()}_getDefaultUserData(){return{seedAffirmation:null,practiceCount:0,journalEntries:[],emotionalEchoes:[],intentionShared:!1,privateIntention:"",collectiveWord:"",intention:"",affirmation:"",releaseList:""}}_bindMethods(){["init","enterRoom","renderRoomDashboard","switchMode","showPracticePopup","showCollectiveIntentionPopup","startCollectiveStep2","startCollectiveStep3","startCollectiveStep4","startCollectiveStep5","startMeditationTimer","startWitnessingTimer","submitWordToCollective","completeCollectivePractice","closeCollectivePopup","closePracticePopup","saveIntentionPractice","submitClosure","showLockedMessage","enterGroupCircle","leaveRoom","_handlePopupClick","_clearTimer"].forEach(e=>{typeof this[e]=="function"&&(this[e]=this[e].bind(this))})}init(){try{this.checkIfWeekActive(),u.injectStyles(this.config.cssPrefix)}catch(e){console.error(`${this.config.name} init error:`,e)}}checkIfWeekActive(){try{if(!v?.currentMoonData){this._hasLoggedWaiting||(this._hasLoggedWaiting=!0),this._retryCheckTimeout||(this._retryCheckTimeout=setTimeout(()=>{this._retryCheckTimeout=null,this.checkIfWeekActive()},500));return}if(window.Core?.state?.currentUser?.is_admin){this.isActive=!0,this.calculateWeekDates(),this.loadUserWeekData();return}const t=v.currentMoonData.phase;this.isActive=this.config.phaseRanges.some(([o,r])=>t>=o&&t<=r),this.isActive&&(this.calculateWeekDates(),this.loadUserWeekData())}catch(e){console.error("checkIfWeekActive error:",e),this.isActive=!1}}calculateWeekDates(){try{const e=v?.currentMoonData;if(!e){console.error("No moon data");return}const{phase:t,age:o}=e,r=d.CONSTANTS,i=new Date,a=new Date(i-o*r.MS_PER_DAY);let s=this.config.phaseRanges.find(([l,m])=>t>=l&&t<=m);if(!s&&(s=window.Core?.state?.currentUser?.is_admin?this.config.phaseRanges[0]:null,!s)){this._calculateNextOccurrence(t,o);return}this.weekStartDate=new Date(a.getTime()+s[0]*r.LUNAR_CYCLE_DAYS*r.MS_PER_DAY),this.weekEndDate=new Date(a.getTime()+s[1]*r.LUNAR_CYCLE_DAYS*r.MS_PER_DAY)}catch(e){console.error("calculateWeekDates error:",e)}}_calculateNextOccurrence(e,t){try{const o=d.CONSTANTS,r=new Date,i=new Date(r-t*o.MS_PER_DAY),[a,s]=this.config.phaseRanges[0],l=e<a?i:new Date(i.getTime()+o.LUNAR_CYCLE_DAYS*o.MS_PER_DAY);this.weekStartDate=new Date(l.getTime()+a*o.LUNAR_CYCLE_DAYS*o.MS_PER_DAY),this.weekEndDate=new Date(l.getTime()+s*o.LUNAR_CYCLE_DAYS*o.MS_PER_DAY)}catch(o){console.error("_calculateNextOccurrence error:",o)}}getDaysRemaining(){return this.weekEndDate?Math.max(0,Math.ceil((this.weekEndDate-new Date)/d.CONSTANTS.MS_PER_DAY)):0}shouldShowClosure(){const e=this.getDaysRemaining();return e>0&&e<=2}loadUserWeekData(){try{const e=localStorage.getItem(this.config.storageKey);e&&(this.userWeekData={...this.userWeekData,...JSON.parse(e)})}catch(e){console.error("loadUserWeekData error:",e)}}saveUserWeekData(e=!1){if(e){this._performSave();return}clearTimeout(this.saveDebounceTimer),this.saveDebounceTimer=setTimeout(()=>this._performSave(),d.CONSTANTS.SAVE_DEBOUNCE_MS)}_performSave(){try{localStorage.setItem(this.config.storageKey,JSON.stringify(this.userWeekData))}catch(e){console.error("_performSave error:",e),window.Core?.showToast("Failed to save data")}}_sanitizeInput(e){return typeof e=="string"?e.replace(/<[^>]*>/g,"").trim():""}_validateIntention(e){const t=d.CONSTANTS;return e?.trim()?e.length>t.MAX_INTENTION_LENGTH?{valid:!1,error:`Intention must be under ${t.MAX_INTENTION_LENGTH} characters`}:{valid:!0}:{valid:!1,error:"Intention cannot be empty"}}_validateAffirmation(e){const t=d.CONSTANTS;return e?.trim()?e.length>t.MAX_AFFIRMATION_LENGTH?{valid:!1,error:`Affirmation must be under ${t.MAX_AFFIRMATION_LENGTH} characters`}:{valid:!0}:{valid:!1,error:"Affirmation cannot be empty"}}_validateCollectiveWord(e){return e?.trim()?e.length>d.CONSTANTS.MAX_WORD_LENGTH?{valid:!1,error:`Word must be under ${d.CONSTANTS.MAX_WORD_LENGTH} characters`}:e.trim().includes(" ")?{valid:!1,error:"Please enter only one word"}:{valid:!0}:{valid:!1,error:"Word cannot be empty"}}enterRoom(){try{const e=window.Core?.state?.currentUser?.is_admin;if(!this.isActive&&!e){window.Core?.showToast(`${this.config.emoji} ${this.config.name} room opens during the ${this.config.name} phase`);return}window.Core?.navigateTo("practiceRoomView"),window.PracticeRoom?.stopHubPresence?.(),window.currentLunarRoom=this,this.renderRoomDashboard(),this._setPresence(),this._loadCollectiveWords(),setTimeout(()=>this._refreshLivePresence(),300)}catch(e){console.error("enterRoom error:",e),window.Core?.showToast("Failed to enter room")}}leaveRoom(){try{this._clearPresence(),["_presenceSub","_collectiveWordsSub"].forEach(e=>{if(this[e]){try{this[e].unsubscribe()}catch{}this[e]=null}}),this._clearTimer(),this._removeEventListeners(),window.currentLunarRoom=null,window.PracticeRoom?.startHubPresence?.(),window.Core?.navigateTo("hubView")}catch(e){console.error("leaveRoom error:",e)}}renderRoomDashboard(){var e;try{const t=(e=this.domCache).dynamicContent??(e.dynamicContent=document.getElementById("dynamicRoomContent"));if(!t){console.error("dynamicRoomContent not found");return}const o=new Date,r=this.getDaysRemaining(),i=Math.ceil((this.weekStartDate-o)/d.CONSTANTS.MS_PER_DAY),a=this.getLivingPresenceCount(),s=this.isActive?`${r} ${r===1?"day":"days"} remaining`:`${i} ${i===1?"day":"days"} until ${this.config.name}`;t.innerHTML=`
                <div class="lunar-room-bg">
                    <div class="${this.config.cssPrefix}-starfield lunar-starfield">
                        ${u.generateStarfield()}
                    </div>
                    ${u.renderTopBar({emoji:this.config.emoji,name:this.config.name,daysText:s,livingPresenceCount:a,cssPrefix:this.config.cssPrefix})}
                    <div class="${this.config.cssPrefix}-content-wrapper lunar-content-wrapper">
                        ${u.renderMoonVisual({cssPrefix:this.config.cssPrefix,sphereClass:`${this.config.cssPrefix}-moon-sphere`,glowClass:`${this.config.cssPrefix}-moon-glow`})}
                        ${u.renderIntroCard({imageUrl:this.config.imageUrl,description:this.config.description})}
                        ${u.renderModeToggle({cssPrefix:this.config.cssPrefix,globalName:this.config.globalName})}
                        ...
                        <div class="lunar-wisdom-section">${u.renderWisdomText(this.config.wisdomQuote)}</div>
                    </div>
            ...
            ${e?this._renderSavedInputs():""}`}_renderPracticeCards(){const e=Object.entries(this.config.practices),t=e[0]?.[0],o=!!(this.userWeekData.intention||this.userWeekData.affirmation);return e.map(([r,i])=>{const a=r===t&&o;return`
                <div class="lunar-practice-card ${this.config.cssPrefix}-practice-card${a?" locked":""}"
                     data-practice="${r}" data-locked="${a}"
                     tabindex="0" role="button"
                     aria-label="${i.title}${a?" - Complete":""}">
                    <div class="lunar-practice-icon" style="color:${i.color};">${i.icon}</div>
                    <div class="lunar-practice-info">
                        <h4>${i.title}</h4>
                        <p>${i.description}</p>
                    </div>
                    ${a?'<div class="lunar-lock-badge">✓ Complete</div>':""}
                </div>`}).join("")}_renderSavedInputs(){const{intention:e,affirmation:t,releaseList:o}=this.userWeekData,r=this.config.savedInputLabels,i=a=>c._escapeHtml(a);return`
            ...
            <div class="lunar-group-intro">
                ...
                <div class="lunar-group-avatars">${u.renderMockAvatars(e)}</div>
            ...
            </div>`}_attachEventListeners(e){this._removeEventListeners();const t=o=>{const r=o.target.closest(".lunar-practice-card");if(r&&(o.type==="click"||o.key==="Enter"||o.key===" ")){o.preventDefault(),r.dataset.locked==="true"?this.showLockedMessage():this.showPracticePopup(r.dataset.practice);return}if(o.type!=="click")return;const i=o.target.closest("[data-action]");if(i)switch(i.dataset.action){case"switch-mode":this.switchMode(i.dataset.mode);break;case"enter-group-circle":this.enterGroupCircle();break;case"submit-closure":this.submitClosure();break;case"back-to-hub":window.Rituals?Rituals.showClosing():this.leaveRoom();break}};e.addEventListener("click",t),e.addEventListener("keydown",t),this.eventListeners.push({element:e,type:"click",handler:t},{element:e,type:"keydown",handler:t})}_removeEventListeners(){this.eventListeners.forEach(({element:e,type:t,handler:o})=>e?.removeEventListener(t,o)),this.eventListeners=[]}switchMode(e){document.querySelectorAll(".lunar-practice-mode").forEach(t=>t.classList.toggle("active",t.dataset.mode===e)),document.querySelectorAll(".lunar-mode-btn").forEach(t=>{const o=t.dataset.mode===e;t.classList.toggle("active",o),t.setAttribute("aria-pressed",String(o))})}showPracticePopup(e){try{const t=this.config.practices[e];if(!t){console.error(`Practice not found: ${e}`);return}this.currentPractice=e;const o=e===Object.keys(this.config.practices)[0],r=u.createPopup({icon:t.icon,title:t.title,subtitle:t.description,content:this.getPracticeContent(e),cssPrefix:this.config.cssPrefix,hasFooter:!o,onClose:()=>this.closePracticePopup()});(document.getElementById("communityHubFullscreenContainer")??document.body).appendChild(r),this.domCache.popup=r,this._attachPopupListeners(r,e)}catch(t){console.error("showPracticePopup error:",t)}}_attachPopupListeners(e,t){e.querySelectorAll('[data-action="close-popup"]').forEach(o=>o.addEventListener("click",this.closePracticePopup)),e.addEventListener("click",this._handlePopupClick),t===Object.keys(this.config.practices)[0]&&(e.querySelector('[data-action="save-practice"]')?.addEventListener("click",this.saveIntentionPractice),e.querySelectorAll("[data-affirmation]").forEach(o=>o.addEventListener("click",()=>{const r=e.querySelector("#affirmationText");r&&(r.value=o.dataset.affirmation)})))}_handlePopupClick(e){e.target===e.currentTarget&&this.closePracticePopup()}getPracticeContent(e){try{const t=this.config.practiceContent[e];return t?t(this.userWeekData,this.config.prebuiltAffirmations):`<div class="lunar-popup-section"><h3>Practice</h3><p>${this.config.practices[e].description}</p></div>`}catch(t){return console.error("getPracticeContent error:",t),"<p>Error loading practice content</p>"}}closePracticePopup(){const e=this.domCache.popup??document.querySelector(".lunar-practice-popup");e&&(e.remove(),this.domCache.popup=null)}saveIntentionPractice(){try{const e=document.getElementById("intentionText"),t=document.getElementById("affirmationText"),o=document.getElementById("releaseListText");if(!e||!t||!o){console.error("Form elements not found");return}const r=this._sanitizeInput(e.value),i=this._sanitizeInput(t.value),a=this._sanitizeInput(o.value),s=this._validateIntention(r);if(!s.valid){window.Core?.showToast(s.error);return}const l=this._validateAffirmation(i);if(!l.valid){window.Core?.showToast(l.error);return}Object.assign(this.userWeekData,{intention:r,affirmation:i,releaseList:a}),this.userWeekData.practiceCount++,this.saveUserWeekData(!0),window.Core?.showToast("Practice saved"),this.closePracticePopup(),this.renderRoomDashboard()}catch(e){console.error("saveIntentionPractice error:",e),window.Core?.showToast("Failed to save practice")}}showLockedMessage(){window.Core?.showToast("✓ Practice completed for this cycle")}showCollectiveIntentionPopup(){try{const e=u.createPopup({icon:"🌙",title:"Collective Intention Circle",subtitle:`Join others in ${this.config.collectiveFocus}`,content:this._getCollectiveStep1Content(),cssPrefix:this.config.cssPrefix,hasFooter:!1,onClose:()=>this.closeCollectivePopup()});(document.getElementById("communityHubFullscreenContainer")??document.body).appendChild(e),this.domCache.popup=e,e.querySelectorAll('[data-action="close-popup"]').forEach(t=>t.addEventListener("click",this.closeCollectivePopup)),e.querySelector('[data-action="begin-collective"]')?.addEventListener("click",this.startCollectiveStep2),e.addEventListener("click",this._handlePopupClick)}catch(e){console.error("showCollectiveIntentionPopup error:",e)}}_getCollectiveStep1Content(){return`
            ...
            </div>`}startCollectiveStep2(){const e=document.getElementById("collectiveIntentionContent");e&&(e.innerHTML=`
            <div class="lunar-popup-section" style="text-align:center;">
            ...
            </div>`,e.querySelector('[data-action="start-meditation"]')?.addEventListener("click",this.startMeditationTimer),e.querySelector('[data-action="skip-meditation"]')?.addEventListener("click",this.startCollectiveStep3))}startMeditationTimer(){document.getElementById("startMeditationBtn")?.style.setProperty("display","none"),this._runTimer("meditationTimer",d.CONSTANTS.MEDITATION_DURATION,()=>{const e=document.getElementById("skipToIntentionBtn");e&&(e.textContent="Continue to Intention",e.style.display="block")}),setTimeout(()=>{const e=document.getElementById("skipToIntentionBtn");e&&(e.style.display="block")},1e4)}startCollectiveStep3(){this._clearTimer();const e=document.getElementById("collectiveIntentionContent");e&&(e.innerHTML=`
            <div class="lunar-popup-section">
                ...
                >${c._escapeHtml(this.userWeekData.privateIntention||"")}</textarea>
                <button class="lunar-popup-btn" data-action="save-private-intention">Continue</button>
            </div>`,e.querySelector('[data-action="save-private-intention"]')?.addEventListener("click",this.startCollectiveStep4))}startCollectiveStep4(){const e=document.getElementById("privateIntentionText");e&&(this.userWeekData.privateIntention=this._sanitizeInput(e.value));const t=document.getElementById("collectiveIntentionContent");t&&(t.innerHTML=`
            <div class="lunar-popup-section">
                ...
                <div class="lunar-intention-preview">
                    ...
                    <p>${c._escapeHtml(this.userWeekData.privateIntention||"No intention set")}</p>
                </div>
            ...
            </div>`,t.querySelector('[data-action="submit-collective-word"]')?.addEventListener("click",()=>{const o=document.getElementById("collectiveWordInput");o&&this.submitWordToCollective(o.value)}))}submitWordToCollective(e){const t=this._sanitizeInput(e),o=this._validateCollectiveWord(t);if(!o.valid){window.Core?.showToast(o.error);return}this.userWeekData.collectiveWord=t,this.userWeekData.intentionShared=!0,this.collectiveWords||(this.collectiveWords=this.getMockCollectiveWords()),this.collectiveWords.push({word:t,timestamp:Date.now()}),p()?.ready&&p()?.sendRoomMessage(`${this._getLunarRoomId()}-collective`,t).catch(r=>console.error("[LunarRoom] submitWordToCollective DB error:",r)),this.startCollectiveStep5()}startCollectiveStep5(){const e=document.getElementById("collectiveIntentionContent");e&&(e.innerHTML=`
            <div class="lunar-popup-section" style="text-align:center;">
                ...
                <div id="wordCloud" class="lunar-word-cloud">
                    ${u.renderWordCloud(this.collectiveWords??this.getMockCollectiveWords())}
                </div>
