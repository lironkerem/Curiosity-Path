...
import{_ as m}from"./communityHub-DUqfsECp.js";import{c as v}from"./modal-1yjDmyAf.js";import{a as p}from"./controller-3q4LAI6z.js";function k(r){const e=window.archetypesEngine,t=e.setActiveShadow(r);if(!t)return p("Shadow journey not found","error");const{modal:n}=v({id:"subShadowJourneyModal",title:`${t.icon} ${t.title}`,subtitle:`${t.tagline}<br><span style="font-size:0.85rem;display:inline-flex;align-items:center;gap:0.25rem;flex-wrap:wrap;word-break:break-word;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon" style="width:0.85rem;height:0.85rem;vertical-align:middle;"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l2 2"/></svg> ${t.estimatedTime}</span>`,content:'<div id="sub-shadow-journey-content" style="overflow-y:auto;max-height:calc(90vh - 200px)"></div>',actions:"",onClose:()=>{e.saveUserState(),window.AppController.renderDashboard()}}),a=n.querySelector(".modal-card");a.style.maxWidth="700px",a.style.maxHeight="90vh",d(n)}async function d(r){var c,l;const e=window.archetypesEngine,t=r.querySelector("#sub-shadow-journey-content"),n=e.state.selectedStepIndex,a=e.getActiveJourney();if(!a)return;const s=a.steps.length;if(n>=s)return b(r);const o=e.getStep(n),g=Math.round(n/s*100),u=e.state.answers[o.id]||"";t.innerHTML=`
    ...
    </div>`,s.querySelector("#complete-journey").addEventListener("click",()=>{window.AppController.addLightParticles(a),m(()=>import("./controller-3q4LAI6z.js").then(o=>o.u),__vite__mapDeps([0,1])).then(o=>o.showToast(`${t.title} complete! +${a} Light-Particles earned.`)),e.completeJourney(),e.clearUserState(),r.remove(),window.AppController.renderDashboard()})}export{k as openSubShadowJourneyModal,d as renderStep};
