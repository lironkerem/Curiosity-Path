...
var St=Object.defineProperty;var Mt=(t,e,o)=>e in t?St(t,e,{enumerable:!0,configurable:!0,writable:!0,value:o}):t[e]=o;var Xe=(t,e,o)=>Mt(t,typeof e!="symbol"?e+"":e,o);import{_ as be}from"./shadow-alchemy-YpVrgYlJ.js";const ht=window.AppSupabase||null;window.CommunitySupabase=ht;const u={_sb:null,_uid:null,_subs:{},_heartbeatTimer:null,async init(){if(this._sb=window.AppSupabase||ht,!this._sb)return console.error("[CommunityDB] CommunitySupabase not ready — window.AppSupabase is null"),!1;const{data:{user:t},error:e}=await this._sb.auth.getUser();return e||!t?(console.error("[CommunityDB] No authenticated user:",e==null?void 0:e.message),!1):(this._uid=t.id,!0)},get userId(){return this._uid},get ready(){return!!(this._sb&&this._uid)},_err(t,e){console.error(`[CommunityDB] ${t}:`,(e==null?void 0:e.message)??e)},_profileSelect:"id, name, emoji, avatar_url",_ago(t){return new Date(Date.now()-t).toISOString()},_todayUTC(){const t=new Date;return t.setUTCHours(0,0,0,0),t.toISOString()},async getMyProfile(){if(!this.ready)return null;const{data:t,error:e}=await this._sb.from("profiles").select("*").eq("id",this._uid).single();return e?(this._err("getMyProfile",e),null):t},async getProfile(t){if(!this.ready)return null;const{data:e,error:o}=await this._sb.from("profiles").select("id, name, emoji, avatar_url, inspiration, community_status, community_role, total_sessions, total_minutes, gifts_given, birthday, country").eq("id",t).single();return o?(this._err("getProfile",o),null):e},_parseProgress(t){var o,i,n,r,a,s;const e=typeof t=="string"?JSON.parse(t):t;return{xp:e.xp??0,karma:e.karma??0,level:e.level??1,badges:e.badges??[],unlockedFeatures:e.unlockedFeatures??[],streak:((o=e.streaks)==null?void 0:o.current)??((i=e.streak)==null?void 0:i.current)??0,longestStreak:((n=e.streaks)==null?void 0:n.longest)??0,totalSessions:((r=e.stats)==null?void 0:r.totalSessions)??0,totalMeditations:((a=e.stats)==null?void 0:a.totalMeditations)??0,totalReadings:((s=e.stats)==null?void 0:s.totalReadings)??0,totalTarotSpreads:e.totalTarotSpreads??0,totalJournalEntries:e.totalJournalEntries??0,totalWellnessRuns:e.totalWellnessRuns??0,totalHappinessViews:e.totalHappinessViews??0}},async getUserProgress(t){if(!this.ready)return null;try{const{data:e,error:o}=await this._sb.from("user_progress").select("payload").eq("user_id",t).single();return o||!e?null:this._parseProgress(e.payload)}catch(e){return this._err("getUserProgress",e),null}},getOwnGamificationState(){var e;const t=(e=window.app)==null?void 0:e.gamification;return t?this._parseProgress(t.state??t):null},async uploadAvatar(t){if(!this.ready)return null;try{const e=t.name.split(".").pop().toLowerCase()||"jpg",o=`avatars/${this._uid}.${e}`,{error:i}=await this._sb.storage.from("community-avatars").upload(o,t,{upsert:!0,contentType:t.type});if(i)return this._err("uploadAvatar upload",i),null;const{data:n}=this._sb.storage.from("community-avatars").getPublicUrl(o),r=n==null?void 0:n.publicUrl;if(!r)return null;const a=`${r}?t=${Date.now()}`;return await this.updateProfile({avatar_url:a})?a:null}catch(e){return this._err("uploadAvatar",e),null}},async updateProfile(t){if(!this.ready)return!1;const{error:e}=await this._sb.from("profiles").update({...t,updated_at:new Date().toISOString()}).eq("id",this._uid);return e?(this._err("updateProfile",e),!1):!0},async setPresence(t="online",e="✨ Available",o=null){if(!this.ready)return!1;const i=new Date().toISOString(),{error:n}=await this._sb.from("community_presence").upsert({user_id:this._uid,status:t,activity:e,room_id:o,last_seen:i,updated_at:i},{onConflict:"user_id"});return n&&this._err("setPresence",n),!n},async setOffline(){return this.setPresence("offline","💤 Offline",null)},async getActiveMembers(){if(!this.ready)return[];const{data:t,error:e}=await this._sb.from("community_presence").select(`user_id, status, activity, room_id, is_phantom, last_seen, profiles ( ${this._profileSelect} )`).neq("status","offline").gte("last_seen",this._ago(5*6e4)).order("is_phantom",{ascending:!1}).order("last_seen",{ascending:!1});return e?(this._err("getActiveMembers",e),[]):t||[]},async getRoomParticipants(t){if(!this.ready)return[];const{data:e,error:o}=await this._sb.from("community_presence").select(`user_id, status, activity, room_id, is_phantom, last_seen, profiles ( ${this._profileSelect} )`).or(`room_id.eq.${t},is_phantom.eq.true`).neq("status","offline").gte("last_seen",this._ago(5*6e4)).order("is_phantom",{ascending:!1}).order("last_seen",{ascending:!0});return o?(this._err("getRoomParticipants",o),[]):e||[]},subscribeToPresence(t){return this._subs.presence&&this._subs.presence.unsubscribe(),this._subs.presence=this._sb.channel("community-presence").on("postgres_changes",{event:"*",schema:"public",table:"community_presence"},async()=>t(await this.getActiveMembers())).subscribe(),this._subs.presence},startHeartbeat(t=6e4){this.stopHeartbeat(),this._heartbeatTimer=setInterval(async()=>{var o,i,n;if(!this.ready)return;const e=(o=typeof Core<"u"?Core:window.Core)==null?void 0:o.state;await this.setPresence(((i=e==null?void 0:e.currentUser)==null?void 0:i.status)||"online",((n=e==null?void 0:e.currentUser)==null?void 0:n.activity)||"✨ Available",(e==null?void 0:e.currentRoom)||null)},t),window.addEventListener("beforeunload",()=>this._cleanup())},stopHeartbeat(){this._heartbeatTimer&&(clearInterval(this._heartbeatTimer),this._heartbeatTimer=null)},_reflectionSelect:"id, content, appreciation_count, created_at, profiles ( id, name, emoji, avatar_url )",async getReflections(t=30){if(!this.ready)return[];const{data:e,error:o}=await this._sb.from("reflections").select(this._reflectionSelect).order("created_at",{ascending:!1}).limit(t);return o?(this._err("getReflections",o),[]):e||[]},async postReflection(t){if(!this.ready)return null;const{data:e,error:o}=await this._sb.from("reflections").insert({user_id:this._uid,content:t}).select(this._reflectionSelect).single();return o?(this._err("postReflection",o),null):e},async deleteReflection(t){if(!this.ready)return!1;const{error:e}=await this._sb.from("reflections").delete().eq("id",t);return e?(this._err("deleteReflection",e),!1):!0},async updateReflection(t,e){if(!this.ready)return!1;const{error:o}=await this._sb.from("reflections").update({content:e}).eq("id",t).eq("user_id",this._uid);return o?(this._err("updateReflection",o),!1):!0},subscribeToReflections(t){return this._subs.reflections&&this._subs.reflections.unsubscribe(),this._subs.reflections=this._sb.channel("community-reflections").on("postgres_changes",{event:"INSERT",schema:"public",table:"reflections"},async({new:e})=>{const{data:o}=await this._sb.from("reflections").select(this._reflectionSelect).eq("id",e.id).single();o&&t(o)}).subscribe(),this._subs.reflections},async getMyAppreciations(){if(!this.ready)return new Set;const{data:t,error:e}=await this._sb.from("appreciations").select("reflection_id").eq("user_id",this._uid);return e?(this._err("getMyAppreciations",e),new Set):new Set(t.map(o=>o.reflection_id))},async toggleAppreciation(t,e){if(!this.ready)return null;if(e){const{error:o}=await this._sb.from("appreciations").delete().eq("user_id",this._uid).eq("reflection_id",t);return o?(this._err("removeAppreciation",o),null):{appreciated:!1}}else{const{error:o}=await this._sb.from("appreciations").insert({user_id:this._uid,reflection_id:t});return o?(this._err("addAppreciation",o),null):{appreciated:!0}}},async getReflectionCount(t){if(!this.ready)return null;const{data:e,error:o}=await this._sb.from("reflections").select("appreciation_count").eq("id",t).single();return o?(this._err("getReflectionCount",o),null):(e==null?void 0:e.appreciation_count)??null},async getMyUserAppreciations(){if(!this.ready)return new Set;const{data:t,error:e}=await this._sb.from("user_appreciations").select("appreciated_user_id").eq("user_id",this._uid);return e?(this._err("getMyUserAppreciations",e),new Set):new Set(t.map(o=>o.appreciated_user_id))},async toggleUserAppreciation(t,e){if(!this.ready)return null;if(e){const{error:o}=await this._sb.from("user_appreciations").delete().eq("user_id",this._uid).eq("appreciated_user_id",t);return o?(this._err("removeUserAppreciation",o),null):{appreciated:!1}}else{const{error:o}=await this._sb.from("user_appreciations").insert({user_id:this._uid,appreciated_user_id:t});return o?(this._err("addUserAppreciation",o),null):{appreciated:!0}}},async getUserAppreciationCount(t){if(!this.ready)return 0;const{count:e,error:o}=await this._sb.from("user_appreciations").select("*",{count:"exact",head:!0}).eq("appreciated_user_id",t);return o?(this._err("getUserAppreciationCount",o),0):e||0},_roomMsgSelect:"id, message, created_at, profiles ( id, name, emoji, avatar_url )",async getRoomMessages(t,e=50){if(!this.ready)return[];const{data:o,error:i}=await this._sb.from("room_messages").select(this._roomMsgSelect).eq("room_id",t).order("created_at",{ascending:!0}).limit(e);return i?(this._err("getRoomMessages",i),[]):o||[]},async sendRoomMessage(t,e){if(!this.ready)return null;const{data:o,error:i}=await this._sb.from("room_messages").insert({user_id:this._uid,room_id:t,message:e}).select(this._roomMsgSelect).single();return i?(this._err("sendRoomMessage",i),null):o},subscribeToRoomChat(t,e){const o=`room-${t}`;return this._subs[o]&&this._subs[o].unsubscribe(),this._subs[o]=this._sb.channel(`room-chat-${t}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"room_messages",filter:`room_id=eq.${t}`},async({new:i})=>{const{data:n}=await this._sb.from("room_messages").select(this._roomMsgSelect).eq("id",i.id).single();n&&e(n)}).subscribe(),this._subs[o]},unsubscribeFromRoomChat(t){this._unsub(`room-${t}`)},async sendWhisper(t,e){if(!this.ready)return null;const{data:o,error:i}=await this._sb.from("whispers").insert({sender_id:this._uid,recipient_id:t,message:e}).select().single();return i?(this._err("sendWhisper",i),null):o},async getWhispers(t){if(!this.ready)return[];const{data:e,error:o}=await this._sb.from("whispers").select(`
        ...
        `)},formatTime(t){if(typeof t!="number"||t<0)return"0:00";const e=Math.floor(t/60),o=Math.floor(t%60);return`${e}:${o.toString().padStart(2,"0")}`},getAvatarGradient(t){(!t||typeof t!="string")&&(t="default");const e=t.split("").reduce((o,i)=>o+i.charCodeAt(0),0);return this.config.AVATAR_GRADIENTS[Math.abs(e)%this.config.AVATAR_GRADIENTS.length]}};window.Core=x;const xo=Object.freeze(Object.defineProperty({__proto__:null,Core:x},Symbol.toStringTag,{value:"Module"})),At=["🏅","🎖️","🌟","👑","🧪","🕉️","🦸","🌱","🎪","🌙","☀️","⚡","🌊","💜","🔱","🔥","💎","🦋","🌸","🍀","🌈","⭐","🎯","🏆","🎗️","🌀","🔮","💫","🧘","🦅","🐉","🌺","🎵","💡","🌿","🦁","🐬","🌍","🎭","🛡️","⚔️","🗝️","🧬","🌠","🎋"],Rt={state:{isOpen:!1,currentUserId:null,isAppreciated:!1,appreciationCount:0},_LEVEL_TITLES:{1:"Seeker",2:"Practitioner",3:"Adept",4:"Healer",5:"Master",6:"Sage",7:"Enlightened",8:"Buddha",9:"Light",10:"Emptiness"},_STATUS_RINGS:{online:{c:"var(--ring-available,#6b9b37)",s:"rgba(107,155,55,0.2)"},available:{c:"var(--ring-available,#6b9b37)",s:"rgba(107,155,55,0.2)"},away:{c:"var(--ring-guiding,#e53e3e)",s:"rgba(229,62,62,0.2)"},silent:{c:"var(--ring-silent,#7c3aed)",s:"rgba(124,58,237,0.2)"},deep:{c:"var(--ring-deep,#1e40af)",s:"rgba(30,64,175,0.2)"},offline:{c:"var(--ring-offline,#9ca3af)",s:"rgba(156,163,175,0.2)"}},_RARITY_COLORS:{common:"#9ca3af",uncommon:"#10b981",rare:"#3b82f6",epic:"#a855f7",legendary:"#f59e0b"},_RARITY_LABELS:{common:"Common",uncommon:"Uncommon",rare:"Rare",epic:"Epic",legendary:"Legendary"},_COUNTRY_CODES:{israel:"IL","united states":"US",usa:"US",us:"US","united kingdom":"GB",uk:"GB",canada:"CA",australia:"AU",germany:"DE",france:"FR",spain:"ES",italy:"IT",netherlands:"NL",belgium:"BE",switzerland:"CH",sweden:"SE",norway:"NO",denmark:"DK",finland:"FI",poland:"PL",portugal:"PT",austria:"AT",india:"IN",china:"CN",japan:"JP","south korea":"KR",brazil:"BR",mexico:"MX",argentina:"AR","south africa":"ZA",russia:"RU",ukraine:"UA",greece:"GR",turkey:"TR",egypt:"EG","new zealand":"NZ",ireland:"IE",singapore:"SG",thailand:"TH",indonesia:"ID",malaysia:"MY",philippines:"PH"},_ADMIN_SUB_IDS:["adminSubRole","adminSubXp","adminSubKarma","adminSubBadge","adminSubPremium","adminSubMessage","adminSubCustomBadge"],_buildEmojiPicker(t,e){return`
            <div id="${e}" style="display:none;margin-top:6px;padding:8px;border-radius:10px;
                ...
                ${At.map(o=>`<button type="button" onclick="MemberProfileModal._pickEmoji('${t}','${e}','${o}')"
    ...
    <div class="modal-overlay" id="guidelinesModal" onclick="SafetyBar.handleOverlayClick(event,'guidelines')">
        ...
        <div style="margin-bottom:12px;">
                ...
                <div class="admin-notif-row ${i.read?"":"unread"}" id="adminNotif_${i.id}">
                    ...
                    <div style="flex:1;min-width:0;">
                        ...
                        <div style="font-size:0.82rem;color:var(--text-muted,#888);">
                            From: <strong>${((n=i.payload)==null?void 0:n.sender_name)||"Unknown"}</strong>
                            ${(r=i.payload)!=null&&r.room?`· ${i.payload.room}`:""}
                        </div>
                        ${(a=i.payload)!=null&&a.message?`<div style="font-size:0.82rem;margin-top:4px;font-style:italic;">"${this._esc(i.payload.message)}"</div>`:""}
                        ${(s=i.payload)!=null&&s.reason?`<div style="font-size:0.82rem;margin-top:4px;">Reason: ${this._esc(i.payload.reason)}</div>`:""}
                        ${(l=i.payload)!=null&&l.details?`<div style="font-size:0.82rem;color:var(--text-muted,#888);">${this._esc(i.payload.details)}</div>`:""}
                        ${(d=i.payload)!=null&&d.issueType?`<div style="font-size:0.82rem;margin-top:4px;">Type: ${this._esc(i.payload.issueType)}</div>`:""}
                        ${(c=i.payload)!=null&&c.description?`<div style="font-size:0.82rem;color:var(--text-muted,#888);">${this._esc(i.payload.description)}</div>`:""}
                    </div>
                    ${i.read?"":`
            ...
            <div style="margin-bottom:16px;">
                ...
                <div id="bulkPanel_badge" style="display:none;">
                    ...
                    <select id="bulkBadgeSelect" style="${i}">
                        ${o}
                    </select>
                ...
                <div id="bulkPanel_customBadge" style="display:none;">
                    ...
                    <select id="bulkCustomRarity" style="${i}">
                        <option value="common">Common</option>
                        <option value="uncommon">Uncommon</option>
                        <option value="rare">Rare</option>
                        <option value="epic" selected>Epic</option>
                ...
                <div id="bulkPanel_unlock" style="display:none;">
                    ...
                    <select id="bulkUnlockSelect" style="${i}">
            ...
            ${a}`}_showBlessingAnimation(e){if(document.getElementById("blessingAnimationOverlay"))return;const o=3e3,i=window.innerWidth<768,n=window.innerWidth,r=window.innerHeight,a=n/2,s=r/2,l=document.createElement("div");l.id="blessingAnimationOverlay",l.style.cssText=["position:fixed;inset:0;pointer-events:none;z-index:999999;","backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);","background:rgba(0,0,0,0);","transition:background 0.2s ease;"].join(""),document.body.appendChild(l),requestAnimationFrame(()=>{l.style.background="rgba(0,0,0,0.25)"});const d=document.createElement("canvas");d.width=n,d.height=r,d.style.cssText="position:absolute;inset:0;width:100%;height:100%;background:transparent;",l.appendChild(d);const c=d.getContext("2d"),p=document.createElement("div");p.style.cssText=["position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) scale(0.6);","color:rgba(255,230,120,0.95);font-size:clamp(36px,8vw,72px);font-weight:300;","letter-spacing:0.25em;text-transform:uppercase;white-space:nowrap;","font-family:var(--serif,Georgia,serif);","text-shadow:0 0 40px rgba(255,200,60,0.8),0 0 80px rgba(255,180,40,0.4);","opacity:0;transition:opacity 0.35s ease,transform 0.5s cubic-bezier(0.34,1.56,0.64,1);","pointer-events:none;"].join(""),p.textContent="✦  Blessed  ✦",l.appendChild(p),setTimeout(()=>{p.style.opacity="1",p.style.transform="translate(-50%,-50%) scale(1)"},o*.42),setTimeout(()=>{p.style.opacity="0",p.style.transform="translate(-50%,-50%) scale(1.15)",p.style.transition="opacity 0.4s ease,transform 0.4s ease"},o-600);const h=(e==null?void 0:e.name)??null;if(h){const S=document.createElement("div");S.style.cssText=["position:absolute;bottom:18%;left:50%;transform:translateX(-50%);","color:rgba(255,230,160,0.9);font-size:15px;font-weight:400;","letter-spacing:0.18em;text-transform:uppercase;white-space:nowrap;","font-family:var(--serif,Georgia,serif);","opacity:0;transition:opacity 0.6s ease 0.6s;"].join(""),S.textContent=`✦  ${h} blesses this room  ✦`,l.appendChild(S),requestAnimationFrame(()=>{S.style.opacity="1"}),setTimeout(()=>{S.style.opacity="0",S.style.transition="opacity 0.4s ease"},o-500)}const m=i?120:260,g=[];for(let S=0;S<m;S++){const _=Math.random()*Math.PI*2,k=20+Math.random()*Math.max(n,r)*.55,B=Math.random()<.75,O=B?38+Math.random()*18:265+Math.random()*25,E=B?85+Math.random()*15:70+Math.random()*20,z=60+Math.random()*25;g.push({angle:_,radius:k,x:a+Math.cos(_)*k,y:s+Math.sin(_)*k,size:1+Math.random()*(i?2:3),hue:O,sat:E,lit:z,delay:Math.random()*.3,speed:.8+Math.random()})}const f=performance.now();let v;const M=S=>{const _=(S-f)/o,k=.42,B=.48,O=1;if(c.globalCompositeOperation="destination-out",c.globalAlpha=.18,c.fillStyle="rgba(0,0,0,1)",c.fillRect(0,0,n,r),c.globalCompositeOperation="source-over",c.globalAlpha=1,_>k*.6&&_<O*.85){const E=_<B?(_-k*.6)/(B-k*.6):1-(_-B)/(O-B),z=Math.max(0,Math.min(1,E)),q=80+E*120,L=c.createRadialGradient(a,s,0,a,s,q);L.addColorStop(0,`rgba(255,220,100,${.55*z})`),L.addColorStop(.5,`rgba(200,150, 60,${.25*z})`),L.addColorStop(1,"rgba(200,150, 60,0)"),c.globalAlpha=1,c.fillStyle=L,c.beginPath(),c.arc(a,s,q,0,Math.PI*2),c.fill()}g.forEach(E=>{const z=Math.max(0,_-E.delay*.3);let q,L,J;if(z<k){const F=z/k,Z=1-(1-F)*(1-F),$=E.radius*(1-Z*.97),I=Z*Math.PI*2.5*(E.radius>0?1:-1);q=a+Math.cos(E.angle+I)*$,L=s+Math.sin(E.angle+I)*$,J=Math.min(1,Z*2.5)}else if(z<B)q=a+(Math.random()-.5)*8,L=s+(Math.random()-.5)*8,J=1;else{const F=(z-B)/(O-B),Z=F*F,$=E.angle+Math.PI*(.7+Math.random()*.6),I=Z*Math.max(n,r)*.7*E.speed;q=a+Math.cos($)*I,L=s+Math.sin($)*I,J=Math.max(0,1-F*1.15)}J<=0||(c.globalAlpha=J,c.fillStyle=`hsl(${E.hue},${E.sat}%,${E.lit}%)`,c.shadowBlur=i?4:8,c.shadowColor=`hsl(${E.hue},100%,70%)`,c.beginPath(),c.arc(q,L,E.size,0,Math.PI*2),c.fill())}),c.globalAlpha=1,c.shadowBlur=0,_<1?v=requestAnimationFrame(M):(l.style.transition="opacity 0.3s ease",l.style.opacity="0",setTimeout(()=>l.remove(),350),cancelAnimationFrame(v))};v=requestAnimationFrame(M)}createPracticeView(){var i;const e=[this.buildHeader(),this.buildBody(),this.buildInstructionsModal(),((i=this.buildAdditionalModals)==null?void 0:i.call(this))??""].join("");G.injectModals();const o=document.getElementById("dynamicRoomContent");if(o)o.innerHTML=e;else{if(document.getElementById(`${this.roomId}View`))return;document.body.insertAdjacentHTML("beforeend",`<div class="view practice-space" id="${this.roomId}View">${e}</div>`)}}buildHeader(){var e;return`
        ...
        <div class="practice-room ${i?"active":"in-session"}"
            ...
            ${!i&&o?`<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:2;pointer-events:none;">${et}</div>`:""}
        ...
        </button>`},cleanupCycle(){this._cycleInterval&&(clearInterval(this._cycleInterval),this._cycleInterval=null)}},Te=t=>t.charAt(0).toUpperCase()+t.slice(1);function Fe(t,e,o,i){var l,d;const n=o||t.charAt(0).toUpperCase(),r=((d=(l=window.Core)==null?void 0:l.getAvatarGradient)==null?void 0:d.call(l,i||t))??"linear-gradient(135deg, #667eea 0%, #764ba2 100%)",a=e?`<img src="${e}" width="36" height="36" style="width:36px;height:36px;object-fit:cover;border-radius:50%;display:block;flex-shrink:0;" alt="${t}" loading="lazy" decoding="async">`:`<span style="color:white;font-size:13px;font-weight:600;line-height:1;">${n}</span>`,s=e?"background:transparent;":`background:${r};`;return{inner:a,bg:s,gradient:r,initial:n}}const tt=t=>new Date(t).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),De={initChatState(t=["main"]){this.chatChannels=t,this.state.messages=Object.fromEntries(t.map(e=>[e,[]]))},async initializeChat(){await Promise.all(this.chatChannels.map(t=>this.loadRoomChatFromDB(t))),setTimeout(()=>this._injectSenderAvatar(),200)},_getDbRoomId(t="main"){return t==="main"?this.roomId:`${this.roomId}-${t}`},_msgContainerId(t){return`${this.roomId}${Te(t)}Messages`},_inputId(t){return`${this.roomId}${Te(t)}Input`},_rowToMsgData(t,e=!1){var c,p;const o=t.profiles||{},i=((p=(c=window.Core)==null?void 0:c.state)==null?void 0:p.currentUser)||{},n=e?i.name||"You":o.name||"Member",r=e?i.avatar_url||"":o.avatar_url||"",a=e?i.emoji||"":o.emoji||"",s=o.id||null,{gradient:l,initial:d}=Fe(n,r,a,s);return{name:n,initial:d,avatarUrl:r,avatarBg:l,userId:s,time:tt(t.created_at),text:t.message,country:null}},async sendMessage(t="main",e=null){var c,p,h;const o=document.getElementById(e||this._inputId(t));if(!(o!=null&&o.value.trim()))return;const i=o.value.trim(),n=document.getElementById(this._msgContainerId(t)),r=((p=(c=window.Core)==null?void 0:c.state)==null?void 0:p.currentUser)||{},a=r.name||"You",{gradient:s,initial:l}=Fe(a,r.avatar_url||"",r.emoji||"",r.id),d={name:a,initial:l,avatarUrl:r.avatar_url||"",avatarBg:s,userId:r.id||null,time:tt(new Date),text:i,country:null,timestamp:Date.now(),...(h=this.getCustomMessageData)==null?void 0:h.call(this,t)};if(this.state.messages[t].push(d),n&&(n.insertAdjacentHTML("beforeend",this.buildMessageHTML(d)),n.scrollTop=n.scrollHeight),o.value="",window.Core.showToast("Message sent"),u!=null&&u.ready)try{await u.sendRoomMessage(this._getDbRoomId(t),i)}catch(m){console.error("[ChatMixin] sendRoomMessage error:",m)}},async loadRoomChatFromDB(t="main"){var n,r,a;if(!(u!=null&&u.ready)){this.loadMessagesFromStorage(t),this.renderSavedMessages(t);return}const e=this._getDbRoomId(t),o=document.getElementById(this._msgContainerId(t)),i=(a=(r=(n=window.Core)==null?void 0:n.state)==null?void 0:r.currentUser)==null?void 0:a.id;try{const[s,l]=await Promise.all([u.getRoomMessages(e,50),u.getBlockedUsers()]);o&&s.length&&(s.filter(d=>{var c;return!l.has((c=d.profiles)==null?void 0:c.id)}).forEach(d=>{var p;const c=((p=d.profiles)==null?void 0:p.id)===i;o.insertAdjacentHTML("beforeend",this.buildMessageHTML(this._rowToMsgData(d,c)))}),o.scrollTop=o.scrollHeight),u.subscribeToRoomChat(e,async d=>{var p,h;((p=d.profiles)==null?void 0:p.id)===i||(await u.getBlockedUsers()).has((h=d.profiles)==null?void 0:h.id)||!o||(o.insertAdjacentHTML("beforeend",this.buildMessageHTML(this._rowToMsgData(d))),o.scrollTop=o.scrollHeight)})}catch(s){console.error(`[ChatMixin] loadRoomChatFromDB error (${t}):`,s)}},buildMessageHTML(t){const e=t.avatarUrl?"":`<span style="color:white;font-size:13px;font-weight:600;line-height:1;">${t.initial}</span>`,o=t.avatarUrl?`background-image:url('${t.avatarUrl}');background-size:cover;background-position:center;`:`background:${t.avatarBg};`,i=t.userId?`<span class="campfire-msg-name" role="button" tabindex="0" style="cursor:pointer;" onclick="openMemberProfileAboveRoom('${t.userId}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openMemberProfileAboveRoom('${t.userId}');}" aria-label="View profile of ${t.name}">${t.name}</span>`:`<span class="campfire-msg-name">${t.name}</span>`;return`
        <div class="campfire-msg">
            <div class="campfire-msg-avatar" aria-hidden="true" style="${o}width:36px;height:36px;min-width:36px;overflow:hidden;display:flex;align-items:center;justify-content:center;flex-shrink:0;">${e}</div>
        ...
        </div>`},buildChatContainer(t="main",e="Type your message..."){const o=Te(t),i=`${this.roomId}${o}SenderAvatar`,n=`${this.roomId}${o}SendBtn`;return`
        ...
        </div>`},buildSoundSettings(){const t=this.roomId,e=Nt.map(n=>this._soundOption("Bell",n.value,n.label,"previewSound","selectBell")).join(""),o=Ot.map(n=>this._soundOption("Completion",n.value,n.label,"previewSound","selectCompletion")).join(""),i=Ut.map(n=>this._soundOption("Ambient",n.value,n.label,"previewAmbient","selectAmbient")).join("");return`
...
@media(prefers-reduced-motion:reduce){*{animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important;}}
        `}},Ge=()=>window.CommunityDB,xe={location:{latitude:31,longitude:0,name:"Default"},currentMoonData:null,currentLunarRoom:null,lunarRooms:[{phaseRanges:[[0,.068],[.932,1]],phaseName:"New Moon",icon:"🌑",roomName:"New Moon Intentions",description:"Set intentions and plant seeds for the lunar cycle",roomId:"new-moon",energy:"Beginning, Stillness, Potential",practices:["Silent meditation","Intention setting","Vision journaling","Seed planting ritual"]},{phaseRanges:[[.068,.432]],phaseName:"Waxing Moon",icon:"🌓",roomName:"Waxing Growth Practice",description:"Build momentum and cultivate expanding energy",roomId:"waxing-moon",energy:"Growth, Action, Building",practices:["Dynamic movement","Energy cultivation","Goal visualization","Action planning meditation"]},{phaseRanges:[[.432,.568]],phaseName:"Full Moon",icon:"🌕",roomName:"Full Moon Illumination",description:"Celebrate fullness and release what no longer serves",roomId:"full-moon",energy:"Culmination, Release, Clarity",practices:["Celebration ritual","Gratitude meditation","Release ceremony","Moon bathing"]},{phaseRanges:[[.568,.932]],phaseName:"Waning Moon",icon:"🌗",roomName:"Waning Release Practice",description:"Let go and reflect on the lunar journey",roomId:"waning-moon",energy:"Release, Reflection, Rest",practices:["Reflection meditation","Letting go ritual","Rest and restore","Completion ceremony"]}],init(){if(this._initialized)return;this._initialized=!0;const t=()=>this.updateAll();navigator.geolocation?navigator.geolocation.getCurrentPosition(({coords:e})=>{this.location={latitude:e.latitude,longitude:e.longitude,name:"Your Location"},t()},()=>t(),{timeout:5e3,maximumAge:36e5}):t(),setInterval(()=>this.updateAll(),6e5)},updateAll(){this.updateMoonData(),this.updateMoonVisualization(),this.updateLunarRoom(),this.renderLunarCard()},setLocation(t,e,o){this.location={latitude:t,longitude:e,name:o},this.updateAll()},updateMoonData(){const t=new Date,e=Ae.getMoonIllumination(t),o=Ae.getMoonTimes(t,this.location.latitude,this.location.longitude);this.currentMoonData={phase:e.phase,fraction:e.fraction,angle:e.angle,rise:o.rise,set:o.set,phaseName:this.getMoonPhaseName(e.phase),age:e.phase*29.53,nextFullMoon:this.getNextFullMoon(t)},this._renderMoonInfo()},_renderMoonInfo(){const t=this.currentMoonData;if(!t)return;const e=(i,n)=>{const r=document.getElementById(i);r&&(r.textContent=n)};e("moonPhaseName",t.phaseName),e("moonIllumination",`${Math.round(t.fraction*100)}% illuminated`),e("moonAge",`${t.age.toFixed(1)} days old`),e("moonrise",t.rise?this.formatTime(t.rise):"No rise today"),e("moonset",t.set?this.formatTime(t.set):"No set today");const o=Math.ceil((t.nextFullMoon-new Date)/864e5);e("nextPhase",`Next Full Moon: ${this.formatDate(t.nextFullMoon)} (${o} days)`)},updateMoonVisualization(){this.currentMoonData&&this.drawMoon(this.currentMoonData.phase,this.currentMoonData.angle)},drawMoon(t,e){const o=document.getElementById("moonSvg");if(!o)return;const i=50,n=60,r=60;o.innerHTML="";const a=document.createElementNS("http://www.w3.org/2000/svg","g");a.setAttribute("transform",`rotate(${(e*180/Math.PI*-1).toFixed(2)}, ${n}, ${r})`),o.appendChild(a);const s=document.createElementNS("http://www.w3.org/2000/svg","circle");s.setAttribute("cx",n),s.setAttribute("cy",r),s.setAttribute("r",i),s.setAttribute("fill","#2d3748"),a.appendChild(s);const l=Math.cos(t*2*Math.PI)*i,d=document.createElementNS("http://www.w3.org/2000/svg","path");d.setAttribute("fill","#f7fafc"),d.setAttribute("d",t<.5?`M ${n},${r-i} A ${i},${i} 0 0 1 ${n},${r+i} Q ${n+l},${r} ${n},${r-i}`:`M ${n},${r-i} Q ${n+l},${r} ${n},${r+i} A ${i},${i} 0 0 1 ${n},${r-i}`),a.appendChild(d),[{x:.3,y:-.2,r:.15},{x:-.2,y:.3,r:.1},{x:.1,y:.4,r:.08}].forEach(({x:c,y:p,r:h})=>{if(!(t<.5&&c>0||t>=.5&&c<0||t>.45&&t<.55))return;const g=document.createElementNS("http://www.w3.org/2000/svg","circle");g.setAttribute("cx",n+c*i),g.setAttribute("cy",r+p*i),g.setAttribute("r",h*i),g.setAttribute("fill","rgba(0,0,0,0.15)"),a.appendChild(g)})},getMoonPhaseName(t){return t<.03||t>.97?"New Moon":t<.22?"Waxing Crescent":t<.28?"First Quarter":t<.47?"Waxing Gibbous":t<.53?"Full Moon":t<.72?"Waning Gibbous":t<.78?"Last Quarter":"Waning Crescent"},getNextFullMoon(t){const e=Ae.getMoonIllumination(t).phase,o=e<.5?(.5-e)*29.53:(1.5-e)*29.53;return new Date(t.getTime()+o*864e5)},getLunarRoomByPhase(t){return this.lunarRooms.find(e=>e.phaseRanges.some(([o,i])=>t>=o&&t<=i))??this.lunarRooms[0]},updateLunarRoom(){var e;if(!this.currentMoonData)return;const t=this.getLunarRoomByPhase(this.currentMoonData.phase);(e=this.currentLunarRoom)==null||e.roomId,t.roomId,this.currentLunarRoom=t},getCurrentRoom(){return this.currentLunarRoom},_ROOM_MODULES:{"new-moon":()=>be(()=>Promise.resolve().then(()=>ao),void 0),"waxing-moon":()=>be(()=>Promise.resolve().then(()=>lo),void 0),"full-moon":()=>be(()=>Promise.resolve().then(()=>uo),void 0),"waning-moon":()=>be(()=>Promise.resolve().then(()=>mo),void 0)},_roomExportName:{"new-moon":"NewMoonRoom","waxing-moon":"WaxingMoonRoom","full-moon":"FullMoonRoom","waning-moon":"WaningMoonRoom"},async _loadAndEnterRoom(t){var o,i,n;const e=this._ROOM_MODULES[t];if(!e){(o=window.Core)==null||o.showToast(`Unknown room: ${t}`);return}try{const a=(await e())[this._roomExportName[t]];a?a.enterRoom():(i=window.Core)==null||i.showToast(`${t} failed to initialise`)}catch(r){console.error(`[LunarEngine] Failed to load ${t}:`,r),(n=window.Core)==null||n.showToast(`Failed to load ${t}`)}},joinLunarRoom(){var t;if(!this.currentLunarRoom){(t=window.Core)==null||t.showToast("Lunar room not ready");return}this._loadAndEnterRoom(this.currentLunarRoom.roomId)},adminJoinRoom(t){this._loadAndEnterRoom(t)},renderLunarCard(){var n,r,a;const t=document.getElementById("lunarContainer");if(!t){console.warn("LunarEngine: #lunarContainer not found");return}if(V.injectStyles(),!this.currentMoonData||!this.currentLunarRoom){t.innerHTML='<p style="color:var(--text-muted);padding:20px;">Loading lunar data...</p>';return}const{currentMoonData:e,currentLunarRoom:o}=this,i=(a=(r=(n=window.Core)==null?void 0:n.state)==null?void 0:r.currentUser)==null?void 0:a.is_admin;t.innerHTML=`
            ...
            </div>`},_attachAdminListeners(){const t=document.querySelector(".lunar-admin-header");if(t&&!t._lunarBound){const e=()=>{const o=document.getElementById(t.dataset.panel),i=document.getElementById(t.dataset.toggle),n=o.style.display!=="none";o.style.display=n?"none":"block",i.textContent=n?"▶":"▼",t.setAttribute("aria-expanded",String(!n))};t.addEventListener("click",e),t.addEventListener("keydown",o=>{(o.key==="Enter"||o.key===" ")&&(o.preventDefault(),e())}),t._lunarBound=!0}document.querySelectorAll(".lunar-admin-room-btn").forEach(e=>{e._lunarBound||(e.addEventListener("click",()=>this.adminJoinRoom(e.dataset.roomId)),e._lunarBound=!0)})},_refreshOuterCard(){var o,i,n;if(!((o=Ge())!=null&&o.ready)){this._outerCardRetry||(this._outerCardRetry=setTimeout(()=>{this._outerCardRetry=null,this._refreshOuterCard()},500));return}const t=(i=this.currentLunarRoom)==null?void 0:i.roomId;if(!t)return;const e=async()=>{var r;try{const a=await((r=Ge())==null?void 0:r.getRoomParticipants(t)),s=document.getElementById("lunarRoomPresence");s&&(s.textContent=`${a.length} present`)}catch(a){console.warn("[LunarEngine] _refreshOuterCard error:",a)}};if(e(),this._attachCardListeners(),this._attachAdminListeners(),this._outerCardSub)try{this._outerCardSub.unsubscribe()}catch{}this._outerCardSub=(n=Ge())==null?void 0:n.subscribeToPresence(e)},formatTime(t){return!t||isNaN(t.getTime())?"N/A":t.toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})},formatDate(t){return t.toLocaleDateString("en-US",{month:"short",day:"numeric"})},getMoonData(){return this.currentMoonData},renderLunarRoom(){this.renderLunarCard()},injectAdminUI(){var o,i,n;if(!((n=(i=(o=window.Core)==null?void 0:o.state)==null?void 0:i.currentUser)==null?void 0:n.is_admin))return;const e=document.querySelector("#lunarContainer .celestial-card-full");if(e&&!document.getElementById("lunarAdminPanel")){const r=document.createElement("div");r.innerHTML=this._renderAdminSection(),e.appendChild(r.firstElementChild),this._attachAdminListeners()}}};window.LunarEngine=xe;const Q=()=>window.CommunityDB,U=class U{constructor(e){if(!e)throw new Error("LunarRoom: phaseConfig is required");this.config=e,this.isActive=!1,this.currentPractice=null,this.collectiveTimer=null,this.collectiveWords=null,this.saveDebounceTimer=null,this._retryCheckTimeout=null,this._hasLoggedWaiting=!1,this.eventListeners=[],this.userWeekData=this._getDefaultUserData(),this.weekStartDate=null,this.weekEndDate=null,this.domCache={dynamicContent:null,popup:null},this._bindMethods()}_getDefaultUserData(){return{seedAffirmation:null,practiceCount:0,journalEntries:[],emotionalEchoes:[],intentionShared:!1,privateIntention:"",collectiveWord:"",intention:"",affirmation:"",releaseList:""}}_bindMethods(){["init","enterRoom","renderRoomDashboard","switchMode","showPracticePopup","showCollectiveIntentionPopup","startCollectiveStep2","startCollectiveStep3","startCollectiveStep4","startCollectiveStep5","startMeditationTimer","startWitnessingTimer","submitWordToCollective","completeCollectivePractice","closeCollectivePopup","closePracticePopup","saveIntentionPractice","submitClosure","showLockedMessage","enterGroupCircle","leaveRoom","_handlePopupClick","_clearTimer"].forEach(e=>{typeof this[e]=="function"&&(this[e]=this[e].bind(this))})}init(){try{this.checkIfWeekActive(),V.injectStyles(this.config.cssPrefix)}catch(e){console.error(`${this.config.name} init error:`,e)}}checkIfWeekActive(){var e,o,i;try{if(!(xe!=null&&xe.currentMoonData)){this._hasLoggedWaiting||(this._hasLoggedWaiting=!0),this._retryCheckTimeout||(this._retryCheckTimeout=setTimeout(()=>{this._retryCheckTimeout=null,this.checkIfWeekActive()},500));return}if((i=(o=(e=window.Core)==null?void 0:e.state)==null?void 0:o.currentUser)==null?void 0:i.is_admin){this.isActive=!0,this.calculateWeekDates(),this.loadUserWeekData();return}const r=xe.currentMoonData.phase;this.isActive=this.config.phaseRanges.some(([a,s])=>r>=a&&r<=s),this.isActive&&(this.calculateWeekDates(),this.loadUserWeekData())}catch(n){console.error("checkIfWeekActive error:",n),this.isActive=!1}}calculateWeekDates(){var e,o,i;try{const n=xe==null?void 0:xe.currentMoonData;if(!n){console.error("No moon data");return}const{phase:r,age:a}=n,s=U.CONSTANTS,l=new Date,d=new Date(l-a*s.MS_PER_DAY);let c=this.config.phaseRanges.find(([p,h])=>r>=p&&r<=h);if(!c&&(c=((i=(o=(e=window.Core)==null?void 0:e.state)==null?void 0:o.currentUser)==null?void 0:i.is_admin)?this.config.phaseRanges[0]:null,!c)){this._calculateNextOccurrence(r,a);return}this.weekStartDate=new Date(d.getTime()+c[0]*s.LUNAR_CYCLE_DAYS*s.MS_PER_DAY),this.weekEndDate=new Date(d.getTime()+c[1]*s.LUNAR_CYCLE_DAYS*s.MS_PER_DAY)}catch(n){console.error("calculateWeekDates error:",n)}}_calculateNextOccurrence(e,o){try{const i=U.CONSTANTS,n=new Date,r=new Date(n-o*i.MS_PER_DAY),[a,s]=this.config.phaseRanges[0],l=e<a?r:new Date(r.getTime()+i.LUNAR_CYCLE_DAYS*i.MS_PER_DAY);this.weekStartDate=new Date(l.getTime()+a*i.LUNAR_CYCLE_DAYS*i.MS_PER_DAY),this.weekEndDate=new Date(l.getTime()+s*i.LUNAR_CYCLE_DAYS*i.MS_PER_DAY)}catch(i){console.error("_calculateNextOccurrence error:",i)}}getDaysRemaining(){return this.weekEndDate?Math.max(0,Math.ceil((this.weekEndDate-new Date)/U.CONSTANTS.MS_PER_DAY)):0}shouldShowClosure(){const e=this.getDaysRemaining();return e>0&&e<=2}loadUserWeekData(){try{const e=localStorage.getItem(this.config.storageKey);e&&(this.userWeekData={...this.userWeekData,...JSON.parse(e)})}catch(e){console.error("loadUserWeekData error:",e)}}saveUserWeekData(e=!1){if(e){this._performSave();return}clearTimeout(this.saveDebounceTimer),this.saveDebounceTimer=setTimeout(()=>this._performSave(),U.CONSTANTS.SAVE_DEBOUNCE_MS)}_performSave(){var e;try{localStorage.setItem(this.config.storageKey,JSON.stringify(this.userWeekData))}catch(o){console.error("_performSave error:",o),(e=window.Core)==null||e.showToast("Failed to save data")}}_sanitizeInput(e){return typeof e=="string"?e.replace(/<[^>]*>/g,"").trim():""}_validateIntention(e){const o=U.CONSTANTS;return e!=null&&e.trim()?e.length>o.MAX_INTENTION_LENGTH?{valid:!1,error:`Intention must be under ${o.MAX_INTENTION_LENGTH} characters`}:{valid:!0}:{valid:!1,error:"Intention cannot be empty"}}_validateAffirmation(e){const o=U.CONSTANTS;return e!=null&&e.trim()?e.length>o.MAX_AFFIRMATION_LENGTH?{valid:!1,error:`Affirmation must be under ${o.MAX_AFFIRMATION_LENGTH} characters`}:{valid:!0}:{valid:!1,error:"Affirmation cannot be empty"}}_validateCollectiveWord(e){return e!=null&&e.trim()?e.length>U.CONSTANTS.MAX_WORD_LENGTH?{valid:!1,error:`Word must be under ${U.CONSTANTS.MAX_WORD_LENGTH} characters`}:e.trim().includes(" ")?{valid:!1,error:"Please enter only one word"}:{valid:!0}:{valid:!1,error:"Word cannot be empty"}}enterRoom(){var e,o,i,n,r,a,s,l;try{const d=(i=(o=(e=window.Core)==null?void 0:e.state)==null?void 0:o.currentUser)==null?void 0:i.is_admin;if(!this.isActive&&!d){(n=window.Core)==null||n.showToast(`${this.config.emoji} ${this.config.name} room opens during the ${this.config.name} phase`);return}(r=window.Core)==null||r.navigateTo("practiceRoomView"),(s=(a=window.PracticeRoom)==null?void 0:a.stopHubPresence)==null||s.call(a),window.currentLunarRoom=this,this.renderRoomDashboard(),this._setPresence(),this._loadCollectiveWords(),setTimeout(()=>this._refreshLivePresence(),300)}catch(d){console.error("enterRoom error:",d),(l=window.Core)==null||l.showToast("Failed to enter room")}}leaveRoom(){var e,o,i;try{this._clearPresence(),["_presenceSub","_collectiveWordsSub"].forEach(n=>{if(this[n]){try{this[n].unsubscribe()}catch{}this[n]=null}}),this._clearTimer(),this._removeEventListeners(),window.currentLunarRoom=null,(o=(e=window.PracticeRoom)==null?void 0:e.startHubPresence)==null||o.call(e),(i=window.Core)==null||i.navigateTo("hubView")}catch(n){console.error("leaveRoom error:",n)}}renderRoomDashboard(){var e;try{const o=(e=this.domCache).dynamicContent??(e.dynamicContent=document.getElementById("dynamicRoomContent"));if(!o){console.error("dynamicRoomContent not found");return}const i=new Date,n=this.getDaysRemaining(),r=Math.ceil((this.weekStartDate-i)/U.CONSTANTS.MS_PER_DAY),a=this.getLivingPresenceCount(),s=this.isActive?`${n} ${n===1?"day":"days"} remaining`:`${r} ${r===1?"day":"days"} until ${this.config.name}`;o.innerHTML=`
                <div class="lunar-room-bg">
                    <div class="${this.config.cssPrefix}-starfield lunar-starfield">
                        ${V.generateStarfield()}
                    </div>
                    ${V.renderTopBar({emoji:this.config.emoji,name:this.config.name,daysText:s,livingPresenceCount:a,cssPrefix:this.config.cssPrefix})}
                    <div class="${this.config.cssPrefix}-content-wrapper lunar-content-wrapper">
                        ${V.renderMoonVisual({cssPrefix:this.config.cssPrefix,sphereClass:`${this.config.cssPrefix}-moon-sphere`,glowClass:`${this.config.cssPrefix}-moon-glow`})}
                        ${V.renderIntroCard({imageUrl:this.config.imageUrl,description:this.config.description})}
                        ${V.renderModeToggle({cssPrefix:this.config.cssPrefix,globalName:this.config.globalName})}
                        ...
                        <div class="lunar-wisdom-section">${V.renderWisdomText(this.config.wisdomQuote)}</div>
                    </div>
            ...
            <div class="lunar-group-intro">
                ...
                <div class="lunar-group-avatars">${V.renderMockAvatars(e)}</div>
            ...
            </div>`,(o=e.querySelector('[data-action="start-meditation"]'))==null||o.addEventListener("click",this.startMeditationTimer),(i=e.querySelector('[data-action="skip-meditation"]'))==null||i.addEventListener("click",this.startCollectiveStep3))}startMeditationTimer(){var e;(e=document.getElementById("startMeditationBtn"))==null||e.style.setProperty("display","none"),this._runTimer("meditationTimer",U.CONSTANTS.MEDITATION_DURATION,()=>{const o=document.getElementById("skipToIntentionBtn");o&&(o.textContent="Continue to Intention",o.style.display="block")}),setTimeout(()=>{const o=document.getElementById("skipToIntentionBtn");o&&(o.style.display="block")},1e4)}startCollectiveStep3(){var o;this._clearTimer();const e=document.getElementById("collectiveIntentionContent");e&&(e.innerHTML=`
            <div class="lunar-popup-section">
            ...
            <div class="lunar-popup-section" style="text-align:center;">
                ...
                <div id="wordCloud" class="lunar-word-cloud">
                    ${V.renderWordCloud(this.collectiveWords??this.getMockCollectiveWords())}
                </div>
        ...
        </div>`}},A={storage:{_key:t=>`${_e.STORAGE_PREFIX}${t}${_e.STORAGE_KEY_SUFFIX}`,get(t){const e=localStorage.getItem(this._key(t));if(!e)return null;try{return JSON.parse(e)}catch(o){return console.error(`[SolarUI] Error loading ${t} data:`,o),null}},set(t,e){try{return localStorage.setItem(this._key(t),JSON.stringify(e)),!0}catch(o){return console.error(`[SolarUI] Error saving ${t} data:`,o),!1}},clear(t){localStorage.removeItem(this._key(t))}},utils:{calculateDaysRemaining:t=>Math.ceil((t-new Date)/864e5),generateFloatingElements(t,e=_e.FLOATING_ELEMENT_COUNT){const{FLOATING_ELEMENT_DELAY_MAX:o,FLOATING_ELEMENT_DURATION_MIN:i,FLOATING_ELEMENT_DURATION_RANGE:n}=_e;let r="";for(let a=0;a<e;a++){const s=t[Math.floor(Math.random()*t.length)],l=(Math.random()*100).toFixed(2),d=(Math.random()*o).toFixed(2),c=(i+Math.random()*n).toFixed(2);r+=`<div class="solar-floating-element"
...
@media (max-width:380px) {
  ...
  .solar-inactive-card{padding:1.25rem 0.875rem !important;}
    ...
    `}};window.SolarUIManager=A;const $e={location:{latitude:31,longitude:0,name:"Default"},currentSolarData:null,currentSolarRoom:null,solarRooms:[{season:"spring",icon:"🌱",roomName:"Spring Awakening",roomId:"spring-equinox",description:"Renewal energy and fresh beginnings",energy:"Renewal, Growth, Awakening",practices:["Dawn meditation","Renewal rituals","Energy activation","New beginning visualization"]},{season:"summer",icon:"☀️",roomName:"Summer Radiance",roomId:"summer-solstice",description:"Peak vitality and expansive energy",energy:"Vitality, Expansion, Power",practices:["Solar noon meditation","Fire ceremony","Peak energy practice","Celebration ritual"]},{season:"autumn",icon:"🍂",roomName:"Autumn Harvest",roomId:"autumn-equinox",description:"Gratitude and preparation for rest",energy:"Gratitude, Harvest, Balance",practices:["Gratitude meditation","Harvest ritual","Balance practice","Reflection ceremony"]},{season:"winter",icon:"❄️",roomName:"Winter Stillness",roomId:"winter-solstice",description:"Deep rest and inner contemplation",energy:"Rest, Stillness, Contemplation",practices:["Deep rest meditation","Inner journey","Stillness practice","Contemplative silence"]}],init(){if(this._initialized)return;this._initialized=!0;const t=()=>this.updateAll();navigator.geolocation?navigator.geolocation.getCurrentPosition(({coords:e})=>{this.location={latitude:e.latitude,longitude:e.longitude,name:"Your Location"},t()},t,{timeout:5e3,maximumAge:36e5}):t(),setInterval(t,_e.UPDATE_INTERVAL_MS)},updateAll(){this.updateSolarData(),this.updateSolarVisualization(),this.updateSolarRoom(),this.renderSolarCard()},setLocation(t,e,o){this.location={latitude:t,longitude:e,name:o},this.updateAll()},updateSolarData(){const t=new Date,{latitude:e,longitude:o}=this.location,i=Ae.getTimes(t,e,o),n=this.calculateSunDeclination(t),{name:r,days:a}=this._getNextTransition(t);this.currentSolarData={sunrise:i.sunrise,sunset:i.sunset,solarNoon:i.solarNoon,declination:n,seasonInfo:this.getSeasonInfo(t),currentSeason:this.getCurrentSeason(),nextSeasonName:r,daysToNextSeason:a,position:Ae.getPosition(t,e,o)},this.renderSolarInfo()},renderSolarInfo(){const t=this.currentSolarData;if(!t)return;const e=(i,n)=>{const r=document.getElementById(i);r&&(r.textContent=n)};e("sunrise",this.formatTime(t.sunrise)),e("sunset",this.formatTime(t.sunset)),e("solarNoon",this.formatTime(t.solarNoon)),e("seasonInfo",t.seasonInfo);const o=document.getElementById("solarDeclination");if(o){const i=o.querySelector(".decl-value");i&&(i.textContent=`${Math.abs(t.declination).toFixed(1)}° ${t.declination>=0?"N":"S"}`)}},calculateSunDeclination(t){const e=this.getDayOfYear(t);return 23.44*Math.sin(2*Math.PI*(e-81)/365)},getDayOfYear(t){return Math.floor((t-new Date(t.getFullYear(),0,0))/864e5)},getSeasonDates(t){if(this._seasonDatesCache||(this._seasonDatesCache={}),this._seasonDatesCache[t])return this._seasonDatesCache[t];const e=(t-2e3)/1e3,o=m=>{const g={spring:[245162380984e-5,365242.37404,.05169,-.00411,-57e-5],summer:[245171656767e-5,365241.62603,.00325,.00888,-3e-4],autumn:[245181021715e-5,365242.01767,-.11575,.00337,78e-5],winter:[245190005952e-5,365242.74049,-.06223,-.00823,32e-5]}[m];return g[0]+g[1]*e+g[2]*e**2+g[3]*e**3+g[4]*e**4},i=m=>{const g=(m-2451545)/36525,f=35999.373*g-2.47,v=1+.0334*Math.cos(f*Math.PI/180)+7e-4*Math.cos(2*f*Math.PI/180),M=[[485,324.96,1934.136],[203,337.23,32964.467],[199,342.08,20.186],[182,27.85,445267.112],[156,73.14,45036.886],[136,171.52,22518.443],[77,222.54,65928.934],[74,296.72,3034.906],[70,243.58,9037.513],[58,119.81,33718.148],[52,297.17,150.678],[50,21.02,2281.226]].reduce((S,[_,k,B])=>S+_*Math.cos((k+B*g)*Math.PI/180),0);return m+1e-5*M/v},n=m=>new Date((m-24405875e-1)*864e5),r=m=>new Date(m.getTime()-864e5),a=m=>n(i(o(m))),s=a("spring"),l=a("summer"),d=a("autumn"),c=a("winter"),p=n(i((()=>{const m=(t+1-2e3)/1e3,g=[245162380984e-5,365242.37404,.05169,-.00411,-57e-5];return g[0]+g[1]*m+g[2]*m**2+g[3]*m**3+g[4]*m**4})())),h={spring:{start:s,end:r(l)},summer:{start:l,end:r(d)},autumn:{start:d,end:r(c)},winter:{start:c,end:r(p)}};return this._seasonDatesCache[t]=h,h},getCurrentSeason(){const t=new Date,e=t.getFullYear(),o=this.location.latitude>=0,i={spring:"autumn",summer:"winter",autumn:"spring",winter:"summer"},n=this.getSeasonDates(e-1);if(t>=n.winter.start&&t<=n.winter.end)return o?"winter":"summer";const r=this.getSeasonDates(e);for(const a of["spring","summer","autumn","winter"]){const{start:s,end:l}=r[a];if(t>=s&&t<=l)return o?a:i[a]}return"winter"},_getUpcomingTransitions(t){const e=t.getFullYear(),o=this.getSeasonDates(e),i=this.getSeasonDates(e+1);return[{name:"Spring Equinox",date:o.spring.start},{name:"Summer Solstice",date:o.summer.start},{name:"Autumn Equinox",date:o.autumn.start},{name:"Winter Solstice",date:o.winter.start},{name:"Spring Equinox",date:i.spring.start}].filter(n=>n.date>t).sort((n,r)=>n.date-r.date)},getSeasonInfo(t){const e=this._getUpcomingTransitions(t)[0];return`${Math.ceil((e.date-t)/864e5)} days to ${e.name}`},_getNextTransition(t){const e=this._getUpcomingTransitions(t)[0];return{name:e.name.split(" ")[0],days:Math.ceil((e.date-t)/864e5)}},getNextSeasonInfo(t){return this._getNextTransition(t)},updateSolarVisualization(){this.currentSolarData&&this.drawWheelOfYear(this.currentSolarData)},drawWheelOfYear(t){var Z;const e=document.getElementById("solarSvg");if(!e)return;const o=280,i=140,n=140,r=110,a=52;e.setAttribute("viewBox",`0 0 ${o} ${o}`),e.setAttribute("width","100%"),e.style.height="auto",e.innerHTML="";const s="http://www.w3.org/2000/svg",l=($,I)=>{const Y=document.createElementNS(s,$);return Object.entries(I).forEach(([K,le])=>Y.setAttribute(K,le)),Y},d=l("defs",{}),c=l("clipPath",{id:"woyClip"});c.appendChild(l("circle",{cx:i,cy:n,r:r-1})),d.appendChild(c),e.appendChild(d);const p=[{d:`M${i},${n} L${i},${n-r} L${i+r},${n-r} L${i+r},${n} Z`,fill:"#C0DD97"},{d:`M${i},${n} L${i+r},${n} L${i+r},${n+r} L${i},${n+r} Z`,fill:"#FAC775"},{d:`M${i},${n} L${i},${n+r} L${i-r},${n+r} L${i-r},${n} Z`,fill:"#F0957B"},{d:`M${i},${n} L${i-r},${n} L${i-r},${n-r} L${i},${n-r} Z`,fill:"#B5D4F4"}],h=l("g",{"clip-path":"url(#woyClip)"});p.forEach(({d:$,fill:I})=>h.appendChild(l("path",{d:$,fill:I,opacity:"0.45",stroke:"none"}))),e.appendChild(h),e.appendChild(l("circle",{cx:i,cy:n,r,fill:"none",stroke:"var(--neuro-shadow-light)","stroke-width":"1.5"})),e.appendChild(l("circle",{cx:i,cy:n,r:a,fill:"none",stroke:"var(--neuro-shadow-light)","stroke-width":"1","stroke-dasharray":"3 3",opacity:"0.6"})),e.appendChild(l("line",{x1:i-r,y1:n,x2:i+r,y2:n,stroke:"var(--neuro-shadow-light)","stroke-width":"0.8"})),e.appendChild(l("line",{x1:i,y1:n-r,x2:i,y2:n+r,stroke:"var(--neuro-shadow-light)","stroke-width":"0.8"}));const m=($,I,Y,K,le,de)=>{const ce=l("text",{x:$,y:I,"text-anchor":"middle",fill:le,"font-size":"9","font-weight":"600"});ce.textContent=Y,e.appendChild(ce);const y=l("text",{x:$,y:I+11,"text-anchor":"middle",fill:de,"font-size":"7.5"});y.textContent=K,e.appendChild(y)};m(i+55,n-40,"Spring","Mar · Apr · May","#3B6D11","#639922"),m(i+55,n+52,"Summer","Jun · Jul · Aug","#854F0B","#BA7517"),m(i-55,n+52,"Autumn","Sep · Oct · Nov","#993C1D","#D85A30"),m(i-55,n-40,"Winter","Dec · Jan · Feb","#185FA5","#378ADD");const g=($,I,Y)=>{const K=l("text",{x:$,y:I,"text-anchor":"middle","font-size":"11"});K.textContent=Y,e.appendChild(K)};g(i,n-r+8,"🌱"),g(i+r-6,n+4,"☀️"),g(i,n+r-2,"🍂"),g(i-r+6,n+4,"❄️"),[[i,n-r,i,n-r+8],[i+r,n,i+r-8,n],[i,n+r,i,n+r-8],[i-r,n,i-r+8,n]].forEach(([$,I,Y,K])=>e.appendChild(l("line",{x1:$,y1:I,x2:Y,y2:K,stroke:"var(--neuro-text)","stroke-width":"1.5","stroke-linecap":"round"})));const f=79,v=new Date,M=this.getDayOfYear(v),S=-Math.PI/2+(M-f)/365*2*Math.PI,_=i+r*Math.cos(S),k=n+r*Math.sin(S);for(let $=0;$<8;$++){const I=$*Math.PI/4;e.appendChild(l("line",{x1:_+Math.cos(I)*10,y1:k+Math.sin(I)*10,x2:_+Math.cos(I)*15,y2:k+Math.sin(I)*15,stroke:"#EF9F27","stroke-width":"1.5","stroke-linecap":"round"}))}e.appendChild(l("circle",{cx:_,cy:k,r:"8",fill:"#EF9F27",stroke:"#BA7517","stroke-width":"1.2"})),e.appendChild(l("circle",{cx:_,cy:k,r:"3.5",fill:"#FCDE5A",opacity:"0.9"}));const{nextSeasonName:B,daysToNextSeason:O}=t,E=v.toLocaleDateString("en-US",{month:"short",day:"numeric"}),z=this.getCurrentSeason(),q=z.charAt(0).toUpperCase()+z.slice(1),L=((Z=this.getSeasonDates(v.getFullYear())[z])==null?void 0:Z.start)||v,J=Math.max(1,Math.floor((v-L)/864e5)+1),F=($,I,Y,K,le)=>{const de=l("text",{x:i,y:I,"text-anchor":"middle",fill:le,"font-size":Y,"font-weight":K});de.textContent=$,e.appendChild(de)};F(E,n-10,"9","700","var(--neuro-text)"),F(`${J} days in ${q}`,n+3,"7.5","400","var(--neuro-text-light)"),F(`${O} days to ${B}`,n+15,"7.5","400","#BA7517")},updateSolarRoom(){const t=this.getCurrentSeason(),e=this.solarRooms.find(o=>o.season===t);e&&e!==this.currentSolarRoom&&(this.currentSolarRoom=e,this.renderSolarRoom(e))},renderSolarRoom(t){const e=(i,n)=>{const r=document.getElementById(i);r&&(r.textContent=n)};e("solarRoomIcon",t.icon),e("solarRoomName",t.roomName),e("solarRoomDesc",t.description);const o=document.getElementById("solarPracticeRoom");o&&(o.setAttribute("data-room-id",t.roomId),o.setAttribute("data-room-energy",t.energy))},getCurrentRoom(){return this.currentSolarRoom},renderSolarCard(){var r,a,s;const t=document.getElementById("solarContainer");if(!t){console.warn("SolarEngine: solarContainer not found");return}if(!this.currentSolarData||!this.currentSolarRoom){t.innerHTML='<p style="color:var(--text-muted);padding:20px;">Loading solar data...</p>';return}const{currentSolarData:e,currentSolarRoom:o}=this,i=(s=(a=(r=window.Core)==null?void 0:r.state)==null?void 0:a.currentUser)==null?void 0:s.is_admin,n=e.currentSeason.charAt(0).toUpperCase()+e.currentSeason.slice(1);t.innerHTML=`
      ...
      <div class="solar-popup-section" style="text-align:center;">
        ...
        <div class="modal-overlay" id="${this.roomId}SetupModal" style="z-index:200001;">
            ...
            </ul>`}}Object.assign(je.prototype,ft);Object.assign(je.prototype,yt);Object.assign(je.prototype,De);const Qt=new je;window.DeepWorkRoom=Qt;var ct;(ct=window.dispatchRoomReady)==null||ct.call(window,"deepwork");class bt extends D{constructor(){super({roomId:"campfire",roomType:"always-open",name:"Community Campfire",icon:"🔥",description:"A warm space to share, reflect, and connect with the community. Pull up a chair. Real conversations, real people, real connection.",energy:"Social",imageUrl:"/Community/Campfire.webp",participants:12}),this.initChatState(["main"])}onEnter(){this.loadRoomChatFromDB("main"),this._injectSenderAvatar("main")}getHeaderGradient(){return"background:linear-gradient(135deg,rgba(234,88,12,0.1) 0%,rgba(251,146,60,0.05) 100%);"}getParticipantText(){return`${this.state.participants} around the fire`}buildBody(){return`
        <div class="ps-body">
        ...
        <div style="border:2px solid var(--border);border-radius:var(--radius-lg);padding:20px;background:var(--surface);margin-top:16px;">
            ...
            <div id="${this.roomId}MasteryChakras" style="display:flex;flex-wrap:wrap;gap:8px;justify-content:center;">
                ...
                <div style="display:flex;gap:10px;align-items:baseline;padding:8px 10px;background:var(--background);border-radius:var(--radius-md);border:1px solid var(--border);">
                    ...
                    opacity:${h?"1":"0.5"};
                    transition:all 0.3s ease;">${p.label}</span>`}).join(""),this._masteryLoaded=!0}catch(a){console.warn("[ReikiRoom] load mastery failed",a),o&&(o.textContent="— / 7")}}onEnter(){this.initializeChat(),requestAnimationFrame(()=>{var e;(e=document.querySelector(`#${this.roomId}View .tarot-main`))==null||e.scrollTo(0,0),this._loadCommunityEnergy()})}getActions(){return{...super.getActions(),startPersonalSession:()=>this.startPersonalSession(),prevChakraImage:e=>{this._actionEl(e).dataset.scope==="daily"?this.previousDailyImage():this.previousPersonalImage()},nextChakraImage:e=>{this._actionEl(e).dataset.scope==="daily"?this.nextDailyImage():this.nextPersonalImage()},toggleChakraJournal:e=>this._toggleChakraJournalLog(this._actionEl(e).dataset.prefix),saveChakraJournal:e=>{const o=this._actionEl(e);this._saveChakraJournalEntry(o.dataset.prefix,o.dataset.key)},submitCommunityEnergy:()=>this._submitCommunityEnergy()}}getInstructions(){return`
            ...
            </ul>`}}Object.assign(Je.prototype,De);Object.assign(Je.prototype,vt);const oo=new Je;window.ReikiRoom=oo;var mt;(mt=window.dispatchRoomReady)==null||mt.call(window,"reiki");const io={state:{isEnabled:!1,isRendered:!1},config:{FEATURE_ENABLED:!1},getHTML(){return this.config.FEATURE_ENABLED?`
