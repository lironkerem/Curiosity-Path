...
import"./vendor-suncalc-BBoG6fSx.js";const ee="modulepreload",te=function(e){return"/"+e},U={},m=function(t,i,o){let n=Promise.resolve();if(i&&i.length>0){let a=function(c){return Promise.all(c.map(u=>Promise.resolve(u).then(p=>({status:"fulfilled",value:p}),p=>({status:"rejected",reason:p}))))};document.getElementsByTagName("link");const s=document.querySelector("meta[property=csp-nonce]"),l=s?.nonce||s?.getAttribute("nonce");n=a(i.map(c=>{if(c=te(c),c in U)return;U[c]=!0;const u=c.endsWith(".css"),p=u?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${p}`))return;const h=document.createElement("link");if(h.rel=u?"stylesheet":ee,u||(h.as="script"),h.crossOrigin="",h.href=c,l&&h.setAttribute("nonce",l),document.head.appendChild(h),u)return new Promise((v,C)=>{h.addEventListener("load",v),h.addEventListener("error",()=>C(new Error(`Unable to preload CSS for ${c}`)))})}))}function r(a){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=a,window.dispatchEvent(s),!s.defaultPrevented)throw a}return n.then(a=>{for(const s of a||[])s.status==="rejected"&&r(s.reason);return t().catch(r)})},N=window.AppSupabase||null;window.CommunitySupabase=N;const d={_sb:null,_uid:null,_subs:{},_heartbeatTimer:null,async init(){if(this._sb=window.AppSupabase||N,!this._sb)return console.error("[CommunityDB] CommunitySupabase not ready — window.AppSupabase is null"),!1;const{data:{user:e},error:t}=await this._sb.auth.getUser();return t||!e?(console.error("[CommunityDB] No authenticated user:",t?.message),!1):(this._uid=e.id,!0)},get userId(){return this._uid},get ready(){return!!(this._sb&&this._uid)},_err(e,t){console.error(`[CommunityDB] ${e}:`,t?.message??t)},_profileSelect:"id, name, emoji, avatar_url",_ago(e){return new Date(Date.now()-e).toISOString()},_todayUTC(){const e=new Date;return e.setUTCHours(0,0,0,0),e.toISOString()},async getMyProfile(){if(!this.ready)return null;const{data:e,error:t}=await this._sb.from("profiles").select("*").eq("id",this._uid).single();return t?(this._err("getMyProfile",t),null):e},async getProfile(e){if(!this.ready)return null;const{data:t,error:i}=await this._sb.from("profiles").select("id, name, emoji, avatar_url, inspiration, community_status, community_role, total_sessions, total_minutes, gifts_given, birthday, country").eq("id",e).single();return i?(this._err("getProfile",i),null):t},_parseProgress(e){const t=typeof e=="string"?JSON.parse(e):e;return{xp:t.xp??0,karma:t.karma??0,level:t.level??1,badges:t.badges??[],unlockedFeatures:t.unlockedFeatures??[],streak:t.streaks?.current??t.streak?.current??0,longestStreak:t.streaks?.longest??0,totalSessions:t.stats?.totalSessions??0,totalMeditations:t.stats?.totalMeditations??0,totalReadings:t.stats?.totalReadings??0,totalTarotSpreads:t.totalTarotSpreads??0,totalJournalEntries:t.totalJournalEntries??0,totalWellnessRuns:t.totalWellnessRuns??0,totalHappinessViews:t.totalHappinessViews??0}},async getUserProgress(e){if(!this.ready)return null;try{const{data:t,error:i}=await this._sb.from("user_progress").select("payload").eq("user_id",e).single();return i||!t?null:this._parseProgress(t.payload)}catch(t){return this._err("getUserProgress",t),null}},getOwnGamificationState(){const e=window.app?.gamification;return e?this._parseProgress(e.state??e):null},async uploadAvatar(e){if(!this.ready)return null;try{const t=e.name.split(".").pop().toLowerCase()||"jpg",i=`avatars/${this._uid}.${t}`,{error:o}=await this._sb.storage.from("community-avatars").upload(i,e,{upsert:!0,contentType:e.type});if(o)return this._err("uploadAvatar upload",o),null;const{data:n}=this._sb.storage.from("community-avatars").getPublicUrl(i),r=n?.publicUrl;if(!r)return null;const a=`${r}?t=${Date.now()}`;return await this.updateProfile({avatar_url:a})?a:null}catch(t){return this._err("uploadAvatar",t),null}},async updateProfile(e){if(!this.ready)return!1;const{error:t}=await this._sb.from("profiles").update({...e,updated_at:new Date().toISOString()}).eq("id",this._uid);return t?(this._err("updateProfile",t),!1):!0},async setPresence(e="online",t="✨ Available",i=null){if(!this.ready)return!1;const o=new Date().toISOString(),{error:n}=await this._sb.from("community_presence").upsert({user_id:this._uid,status:e,activity:t,room_id:i,last_seen:o,updated_at:o},{onConflict:"user_id"});return n&&this._err("setPresence",n),!n},async setOffline(){return this.setPresence("offline","💤 Offline",null)},async getActiveMembers(){if(!this.ready)return[];const{data:e,error:t}=await this._sb.from("community_presence").select(`user_id, status, activity, room_id, is_phantom, last_seen, profiles ( ${this._profileSelect} )`).neq("status","offline").gte("last_seen",this._ago(5*6e4)).order("is_phantom",{ascending:!1}).order("last_seen",{ascending:!1});return t?(this._err("getActiveMembers",t),[]):e||[]},async getRoomParticipants(e){if(!this.ready)return[];const{data:t,error:i}=await this._sb.from("community_presence").select(`user_id, status, activity, room_id, is_phantom, last_seen, profiles ( ${this._profileSelect} )`).or(`room_id.eq.${e},is_phantom.eq.true`).neq("status","offline").gte("last_seen",this._ago(5*6e4)).order("is_phantom",{ascending:!1}).order("last_seen",{ascending:!0});return i?(this._err("getRoomParticipants",i),[]):t||[]},subscribeToPresence(e){return this._subs.presence&&this._subs.presence.unsubscribe(),this._subs.presence=this._sb.channel("community-presence").on("postgres_changes",{event:"*",schema:"public",table:"community_presence"},async()=>e(await this.getActiveMembers())).subscribe(),this._subs.presence},startHeartbeat(e=6e4){this.stopHeartbeat(),this._heartbeatTimer=setInterval(async()=>{if(!this.ready)return;const t=(typeof Core<"u"?Core:window.Core)?.state;await this.setPresence(t?.currentUser?.status||"online",t?.currentUser?.activity||"✨ Available",t?.currentRoom||null)},e),window.addEventListener("beforeunload",()=>this._cleanup())},stopHeartbeat(){this._heartbeatTimer&&(clearInterval(this._heartbeatTimer),this._heartbeatTimer=null)},_reflectionSelect:"id, content, appreciation_count, created_at, profiles ( id, name, emoji, avatar_url )",async getReflections(e=30){if(!this.ready)return[];const{data:t,error:i}=await this._sb.from("reflections").select(this._reflectionSelect).order("created_at",{ascending:!1}).limit(e);return i?(this._err("getReflections",i),[]):t||[]},async postReflection(e){if(!this.ready)return null;const{data:t,error:i}=await this._sb.from("reflections").insert({user_id:this._uid,content:e}).select(this._reflectionSelect).single();return i?(this._err("postReflection",i),null):t},async deleteReflection(e){if(!this.ready)return!1;const{error:t}=await this._sb.from("reflections").delete().eq("id",e);return t?(this._err("deleteReflection",t),!1):!0},async updateReflection(e,t){if(!this.ready)return!1;const{error:i}=await this._sb.from("reflections").update({content:t}).eq("id",e).eq("user_id",this._uid);return i?(this._err("updateReflection",i),!1):!0},subscribeToReflections(e){return this._subs.reflections&&this._subs.reflections.unsubscribe(),this._subs.reflections=this._sb.channel("community-reflections").on("postgres_changes",{event:"INSERT",schema:"public",table:"reflections"},async({new:t})=>{const{data:i}=await this._sb.from("reflections").select(this._reflectionSelect).eq("id",t.id).single();i&&e(i)}).subscribe(),this._subs.reflections},async getMyAppreciations(){if(!this.ready)return new Set;const{data:e,error:t}=await this._sb.from("appreciations").select("reflection_id").eq("user_id",this._uid);return t?(this._err("getMyAppreciations",t),new Set):new Set(e.map(i=>i.reflection_id))},async toggleAppreciation(e,t){if(!this.ready)return null;if(t){const{error:i}=await this._sb.from("appreciations").delete().eq("user_id",this._uid).eq("reflection_id",e);return i?(this._err("removeAppreciation",i),null):{appreciated:!1}}else{const{error:i}=await this._sb.from("appreciations").insert({user_id:this._uid,reflection_id:e});return i?(this._err("addAppreciation",i),null):{appreciated:!0}}},async getReflectionCount(e){if(!this.ready)return null;const{data:t,error:i}=await this._sb.from("reflections").select("appreciation_count").eq("id",e).single();return i?(this._err("getReflectionCount",i),null):t?.appreciation_count??null},async getMyUserAppreciations(){if(!this.ready)return new Set;const{data:e,error:t}=await this._sb.from("user_appreciations").select("appreciated_user_id").eq("user_id",this._uid);return t?(this._err("getMyUserAppreciations",t),new Set):new Set(e.map(i=>i.appreciated_user_id))},async toggleUserAppreciation(e,t){if(!this.ready)return null;if(t){const{error:i}=await this._sb.from("user_appreciations").delete().eq("user_id",this._uid).eq("appreciated_user_id",e);return i?(this._err("removeUserAppreciation",i),null):{appreciated:!1}}else{const{error:i}=await this._sb.from("user_appreciations").insert({user_id:this._uid,appreciated_user_id:e});return i?(this._err("addUserAppreciation",i),null):{appreciated:!0}}},async getUserAppreciationCount(e){if(!this.ready)return 0;const{count:t,error:i}=await this._sb.from("user_appreciations").select("*",{count:"exact",head:!0}).eq("appreciated_user_id",e);return i?(this._err("getUserAppreciationCount",i),0):t||0},_roomMsgSelect:"id, message, created_at, profiles ( id, name, emoji, avatar_url )",async getRoomMessages(e,t=50){if(!this.ready)return[];const{data:i,error:o}=await this._sb.from("room_messages").select(this._roomMsgSelect).eq("room_id",e).order("created_at",{ascending:!0}).limit(t);return o?(this._err("getRoomMessages",o),[]):i||[]},async sendRoomMessage(e,t){if(!this.ready)return null;const{data:i,error:o}=await this._sb.from("room_messages").insert({user_id:this._uid,room_id:e,message:t}).select(this._roomMsgSelect).single();return o?(this._err("sendRoomMessage",o),null):i},subscribeToRoomChat(e,t){const i=`room-${e}`;return this._subs[i]&&this._subs[i].unsubscribe(),this._subs[i]=this._sb.channel(`room-chat-${e}`).on("postgres_changes",{event:"INSERT",schema:"public",table:"room_messages",filter:`room_id=eq.${e}`},async({new:o})=>{const{data:n}=await this._sb.from("room_messages").select(this._roomMsgSelect).eq("id",o.id).single();n&&t(n)}).subscribe(),this._subs[i]},unsubscribeFromRoomChat(e){this._unsub(`room-${e}`)},async sendWhisper(e,t){if(!this.ready)return null;const{data:i,error:o}=await this._sb.from("whispers").insert({sender_id:this._uid,recipient_id:e,message:t}).select().single();return o?(this._err("sendWhisper",o),null):i},async getWhispers(e){if(!this.ready)return[];const{data:t,error:i}=await this._sb.from("whispers").select(`
        ...
        `)},formatTime(e){if(typeof e!="number"||e<0)return"0:00";const t=Math.floor(e/60),i=Math.floor(e%60);return`${t}:${i.toString().padStart(2,"0")}`},getAvatarGradient(e){(!e||typeof e!="string")&&(e="default");const t=e.split("").reduce((i,o)=>i+o.charCodeAt(0),0);return this.config.AVATAR_GRADIENTS[Math.abs(t)%this.config.AVATAR_GRADIENTS.length]}};window.Core=k;const g='xmlns="http://www.w3.org/2000/svg" class="lucide-avatar-icon" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"',I={user:`<svg ${g}><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>`,aries:`<svg ${g}><path d="M12 20V10c0-3.31-2.69-6-6-6"/><path d="M12 20V10c0-3.31 2.69-6 6-6"/><circle cx="6" cy="4" r="2"/><circle cx="18" cy="4" r="2"/></svg>`,taurus:`<svg ${g}><circle cx="12" cy="14" r="7"/><path d="M5 9C5 6 7 3 12 3s7 3 7 6"/><path d="M9 9H4"/><path d="M20 9h-5"/></svg>`,gemini:`<svg ${g}><line x1="8" y1="4" x2="8" y2="20"/><line x1="16" y1="4" x2="16" y2="20"/><line x1="8" y1="4" x2="16" y2="4"/><line x1="8" y1="20" x2="16" y2="20"/><line x1="8" y1="12" x2="16" y2="12"/></svg>`,cancer:`<svg ${g}><circle cx="8" cy="12" r="4"/><circle cx="16" cy="12" r="4"/><path d="M4 8c0-2 1-4 4-4"/><path d="M20 16c0 2-1 4-4 4"/></svg>`,leo:`<svg ${g}><circle cx="8" cy="8" r="4"/><path d="M12 8h4a4 4 0 0 1 0 8h-1"/><path d="M15 16v4"/></svg>`,virgo:`<svg ${g}><line x1="6" y1="4" x2="6" y2="16"/><line x1="12" y1="4" x2="12" y2="16"/><path d="M6 12c0 2.5 2 4 6 4s6-1.5 6-4V8a6 6 0 0 0-12 0"/><path d="M15 16v4"/><path d="M17 16l1.5 4"/></svg>`,libra:`<svg ${g}><line x1="3" y1="20" x2="21" y2="20"/><line x1="12" y1="4" x2="12" y2="16"/><path d="M6 16a6 6 0 0 0 12 0"/></svg>`,scorpio:`<svg ${g}><line x1="6" y1="4" x2="6" y2="16"/><line x1="12" y1="4" x2="12" y2="16"/><line x1="18" y1="4" x2="18" y2="12"/><path d="M6 12c0 2.5 2 4 6 4s6-1.5 6-4"/><polyline points="15 9 18 12 21 9"/></svg>`,sagittarius:`<svg ${g}><line x1="5" y1="19" x2="19" y2="5"/><polyline points="12 5 19 5 19 12"/><line x1="5" y1="19" x2="12" y2="12"/></svg>`,capricorn:`<svg ${g}><path d="M6 20V8a4 4 0 0 1 8 0v4a4 4 0 0 0 4 4h0"/><path d="M18 16l2 2-2 2"/></svg>`,aquarius:`<svg ${g}><path d="M3 10c2-2 4 0 6 0s4-2 6 0 4 0 6 0"/><path d="M3 16c2-2 4 0 6 0s4-2 6 0 4 0 6 0"/></svg>`,pisces:`<svg ${g}><line x1="12" y1="4" x2="12" y2="20"/><path d="M4 8c2 2 4 4 8 4s6-2 8-4"/><path d="M4 16c2-2 4-4 8-4s6 2 8 4"/></svg>`,meditation:`<svg ${g}><circle cx="12" cy="5" r="2"/><path d="M12 7v5l-3 3"/><path d="M12 12l3 3"/><path d="M6 17c0-2 2-4 6-4s6 2 6 4"/></svg>`,moon:`<svg ${g}><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>`,sun:`<svg ${g}><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>`,star:`<svg ${g}><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>`,crystal:`<svg ${g}><circle cx="12" cy="12" r="10"/><path d="M12 8l4 4-4 4-4-4z"/></svg>`,butterfly:`<svg ${g}><path d="M12 22V12"/><path d="M12 12C12 12 8 9 4 10c-3 1-3 5 0 6 2 1 5 0 8-4z"/><path d="M12 12C12 12 16 9 20 10c3 1 3 5 0 6-2 1-5 0-8-4z"/><circle cx="12" cy="5" r="2"/></svg>`,leaf:`<svg ${g}><path d="M2 22c1.25-1.25 2.5-2.5 3.75-3.75"/><path d="M22 2C11 2 2 11 2 22c5.5 0 11-2.5 14.5-6S22 7.5 22 2z"/></svg>`,flower:`<svg ${g}><circle cx="12" cy="12" r="3"/><path d="M12 2a4 4 0 0 1 0 8 4 4 0 0 1 0-8z"/><path d="M12 14a4 4 0 0 1 0 8 4 4 0 0 1 0-8z"/><path d="M2 12a4 4 0 0 1 8 0 4 4 0 0 1-8 0z"/><path d="M14 12a4 4 0 0 1 8 0 4 4 0 0 1-8 0z"/></svg>`,om:`<svg ${g}><circle cx="12" cy="12" r="10"/><path d="M8 12a4 4 0 0 0 8 0"/><circle cx="12" cy="8" r="1"/></svg>`,clover:`<svg ${g}><path d="M12 12c-2-2.5-4-3-6-2s-3 4-1 6 5 2 7-4z"/><path d="M12 12c2-2.5 4-3 6-2s3 4 1 6-5 2-7-4z"/><path d="M12 12c-2.5-2-3-4-2-6s4-3 6-1-2 5-4 7z"/><path d="M12 12c2.5-2 3-4 2-6s-4-3-6-1 2 5 4 7z"/><line x1="12" y1="12" x2="12" y2="22"/></svg>`},W={"👤":"user","♈️":"aries","♉️":"taurus","♊️":"gemini","♋️":"cancer","♌️":"leo","♍️":"virgo","♎️":"libra","♏️":"scorpio","♐️":"sagittarius","♑️":"capricorn","♒️":"aquarius","♓️":"pisces","🧘‍♀️":"meditation","🌙":"moon","☀️":"sun","🌟":"star","🔮":"crystal","🦋":"butterfly","🌿":"leaf","🌸":"flower","🕉️":"om","🍀":"clover"};function B(e){return I[e]||I[W[e]]||I.user}const xe=Object.freeze(Object.defineProperty({__proto__:null,AVATAR_ICONS:I,EMOJI_TO_KEY:W,renderAvatarIcon:B},Symbol.toStringTag,{value:"Module"})),ie=["🏅","🎖️","🌟","👑","🧪","🕉️","🦸","🌱","🎪","🌙","☀️","⚡","🌊","💜","🔱","🔥","💎","🦋","🌸","🍀","🌈","⭐","🎯","🏆","🎗️","🌀","🔮","💫","🧘","🦅","🐉","🌺","🎵","💡","🌿","🦁","🐬","🌍","🎭","🛡️","⚔️","🗝️","🧬","🌠","🎋"],F={state:{isOpen:!1,currentUserId:null,isAppreciated:!1,appreciationCount:0},_LEVEL_TITLES:{1:"Seeker",2:"Practitioner",3:"Adept",4:"Healer",5:"Master",6:"Sage",7:"Enlightened",8:"Buddha",9:"Light",10:"Emptiness"},_STATUS_RINGS:{online:{c:"var(--ring-available,#6b9b37)",s:"rgba(107,155,55,0.2)"},available:{c:"var(--ring-available,#6b9b37)",s:"rgba(107,155,55,0.2)"},away:{c:"var(--ring-guiding,#e53e3e)",s:"rgba(229,62,62,0.2)"},silent:{c:"var(--ring-silent,#7c3aed)",s:"rgba(124,58,237,0.2)"},deep:{c:"var(--ring-deep,#1e40af)",s:"rgba(30,64,175,0.2)"},offline:{c:"var(--ring-offline,#9ca3af)",s:"rgba(156,163,175,0.2)"}},_RARITY_COLORS:{common:"#9ca3af",uncommon:"#10b981",rare:"#3b82f6",epic:"#a855f7",legendary:"#f59e0b"},_RARITY_LABELS:{common:"Common",uncommon:"Uncommon",rare:"Rare",epic:"Epic",legendary:"Legendary"},_COUNTRY_CODES:{israel:"IL","united states":"US",usa:"US",us:"US","united kingdom":"GB",uk:"GB",canada:"CA",australia:"AU",germany:"DE",france:"FR",spain:"ES",italy:"IT",netherlands:"NL",belgium:"BE",switzerland:"CH",sweden:"SE",norway:"NO",denmark:"DK",finland:"FI",poland:"PL",portugal:"PT",austria:"AT",india:"IN",china:"CN",japan:"JP","south korea":"KR",brazil:"BR",mexico:"MX",argentina:"AR","south africa":"ZA",russia:"RU",ukraine:"UA",greece:"GR",turkey:"TR",egypt:"EG","new zealand":"NZ",ireland:"IE",singapore:"SG",thailand:"TH",indonesia:"ID",malaysia:"MY",philippines:"PH"},_ADMIN_SUB_IDS:["adminSubRole","adminSubXp","adminSubKarma","adminSubBadge","adminSubPremium","adminSubMessage","adminSubCustomBadge"],_buildEmojiPicker(e,t){return`
            <div id="${t}" style="display:none;margin-top:6px;padding:8px;border-radius:10px;
                ...
                ${ie.map(i=>`<button type="button" onclick="MemberProfileModal._pickEmoji('${e}','${t}','${i}')"
            ...
            </div>`},_pickEmoji(e,t,i){const o=document.getElementById(e);o&&(o.value=i);const n=document.getElementById(t);n&&(n.style.display="none")},_toggleEmojiPicker(e){const t=document.getElementById(e);t&&(t.style.display=t.style.display==="none"?"flex":"none")},init(){if(document.getElementById("memberProfileModal"))return;window.addEventListener("statusChanged",o=>{const{status:n}=o.detail||{};if(!n||!this.state.isOpen||!(this.state.currentUserId===window.Core.state.currentUser?.id))return;const a=document.getElementById("memberModalStatusRing");if(!a)return;const s=this._STATUS_RINGS[n]||this._STATUS_RINGS.offline;a.style.borderColor=s.c,a.style.boxShadow=`0 0 0 3px ${s.s}`}),window.addEventListener("avatarChanged",o=>{const{userId:n,emoji:r,avatarUrl:a}=o.detail||{};if(!n||!this.state.isOpen||this.state.currentUserId!==n)return;const s=document.getElementById("memberModalAvatar");s&&(a?(s.style.background="transparent",s.innerHTML=`<img src="${a}" loading="lazy" decoding="async" width="80" height="80"
                    style="width:100%;height:100%;object-fit:cover;border-radius:50%;" alt="avatar">`):r&&(s.style.background="",s.innerHTML=`<span>${B(r)}</span>`))});const t=(window.BADGE_REGISTRY||[]).map(o=>`<option value="${o.id}" data-icon="${o.icon}" data-rarity="${o.rarity}" data-xp="${o.xp}" data-desc="${o.desc}">${o.icon} ${o.name}</option>`).join(""),i=document.createElement("div");i.innerHTML=`
            ...
            <button onclick="MemberProfileModal._closeAdminSubs()"
                    style="padding:8px 14px;border-radius:10px;border:none;cursor:pointer;
                           font-size:0.88rem;background:var(--neuro-shadow-light,rgba(0,0,0,0.06));
                           color:var(--neuro-text);">Cancel</button>
        </div>`},async open(e){if(this.init(),!e)return;const t=e===window.Core.state.currentUser?.id;this.state.currentUserId=e,this.state.isOpen=!0;const i=document.getElementById("memberProfileModal"),o=document.getElementById("memberProfileModalInner"),n=document.getElementById("memberModalLoading"),r=document.getElementById("memberModalContent");this._hideActionPanels(),n.style.display="block",r.style.display="none",i.style.display="flex",requestAnimationFrame(()=>{i.style.opacity="1",o.style.transform="translateY(0)"}),document.body.style.overflow="hidden";try{const a=await d.getProfile(e);if(!a){window.Core.showToast("Could not load member profile"),this.close();return}if(this._populate(a),!t){const p=d.subscribeToPresence(h=>{const v=h.find(x=>x.user_id===e);if(!v||!this.state.isOpen)return;const C=document.getElementById("memberModalStatusRing");if(!C)return;const w=this._STATUS_RINGS[v.status]||this._STATUS_RINGS.offline;C.style.borderColor=w.c,C.style.boxShadow=`0 0 0 3px ${w.s}`});this._presenceUnsub=()=>{try{p?.unsubscribe()}catch{}}}const s=document.getElementById("memberModalActions"),l=document.getElementById("memberModalAppreciateBtn");s&&(s.style.display=t?"none":"flex"),l&&(l.style.display=t?"none":"block");const c=document.getElementById("memberModalAdminSection"),u=window.Core.state.currentUser?.is_admin;if(c){if(c.style.display=u?"block":"none",u&&a.community_role){const C=document.getElementById("adminRoleSelect");C&&(C.value=a.community_role)}const p=document.getElementById("memberModalAdminBody"),h=document.getElementById("memberModalAdminToggle");p&&(p.style.display="none"),h&&(h.textContent="▶"),this._closeAdminSubs();const v=document.querySelector(`button[onclick*="_openAdminSub('role')"]`);v&&(v.style.display=t?"none":"inline-block")}t||(this.state.isAppreciated=!1,this.state.appreciationCount=0,this._updateAppreciateBtn(),Promise.all([d.getMyUserAppreciations(),d.getUserAppreciationCount(e)]).then(([p,h])=>{this.state.isAppreciated=p.has(e),this.state.appreciationCount=h,this._updateAppreciateBtn()}).catch(()=>{})),d.getUserProgress(e).then(p=>{p&&this._populateGamification(p)}).catch(()=>{}),this._loadMemberCommunityStats(e).catch(()=>{}),n.style.display="none",r.style.display="block"}catch(a){console.error("[MemberProfileModal] open error:",a),window.Core.showToast("Could not load member profile"),this.close()}},close(){const e=document.getElementById("memberProfileModal"),t=document.getElementById("memberProfileModalInner");e&&(e.style.opacity="0",t.style.transform="translateY(16px)",setTimeout(()=>{e.style.display="none",this.state.isOpen=!1,this.state.currentUserId=null,document.body.style.overflow="",this._hideActionPanels(),this._presenceUnsub&&(this._presenceUnsub(),this._presenceUnsub=null)},250))},_populate(e){const t=document.getElementById("memberModalAvatar");t&&(e.avatar_url?(t.style.background="transparent",t.innerHTML=`<img src="${e.avatar_url}" loading="lazy" decoding="async" width="80" height="80"
                    style="width:100%;height:100%;object-fit:cover;border-radius:50%;"
                    alt="${this._esc(e.name)}">`):(t.style.background=window.Core.getAvatarGradient(e.id),t.innerHTML=`<span>${this._esc(e.emoji||(e.name||"?").charAt(0).toUpperCase())}</span>`));const i=document.getElementById("memberModalStatusRing");if(i){const r=this._STATUS_RINGS[e.community_status]||this._STATUS_RINGS.offline;i.style.borderColor=r.c,i.style.boxShadow=`0 0 0 3px ${r.s}`}this._setText("memberModalName",e.name||"Member"),this._setHTML("memberModalRole",`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> ${e.community_role||"Member"}`),this._setText("memberModalInspiration",e.inspiration?`"${e.inspiration}"`:"");const o=document.getElementById("memberModalLocation"),n=document.getElementById("memberModalMetaSep");if(o){const r=[];if(e.birthday)try{const a=new Date(e.birthday+"T00:00:00");r.push(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M20 21v-8a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8"/><path d="M4 16s.5-1 2-1 2.5 2 4 2 2.5-2 4-2 2 1 2 1"/><path d="M2 21h20"/><path d="M7 8v3"/><path d="M12 8v3"/><path d="M17 8v3"/><path d="M7 4h.01"/><path d="M12 4h.01"/><path d="M17 4h.01"/></svg> ${a.toLocaleDateString(void 0,{month:"long",day:"numeric"})}`)}catch{r.push(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M20 21v-8a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8"/><path d="M4 16s.5-1 2-1 2.5 2 4 2 2.5-2 4-2 2 1 2 1"/><path d="M2 21h20"/><path d="M7 8v3"/><path d="M12 8v3"/><path d="M17 8v3"/><path d="M7 4h.01"/><path d="M12 4h.01"/><path d="M17 4h.01"/></svg> ${e.birthday}`)}e.country&&r.push(`${this._countryFlag(e.country)} ${e.country}`),r.length?(o.innerHTML=r.map((a,s)=>`${s>0?'<span style="width:1px;height:16px;background:rgba(0,0,0,0.1);flex-shrink:0;display:inline-block;"></span>':""}
                     <span style="font-size:0.76rem;font-weight:600;color:var(--text-muted);padding:5px 12px;white-space:nowrap;">${a}</span>`).join(""),n&&(n.style.display="")):(o.innerHTML="",n&&(n.style.display="none"))}},_populateGamification(e){const t=this._LEVEL_TITLES[e.level]||"Seeker",i=t.match(/^[aeiou]/i)?"An":"A",o=document.getElementById("memberModalLevel");o&&(o.textContent=`${i} ${t} - Level ${e.level}`);const n=document.getElementById("memberModalXpBar");if(n){const r=[0,800,2e3,4200,7e3,12e3,3e4,6e4,18e4,45e4],a=r[e.level-1]||0,s=r[e.level]||r[r.length-1],l=s>a?Math.min(100,Math.round((e.xp-a)/(s-a)*100)):100;requestAnimationFrame(()=>{n.style.width=l+"%"})}this._setText("memberModalXP",(e.xp??0).toLocaleString()),this._setText("memberModalKarma",(e.karma??0).toLocaleString()),this._setText("memberModalBadgeCount",(e.badges||[]).length)},async _loadMemberCommunityStats(e){if(!d.ready)return;const t=d._sb;try{const[i,o]=await Promise.all([t.from("room_blessings").select("*",{count:"exact",head:!0}).eq("user_id",e),t.from("room_entries").select("room_id").eq("user_id",e)]);this._setText("memberModalBlessings",!i.error&&i.count!=null?i.count.toLocaleString():"0");const n=o.data;if(!o.error&&n?.length){const r={};for(const s of n)r[s.room_id]=(r[s.room_id]||0)+1;const a=Object.entries(r).sort((s,l)=>l[1]-s[1])[0][0];this._setText("memberModalFavRoom",a.replace(/-/g," ").replace(/\b\w/g,s=>s.toUpperCase()))}else this._setText("memberModalFavRoom","-")}catch(i){console.warn("[MemberProfileModal] _loadMemberCommunityStats:",i)}},async toggleAppreciate(){const e=document.getElementById("memberModalAppreciateBtn");if(!(!e||!this.state.currentUserId)){e.disabled=!0;try{const t=await d.toggleUserAppreciation(this.state.currentUserId,this.state.isAppreciated);if(!t){window.Core.showToast("Could not update - please try again");return}this.state.isAppreciated=t.appreciated,this.state.appreciationCount=await d.getUserAppreciationCount(this.state.currentUserId),this._updateAppreciateBtn(),window.Core.showToast(t.appreciated?"Appreciation sent":"Appreciation removed")}catch(t){console.error("[MemberProfileModal] toggleAppreciate error:",t),window.Core.showToast("Could not update - please try again")}finally{e.disabled=!1}}},_updateAppreciateBtn(){const e=document.getElementById("memberModalAppreciateBtn");if(!e)return;const t=this.state.appreciationCount??"",i=t!==""?` (${t})`:"";this.state.isAppreciated?(e.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M12 3L13.5 8.5L19 10L13.5 11.5L12 17L10.5 11.5L5 10L10.5 8.5Z"/><path d="M5 3L5.75 5.25L8 6L5.75 6.75L5 9L4.25 6.75L2 6L4.25 5.25Z"/><path d="M19 14L19.75 16.25L22 17L19.75 17.75L19 20L18.25 17.75L16 17L18.25 16.25Z"/></svg> Appreciated${i}`,e.style.background="var(--primary,#667eea)",e.style.color="#fff",e.style.boxShadow="inset 2px 2px 5px rgba(0,0,0,0.15)"):(e.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M12 3L13.5 8.5L19 10L13.5 11.5L12 17L10.5 11.5L5 10L10.5 8.5Z"/><path d="M5 3L5.75 5.25L8 6L5.75 6.75L5 9L4.25 6.75L2 6L4.25 5.25Z"/><path d="M19 14L19.75 16.25L22 17L19.75 17.75L19 20L18.25 17.75L16 17L18.25 16.25Z"/></svg> Appreciate${i}`,e.style.background="var(--neuro-bg,#f0f0f3)",e.style.color="var(--neuro-text)",e.style.boxShadow="3px 3px 8px rgba(0,0,0,0.1),-2px -2px 6px rgba(255,255,255,0.7)")},startWhisper(){this._hideActionPanels();const e=document.getElementById("memberModalWhisperPanel");e&&(e.style.display="block",document.getElementById("memberModalWhisperText")?.focus())},cancelWhisper(){const e=document.getElementById("memberModalWhisperPanel");if(!e)return;e.style.display="none";const t=document.getElementById("memberModalWhisperText");t&&(t.value="")},async sendWhisper(){const t=document.getElementById("memberModalWhisperText")?.value.trim();if(!t){window.Core.showToast("Please write a message first");return}await this._withBtnState("#memberModalWhisperPanel button","Sending...","Send",async()=>{await d.sendWhisper(this.state.currentUserId,t)?(window.Core.showToast("Whisper sent"),this.cancelWhisper()):window.Core.showToast("Could not send - please try again")})},startReport(){this._hideActionPanels();const e=document.getElementById("memberModalReportPanel");e&&(e.style.display="block")},cancelReport(){const e=document.getElementById("memberModalReportPanel");if(!e)return;e.style.display="none";const t=document.getElementById("memberModalReportReason"),i=document.getElementById("memberModalReportDetails");t&&(t.value=""),i&&(i.value="")},async submitReport(){const e=document.getElementById("memberModalReportReason")?.value,t=document.getElementById("memberModalReportDetails")?.value.trim()||"";if(!e){window.Core.showToast("Please select a reason");return}await this._withBtnState("#memberModalReportPanel button","Submitting...","Submit Report",async()=>{await d.submitReport(this.state.currentUserId,e,t)?(window.Core.showToast("Report submitted - thank you"),this.cancelReport()):window.Core.showToast("Could not submit - please try again")})},async startBlock(){const e=document.getElementById("memberModalName")?.textContent||"this member";if(confirm(`Block ${e}? Their content will be hidden from you.`))try{await d.blockUser(this.state.currentUserId)?(window.Core.showToast(`${e} blocked`),this.close(),window.ActiveMembers?.refresh()):window.Core.showToast("Could not block - please try again")}catch(t){console.error("[MemberProfileModal] blockUser error:",t),window.Core.showToast("Could not block - please try again")}},_toggleAdminPanel(){const e=document.getElementById("memberModalAdminBody"),t=document.getElementById("memberModalAdminToggle");if(!e)return;const i=e.style.display!=="none";e.style.display=i?"none":"block",t.textContent=i?"▶":"▼",i&&this._closeAdminSubs()},_openAdminSub(e){this._closeAdminSubs();const t=e.charAt(0).toUpperCase()+e.slice(1),i=document.getElementById(`adminSub${t}`);i&&(i.style.display="block")},_closeAdminSubs(){for(const e of this._ADMIN_SUB_IDS){const t=document.getElementById(e);t&&(t.style.display="none")}},async _adminChangeRole(){const e=document.getElementById("adminRoleSelect")?.value;!e||!this.state.currentUserId||await this._withBtnState("#adminSubRole button","Saving...","Save Role",async()=>{const t={community_role:e};t.is_vip=e==="VIP",t.is_admin=e==="Admin";const{error:i}=await d._sb.rpc("admin_update_profile",{target_user_id:this.state.currentUserId,new_role:t.community_role,new_is_vip:t.is_vip,new_is_admin:t.is_admin});if(i)throw i;this._setHTML("memberModalRole",`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> ${e}`),await this._adminPushNotify(this.state.currentUserId,"👤 Role Updated",`Your community role has been changed to ${e}.`),window.Core.showToast(`Role changed to ${e}`),this._closeAdminSubs(),await this._safeRefresh(this.state.currentUserId)})},async _adminSendXP(){const e=parseInt(document.getElementById("adminXpAmount")?.value,10);if(!e||e<1){window.Core.showToast("Enter a valid XP amount");return}await this._withBtnState("#adminSubXp button","Sending...","Send XP",async()=>{if(!await d.adminUpdateGamification(this.state.currentUserId,{xpDelta:e}))throw new Error("Save failed");await this._adminPushNotify(this.state.currentUserId,"🎁 Gift from Aanandoham!",`You received +${e} XP!`),window.Core.showToast(`Sent ${e} XP`),this._closeAdminSubs(),await this._safeRefresh(this.state.currentUserId)})},async _adminSendKarma(){const e=parseInt(document.getElementById("adminKarmaAmount")?.value,10);if(!e||e<1){window.Core.showToast("Enter a valid Karma amount");return}await this._withBtnState("#adminSubKarma button","Sending...","Send Karma",async()=>{if(!await d.adminUpdateGamification(this.state.currentUserId,{karmaDelta:e}))throw new Error("Save failed");await this._adminPushNotify(this.state.currentUserId,"🎁 Gift from Aanandoham!",`You received +${e} Karma!`),window.Core.showToast(`Sent ${e} Karma`),this._closeAdminSubs(),await this._safeRefresh(this.state.currentUserId)})},async _adminSendBadge(){const t=document.getElementById("adminBadgeSelect")?.selectedOptions[0];if(!t)return;const o=(window.BADGE_REGISTRY||[]).find(r=>r.id===t.value)||{},n={id:t.value,name:o.name||t.textContent.replace(/^.+? /,"").trim(),icon:o.icon||t.dataset.icon||"🏅",rarity:o.rarity||t.dataset.rarity||"common",xp:o.xp??parseInt(t.dataset.xp,10)??0,description:o.desc||t.dataset.desc||""};await this._withBtnState("#adminSubBadge button","Awarding...","Award Badge",async()=>{if(!await d.adminUpdateGamification(this.state.currentUserId,{badgeId:n.id,badgeName:n.name,badgeIcon:n.icon,badgeRarity:n.rarity,badgeXp:n.xp,badgeDesc:n.description}))throw new Error("Save failed");await this._adminPushNotify(this.state.currentUserId,"🎖️ New Badge Earned!",`You received the ${n.name} badge!`),window.Core.showToast(`Awarded ${n.icon} ${n.name}`),this._closeAdminSubs(),await this._safeRefresh(this.state.currentUserId)})},async _adminSendCustomBadge(){const e=document.getElementById("adminCustomBadgeIcon")?.value.trim()||"🏅",t=document.getElementById("adminCustomBadgeName")?.value.trim(),i=document.getElementById("adminCustomBadgeDesc")?.value.trim()||"",o=document.getElementById("adminCustomBadgeRarity")?.value||"epic",n=parseInt(document.getElementById("adminCustomBadgeXp")?.value,10)||0,r=parseInt(document.getElementById("adminCustomBadgeKarma")?.value,10)||0;if(!t){window.Core.showToast("Please enter a badge name");return}const a="custom_"+t.toLowerCase().replace(/\s+/g,"_").replace(/[^a-z0-9_]/g,"")+"_"+Date.now();await this._withBtnState("#adminSubCustomBadge button","Awarding...","Award Custom Badge",async()=>{if(!await d.adminUpdateGamification(this.state.currentUserId,{badgeId:a,badgeName:t,badgeIcon:e,badgeRarity:o,badgeXp:n,badgeDesc:i}))throw new Error("Save failed");r>0&&await d.adminUpdateGamification(this.state.currentUserId,{karmaDelta:r}),await this._adminPushNotify(this.state.currentUserId,"🎖️ Special Badge Earned!",`You received the ${t} badge!`),window.Core.showToast(`Awarded custom badge: ${e} ${t}`),this._closeAdminSubs(),await this._safeRefresh(this.state.currentUserId)})},async _adminUnlockPremium(){const e=[...document.querySelectorAll("#adminSubPremium input[type=checkbox]:checked")].map(t=>t.value);if(!e.length){window.Core.showToast("Select at least one feature");return}await this._withBtnState("#adminSubPremium button","Unlocking...","Unlock Selected",async()=>{let t=0;const i=[];for(const n of e)await d.adminUpdateGamification(this.state.currentUserId,{unlockFeature:n})?t++:i.push(n);if(i.length&&console.warn("[AdminPanel] Failed to unlock:",i),!t)throw new Error("All unlocks failed");const o=e.map(n=>n.replace(/_/g," ").replace(/\b\w/g,r=>r.toUpperCase())).join(", ");await this._adminPushNotify(this.state.currentUserId,"🔓 New Features Unlocked!",`Admin unlocked: ${o}`),window.Core.showToast(`Unlocked ${t}/${e.length} feature(s)${i.length?` (${i.length} failed)`:""}`),this._closeAdminSubs(),await this._safeRefresh(this.state.currentUserId)})},async _adminSendMessage(){const e=document.getElementById("adminMessageTitle")?.value.trim(),t=document.getElementById("adminMessageContent")?.value.trim();if(!e){window.Core.showToast("Please enter a message title");return}if(!t){window.Core.showToast("Please enter a message");return}await this._withBtnState("#adminSubMessage button","Sending...","Send Message",async()=>{const{data:i}=await d._sb.from("user_progress").select("payload").eq("user_id",this.state.currentUserId).single(),o=i?.payload||{},n=o.adminMessages||[];n.push({id:Date.now()+Math.random(),title:e,content:t,date:new Date().toISOString(),read:!1});const{error:r}=await d._sb.from("user_progress").update({payload:{...o,adminMessages:n},updated_at:new Date().toISOString()}).eq("user_id",this.state.currentUserId);if(r)throw r;const a=t.length>80?t.slice(0,80)+"...":t;await this._adminPushNotify(this.state.currentUserId,`💬 ${e}`,a),window.Core.showToast("Message sent"),this._closeAdminSubs(),document.getElementById("adminMessageTitle").value="",document.getElementById("adminMessageContent").value=""})},async _safeRefresh(e){const t=window.Core.state.currentUser?.id;if(e===t){const o=window.app?.gamification;o?.saveTimeout&&(clearTimeout(o.saveTimeout),o.saveTimeout=null),window.DB?.clearCache?.(),window.app?.state?.clearCache?.();try{const n=await d.getUserProgress(e);n&&o?.state&&(n.xp!==void 0&&(o.state.xp=n.xp),n.karma!==void 0&&(o.state.karma=n.karma),n.level!==void 0&&(o.state.level=n.level),Array.isArray(n.badges)&&(o.state.badges=n.badges),Array.isArray(n.unlockedFeatures)&&(o.state.unlockedFeatures=n.unlockedFeatures)),n&&window.app?.state?.data&&(n.xp!==void 0&&(window.app.state.data.xp=n.xp),n.karma!==void 0&&(window.app.state.data.karma=n.karma),n.level!==void 0&&(window.app.state.data.level=n.level),Array.isArray(n.badges)&&(window.app.state.data.badges=n.badges),Array.isArray(n.unlockedFeatures)&&(window.app.state.data.unlockedFeatures=n.unlockedFeatures))}catch(n){console.warn("[_safeRefresh] pre-patch failed:",n)}}await this._refreshMainProfileStats(e)},async _refreshMainProfileStats(e){try{const t=window.Core.state.currentUser?.id;if(!t)return;const i=e||t,o=await d.getUserProgress(i);if(!o)return;if(i===t){o.xp!==void 0&&(window.Core.state.currentUser.xp=o.xp),o.karma!==void 0&&(window.Core.state.currentUser.karma=o.karma),o.level!==void 0&&(window.Core.state.currentUser.level=o.level),await window.app?.gamification?.reloadFromDatabase?.();const n=document.getElementById("profileGamificationXP");n&&o.xp!==void 0&&(n.textContent=o.xp.toLocaleString()),window.ProfileModule?.refreshGamification?.()}this._populateGamification(o)}catch(t){console.warn("[AdminPanel] _refreshMainProfileStats:",t)}},async _adminPushNotify(e,t,i){try{const o=await fetch("/api/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:e,payload:{title:t,body:i,icon:"/icons/icon-192x192.png",data:{url:"/"}}})});o.ok||console.warn(`[MemberProfileModal] push notify non-200: ${o.status}`)}catch(o){console.warn("[MemberProfileModal] push notify unavailable:",o.message)}},_hideActionPanels(){["memberModalWhisperPanel","memberModalReportPanel"].forEach(o=>{const n=document.getElementById(o);n&&(n.style.display="none")});const e=document.getElementById("memberModalWhisperText");e&&(e.value="");const t=document.getElementById("memberModalReportReason"),i=document.getElementById("memberModalReportDetails");t&&(t.value=""),i&&(i.value=""),this._closeAdminSubs()},_setText(e,t){const i=document.getElementById(e);i&&(i.textContent=t)},_setHTML(e,t){const i=document.getElementById(e);i&&(i.innerHTML=t)},async _withBtnState(e,t,i,o){const n=document.querySelector(e);n&&(n.disabled=!0,n.textContent=t);try{await o()}catch(r){console.error(`[AdminPanel] ${i} error:`,r),window.Core.showToast(`Could not complete: ${i}`)}finally{n&&(n.disabled=!1,n.textContent=i)}},_countryFlag(e){const t=this._COUNTRY_CODES[e.toLowerCase().trim()];return t?[...t].map(i=>String.fromCodePoint(127462+i.charCodeAt(0)-65)).join(""):"🌍"},_esc(e){if(!e||typeof e!="string")return"";const t=document.createElement("div");return t.textContent=e,t.innerHTML}};window.MemberProfileModal=F;const _e=Object.freeze(Object.defineProperty({__proto__:null,MemberProfileModal:F},Symbol.toStringTag,{value:"Module"})),M={_instances:new Set,_subscribed:!1,_retryTimer:null,_blockedCache:null,register(e){this._instances.add(e),this._subscribed||this._subscribe()},unregister(e){this._instances.delete(e),this._instances.size===0&&(this._unsubscribe(),this._blockedCache=null)},async getBlocked(){return this._blockedCache||(this._blockedCache=d.getBlockedUsers().catch(()=>new Set)),this._blockedCache},invalidateBlocked(){this._blockedCache=null},_subscribe(){if(!d.ready){this._retryTimer=setInterval(()=>{d.ready&&(clearInterval(this._retryTimer),this._retryTimer=null,this._doSubscribe())},300);return}this._doSubscribe()},_doSubscribe(){const e=d.subscribeToPresence(async t=>{const i=await this.getBlocked(),o=t.filter(n=>!i.has(n.user_id));this._instances.forEach(n=>n._onPresenceUpdate(o))});this._subscribed=!!e},_unsubscribe(){clearInterval(this._retryTimer),this._retryTimer=null,this._subscribed=!1}},oe=new Set(["online","available","away","guiding","silent","deep","offline"]),G={online:"online",available:"online",away:"away",guiding:"away",silent:"silent",deep:"deep",offline:"offline"},ne=1e4,re=150;class q{constructor(t){if(!(t instanceof HTMLElement))throw new TypeError("[ActiveMembersWidget] containerEl must be an HTMLElement");this.container=t,this.isRendered=!1,this._destroyed=!1}render(){this.container.innerHTML=this._buildShell("Loading..."),this._waitForDB().then(()=>Promise.all([d.getActiveMembers(),M.getBlocked()])).then(([t,i])=>{if(this._destroyed)return;const o=t.filter(n=>!i.has(n.user_id));this._paint(o),M.register(this),this.isRendered=!0}).catch(t=>{this._destroyed||(console.error("[ActiveMembersWidget] render error:",t),this.container.innerHTML=this._buildShell("Could not load members."))})}async refresh(){this.isRendered=!1,this.render()}updateMemberStatus(t,i){if(!oe.has(i))return;const n=this.container.querySelector(`[data-member-id="${t}"]`)?.querySelector(".member-mini-status");n&&(["online","away","offline","silent","deep"].forEach(r=>n.classList.remove(r)),n.classList.add(G[i]||"offline"),n.setAttribute("aria-label",i),n.setAttribute("title",V(i)))}updateMemberActivity(t,i){if(!i||typeof i!="string")return;const o=this.container.querySelector(`[data-member-id="${t}"] .member-mini-info`);o&&(o.textContent=i)}updateMemberAvatar(t,{emoji:i,avatarUrl:o}={}){const r=this.container.querySelector(`[data-member-id="${t}"]`)?.querySelector(".member-mini-avatar");r&&(o?(r.style.background="transparent",r.innerHTML=`<img src="${o}"
                style="width:100%;height:100%;object-fit:cover;border-radius:inherit;"
                alt="" loading="lazy" decoding="async">`):i&&(r.style.background="",r.innerHTML=`<span class="member-avatar-icon">${B(i)}</span>`))}destroy(){this._destroyed=!0,M.unregister(this),this.isRendered=!1}_waitForDB(){return d.ready?Promise.resolve():new Promise((t,i)=>{const o=Date.now(),n=setInterval(()=>{this._destroyed?(clearInterval(n),i(new Error("widget destroyed"))):d.ready?(clearInterval(n),t()):Date.now()-o>ne&&(clearInterval(n),i(new Error("CommunityDB not ready after timeout")))},re)})}_onPresenceUpdate(t){this._paint(t)}_paint(t){const i=t.filter(r=>r.status==="online"||r.status==="available").length,o=this.container.querySelector(".active-members-online-count"),n=this.container.querySelector(".active-members-grid");o&&n?(o.textContent=`${i} online`,n.innerHTML=j(t)):this.container.innerHTML=this._buildShell(`${i} online`,t)}_buildShell(t,i=null){const o=i===null?"":`
            ...
            <section class="section" aria-labelledby="activeMembersTitle-${this._uid}">
                ...
                ${o}
            </section>`}get _uid(){return this.__uid||(this.__uid=Math.random().toString(36).slice(2,7)),this.__uid}}function j(e){return e?.length?e.map(ae).join(""):'<div style="color:var(--text-muted);font-size:13px;padding:12px">No members online</div>'}function ae(e){if(!e)return"";const t=e.profiles||{},i=t.name||"Member",o=t.emoji||"",n=t.avatar_url||"",r=e.status||"online",a=e.activity||"✨ Available",s=e.user_id,l=window.Core?.getAvatarGradient(s||i),c=R(i),u=G[r]||"offline",p=n?`<img src="${n}"
               style="width:100%;height:100%;object-fit:cover;border-radius:inherit;"
               alt="${c}" loading="lazy">`:o?`<span class="member-avatar-icon">${B(o)}</span>`:`<span>${R(i.charAt(0).toUpperCase())}</span>`;return`
        <div class="member-card-mini"
             onclick="window._activeMembersHandleView('${s}')"
             data-member-id="${s}"
             role="button"
             tabindex="0"
             aria-label="View ${c}'s profile"
            ...
            <div class="member-mini-status ${u}"
                 aria-label="${r}"
                 title="${V(r)}"></div>
            <div class="member-mini-name">${c}</div>
            <div class="member-mini-info">${R(a)}</div>
        </div>`}function R(e){if(!e||typeof e!="string")return"";const t=document.createElement("div");return t.textContent=e,t.innerHTML}function V(e){return!e||typeof e!="string"?"":e.charAt(0).toUpperCase()+e.slice(1)}window._activeMembersHandleView=function(e){e&&(window.MemberProfileModal?window.MemberProfileModal.open(e):window.Core?.showToast("Member profiles loading..."))};window._openWhisperModal=async function(){if(!window.WhisperModal)try{const e=await m(()=>Promise.resolve().then(()=>le),void 0);window.WhisperModal=e.WhisperModal??e.default}catch(e){console.warn("[ActiveMembers] WhisperModal failed to load:",e),window.Core?.showToast("Whispers unavailable — please try again.");return}window.WhisperModal?.open()};window.addEventListener("avatarChanged",e=>{const{userId:t,emoji:i,avatarUrl:o}=e.detail||{};t&&M._instances.forEach(n=>{n.updateMemberAvatar(t,{emoji:i,avatarUrl:o})})});window.ActiveMembers={async render(){M._instances.size>0&&M._instances.forEach(e=>e.refresh())},updateMemberStatus(e,t){M._instances.forEach(i=>i.updateMemberStatus(e,t))},updateMemberActivity(e,t){M._instances.forEach(i=>i.updateMemberActivity(e,t))},async refresh(){M._instances.forEach(e=>e.refresh())},get state(){return{isRendered:M._instances.size>0}}};const ke=Object.freeze(Object.defineProperty({__proto__:null,ActiveMembersWidget:q,PresenceManager:M},Symbol.toStringTag,{value:"Module"})),O=["Enter with intention, leave with gratitude","This space holds you. Enter with presence.","Breathe in. You are welcome here.","Leave the noise behind. Step into stillness.","You are exactly where you need to be.","Enter gently. This moment is yours.","Set down what you carry. Enter with an open heart.","The space is ready. So are you.","Come as you are. This is a place of welcome.","Arrive fully. Begin with intention."];class se{constructor(t){this.app=t,this.initialized=!1,this._activeMembersWidget=null}async render(){const t=document.getElementById("community-hub-tab");if(!t){console.error("[CommunityHub] Tab element not found");return}if(this.createFullscreenRoomContainer(),this.initialized||(t.innerHTML=this._buildTabHTML()),requestAnimationFrame(()=>requestAnimationFrame(()=>this._showRitualImmediately())),this.initialized?this._refreshHubPresence():(await this.initializeCommunityHub(),this.initialized=!0),this._attachHubVisibility(),window._pendingHubScrollTarget){const i=window._pendingHubScrollTarget;window._pendingHubScrollTarget=null,requestAnimationFrame(()=>requestAnimationFrame(()=>{const o=document.getElementById(i);o&&o.scrollIntoView({behavior:"smooth",block:"start"})}))}if(window._pendingRoomOpen){const i=window._pendingRoomOpen;window._pendingRoomOpen=null;const o=window[`${i}_enterRoom`];if(typeof o=="function")o();else{const n=r=>{r.detail?.roomKey===i&&(document.removeEventListener("practiceRoomReady",n),window[`${i}_enterRoom`]?.())};document.addEventListener("practiceRoomReady",n),setTimeout(()=>document.removeEventListener("practiceRoomReady",n),8e3)}}}_buildTabHTML(){return`
    ...
    `}_buildCandleSVG(t){return`
      ...
      </svg>`}_buildRitualCard({id:t,textId:i,type:o,action:n,label:r,buttonText:a}){return`
    ...
    `,document.body.appendChild(t),!document.getElementById("openingOverlay")){const i=document.createElement("div");i.innerHTML=this._buildRitualCard({id:"openingOverlay",textId:"openingRitualText",type:"opening",action:"ritual-opening",label:"Enter the space",buttonText:"Enter the Space"}),(document.getElementById("app-container")||document.body).appendChild(i.firstElementChild)}}_showRitualImmediately(){if(window.Rituals?.state?.hasSeenOpening)return;const t=document.getElementById("openingOverlay");if(!t)return;const i=document.getElementById("openingRitualText");i&&(i.textContent=`"${O[Math.floor(Math.random()*O.length)]}"`),document.body.classList.add("ritual-active"),t.classList.add("active"),setTimeout(()=>{window.Rituals?window.Rituals.completeOpening():(t.classList.remove("active"),document.body.classList.remove("ritual-active"))},5e3)}_refreshHubPresence(){try{this._activeMembersWidget?.refresh(),window.PracticeRoom?._hubRooms?.length&&PracticeRoom.startHubPresence(),window.CollectiveFieldDB?.refreshCount&&window.CollectiveFieldDB.refreshCount()}catch(t){console.warn("[CommunityHub] _refreshHubPresence fallback to Core.init",t),k.state.initialized=!1,k.init()}}_preloadYouTubeAPI(){if(window.YT?.Player||document.querySelector('script[src*="youtube.com/iframe_api"]'))return;const t=document.createElement("script");t.src="https://www.youtube.com/iframe_api",t.async=!0,document.head.appendChild(t)}_attachHubVisibility(){this._hubVisibilityHandler||(this._hubVisibilityHandler=()=>{if(document.hidden)this._hubChatRooms?.forEach(t=>{try{d.unsubscribeFromRoomChat(t)}catch{}});else{const t=k?.state?.currentRoom;t&&this._hubChatResubscribe&&this._hubChatResubscribe(t)}},document.addEventListener("visibilitychange",this._hubVisibilityHandler))}registerHubChatRooms(t,i){this._hubChatRooms=t,this._hubChatResubscribe=i}loadStylesheet(t,{critical:i=!1}={}){if(document.querySelector(`link[href="${t}"]`))return;const o=document.createElement("link");o.rel="stylesheet",o.href=t,i||(o.rel="preload",o.as="style",o.onload=function(){this.rel="stylesheet"}),document.head.appendChild(o)}loadScript(t){return new Promise((i,o)=>{if(document.querySelector(`script[src="${t}"]`))return i();const n=Object.assign(document.createElement("script"),{src:t,async:!0});n.onload=i,n.onerror=()=>o(new Error(`Failed to load: ${t}`)),document.body.appendChild(n)})}async _initAdminDashboard(){const t=k?.state?.currentUser;if(!(!t?.isAdmin&&!t?.isModerator))try{await m(()=>Promise.resolve().then(()=>ce),void 0)}catch(i){console.warn("[CommunityHub] AdminDashboard failed to load:",i)}}async initializeCommunityHub(){try{if(this._preloadYouTubeAPI(),await Promise.all([m(()=>Promise.resolve().then(()=>ue),void 0),m(()=>Promise.resolve().then(()=>pe),void 0),m(()=>Promise.resolve().then(()=>me),void 0),m(()=>Promise.resolve().then(()=>he),void 0),m(()=>Promise.resolve().then(()=>ge),void 0),m(()=>Promise.resolve().then(()=>fe),void 0)]),await Promise.all([m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.Y),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.C),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.a),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.S),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.T),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.b),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.c),[])]),await m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.P),[]),!k?.init)throw new Error("Core module not found");await d.init(),window.CommunityDB=d,await k.init(),window.Core=k;const t=document.getElementById("activeMembersContainer");if(t&&(this._activeMembersWidget?.destroy(),this._activeMembersWidget=new q(t),this._activeMembersWidget.render()),await Promise.all([Promise.all([m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.s),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.g),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.o),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.d),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.e),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.f),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.t),[]),m(()=>import("./community-rooms-D7xiuO35.js").then(i=>i.r),[])]),m(()=>import("./community-lunar-DnMy1h9w.js").then(i=>i.l),__vite__mapDeps([0,1])).then(()=>m(()=>import("./community-lunar-DnMy1h9w.js").then(i=>i.a),__vite__mapDeps([0,1]))).then(()=>m(()=>import("./community-lunar-DnMy1h9w.js").then(i=>i.b),__vite__mapDeps([0,1]))).then(()=>m(()=>import("./community-lunar-DnMy1h9w.js").then(i=>i.c),__vite__mapDeps([0,1]))),m(()=>import("./community-solar-B-aV0uDf.js").then(i=>i.s),__vite__mapDeps([2,1])).then(()=>m(()=>import("./community-solar-B-aV0uDf.js").then(i=>i.a),__vite__mapDeps([2,1]))).then(()=>m(()=>import("./community-solar-B-aV0uDf.js").then(i=>i.c),__vite__mapDeps([2,1]))).then(()=>m(()=>import("./community-solar-B-aV0uDf.js").then(i=>i.b),__vite__mapDeps([2,1]))),m(()=>Promise.resolve().then(()=>be),void 0),m(()=>Promise.resolve().then(()=>ye),void 0),this._initAdminDashboard()]),window.LunarEngine?.init?.(),window.CollectiveField?.render(),window.Resonance?.render(),window.UpcomingEvents?.render(),window.ProfileModule){window.ProfileModule.state.isInitialized=!1;try{window.ProfileModule.init()}catch(i){console.warn("[CommunityHub] ProfileModule re-init:",i)}}if(window.CommunityModule){window.CommunityModule.state.isInitialized=!1;try{window.CommunityModule.init()}catch(i){console.warn("[CommunityHub] CommunityModule re-init:",i)}}if(window.Rituals&&(window.Rituals.state.hasSeenOpening=!1),window.CollectiveFieldDB&&await window.CollectiveFieldDB.init(),window._pendingHubScrollTarget){const i=window._pendingHubScrollTarget;window._pendingHubScrollTarget=null,requestAnimationFrame(()=>requestAnimationFrame(()=>{const o=document.getElementById(i);o&&o.scrollIntoView({behavior:"smooth",block:"start"})}))}}catch(t){console.error("❌ Failed to load Community Hub:",t);const i=document.getElementById("community-hub-main-content");i&&(i.innerHTML=`
          <div class="card" style="text-align:center; padding:var(--spacing-xl)">
          ...
          </div>`)}}destroy(){this._activeMembersWidget&&(this._activeMembersWidget.destroy(),this._activeMembersWidget=null),this._hubVisibilityHandler&&(document.removeEventListener("visibilitychange",this._hubVisibilityHandler),this._hubVisibilityHandler=null)}}const Me=Object.freeze(Object.defineProperty({__proto__:null,default:se},Symbol.toStringTag,{value:"Module"})),Y={state:{isOpen:!1,view:"inbox",threadPartnerId:null,threadPartnerName:null,realtimeSub:null,bgSub:null},init(){if(document.getElementById("whisperModal"))return;const e=document.createElement("div");e.innerHTML=`
            <div id="whisperModal"
            ...
            </div>`,document.body.appendChild(e.firstElementChild),document.getElementById("whisperModal").addEventListener("click",i=>{i.target.id==="whisperModal"&&this.close()}),document.addEventListener("keydown",i=>{i.key==="Escape"&&this.state.isOpen&&this.close()});const t=()=>{d?.ready?this.startBackgroundListener():setTimeout(t,500)};t()},async open(){this._openShared(),await this._showInbox()},async openThread(e,t,i,o){this._openShared(),await this._showThread(e,t,i,o)},_openShared(){this.init(),this._animateIn(),this.state.isOpen=!0,document.body.style.overflow="hidden",this._subscribeRealtime()},close(){const e=document.getElementById("whisperModal"),t=document.getElementById("whisperModalInner");e&&(e.style.opacity="0",t.style.transform="translateY(20px)",setTimeout(()=>{e.style.display="none",this.state.isOpen=!1,this.state.view="inbox",this.state.threadPartnerId=null,document.body.style.overflow="",this.state.realtimeSub?.unsubscribe?.(),this.state.realtimeSub=null},250))},_animateIn(){const e=document.getElementById("whisperModal"),t=document.getElementById("whisperModalInner");e.style.display="flex",requestAnimationFrame(()=>{e.style.opacity="1",t.style.transform="translateY(0)"})},_setView(e){const t=e==="inbox";document.getElementById("whisperInboxView").style.display=t?"block":"none",document.getElementById("whisperThreadView").style.display=t?"none":"flex",document.getElementById("whisperReplyBar").style.display=t?"none":"block",document.getElementById("whisperBackBtn").style.display=t?"none":"inline-flex";const i=document.getElementById("whisperModalSubtitle");t&&(document.getElementById("whisperModalTitle").textContent="Whispers",i.style.display="none")},async _showInbox(){this.state.view="inbox",this.state.threadPartnerId=null,this._setView("inbox");const e=document.getElementById("whisperInboxLoading"),t=document.getElementById("whisperInboxEmpty"),i=document.getElementById("whisperInboxList");e.style.display="block",t.style.display="none",i.innerHTML="";const o=await d.getWhisperInbox();if(e.style.display="none",!o.length){t.style.display="block";return}i.innerHTML=o.map(n=>this._conversationRowHTML(n)).join(""),this._setBadge(o.reduce((n,r)=>n+r.unread,0))},_conversationRowHTML(e){const t=e.partner||{},i=this._escape(t.name||"Member"),o=this._avatarHTML(t,44),n=this._escape(e.lastMessage||""),r=this._relativeTime(e.lastAt),a=this._escape(t.id||""),s=this._escape(t.emoji||""),l=this._escape(t.avatar_url||""),c=e.unread>0;return`
            <div data-partner-id="${a}"
                ...
                <div style="position:relative;flex-shrink:0;">
                    ${o}
                    ${c?`<span style="position:absolute;top:-2px;right:-2px;
                ...
                <div style="flex:1;min-width:0;">
                    <div style="display:flex;justify-content:space-between;
                        ...
                        <span class="whisper-partner-name"
                              style="font-weight:${c?"700":"500"};font-size:0.9rem;
                    ...
                    <div style="font-size:0.8rem;color:var(--text-muted);
                                white-space:nowrap;overflow:hidden;text-overflow:ellipsis;
                                font-weight:${c?"600":"400"};">
                        ${n}
                    </div>
                </div>

                <span class="whisper-unread-badge"
                      style="display:${c?"inline-flex":"none"};
            ...
            </div>`},async _showThread(e,t,i,o){this.state.view="thread",this.state.threadPartnerId=e,this.state.threadPartnerName=t,this._setView("thread"),document.getElementById("whisperModalTitle").textContent=t;const n=document.getElementById("whisperModalSubtitle");n.textContent="Private whisper thread",n.style.display="block";const r=document.getElementById("whisperThreadLoading"),a=document.getElementById("whisperThreadMessages"),s=document.getElementById("whisperThreadView");r.style.display="block",a.innerHTML="";const[l]=await Promise.all([d.getWhispers(e),d.markConversationRead(e)]);r.style.display="none",this._renderThreadMessages(l),setTimeout(()=>{s.scrollTop=s.scrollHeight},50),document.getElementById("whisperReplyText")?.focus(),await this.refreshUnreadBadge()},_renderThreadMessages(e){const t=document.getElementById("whisperThreadMessages");if(!t)return;if(!e.length){t.innerHTML=`
                <div style="text-align:center;padding:3rem;
                            color:var(--text-muted);font-size:0.85rem;">
                    No messages yet — say something ✨
                </div>`;return}const i=d._uid;t.innerHTML=e.map(o=>{const n=o.sender_id===i||o.sender?.id===i;return this._messageBubbleHTML(n,this._escape(o.message),this._relativeTime(o.created_at))}).join("")},_appendMessage(e){const t=document.getElementById("whisperThreadMessages");if(!t)return;t.querySelector("[data-empty]")?.remove();const i=e.sender_id===d._uid,o=document.createElement("div");o.innerHTML=this._messageBubbleHTML(i,this._escape(e.message),this._relativeTime(e.created_at)),t.appendChild(o.firstElementChild),document.getElementById("whisperThreadView")?.scrollTo({top:999999})},_messageBubbleHTML(e,t,i){return`
        ...
        `,document.head.appendChild(e)},injectAdminUI(){window.Core?.state?.currentUser?.is_admin&&this.injectBadge()},injectBadge(){if(document.getElementById("adminDashBadge")||!window.Core?.state?.currentUser?.is_admin)return;this._injectStyles();const e=document.createElement("div");e.id="adminDashBadge",e.style.cssText="padding:0 0 32px;",e.innerHTML=`
        ...
        <div style="margin-bottom:12px;">
            ...
            ${t.map(o=>`
                <div class="admin-notif-row ${o.read?"":"unread"}" id="adminNotif_${o.id}">
                    ...
                    <div style="flex:1;min-width:0;">
                        ...
                        <div style="font-size:0.82rem;color:var(--text-muted,#888);">
                            From: <strong>${f(o.payload?.sender_name||"Unknown")}</strong>
                            ${o.payload?.room?`· ${f(o.payload.room)}`:""}
                        </div>
                        ${o.payload?.message?`<div style="font-size:0.82rem;margin-top:4px;font-style:italic;">"${f(o.payload.message)}"</div>`:""}
                        ${o.payload?.reason?`<div style="font-size:0.82rem;margin-top:4px;">Reason: ${f(o.payload.reason)}</div>`:""}
                        ${o.payload?.details?`<div style="font-size:0.82rem;color:var(--text-muted,#888);">${f(o.payload.details)}</div>`:""}
                        ${o.payload?.issueType?`<div style="font-size:0.82rem;margin-top:4px;">Type: ${f(o.payload.issueType)}</div>`:""}
                        ${o.payload?.description?`<div style="font-size:0.82rem;color:var(--text-muted,#888);">${f(o.payload.description)}</div>`:""}
                    </div>
                    ${o.read?"":`
                ...
                </div>`).join("")}`},async _renderMembers(e){const[t,i,{data:o=[]}]=await Promise.all([CommunityDB.getAdminMemberStats(),CommunityDB.getActiveMembers(),CommunityDB._sb.from("profiles").select("id, name, emoji, avatar_url, community_status").order("name")]),n=Object.fromEntries(i.map(l=>[l.user_id,l])),r=new Set(i.map(l=>l.user_id)),a=o.filter(l=>!r.has(l.id)),s=(l,c)=>{const u=c?.status||"offline",p=c?.activity||"💤 Offline",h=u==="online"?"#22c55e":u==="away"?"#f59e0b":"#aaa",v=l.avatar_url?`<img src="${f(l.avatar_url)}" alt="Member avatar" width="40" height="40" style="width:100%;height:100%;object-fit:cover;" loading="lazy" decoding="async">`:f(l.emoji||(l.name||"?").charAt(0));return`
                <div style="display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:10px;
                    ...
                    <div style="flex:1;min-width:0;">
                        <div style="font-size:0.85rem;font-weight:600;">${f(l.name||"Member")}</div>
                        <div style="font-size:0.75rem;color:var(--text-muted,#888);">${f(p)}</div>
                    </div>
            ...
            ${a.map(l=>s(l,n[l.id]||null)).join("")}`},async _renderEngagement(e){const t=await CommunityDB.getAdminEngagementStats();e.innerHTML=`
            ...
            <div class="admin-stat-grid">
            ...
            </div>`},async _renderLeaderboard(e){const t=await CommunityDB.getLeaderboard(),i=(o,n)=>o.length?o.map((r,a)=>`
                <div style="display:flex;align-items:center;gap:8px;padding:7px 10px;border-radius:8px;
                    ...
                    <span style="flex:1;font-size:0.85rem;font-weight:600;">${f(r.profiles?.name||"Member")}</span>
                    <span style="font-size:0.85rem;font-weight:700;color:var(--neuro-accent);">${r.payload?.[n]||0}</span>
            ...
            </table>`},async _renderRetention(e){const t=await CommunityDB.getRetentionSignals();e.innerHTML=`
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div>
                    <div style="font-size:0.78rem;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;
                ...
                <div>
                    ...
                    ${t.streakMembers?.length?t.streakMembers.map(i=>`
                            <div style="display:flex;align-items:center;gap:8px;padding:6px 10px;border-radius:8px;
                                 ...
                                 onclick="MemberProfileModal?.open('${i.user_id}')">
                                <span>${i.emoji||"🧘"}</span>
                                <span style="font-size:0.83rem;font-weight:600;">${f(i.name||"Member")}</span>
            ...
            <label id="bulkRow_${o.id}"
                ...
                <span style="font-size:0.85rem;font-weight:600;color:var(--neuro-text);">${f(o.name||"Member")}</span>
                <span style="font-size:0.75rem;color:var(--text-muted,#888);margin-left:auto;">${f(o.community_role||"Member")}</span>
            ...
            <div style="margin-bottom:16px;">
                ...
                <select id="bulkBadgeSelect" style="${t}">
                    ${P.map(l=>`<option value="${l.id}" data-icon="${l.icon}" data-rarity="${l.rarity}" data-xp="${l.xp}" data-desc="${l.desc}">${l.icon} ${l.name}</option>`).join("")}
                </select>
                ...
                <select id="bulkUnlockSelect" style="${t}">
                    ${this._UNLOCKS.map(([l,c])=>`<option value="${l}">${c}</option>`).join("")}
                </select>
                ...
                <button onclick="AdminDashboard._bulkSendMessage()" style="${i}">Send Message</button>`},s=document.getElementById("bulkActivePanel");s&&a[e]&&(s.innerHTML=a[e],this._bulkRendered.add(e))},_bulkToggle(e,t){this._bulkSelected[t?"add":"delete"](e),this._bulkUpdateCount()},_bulkSelectAll(){for(const e of this._bulkMembers){this._bulkSelected.add(e.id);const t=document.querySelector(`#bulkRow_${e.id} input[type=checkbox]`);t&&(t.checked=!0)}this._bulkUpdateCount()},_bulkSelectNone(){this._bulkSelected.clear(),document.querySelectorAll("#bulkMemberList input[type=checkbox]").forEach(e=>e.checked=!1),this._bulkUpdateCount()},_bulkUpdateCount(){const e=this._bulkSelected.size,t=document.getElementById("bulkSelectedCount");t&&(t.textContent=`${e} member${e!==1?"s":""} selected`)},_bulkFilterMembers(e){clearTimeout(this._filterTimer),this._filterTimer=setTimeout(()=>{const t=e.toLowerCase(),i=document.querySelectorAll("#bulkMemberList label"),o=[];i.forEach(n=>{const r=n.querySelector("span:nth-child(3)")?.textContent?.toLowerCase()||"";o.push({row:n,show:r.includes(t)})}),o.forEach(({row:n,show:r})=>{n.style.display=r?"":"none"})},120)},_bulkShowTab(e){for(const[t]of this._BULK_TABS){const i=document.getElementById(`bulkTab_${t}`);if(!i)continue;const o=t===e;i.style.background=o?"var(--neuro-accent-a20)":"rgba(0,0,0,0.05)",i.style.color=o?"var(--neuro-accent)":"var(--text-muted,#888)"}this._bulkRendered.delete(e),this._bulkRenderTabContent(e)},_bulkGuard(){return this._bulkSelected.size===0?(window.Core.showToast("Select at least one member first"),!1):!0},async _bulkSendGamification({inputId:e,label:t,xpDelta:i=0,karmaDelta:o=0,notifTitle:n,notifBody:r}){if(!this._bulkGuard())return;const a=parseInt(document.getElementById(e)?.value,10);if(!a||a<1){window.Core.showToast(`Enter a valid ${t} amount`);return}const s=[...this._bulkSelected];window.Core.showToast(`Sending ${a} ${t} to ${s.length} members...`);let l=0;for(const c of s)try{await CommunityDB.adminUpdateGamification(c,{xpDelta:i==="amount"?a:i,karmaDelta:o==="amount"?a:o})?(l++,await this._pushNotify(c,n,r(a))):console.warn("[AdminDashboard] _bulkSendGamification: no success for",c)}catch(u){console.error("[AdminDashboard] _bulkSendGamification error for",c,u)}window.Core.showToast(`Sent ${a} ${t} to ${l}/${s.length} members`)},async _bulkSendXP(){await this._bulkSendGamification({inputId:"bulkXpAmount",label:"XP",xpDelta:"amount",notifTitle:"🎁 Gift from Aanandoham!",notifBody:e=>`You received +${e} XP!`})},async _bulkSendKarma(){await this._bulkSendGamification({inputId:"bulkKarmaAmount",label:"Karma",karmaDelta:"amount",notifTitle:"🎁 Gift from Aanandoham!",notifBody:e=>`You received +${e} Karma!`})},async _bulkSendBadge(){if(!this._bulkGuard())return;const t=document.getElementById("bulkBadgeSelect")?.value;if(!t){window.Core.showToast("Select a badge");return}const i=P.find(u=>u.id===t)||{},o=i.name||t,n=i.icon||"🏅",r=i.rarity||"common",a=i.xp??0,s=i.desc||"",l=[...this._bulkSelected];window.Core.showToast(`Sending badge to ${l.length} members...`);let c=0;for(const u of l)try{await CommunityDB.adminUpdateGamification(u,{badgeId:t,badgeName:o,badgeIcon:n,badgeRarity:r,badgeXp:a,badgeDesc:s})?(c++,await this._pushNotify(u,"🏅 New Badge!",`You earned the ${n} ${o} badge!`)):console.warn("[AdminDashboard] _bulkSendBadge: no success for",u)}catch(p){console.error("[AdminDashboard] _bulkSendBadge error for",u,p)}window.Core.showToast(`Sent badge to ${c}/${l.length} members`)},async _bulkSendCustomBadge(){if(!this._bulkGuard())return;const e=document.getElementById("bulkCustomIcon")?.value.trim()||"🏅",t=document.getElementById("bulkCustomName")?.value.trim(),i=document.getElementById("bulkCustomDesc")?.value.trim()||"",o=document.getElementById("bulkCustomRarity")?.value||"epic",n=parseInt(document.getElementById("bulkCustomXp")?.value,10)||0,r=parseInt(document.getElementById("bulkCustomKarma")?.value,10)||0;if(!t){window.Core.showToast("Please enter a badge name");return}const a="custom_"+t.toLowerCase().replace(/\s+/g,"_").replace(/[^a-z0-9_]/g,"")+"_"+Date.now(),s=[...this._bulkSelected];window.Core.showToast(`Sending custom badge to ${s.length} members...`);let l=0;for(const c of s)try{await CommunityDB.adminUpdateGamification(c,{badgeId:a,badgeName:t,badgeIcon:e,badgeRarity:o,badgeXp:n,badgeDesc:i})?(r>0&&await CommunityDB.adminUpdateGamification(c,{karmaDelta:r}),l++,await this._pushNotify(c,"🎖️ Special Badge!",`You received the ${e} ${t} badge!`)):console.warn("[AdminDashboard] _bulkSendCustomBadge: no success for",c)}catch(u){console.error("[AdminDashboard] _bulkSendCustomBadge error for",c,u)}window.Core.showToast(`Sent custom badge to ${l}/${s.length} members`)},async _bulkSendUnlock(){if(!this._bulkGuard())return;const e=document.getElementById("bulkUnlockSelect"),t=e?.value,i=e?.options[e.selectedIndex]?.text||t;if(!t){window.Core.showToast("Select a feature");return}const o=[...this._bulkSelected];window.Core.showToast(`Unlocking ${i} for ${o.length} members...`);let n=0;for(const r of o)try{await CommunityDB.adminUpdateGamification(r,{unlockFeature:t})?(n++,await this._pushNotify(r,"🔓 Feature Unlocked!",`${i} has been unlocked for you!`)):console.warn("[AdminDashboard] _bulkSendUnlock: no success for",r)}catch(a){console.error("[AdminDashboard] _bulkSendUnlock error for",r,a)}window.Core.showToast(`Unlocked ${i} for ${n}/${o.length} members`)},async _bulkSendMessage(){if(!this._bulkGuard())return;const e=document.getElementById("bulkMessageText")?.value?.trim();if(!e){window.Core.showToast("Write a message first");return}const t=[...this._bulkSelected];window.Core.showToast(`Broadcasting to ${t.length} members...`);let i={sent:0};try{i=await CommunityDB.broadcastMessage(t,e)}catch(o){console.error("[AdminDashboard] _bulkSendMessage error:",o),window.Core.showToast("Failed to send messages");return}if(i.sent>0){for(const o of t)await this._pushNotify(o,"💬 Message from Aanandoham",e.substring(0,80));document.getElementById("bulkMessageText").value="",window.Core.showToast(`Message sent to ${i.sent}/${t.length} members`)}else window.Core.showToast("Failed to send messages")},async _markRead(e){await CommunityDB.markNotificationRead(e);const t=document.getElementById(`adminNotif_${e}`);t&&(t.classList.remove("unread"),t.querySelector("button")?.remove()),this._updateBadge()},async _markAllRead(){await CommunityDB.markAllNotificationsRead(),this._loadSection("notifications"),this._updateBadge()},async _deleteReflection(e,t){if(!confirm("Delete this reflection?"))return;t.disabled=!0,await CommunityDB.deleteReflection(e)?(t.closest(".admin-refl-row")?.remove(),window.Core.showToast("Reflection deleted")):(window.Core.showToast("Could not delete"),t.disabled=!1)},_statCard(e,t){return`
            <div class="admin-stat-card">
                <div class="admin-stat-value">${e}</div>
                <div class="admin-stat-label">${t}</div>
            </div>`},_timeAgo(e){if(!e)return"";const t=Date.now()-new Date(e).getTime(),i=Math.floor(t/6e4),o=Math.floor(t/36e5),n=Math.floor(t/864e5);return i<1?"just now":i<60?`${i}m ago`:o<24?`${o}h ago`:`${n}d ago`},_formatDuration(e){return e?e<60?`${e}s`:e<3600?`${Math.round(e/60)}m`:`${(e/3600).toFixed(1)}h`:"-"},_formatRoomId(e){return(e||"").replace(/-/g," ").replace(/\b\w/g,t=>t.toUpperCase())},_esc:f};window.AdminDashboard=X;const ce=Object.freeze(Object.defineProperty({__proto__:null,AdminDashboard:X},Symbol.toStringTag,{value:"Module"})),K={state:{hasSeenOpening:!1,isInitialized:!1,autoCloseTimer:null},config:{OPENING_AUTO_CLOSE_MS:5e3,CLOSING_AUTO_CLOSE_MS:3e3,OPENING_COOLDOWN_MS:1440*60*1e3,OPENING_TEXTS:["Enter with intention, leave with gratitude","This space holds you. Enter with presence.","Breathe in. You are welcome here.","Leave the noise behind. Step into stillness.","You are exactly where you need to be.","Enter gently. This moment is yours.","Set down what you carry. Enter with an open heart.","The space is ready. So are you.","Come as you are. This is a place of welcome.","Arrive fully. Begin with intention."],CLOSING_TEXTS:["Thank you for holding space with us","Carry the stillness with you as you go.","You showed up. That is enough.","May what was planted here continue to grow.","Go gently. You have done something meaningful.","The practice continues beyond this space.","Thank you for your presence in this circle.","Rest in what was received here today.","You are changed by having paused. Go well.","Until next time - carry the quiet with you."],ROOM_MODULES:["BreathworkRoom","SilentRoom","GuidedRoom","OshoRoom","DeepWorkRoom","CampfireRoom","TarotRoom","ReikiRoom","NewMoonRoom","WaxingMoonRoom","FullMoonRoom","WaningMoonRoom","SpringSolarRoom","SummerSolarRoom","AutumnSolarRoom","WinterSolarRoom"]},init(){if(this.state.isInitialized){console.warn("Rituals already initialized");return}try{this.loadState(),this.setupEventListeners(),this.state.isInitialized=!0}catch(e){console.error("Rituals initialization failed:",e)}},setupEventListeners(){document.addEventListener("keydown",e=>{if(e.key!=="Escape")return;const t=document.getElementById("openingOverlay"),i=document.getElementById("closingOverlay");t?.classList.contains("active")?this.completeOpening():i?.classList.contains("active")&&this.completeClosing()}),document.addEventListener("click",e=>{const t=e.target.closest("[data-action]")?.dataset.action;t==="ritual-opening"?this.completeOpening():t==="ritual-closing"&&this.completeClosing()})},loadState(){try{const e=localStorage.getItem("rituals_lastSeen");if(e){const t=Date.now()-parseInt(e,10);this.state.hasSeenOpening=t<this.config.OPENING_COOLDOWN_MS}}catch(e){console.error("Failed to load rituals state:",e)}},saveState(){try{localStorage.setItem("rituals_lastSeen",Date.now().toString())}catch(e){console.error("Failed to save rituals state:",e)}},completeOpening(){const e=document.getElementById("openingOverlay");e&&(clearTimeout(this.state.autoCloseTimer),this.state.autoCloseTimer=null,e.classList.remove("active"),document.body.classList.remove("ritual-active"),this.state.hasSeenOpening=!0,this.saveState(),k?.showToast?.("Welcome to the space"))},showClosing(){const e=document.getElementById("closingOverlay"),t=document.getElementById("communityHubFullscreenContainer");if(!e){console.warn("Closing overlay not found");return}const i=document.getElementById("closingRitualText");i&&(i.textContent=`"${this._randomText("CLOSING_TEXTS")}"`),document.body.classList.add("ritual-active"),t&&(t.style.display="block",t.style.pointerEvents="auto"),e.classList.add("active"),e.setAttribute("aria-hidden","false");const n=window.matchMedia("(prefers-reduced-motion: reduce)").matches?1e3:this.config.CLOSING_AUTO_CLOSE_MS;this.state.autoCloseTimer=setTimeout(()=>this.completeClosing(),n)},completeClosing(){const e=document.getElementById("closingOverlay"),t=document.getElementById("communityHubFullscreenContainer");if(!e){console.warn("Closing overlay not found");return}e.classList.remove("active"),e.setAttribute("aria-hidden","true"),document.body.classList.remove("ritual-active"),document.body.style.overflow="",clearTimeout(this.state.autoCloseTimer),this.state.autoCloseTimer=null,t&&(t.style.display="none"),this.cleanupActiveRoom(),k?.navigateTo?.("hubView"),k?.showToast?.("Space closed with gratitude")},cleanupActiveRoom(){const e=this.findActiveRoom();if(!e)return;const{module:t,name:i}=e;typeof t.leaveRoom=="function"?t.leaveRoom():typeof t.cleanup=="function"&&t.cleanup()},findActiveRoom(){if(window.currentSolarRoom)return{name:"CurrentSolarRoom",module:window.currentSolarRoom};if(window.currentLunarRoom)return{name:"CurrentLunarRoom",module:window.currentLunarRoom};const t=document.getElementById("dynamicRoomContent")?.children.length>0,i=!!document.querySelector(".ps-header");if(!t||!i)return null;for(const o of this.config.ROOM_MODULES){const n=window[o];if(!n?.roomId)continue;const{roomId:r}=n;if(document.getElementById(`${r}ParticipantStack`)||document.getElementById(`${r}View`)||document.getElementById(`${r}TimerDisplay`))return{name:o,module:n}}return null},getRoomViewId(e,t){return e.roomId?`${e.roomId}PracticeView`:e.viewId?e.viewId:`${t.toLowerCase().replace("room","")}PracticeView`},_randomText(e){const t=this.config[e];return t[Math.floor(Math.random()*t.length)]},reset(){this.state.hasSeenOpening=!1,localStorage.removeItem("rituals_lastSeen")},hasOverlay(e){return!!document.getElementById(`${e}Overlay`)}};window.Rituals=K;const ue=Object.freeze(Object.defineProperty({__proto__:null,Rituals:K},Symbol.toStringTag,{value:"Module"})),Z={state:{isInitialized:!1,appreciatedReflections:new Set,reportingUserId:null},config:{MIN_REFLECTION_LENGTH:1,MAX_REFLECTION_LENGTH:500,MIN_REPORT_LENGTH:10,MIN_MODERATOR_MESSAGE_LENGTH:10,MIN_TECHNICAL_DESCRIPTION_LENGTH:10},async init(){if(!this.state.isInitialized)try{this.state.appreciatedReflections=await d.getMyAppreciations(),this.renderReflectionsHTML(),await this.renderReflections(),this.subscribeToNewReflections(),this._initWhisperBadge(),this.state.isInitialized=!0}catch(e){console.error("[CommunityModule] init failed:",e)}},_initWhisperBadge(){const e=()=>window.WhisperModal?.refreshUnreadBadge();if(d?.ready)e();else{const t=setInterval(()=>{d?.ready&&(clearInterval(t),e())},300)}},renderReflectionsHTML(){const e=document.getElementById("communityReflectionsContainer");if(!e){console.warn("[CommunityModule] #communityReflectionsContainer not found");return}e.innerHTML=this._buildReflectionsShell(),this._setupCharCounter("reflectionInput","charCount")},_buildReflectionsShell(){const e=window.Core?.state?.currentUser||{},t=e.name||"You",i=e.avatar_url||"",o=window.Core.getAvatarGradient(e.id||"me"),n=i?`<img src="${i}" alt="${t}" width="40" height="40" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" loading="lazy" decoding="async">`:e.emoji||t.charAt(0).toUpperCase();return`
        <section class="section">
            <div class="section-header">
        ...
        </section>`},async renderReflections(){const e=document.getElementById("reflectionsContainer");if(e){e.innerHTML='<div style="color:var(--text-muted);font-size:13px;padding:16px;text-align:center">Loading reflections...</div>';try{const[t,i]=await Promise.all([d.getReflections(20),d.getBlockedUsers()]),o=t.filter(n=>!i.has(n.profiles?.id));e.innerHTML=o.length?o.map(n=>this._buildReflectionHTML(n)).join(""):'<div style="color:var(--text-muted);font-size:13px;padding:16px;text-align:center">Be the first to share a reflection ✨</div>'}catch(t){console.error("[CommunityModule] renderReflections error:",t)}}},_buildReflectionHTML(e){const t=e.profiles||{},i=t.name||"Community Member",o=t.avatar_url||"",n=window.Core.getAvatarGradient(t.id||e.id),r=this._timeAgo(e.created_at),a=t.id===window.Core?.state?.currentUser?.id,s=window.Core?.state?.currentUser?.is_admin,l=this.state.appreciatedReflections.has(e.id),c=o?`<img src="${o}" alt="${this._esc(i)}" width="40" height="40" loading="lazy" decoding="async">`:this._esc(t.emoji||i.charAt(0).toUpperCase()),u=o?"background:transparent;":`background:${n};`,p=a?`
            ...
            </div>`},subscribeToNewReflections(){if(!d?.ready){const t=setInterval(()=>{d?.ready&&(clearInterval(t),this.subscribeToNewReflections())},300);return}if(!d.subscribeToReflections(async t=>{t.profiles?.id===window.Core?.state?.currentUser?.id||(await d.getBlockedUsers()).has(t.profiles?.id)||this._prependReflection(t)})){const t=setInterval(()=>{d?.ready&&(clearInterval(t),this.subscribeToNewReflections())},300)}},_prependReflection(e){const t=document.getElementById("reflectionsContainer");if(!t)return;const i=document.createElement("div");i.innerHTML=this._buildReflectionHTML(e);const o=i.firstElementChild;o&&t.insertBefore(o,t.firstChild)},async shareReflection(){const e=document.getElementById("reflectionInput");if(!e)return;const t=e.value.trim();if(t.length<this.config.MIN_REFLECTION_LENGTH){window.Core.showToast("Please write something first");return}if(t.length>this.config.MAX_REFLECTION_LENGTH){window.Core.showToast(`Reflection too long (max ${this.config.MAX_REFLECTION_LENGTH} characters)`);return}try{const i=await d.postReflection(t);if(!i){window.Core.showToast("Could not share reflection - please try again");return}e.value="";const o=document.getElementById("charCount");o&&(o.textContent="0"),this._prependReflection(i),window.Core.showToast("Reflection shared with the community")}catch(i){console.error("[CommunityModule] shareReflection error:",i)}},async deleteReflection(e){if(!confirm("Remove this reflection?"))return;await d.deleteReflection(e)?(document.querySelector(`[data-reflection-id="${e}"]`)?.remove(),window.Core.showToast("Reflection removed")):window.Core.showToast("Could not remove reflection")},editReflection(e){const i=document.querySelector(`[data-reflection-id="${e}"]`)?.querySelector(".ref-content");if(!i)return;const o=i.textContent.trim();i.innerHTML=`
            ...
            </div>`;const n=document.getElementById(`editReflectionInput_${e}`);n&&(this._setupCharCounter(`editReflectionInput_${e}`,`editCharCount_${e}`),n.focus(),n.setSelectionRange(n.value.length,n.value.length))},cancelEditReflection(e){const i=document.querySelector(`[data-reflection-id="${e}"]`)?.querySelector(".ref-content");i&&d.getReflections(20).then(o=>{const n=o.find(r=>r.id===e);n&&(i.textContent=n.content)}).catch(()=>{})},async saveEditReflection(e){const t=document.getElementById(`editReflectionInput_${e}`);if(!t)return;const i=t.value.trim();if(!i){window.Core.showToast("Reflection cannot be empty");return}if(i.length>500){window.Core.showToast("Too long (max 500 characters)");return}if(t.disabled=!0,await d.updateReflection(e,i)){const n=document.querySelector(`[data-reflection-id="${e}"] .ref-content`);n&&(n.textContent=i),window.Core.showToast("Reflection updated")}else t.disabled=!1,window.Core.showToast("Could not update - please try again")},async appreciate(e,t){if(!(!e||!t))try{const i=this.state.appreciatedReflections.has(t),o=await d.toggleAppreciation(t,i);if(!o)return;this.state.appreciatedReflections[o.appreciated?"add":"delete"](t),e.classList.toggle("appreciated",o.appreciated);const n=await d.getReflectionCount(t),r=e.querySelector(".appreciation-count");r&&n!==null&&(r.textContent=`Appreciate (${n})`)}catch(i){console.error("[CommunityModule] appreciate error:",i)}},async whisper(e){if(e)try{const t=await d.getProfile(e);WhisperModal.openThread(e,t?.name||"Member",t?.emoji||"",t?.avatar_url||"")}catch{WhisperModal.openThread(e,"Member","","")}},renderMembers(){},viewMember(e){e&&(window.MemberProfileModal?MemberProfileModal.open(e):window.Core.showToast("Member profiles loading..."))},renderWaves(){const e=document.getElementById("wavesContainer");if(!e)return;const t=[{id:1,title:"Evening Wind Down",time:"Tonight at 8:00 PM",participants:42,progress:67},{id:2,title:"Sunday Morning Stillness",time:"Tomorrow at 7:00 AM",participants:28,progress:45}];e.innerHTML=t.map(i=>`
            ...
            </div>`).join("")},contributeWave(){window.Core.showToast("Contribution recorded! Start your practice.")},showCrisisModal(){this.openModal("crisisModal")},closeCrisisModal(){this.closeModal("crisisModal")},showReportModal(e=null){this.state.reportingUserId=e,this.openModal("reportModal")},closeReportModal(){this.closeModal("reportModal"),this._clearFields(["reportReason","reportDetails"]),this.state.reportingUserId=null},async submitReport(){const e=document.getElementById("reportReason")?.value,t=document.getElementById("reportDetails")?.value?.trim()||"";if(!e){window.Core.showToast("Please select a reason");return}await d.submitReport(this.state.reportingUserId,e,t)?(window.Core.showToast("Report submitted. Thank you for keeping the space safe."),this.closeReportModal()):window.Core.showToast("Could not submit report - please try again")},showBlockModal(){this.openModal("blockModal")},closeBlockModal(){this.closeModal("blockModal"),this._clearFields(["blockUsername"])},async confirmBlock(){const e=document.getElementById("blockUsername")?.value?.trim();if(!e){window.Core.showToast("Please enter a username");return}const t=await d.getUserByName(e);if(!t){window.Core.showToast("User not found");return}await d.blockUser(t.id)?(window.Core.showToast(`${t.name} has been blocked`),this.closeBlockModal(),await this.renderReflections()):window.Core.showToast("Could not block user - please try again")},hideMessagesFromUser(e){document.querySelectorAll(".chat-msg").forEach(t=>{t.querySelector("div")?.textContent.includes(e)&&(t.style.display="none")})},showHelpModal(){this.openModal("helpModal")},closeHelpModal(){this.closeModal("helpModal")},needHelp(){this.showHelpModal()},showModeratorModal(){this.closeHelpModal(),this.openModal("moderatorModal")},closeModeratorModal(){this.closeModal("moderatorModal"),this._clearFields(["moderatorMessage"]);const e=document.getElementById("moderatorUrgency");e&&(e.value="low")},contactModerator(){this.showModeratorModal()},submitModeratorRequest(){const e=document.getElementById("moderatorMessage")?.value?.trim();if(!e||e.length<this.config.MIN_MODERATOR_MESSAGE_LENGTH){window.Core.showToast(`Please describe your situation (at least ${this.config.MIN_MODERATOR_MESSAGE_LENGTH} characters)`);return}window.Core.showToast("Request sent. A moderator will reach out shortly."),this.closeModeratorModal()},showTechnicalModal(){this.closeHelpModal(),this.openModal("technicalModal")},closeTechnicalModal(){this.closeModal("technicalModal"),this._clearFields(["technicalType","technicalDescription","technicalDevice"])},reportTechnicalIssue(){this.showTechnicalModal()},submitTechnicalIssue(){const e=document.getElementById("technicalType")?.value,t=document.getElementById("technicalDescription")?.value?.trim();if(!e){window.Core.showToast("Please select an issue type");return}if(!t||t.length<this.config.MIN_TECHNICAL_DESCRIPTION_LENGTH){window.Core.showToast(`Please provide more details (at least ${this.config.MIN_TECHNICAL_DESCRIPTION_LENGTH} characters)`);return}window.Core.showToast("Issue reported. Our tech team will investigate."),this.closeTechnicalModal()},showGuidelinesModal(){this.closeHelpModal(),this.openModal("guidelinesModal")},closeGuidelinesModal(){this.closeModal("guidelinesModal")},viewGuidelines(){this.showGuidelinesModal()},muteChat(){const e=document.getElementById("psSidebar");if(!e)return;const t=e.classList.contains("muted");e.classList.toggle("muted"),window.Core.showToast(t?"Chat unmuted":"Chat muted"),!t&&e.classList.contains("open")&&(e.classList.remove("open"),document.getElementById("fabChat")?.classList.remove("hidden"))},registerEvent(){window.Core.showToast("Registration confirmed! Check your email.")},openModal(e){const t=document.getElementById(e);t?t.classList.add("active"):console.warn(`[CommunityModule] Modal not found: ${e}`)},closeModal(e){document.getElementById(e)?.classList.remove("active")},_setupCharCounter(e,t){const i=document.getElementById(e),o=document.getElementById(t);i&&o&&i.addEventListener("input",()=>{o.textContent=i.value.length})},_clearFields(e){e.forEach(t=>{const i=document.getElementById(t);i&&(i.value="")})},_timeAgo(e){if(!e)return"";const t=Date.now()-new Date(e).getTime(),i=Math.floor(t/6e4),o=Math.floor(t/36e5),n=Math.floor(t/864e5);return i<1?"Just now":i<60?`${i}m ago`:o<24?`${o}h ago`:`${n}d ago`},_esc(e){if(!e||typeof e!="string")return"";const t=document.createElement("div");return t.textContent=e,t.innerHTML}};window.CommunityModule=Z;const pe=Object.freeze(Object.defineProperty({__proto__:null,CommunityModule:Z},Symbol.toStringTag,{value:"Module"})),b={state:{modalsInjected:!1},init(){},injectModals(){const e=!!document.getElementById("reportModal");if(this.state.modalsInjected&&!e&&(this.state.modalsInjected=!1),!(this.state.modalsInjected||e))try{document.body.insertAdjacentHTML("beforeend",this.getAllModalsHTML()),this.state.modalsInjected=!0}catch(t){console.error("Failed to inject SafetyBar modals:",t)}},getAllModalsHTML(){return`
    ...
    <div class="modal-overlay" id="guidelinesModal" onclick="SafetyBar.handleOverlayClick(event,'guidelines')">
    ...
    </div>`},handleOverlayClick(e,t){e.target===e.currentTarget&&this.closeModal(t)},openModal(e){this.injectModals();const t=document.getElementById(`${e}Modal`);t?t.classList.add("active"):console.warn(`[SafetyBar] openModal: #${e}Modal not found`)},closeModal(e){document.getElementById(`${e}Modal`)?.classList.remove("active")},_switchModal(e,t){this.closeModal(e),this.openModal(t)},async submitReport(){const e=document.getElementById("reportReason")?.value,t=document.getElementById("reportDetails")?.value?.trim();if(!e){window.Core.showToast("Please select a reason");return}window.Community?.submitReport?.();try{await this._writeAdminNotification("report",{reason:e,details:t,room:this._getCurrentRoom()}),await this._pushAdmins("⚠️ New Report",`${this._senderName()} reported: ${e}${t?" - "+t.substring(0,60):""}`)}catch(i){console.error("submitReport admin notify error:",i)}},confirmBlock(){typeof window.Community?.confirmBlock=="function"?window.Community.confirmBlock():console.error("Community.confirmBlock not available")},toggleHelpMePanel(){const e=document.getElementById("helpMePanel");e&&(e.style.display=e.style.display==="none"?"block":"none")},async submitHelpMe(){const e=document.getElementById("helpMeText")?.value?.trim();if(!e){window.Core.showToast("Please write a short message");return}const t=document.querySelector("#helpMePanel button");t&&(t.disabled=!0,t.textContent="Sending...");try{await this._writeAdminNotification("help",{message:e,room:this._getCurrentRoom()}),await this._pushAdmins("🆘 Help Request",`${this._senderName()} needs help: "${e.substring(0,80)}"`),window.Core.showToast("Your message was sent to the admin"),document.getElementById("helpMeText").value="",document.getElementById("helpMePanel").style.display="none",this.closeModal("help")}catch(i){console.error("submitHelpMe error:",i),window.Core.showToast("Could not send - please try again")}finally{t&&(t.disabled=!1,t.textContent="Send to Admin")}},async submitModeratorRequest(){const e=document.getElementById("moderatorUrgency")?.value,t=document.getElementById("moderatorMessage")?.value?.trim();try{await this._writeAdminNotification("moderator",{urgency:e,message:t,room:this._getCurrentRoom()}),await this._pushAdmins("👥 Moderator Request",`${this._senderName()} [${e}]: ${t?t.substring(0,80):"-"}`),window.Core.showToast("Moderator contacted"),this.closeModal("moderator")}catch(i){console.error("submitModeratorRequest error:",i),window.Core.showToast("Could not send - please try again")}},async submitTechnicalIssue(){const e=document.getElementById("technicalType")?.value,t=document.getElementById("technicalDescription")?.value?.trim(),i=document.getElementById("technicalDevice")?.value?.trim();window.Community?.submitTechnicalIssue?.();try{await this._writeAdminNotification("technical",{issueType:e,description:t,device:i,room:this._getCurrentRoom()}),await this._pushAdmins("🔧 Technical Issue",`${this._senderName()}: ${e||"issue"}${t?" - "+t.substring(0,60):""}`)}catch(o){console.error("submitTechnicalIssue admin notify error:",o)}},async _writeAdminNotification(e,t){if(!d?.ready)return;const{error:i}=await d._sb.from("admin_notifications").insert({type:e,from_user_id:d.userId||null,payload:{...t,sender_name:this._senderName(),timestamp:new Date().toISOString()}});i&&console.error("[SafetyBar] _writeAdminNotification:",i.message)},async _pushAdmins(e,t){if(d?.ready)try{const[{data:i}]=await Promise.all([d._sb.from("profiles").select("id").eq("is_admin",!0)]);if(!i?.length)return;const{data:o}=await d._sb.from("push_subscriptions").select("subscription").in("user_id",i.map(n=>n.id));if(!o?.length)return;await Promise.allSettled(o.map(n=>fetch("/api/send",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sub:n.subscription,payload:{title:e,body:t,icon:"/icons/icon-192x192.png",data:{url:"/"}}})}).catch(()=>{})))}catch(i){console.error("[SafetyBar] _pushAdmins error:",i)}},_getCurrentRoom(){return document.querySelector(".room-title, .room-name-inline, #roomTitle")?.textContent?.trim()||"Community Hub"},_senderName(){return window.Core?.state?.currentUser?.name||"A member"}};(function(){const e={showReportModal:()=>b.openModal("report"),showBlockModal:()=>b.openModal("block"),showHelpModal:()=>b.openModal("help"),showCrisisModal:()=>b.openModal("crisis"),showModeratorModal:()=>b.openModal("moderator"),showTechnicalModal:()=>b.openModal("technical"),showGuidelinesModal:()=>b.openModal("guidelines"),muteChat:()=>window.Core?.showToast?.("Chat muted"),closeReportModal:()=>b.closeModal("report"),closeBlockModal:()=>b.closeModal("block"),closeHelpModal:()=>b.closeModal("help"),closeCrisisModal:()=>b.closeModal("crisis"),closeModeratorModal:()=>b.closeModal("moderator"),closeTechnicalModal:()=>b.closeModal("technical"),closeGuidelinesModal:()=>b.closeModal("guidelines")};window.CommunityModule?Object.keys(e).forEach(t=>{typeof window.CommunityModule[t]!="function"&&(window.CommunityModule[t]=e[t])}):window.CommunityModule=e})();window.SafetyBar=b;const me=Object.freeze(Object.defineProperty({__proto__:null,SafetyBar:b},Symbol.toStringTag,{value:"Module"})),_=()=>window.CollectiveField,$=()=>window.CommunityDB?.userId||window.Core?.state?.currentUser?.id||null,T={_realtimeChannels:{},_pollInterval:null,POLL_INTERVAL_MS:3e4,async init(){try{await this._ensureTodayRow(),await this.loadAll(),this._subscribeRealtime(),this._startPolling()}catch(e){console.error("[CollectiveFieldDB] init error:",e)}},async loadAll(){await Promise.all([this.loadFieldState(),this.loadRecentSenders(),this.loadWaveTotal(),this.loadWaveParticipants(),this.loadUserContribution()])},get _sb(){return window.AppSupabase||window.CommunitySupabase||null},_todayUTC(){return new Date().toISOString().slice(0,10)},_err(e,t){console.error(`[CollectiveFieldDB] ${e}:`,t)},async _ensureTodayRow(){const e=this._todayUTC(),{data:t,error:i}=await this._sb.from("collective_field").select("id").eq("date",e).maybeSingle();if(i)throw i;if(!t){const{error:o}=await this._sb.from("collective_field").insert({date:e,energy_level:0,pulse_count_today:0});if(o)throw o}},async loadFieldState(){const{data:e,error:t}=await this._sb.from("collective_field").select("energy_level, pulse_count_today").eq("date",this._todayUTC()).single();if(t){this._err("loadFieldState",t);return}_().updateEnergyLevel(e.energy_level),_().updateCommunityPulseCount(e.pulse_count_today)},async loadRecentSenders(){const{data:e,error:t}=await this._sb.from("collective_pulses").select("user_id, profiles(emoji, avatar_url)").eq("date",this._todayUTC()).order("created_at",{ascending:!1}).limit(5);if(t){this._err("loadRecentSenders",t);return}_().updateRecentSenders((e||[]).map(i=>({emoji:i.profiles?.emoji||"🧘",avatarUrl:i.profiles?.avatar_url||null})))},async recordPulse(){const e=$();if(!e){this._err("recordPulse","no userId");return}const t=this._todayUTC(),{error:i}=await this._sb.rpc("increment_field_pulse",{p_date:t,p_energy_add:5});if(i){this._err("recordPulse RPC",i);return}const{error:o}=await this._sb.from("collective_pulses").insert({user_id:e,date:t});o&&this._err("recordPulse insert",o)},async loadWaveTotal(){const{data:e,error:t}=await this._sb.from("wave_contributions").select("minutes").eq("date",this._todayUTC());if(t){this._err("loadWaveTotal",t);return}_().updateWaveTotalMinutes((e||[]).reduce((i,o)=>i+(o.minutes||0),0))},async loadWaveParticipants(){const{data:e,error:t}=await this._sb.from("wave_contributions").select("user_id").eq("date",this._todayUTC());if(t){this._err("loadWaveParticipants",t);return}_().updateWaveParticipants(new Set((e||[]).map(i=>i.user_id)).size)},async loadUserContribution(){const e=$();if(!e)return;const t=this._todayUTC(),[{data:i,error:o},{data:n,error:r}]=await Promise.all([this._sb.from("wave_contributions").select("minutes").eq("user_id",e).eq("date",t),this._sb.from("wave_contributions").select("minutes").eq("user_id",e)]);if(o){this._err("loadUserContribution today",o);return}if(r){this._err("loadUserContribution allTime",r);return}const a=s=>(s||[]).reduce((l,c)=>l+(c.minutes||0),0);_().updateUserContribution(a(i),a(n))},async logWaveContribution(e,t){const i=$();if(!i){this._err("logWaveContribution","no userId");return}if(e<1)return;const{error:o}=await this._sb.from("wave_contributions").insert({user_id:i,date:this._todayUTC(),minutes:e,completed:t});if(o){this._err("logWaveContribution",o);return}await Promise.all([this.loadWaveTotal(),this.loadWaveParticipants()])},_subscribeRealtime(){const e=this._todayUTC();this._realtimeChannels.field=this._sb.channel("collective_field_changes").on("postgres_changes",{event:"UPDATE",schema:"public",table:"collective_field",filter:`date=eq.${e}`},({new:t})=>{_().updateEnergyLevel(t.energy_level),_().updateCommunityPulseCount(t.pulse_count_today)}).subscribe(),this._realtimeChannels.pulses=this._sb.channel("collective_pulses_inserts").on("postgres_changes",{event:"INSERT",schema:"public",table:"collective_pulses",filter:`date=eq.${e}`},async({new:t})=>{_().receiveExternalPulse({userId:t.user_id,intensity:.7}),await this.loadRecentSenders()}).subscribe(),this._realtimeChannels.wave=this._sb.channel("wave_contributions_inserts").on("postgres_changes",{event:"INSERT",schema:"public",table:"wave_contributions",filter:`date=eq.${e}`},async({new:t})=>{if(typeof t?.minutes=="number"&&t.minutes>0){const i=_().state.waveTotalMinutes||0;_().updateWaveTotalMinutes(i+t.minutes)}await this.loadWaveParticipants()}).subscribe()},_startPolling(){this._pollInterval&&clearInterval(this._pollInterval),this._pollInterval=setInterval(async()=>{try{await this.loadAll()}catch(e){this._err("poll",e)}},this.POLL_INTERVAL_MS)},destroy(){for(const e of Object.values(this._realtimeChannels))try{e.unsubscribe()}catch{}this._realtimeChannels={},this._pollInterval&&(clearInterval(this._pollInterval),this._pollInterval=null)}};window.addEventListener("pagehide",()=>T.destroy());window.CollectiveFieldDB=T;const he=Object.freeze(Object.defineProperty({__proto__:null,CollectiveFieldDB:T},Symbol.toStringTag,{value:"Module"})),J={state:{isEnabled:!1,isRendered:!1},config:{FEATURE_ENABLED:!1},getHTML(){return this.config.FEATURE_ENABLED?`
        ...
        <div style="background:var(--neuro-bg,#f0f0f3);border-radius:20px;padding:24px;
        ...
        </div>`},_switchAdminTab(e){this._readAdminFields(this._adminActiveTab),this._adminActiveTab=e,this.config.ADMIN_TABS.forEach(({id:t})=>{const i=document.getElementById(`adminTab_${t}`);if(!i)return;const o=t===e;i.style.background=o?"var(--neuro-accent)":"var(--neuro-accent-a10)",i.style.color=o?"#fff":"var(--neuro-accent)"}),this._renderAdminTab(e)},_renderAdminTab(e){const t=document.getElementById("adminTabContent");if(!t)return;const i=this._adminDraft[e],{Sessions:o,Workshops:n}=this.config.FLYER_FILES,r=(l,c)=>c.map(u=>{const p=this._flyerBase+l+"/"+u,h=i.image===p;return`<button type="button" onclick="UpcomingEvents._selectFlyer('${e}','${p}','${u}')"
                         style="cursor:pointer;border-radius:8px;overflow:hidden;
                                border:3px solid ${h?"var(--neuro-accent)":"transparent"};
                                transition:border 0.15s;">
                        <img src="${p}" alt="${u}" loading="lazy" decoding="async" style="width:100%;height:auto;display:block;">
            ...
            <div style="font-size:0.78rem;font-weight:700;color:var(--neuro-accent);margin-bottom:12px;
                        text-transform:uppercase;letter-spacing:0.5px;">Editing: ${a[e]}</div>
            ...
            </div>`},_buildCalmWaveHTML(){const{waveTotalMinutes:e,WAVE_GOAL_MINUTES:t,userTodayMinutes:i,userAllTimeMinutes:o}=this.state,n=this.config.DEFAULT_WAVE_PARTICIPANTS,r=Math.min(Math.round(e/t*100),100),a=Math.max(t-e,0),s=Math.floor(a/60),l=a%60,c=s>0?`${s}h ${l}m`:`${l}m`;return`
            <div class="collective-card wave-card-new">
                ...
                <div class="wave-time-block" style="position:relative;">
                    ...
                    <div id="waveCollectiveTime" style="transition:font-size 0.4s ease,opacity 0.4s ease,margin 0.4s ease;">
                        <div id="waveClockDisplay" style="font-size:28px;font-weight:700;letter-spacing:1px;color:var(--text-primary);line-height:1.1;">${c}</div>
            ...
            </svg>`},_cleanup(){const e=this.state,t=["holdInterval","timerInterval"];for(const i of t)e[i]&&(clearInterval(e[i]),e[i]=null);this._lastSentRefreshInterval&&(clearInterval(this._lastSentRefreshInterval),this._lastSentRefreshInterval=null),e.isHolding=!1},handleHoldStart(){const e=this.state;if(e.lastCalmSentAt&&Date.now()-e.lastCalmSentAt<e.COOLDOWN_MS){const i=Math.ceil((e.COOLDOWN_MS-(Date.now()-e.lastCalmSentAt))/1e3);this._toast(`Wait ${i}s before sending again`);return}if(e.isHolding)return;e.isHolding=!0,e.holdProgress=0,document.getElementById("pulseBtn")?.classList.add("holding");const t=Date.now();e.holdInterval=setInterval(()=>{const i=Date.now()-t;e.holdProgress=Math.min(i/e.HOLD_DURATION_MS*100,100);const o=document.getElementById("holdRing");o&&(o.style.strokeDashoffset=100.5-e.holdProgress/100*100.5),e.holdProgress>=100&&(clearInterval(e.holdInterval),e.holdInterval=null,e.isHolding=!1,this._firePulse())},50)},handleHoldEnd(){this.state.isHolding&&this._cancelHold()},handleHoldCancel(){this.state.isHolding&&this._cancelHold()},_cancelHold(){const e=this.state;e.holdInterval&&(clearInterval(e.holdInterval),e.holdInterval=null),e.isHolding=!1,e.holdProgress=0,document.getElementById("pulseBtn")?.classList.remove("holding");const t=document.getElementById("holdRing");t&&(t.style.strokeDashoffset="100.5")},_firePulse(){const e=this.state;e.lastCalmSentAt=Date.now(),e.calmsSentCount++;const t=document.getElementById("pulseBtn");t&&(t.classList.remove("holding"),t.classList.add("fired"),setTimeout(()=>t.classList.remove("fired"),1e3));const i=document.getElementById("holdRing");i&&(i.style.strokeDashoffset="100.5"),this._addEnergy(e.ENERGY_PER_PULSE),this._triggerFieldRipple(e.calmsSentCount),this._triggerAppWideRipple(),this._broadcastPulse(e.calmsSentCount),e.communityPulseCount++,this._addSelfToRecentSenders(),this._refreshEnergyCardMeta(),this._toast("Energy sent")},_addEnergy(e){const t=this.state;t.energyLevel=Math.min(t.energyLevel+e,100),this._updateEnergyBar(t.energyLevel)},_updateEnergyBar(e){const t=document.getElementById("pulseFill"),i=document.getElementById("energyValue");t&&(t.style.width=`${e}%`,t.setAttribute("aria-valuenow",e)),i&&(i.textContent=`${e}%`);const o=document.getElementById("adminEnergyBtn");o&&(o.style.display=window.Core?.state?.currentUser?.is_admin?"inline":"none")},handleContributeWave(){this.state.isContributing?this._endWave().catch(e=>console.error("[CollectiveField] _endWave error:",e)):this._startWave()},_startWave(){const e=this.state;e.isContributing=!0,e.contributeStartedAt=Date.now(),e.countedAsParticipant=!1,document.getElementById("waveBtn")?.classList.add("in-progress");const t=document.getElementById("waveBtnLabel");t&&(t.textContent="End Session"),this._updateWaveStatusLine(),this._toast("Silence started"),e.timerInterval=setInterval(()=>this._waveTick(),1e3)},_resumeWaveTick(){const e=this.state;document.getElementById("waveBtn")?.classList.add("in-progress");const t=document.getElementById("waveBtnLabel");t&&(t.textContent="End Session"),this._updateWaveStatusLine(),e.timerInterval=setInterval(()=>this._waveTick(),1e3)},_waveTick(){const e=this.state;if(!e.isContributing)return;const t=Date.now()-e.contributeStartedAt;!e.countedAsParticipant&&t>=300*1e3&&(e.countedAsParticipant=!0,this._incrementParticipantCount(),this._triggerAvatarRipple(),this._toast("You're in the wave")),this._updateWaveStatusLine(t)},async _endWave(){const e=this.state;clearInterval(e.timerInterval),e.timerInterval=null,e.isContributing=!1;const t=Date.now()-e.contributeStartedAt,i=Math.floor(t/6e4);e.countedAsParticipant&&(this._decrementParticipantCount(),e.countedAsParticipant=!1),document.getElementById("waveBtn")?.classList.remove("in-progress");const o=document.getElementById("waveBtnLabel");if(o&&(o.textContent="Start Silence"),i<1){this._toast("Sit a little longer"),this._updateWaveStatusLine();return}const n=e.waveTotalMinutes,r=e.userTodayMinutes,a=e.userAllTimeMinutes;e.waveTotalMinutes=Math.min(e.waveTotalMinutes+i,e.WAVE_GOAL_MINUTES),e.userTodayMinutes+=i,e.userAllTimeMinutes+=i,this._updateWaveProgress(),this._updateWaveStatusLine(),this._toast(`${i}min contributed to the wave`),e.waveTotalMinutes>=e.WAVE_GOAL_MINUTES&&this._onWaveComplete();try{await T.logWaveContribution(i,!0)}catch(s){console.error("[CollectiveField] logWaveContribution failed — rolling back:",s),e.waveTotalMinutes=n,e.userTodayMinutes=r,e.userAllTimeMinutes=a,this._updateWaveProgress(),this._updateWaveStatusLine(),this._toast("Could not save session — please try again")}},_onWaveComplete(){this._toast("The wave is complete! A new one begins tomorrow.")},_updateWaveProgress(){const e=this.state,t=Math.min(Math.round(e.waveTotalMinutes/e.WAVE_GOAL_MINUTES*100),100),i=document.getElementById("waveFill"),o=document.getElementById("waveProgressValue");i&&(i.style.width=`${t}%`,i.setAttribute("aria-valuenow",t)),o&&(o.textContent=`${t}%`);const n=document.getElementById("adminWaveBtn");n&&(n.style.display=window.Core?.state?.currentUser?.is_admin?"inline":"none")},_updateWaveStatusLine(e){const t=this.state,i=document.getElementById("waveSessionClock"),o=document.getElementById("waveCountUp"),n=document.getElementById("waveCollectiveTime"),r=document.getElementById("waveClockDisplay"),a=document.getElementById("waveContribBlock");if(!r)return;const s=Math.max(t.WAVE_GOAL_MINUTES-t.waveTotalMinutes,0),l=Math.floor(s/60),c=s%60;if(r.textContent=l>0?`${l}h ${c}m`:`${c}m`,e!==void 0&&t.isContributing){const u=Math.floor(e/6e4),p=Math.floor(e%6e4/1e3);o&&(o.textContent=`${String(u).padStart(2,"0")}:${String(p).padStart(2,"0")}`),i&&(i.style.maxHeight="60px",i.style.opacity="1",i.style.marginBottom="8px"),n&&(n.style.fontSize="13px",n.style.opacity="0.45",n.style.marginTop="2px"),a&&(a.style.opacity="0.5")}else{i&&(i.style.maxHeight="0",i.style.opacity="0",i.style.marginBottom="0"),n&&(n.style.fontSize="",n.style.opacity="1",n.style.marginTop=""),a&&(a.style.opacity="1");const u=document.getElementById("userTodayDisplay"),p=document.getElementById("userAllTimeDisplay");u&&(u.textContent=`${t.userTodayMinutes}m`),p&&(p.textContent=`${t.userAllTimeMinutes}m`)}},_triggerFieldRipple(e){const t=document.querySelector(".energy-card svg");if(!t)return;const i=Math.max(1-(e-1)*.2,.2),o=document.createElementNS("http://www.w3.org/2000/svg","circle");Object.entries({cx:"50",cy:"50",r:"10",fill:"none",stroke:"currentColor","stroke-width":"2"}).forEach(([n,r])=>o.setAttribute(n,r)),o.style.cssText=`opacity:${i};transition:r 1s ease-out,opacity 1s ease-out;`,t.appendChild(o),requestAnimationFrame(()=>requestAnimationFrame(()=>{o.style.opacity="0",o.setAttribute("r","48")})),setTimeout(()=>o.remove(),1100)},_triggerAppWideRipple(){if(!document.getElementById("appRippleStyles")){const a=document.createElement("style");a.id="appRippleStyles",a.textContent=`
            ...
            `,document.head.appendChild(a)}let e=document.getElementById("appRippleOverlay");e||(e=document.createElement("div"),e.id="appRippleOverlay",document.body.appendChild(e));const t=document.querySelector(".energy-card"),i=t?t.getBoundingClientRect():null,o=i?i.left+i.width/2:window.innerWidth/2,n=i?i.top+i.height/2:window.innerHeight/2,r=Math.hypot(window.innerWidth,window.innerHeight)*2.2;["ring-1","ring-2","ring-3"].forEach((a,s)=>{const l=document.createElement("div");l.className=`app-wide-ripple ${a}`,l.style.cssText=`--ripple-size:${r}px;left:${o}px;top:${n}px;`,e.appendChild(l),setTimeout(()=>l.remove(),3400+s*300)})},_triggerAvatarRipple(){if(!document.getElementById("waveRippleStyles")){const a=document.createElement("style");a.id="waveRippleStyles",a.textContent=`
                @keyframes waveFloat {
        ...
        <div class="profile-gamification-section"
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;">
                ...
                <div style="background:var(--neuro-bg,#f0f0f3);border-radius:14px;
                     ...
                     onmouseover="this.style.transform='translateY(-2px)'"
                     onmouseout="this.style.transform=''">
                    <div style="font-size:1.3rem;line-height:1;margin-bottom:4px;">${a.emoji}</div>
                    <div ${a.id?`id="${a.id}"`:""} style="font-size:1.15rem;font-weight:800;
                                color:var(--neuro-accent);line-height:1;">${a.value}</div>
                    <div style="font-size:0.65rem;color:var(--text-muted);
                                font-weight:600;text-transform:uppercase;
                                letter-spacing:0.03em;margin-top:3px;">${a.label}</div>
                </div>`).join("")}
            </div>
        </div>
        ...
        <style>
            @keyframes profile-xp-shimmer {
                ...
                100% { transform: translateX(200%); }
        ...
        </style>`},animateGamificationBar(){requestAnimationFrame(()=>{const e=document.getElementById("profileGamificationXpBar");e&&(e.style.width=(e.dataset.width||0)+"%")})},refreshGamification(){const e=document.querySelector(".profile-gamification-section");if(!e)return;const t=document.createElement("div");t.innerHTML=this._getGamificationHTML(),e.replaceWith(t.firstElementChild),this.animateGamificationBar()},renderHTML(){const e=document.getElementById("profileHeroContainer");if(!e){console.warn("profileHeroContainer not found - skipping profile render");return}e.innerHTML=this.getHTML()},populateData(){const e=this._user();if(!e){console.warn("window.Core.state.currentUser not available");return}try{this.updateAvatar(e),this.updateName(e),this.updateKarma(e),this.updateBio(e),this.updateStatusRing(e.status),this.updateRole(e),this.updateBadges(),this.updateBirthday(e),this.updateCountry(e),this.updateProfileLocationRow(e),this.animateGamificationBar(),this.loadActivityData().catch(()=>{}),this.loadCommunityStats().catch(()=>{})}catch(t){console.error("Profile data population error:",t)}},updateAvatar(e){const t=document.getElementById("profileAvatar"),i=document.getElementById("profileAvatarImg"),o=document.getElementById("profileAvatarFallback");if(!t)return;const n=e.avatar_url||e.avatarUrl;if(n&&i){i.src=n,i.style.display="block",o&&(o.style.display="none"),t.style.background="transparent";return}o&&(o.textContent=e.emoji||e.avatar||"?",o.style.display="block",i&&(i.style.display="none")),window.Core?.getAvatarGradient&&(t.style.background=window.Core.getAvatarGradient(e.id||e.name||"default"))},updateName(e){const t=document.getElementById("profileName");t&&(t.textContent=e.name||e.displayName||"Member")},updateKarma(e){const t=y()?.getOwnGamificationState?.(),i=t?.karma??e.karma??0,o=t?.xp??e.xp??0,n=t?.level??e.level??1,r=document.getElementById("profileGamificationXP"),a=document.getElementById("profileGamificationXPNext"),s=document.getElementById("profileGamificationXpBar");if(r&&(r.textContent=o.toLocaleString()),s){const c=[0,800,2e3,4200,7e3,12e3,3e4,6e4,18e4,45e4],u=c[n-1]||0,p=c[n]||c[c.length-1],h=p>u?Math.min(100,Math.round((o-u)/(p-u)*100)):100;s.dataset.width=h,s.style.width=h+"%",a&&(a.textContent=Math.max(0,p-o).toLocaleString())}const l=document.getElementById("statKarma");l&&(l.textContent=i.toLocaleString())},async loadCommunityStats(){if(!y()?.ready)return;const e=this._user()?.id;if(e)try{const[{data:t,error:i},{data:o,error:n}]=await Promise.all([y()?._sb.from("profiles").select("gifts_given").eq("id",e).single(),y()?._sb.from("room_entries").select("room_id").eq("user_id",e)]),r=document.getElementById("statBlessings");if(r){const s=!i&&t?.gifts_given!=null?t.gifts_given:0;r.textContent=s.toLocaleString()}const a=document.getElementById("statFavRoom");if(a)if(!n&&o?.length>0){const s={};o.forEach(c=>{s[c.room_id]=(s[c.room_id]||0)+1});const l=Object.entries(s).sort((c,u)=>u[1]-c[1])[0][0];a.textContent=l.replace(/-/g," ").replace(/\b\w/g,c=>c.toUpperCase())}else a.textContent="-"}catch(t){console.warn("[ProfileModule] loadCommunityStats error:",t)}},updateBio(e){const t=document.getElementById("profileInspiration"),i=e.bio||e.inspiration;t&&i&&(t.textContent=`"${i}"`)},updateRole(e){const t=e.community_role||e.role||"Member",i=e.status||"available",o=document.getElementById("profileRoleBadge");o&&(o.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg> ${t}`);const n=document.getElementById("privateRole");n&&(n.textContent=t);const r=document.getElementById("privateStatus");r&&(r.textContent=i.charAt(0).toUpperCase()+i.slice(1))},updateBirthday(e){const t=document.getElementById("privateBirthday");if(t)if(e.birthday)try{t.textContent=new Date(e.birthday+"T00:00:00").toLocaleDateString(void 0,{month:"long",day:"numeric",year:"numeric"})}catch{t.textContent=e.birthday}else t.textContent="-"},updateCountry(e){const t=document.getElementById("privateCountry");t&&(t.textContent=e.country||"-")},async updateProfileLocationRow(e){let{birthday:t,country:i}=e;if(!t&&!i&&y()?.ready)try{const s=await y()?.getMyProfile();if(s){t=s.birthday,i=s.country;const l=this._user();l&&(l.birthday=t,l.country=i)}}catch{}const o=document.getElementById("profileBirthdayDisplay");if(o)if(t)try{const s=new Date(t+"T00:00:00").toLocaleDateString(void 0,{month:"long",day:"numeric"});o.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M20 21v-8a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8"/><path d="M4 16s.5-1 2-1 2.5 2 4 2 2.5-2 4-2 2 1 2 1"/><path d="M2 21h20"/><path d="M7 8v3"/><path d="M12 8v3"/><path d="M17 8v3"/><path d="M7 4h.01"/><path d="M12 4h.01"/><path d="M17 4h.01"/></svg> ${s}`}catch{o.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M20 21v-8a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8"/><path d="M4 16s.5-1 2-1 2.5 2 4 2 2.5-2 4-2 2 1 2 1"/><path d="M2 21h20"/><path d="M7 8v3"/><path d="M12 8v3"/><path d="M17 8v3"/><path d="M7 4h.01"/><path d="M12 4h.01"/><path d="M17 4h.01"/></svg> ${t}`}else o.textContent="";const n=document.getElementById("profileCountryDisplay");n&&(n.textContent=i?`${this._countryFlag(i)} ${i}`:"");const r=document.getElementById("profileCountrySep");r&&(r.style.display=i?"":"none");const a=document.querySelector("#profileMetaRow .profile-meta-sep");a&&(a.style.display=t||i?"":"none")},async loadActivityData(){const t=window.app?.gamification?.getStatusSummary?.()??null,i=window.app?.state?.getStats?.()??null;if(t&&i){this._setActivityCount("journal",t.totalJournalEntries??0,"entries"),this._setActivityCount("gratitude",i.totalGratitudes??0,"entries"),this._setActivityCount("energy",i.totalEnergyEntries??window.app?.state?.data?.energyEntries?.length??0,"check-ins"),this._setActivityCount("flip",i.totalFlipEntries??window.app?.state?.data?.flipEntries?.length??0,"entries"),this._setActivityCount("tarot",t.totalTarotSpreads??0,"readings"),this._setActivityCount("meditation",t.totalMeditations??i.weeklyMeditations??window.app?.state?.data?.meditationEntries?.length??0,"sessions"),this._activityData={_fromGamification:!0};return}if(window.app?.state?.ready)try{await window.app.state.ready}catch{}const o=window.app?.state?.data;if(o){const n=(o.gratitudeEntries||[]).reduce((r,a)=>r+(a.entries?.length||0),0);this._setActivityCount("journal",o.journalEntries?.length??0,"entries"),this._setActivityCount("gratitude",n,"entries"),this._setActivityCount("energy",o.energyEntries?.length??0,"check-ins"),this._setActivityCount("flip",o.flipEntries?.length??0,"entries"),this._setActivityCount("tarot",o.tarotReadings?.length??0,"readings"),this._setActivityCount("meditation",o.meditationEntries?.length??0,"sessions"),this._activityData=o;return}try{if(y()?.ready){const n=await y()?.getOwnFullProgress();if(n){const r=(n.gratitudeEntries||[]).reduce((a,s)=>a+(s.entries?.length||0),0);this._setActivityCount("journal",n.journalEntries?.length??0,"entries"),this._setActivityCount("gratitude",r,"entries"),this._setActivityCount("energy",n.energyEntries?.length??0,"check-ins"),this._setActivityCount("flip",n.flipEntries?.length??0,"entries"),this._setActivityCount("tarot",n.tarotReadings?.length??0,"readings"),this._setActivityCount("meditation",n.meditationEntries?.length??0,"sessions"),this._activityData=n}}}catch(n){console.warn("[ProfileModule] loadActivityData fallback failed:",n)}},_setActivityCount(e,t,i){const o=document.getElementById(`activityCount_${e}`);o&&(o.textContent=t>0?`${t} ${i}`:`No ${i} yet`)},async openActivityModal(e){this._activityData||await this.loadActivityData();const i={journal:{title:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg> Journal Entries'},gratitude:{title:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/><path d="m12 13-1-1 2-2-3-3-2 2-1-1"/></svg> Gratitude Entries'},energy:{title:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/></svg> Energy Check-ins'},flip:{title:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg> Flip the Script'},tarot:{title:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><rect x="2" y="5" width="11" height="14" rx="2"/><path d="M15.5 5.5 18 3l4 4-5.5 5.5"/><path d="m13 13 4.5 4.5"/><path d="m17.5 17.5 1 1"/></svg> Tarot Spreads'},meditation:{title:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M12 5a3 3 0 1 0-5.997.125 4 4 0 0 0-2.526 5.77 4 4 0 0 0 .556 6.588A4 4 0 1 0 12 18Z"/><path d="M12 5a3 3 0 1 1 5.997.125 4 4 0 0 1 2.526 5.77 4 4 0 0 1-.556 6.588A4 4 0 1 1 12 18Z"/><path d="M15 13a4.5 4.5 0 0 1-3-4 4.5 4.5 0 0 1-3 4"/><path d="M17.599 6.5a3 3 0 0 0 .399-1.375"/><path d="M6.003 5.125A3 3 0 0 0 6.401 6.5"/><path d="M3.477 10.896a4 4 0 0 1 .585-.396"/><path d="M19.938 10.5a4 4 0 0 1 .585.396"/><path d="M6 18a4 4 0 0 1-1.967-.516"/><path d="M19.967 17.484A4 4 0 0 1 18 18"/></svg> Meditation Sessions'}}[e],o=this._activityData?.[`${e}Entries`]||[],n=document.getElementById("activityModalTitle"),r=document.getElementById("activityModalBody");n&&(n.innerHTML=i.title),r&&(r.innerHTML=o.length?o.map(l=>this._renderActivityEntry(e,l)).join(""):'<div style="text-align:center;color:var(--text-muted);padding:2rem;">No entries yet</div>');const a=document.getElementById("activityModal"),s=document.getElementById("activityModalInner");a&&(a.style.display="flex",requestAnimationFrame(()=>{a.style.opacity="1",s.style.transform="translateY(0)"}),a.onclick=l=>{l.target===a&&this.closeActivityModal()},this._activityEscHandler=l=>{l.key==="Escape"&&this.closeActivityModal()},document.addEventListener("keydown",this._activityEscHandler))},closeActivityModal(){const e=document.getElementById("activityModal"),t=document.getElementById("activityModalInner");e&&(e.style.opacity="0",t.style.transform="translateY(16px)",setTimeout(()=>{e.style.display="none"},250),this._activityEscHandler&&(document.removeEventListener("keydown",this._activityEscHandler),this._activityEscHandler=null))},_renderActivityEntry(e,t){const i=this._formatEntryDate(t.timestamp||t.date),o=`border-radius:12px;padding:12px 14px;margin-bottom:10px;
                      background:var(--surface,rgba(0,0,0,0.03));
                      border-left:3px solid var(--neuro-accent);
                      font-size:0.86rem;line-height:1.6;`;switch(e){case"journal":{const n=t.situation||t.feelings||"",r=t.mood?`<span style="margin-left:6px;opacity:0.7;">${t.mood}</span>`:"";return`<div style="${o}">
                    <div style="font-size:0.75rem;color:var(--text-muted);margin-bottom:4px;">${i}${r}</div>
                    <div style="color:var(--neuro-text);">${this._esc(n)}</div>
                </div>`}case"gratitude":{const n=t.entries||[];return`<div style="${o}">
                    <div style="font-size:0.75rem;color:var(--text-muted);margin-bottom:6px;">${i}</div>
                    ${n.map(r=>`<div style="margin-bottom:3px;display:flex;align-items:center;gap:0.3rem;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/><path d="m12 13-1-1 2-2-3-3-2 2-1-1"/></svg> ${this._esc(r)}</div>`).join("")}
                </div>`}case"energy":{const n=t.energy??"-",r=t.notes?`<div style="margin-top:4px;opacity:0.8;">${this._esc(t.notes)}</div>`:"",a=(t.moodTags||[]).length?`<div style="margin-top:5px;display:flex;flex-wrap:wrap;gap:4px;">
                    ...
                    <div style="font-size:1rem;font-weight:700;color:var(--neuro-accent);display:flex;align-items:center;gap:0.3rem;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/></svg> Energy: ${n}/10</div>
                    ${a}${r}
                </div>`}case"flip":{const n=t.original||t.flipped_from||t.situation||t.text||t.content||"",r=t.flipped||t.reframe||"";return`<div style="${o}">
                    <div style="font-size:0.75rem;color:var(--text-muted);margin-bottom:4px;">${i}</div>
                    ${n?`<div style="color:var(--neuro-text);display:flex;align-items:center;gap:0.3rem;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg> ${this._esc(n)}</div>`:""}
                    ${r?`<div style="margin-top:6px;color:var(--neuro-accent);display:flex;align-items:center;gap:0.3rem;"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide-icon"><path d="M15 4V2"/><path d="M15 16v-2"/><path d="M8 9h2"/><path d="M20 9h2"/><path d="M17.8 11.8 19 13"/><path d="M15 9h.01"/><path d="M17.8 6.2 19 5"/><path d="m3 21 9-9"/><path d="M12.2 6.2 11 5"/></svg> ${this._esc(r)}</div>`:""}
                </div>`}case"tarot":{const n=t.spreadType||t.spreadKey||"Spread",r=(t.cards||[]).slice(0,3);return`<div style="${o}">
                    <div style="font-size:0.75rem;color:var(--text-muted);margin-bottom:4px;">${i}</div>
                    <div style="font-weight:700;color:var(--neuro-accent);margin-bottom:4px;">🃏 ${this._esc(n)}</div>
                    ${r.length?`<div style="font-size:0.8rem;color:var(--neuro-text);opacity:0.8;">
                        ${r.map(a=>`<div style="margin-bottom:2px;">• ${this._esc(a.name||"")}</div>`).join("")}
                        ${t.cards?.length>3?`<div style="opacity:0.6;">+${t.cards.length-3} more cards</div>`:""}
                    </div>`:""}
                </div>`}case"meditation":{const n=t.title||t.category||"Session",r=t.duration?`${t.duration} min`:"",a=t.chakra?`· ${t.chakra} chakra`:"";return`<div style="${o}">
                    <div style="font-size:0.75rem;color:var(--text-muted);margin-bottom:4px;">${i}</div>
                    <div style="font-weight:700;color:var(--neuro-accent);display:flex;align-items:center;gap:0.4rem;">
                        🧘 ${this._esc(n)}
                    </div>
                    ${r||a?`<div style="font-size:0.8rem;margin-top:3px;opacity:0.75;">${r}${a}</div>`:""}
                </div>`}default:return""}},updateBadges(){const e=window.app?.gamification?.state?.badges??[],t=document.getElementById("statBadges");t&&(t.textContent=e.length)},updateStatusRing(e){const t=this.config.STATUS_RING[e]||this.config.STATUS_RING.offline,i=document.getElementById("statusRing");i&&(i.style.borderColor=t.color,i.style.boxShadow=`0 0 0 4px ${t.color}33`);const o=document.getElementById("statusPickerDot"),n=document.getElementById("statusPickerLabel");o&&(o.style.background=t.color),n&&(n.textContent=t.label)},toggleStatusPicker(){const e=document.getElementById("statusPickerDropdown");if(!e)return;const t=e.style.display!=="none";if(e.style.display=t?"none":"block",!t){const i=o=>{!document.getElementById("statusPickerBtn")?.contains(o.target)&&!e.contains(o.target)&&(e.style.display="none",document.removeEventListener("click",i))};setTimeout(()=>document.addEventListener("click",i),0)}},async setStatus(e,t,i){const o=document.getElementById("statusPickerDropdown");o&&(o.style.display="none");const n=this._user()?.id;n&&window.ActiveMembers&&window.ActiveMembers.updateMemberStatus(n,e);const r=this.config.STATUS_ACTIVITIES[e]||"✨ Available";this.updateStatusRing(e);const a=document.getElementById("statusPickerDot"),s=document.getElementById("statusPickerLabel");a&&(a.style.background=i),s&&(s.textContent=t);const l=this._user();l&&(l.status=e,l.community_status=e);try{const c=window.Core?.state?.currentRoom||null;await Promise.all([y()?.setPresence(e,r,c),y()?.updateProfile({community_status:e})]),window.Core.showToast(`Status set to ${t}`)}catch(c){console.error("[ProfileModule] setStatus error:",c),window.Core.showToast("Could not update status - please try again")}window.dispatchEvent(new CustomEvent("statusChanged",{detail:{status:e}}))},updatePresenceCount(){window.Core?.updatePresenceCount?.()},toggleProfileView(e){e!=="public"&&e!=="private"||(document.querySelectorAll(".v-btn").forEach(t=>{const i=t.textContent.trim()==="Public View",o=e==="public"&&i||e==="private"&&!i;t.classList.toggle("active",o),t.setAttribute("aria-pressed",String(o))}),document.getElementById("privateDetails")?.classList.toggle("visible",e==="private"),this.state.currentView=e)},async sendPulse(){const e=window.Core?.state;if(!e){console.error("Core not available");return}if(e.pulseSent){window.Core.showToast("Already offered");return}const t=document.getElementById("pulseBtn");if(t)try{t.classList.add("sending"),setTimeout(async()=>{t.classList.remove("sending"),t.classList.add("sent"),t.innerHTML='✓<span class="pulse-ripple"></span>',e.pulseSent=!0;const i=document.getElementById("pulseFill");i&&(i.style.width="50%"),window.Core.showToast("Calm offered to the community"),await y()?.setPresence("online","💗 Offering calm",e.currentRoom||null);const o=this._user();o&&(o.activity="💗 Offering calm")},this.config.PULSE_ANIMATION_DURATION)}catch(i){console.error("Error sending pulse:",i)}},editProfile(){let e=document.getElementById("_avatarFileInput");e||(e=document.createElement("input"),e.id="_avatarFileInput",e.type="file",e.accept="image/jpeg,image/png,image/webp,image/gif",e.style.display="none",document.body.appendChild(e),e.addEventListener("change",()=>{const t=e.files?.[0];t&&this._uploadAvatar(t),e.value=""})),e.click()},async _uploadAvatar(e){if(e.size>5*1024*1024){window.Core.showToast("Image too large - max 5MB");return}const t=new FileReader;t.onload=o=>{const n=document.getElementById("profileAvatarImg"),r=document.getElementById("profileAvatarFallback"),a=document.getElementById("profileAvatar");n&&(n.src=o.target.result,n.style.display="block"),r&&(r.style.display="none"),a&&(a.style.background="transparent")},t.readAsDataURL(e),window.Core.showToast("Uploading photo...");const i=await y()?.uploadAvatar(e);if(i){const o=this._user();o&&(o.avatar_url=i),window.Core.showToast("Profile photo updated")}else window.Core.showToast("Upload failed - please try again"),this.updateAvatar(this._user())},async editRole(){const e=this._user()?.role||"",t=prompt("Enter your community role (e.g. Meditator, Healer, Teacher):",e);if(t===null)return;const i=t.trim().substring(0,60);if(!await y()?.updateProfile({community_role:i||null})){window.Core.showToast("Could not save - please try again");return}const n=this._user();n&&(n.community_role=i||"Member",n.role=i||"Member"),this.updateRole(this._user()),window.Core.showToast("Role updated")},async editInspiration(){const e=document.getElementById("profileInspiration");if(!e)return;const t=e.textContent.replace(/"/g,"").trim(),i=prompt("Edit your inspiration message:",t);if(!i?.trim())return;const o=i.trim().substring(0,this.config.MAX_INSPIRATION_LENGTH).replace(/<[^>]*>/g,"");if(!o)return;if(!await y()?.updateProfile({inspiration:o})){window.Core.showToast("Could not save - please try again");return}e.textContent=`"${o}"`;const r=this._user();r&&(r.inspiration=o),window.Core.showToast("Inspiration updated")},async _createInlineEditor({fieldId:e,dbKey:t,currentValue:i,inputType:o,placeholder:n,maxLength:r,successToast:a,validate:s,onSave:l}){const c=document.getElementById(e),u=c?.closest(".detail-row");if(!u||u.querySelector("input"))return;const p=u.querySelector(".edit-inline-btn");c&&(c.style.display="none"),p&&(p.style.display="none");const h=`flex:1;padding:4px 8px;border-radius:8px;
            border:1px solid rgba(0,0,0,0.15);font-size:0.85rem;
            background:var(--neuro-bg);color:var(--neuro-text);`,v=`margin-left:6px;padding:4px 8px;border-radius:8px;border:none;
            cursor:pointer;font-size:0.85rem;font-weight:700;
            background:var(--neuro-accent);color:#fff;`,C=`margin-left:4px;padding:4px 8px;border-radius:8px;border:none;
            cursor:pointer;font-size:0.85rem;opacity:0.6;
            background:rgba(0,0,0,0.06);color:var(--neuro-text);`,w=document.createElement("input");w.type=o,w.value=i,w.maxLength=r,w.placeholder=n,w.style.cssText=h;const x=document.createElement("button");x.textContent="✓",x.style.cssText=v;const E=document.createElement("button");E.textContent="✕",E.style.cssText=C;const L=()=>{w.remove(),x.remove(),E.remove(),c&&(c.style.display=""),p&&(p.style.display="")},D=async()=>{const S=w.value.trim().substring(0,r);if(s){const H=s(S);if(H){window.Core.showToast(H);return}}if(x.disabled=!0,x.textContent="...",!await y()?.updateProfile({[t]:S||null})){window.Core.showToast("Could not save - please try again"),x.disabled=!1,x.textContent="✓";return}L(),l(S||null),window.Core.showToast(a)};x.onclick=D,E.onclick=L,w.addEventListener("keydown",S=>{S.key==="Enter"&&D(),S.key==="Escape"&&L()}),u.appendChild(w),u.appendChild(x),u.appendChild(E),w.focus()},editBirthday(){const e=document.getElementById("profileBirthdayDisplay");if(!e)return;const t=this._user()?.birthday||"",i=document.createElement("input");i.type="date",i.value=t,i.style.cssText=`padding:3px 8px;border-radius:8px;border:1px solid rgba(0,0,0,0.15);
            font-size:0.8rem;background:var(--neuro-bg);color:var(--neuro-text);`;const o=async()=>{const n=i.value.trim();if(n&&!/^\d{4}-\d{2}-\d{2}$/.test(n)){window.Core.showToast("Invalid date");return}if(!await y()?.updateProfile({birthday:n||null})){window.Core.showToast("Could not save - please try again");return}const a=this._user();a&&(a.birthday=n),i.replaceWith(e),e.style.display="",this.updateBirthday(this._user()),this.updateProfileLocationRow(this._user()),window.Core.showToast("Birthday updated")};i.addEventListener("keydown",n=>{n.key==="Enter"&&o(),n.key==="Escape"&&(i.replaceWith(e),e.style.display="")}),i.addEventListener("blur",o),e.style.display="none",e.parentNode.insertBefore(i,e.nextSibling),i.focus()},editCountry(){const e=document.getElementById("profileCountryDisplay");if(!e)return;const t=this._user()?.country||"",i=document.createElement("input");i.type="text",i.value=t,i.placeholder="Your country",i.maxLength=60,i.style.cssText=`padding:3px 8px;border-radius:8px;border:1px solid rgba(0,0,0,0.15);
            font-size:0.8rem;background:var(--neuro-bg);color:var(--neuro-text);width:90px;`;const o=async()=>{const n=i.value.trim();if(!await y()?.updateProfile({country:n||null})){window.Core.showToast("Could not save - please try again");return}const a=this._user();a&&(a.country=n),i.replaceWith(e),e.style.display="",this.updateCountry(this._user()),this.updateProfileLocationRow(this._user()),window.Core.showToast("Country updated")};i.addEventListener("keydown",n=>{n.key==="Enter"&&o(),n.key==="Escape"&&(i.replaceWith(e),e.style.display="")}),i.addEventListener("blur",o),e.style.display="none",e.parentNode.insertBefore(i,e.nextSibling),i.focus()},setupCharCounter(){const e=document.getElementById("reflectionInput"),t=document.getElementById("charCount");e&&t&&e.addEventListener("input",()=>{t.textContent=e.value.length})},refresh(){this.populateData()}};window.ProfileModule=Q;const be=Object.freeze(Object.defineProperty({__proto__:null,ProfileModule:Q},Symbol.toStringTag,{value:"Module"}));export{q as A,d as C,W as E,b as S,le as W,m as _,A as a,k as b,I as c,xe as d,ke as e,Me as f,_e as m,B as r};
